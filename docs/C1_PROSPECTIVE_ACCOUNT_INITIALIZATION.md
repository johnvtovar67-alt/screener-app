...
The prospective policy attributes each current Swing holding and cash 25%/50%/25%. Each sleeve runs in the original $100,000 units, with a recorded conversion back to actual account units. This preserves the original minimum trade, integer model sizing, and portfolio parameters instead of silently retuning them to a smaller account. Summing the three sleeves recovers the entered share quantities and cash exactly within numeric tolerance.
...
- Map generated model-unit changes to account-sized whole-share orders, with actual-fill feedback rather than assumed completed fills.
- Apply the external SCHW purchase block to the actual execution path.
