...
import {
  C1_ACCOUNT_MOMENTUM_EXIT_POLICY,
  C1_FROZEN_OPTIONS,
} from "./c1FrozenOptions";
...
export const V11_PRODUCTION_MAX_POSITION_PCT = C1_FROZEN_OPTIONS.base.buyMaxPositionPct * 100;
export const V11_PRODUCTION_MAX_SECTOR_POSITIONS = C1_FROZEN_OPTIONS.base.maxSectorPositions;
export const V11_PRODUCTION_MAX_ENTRY_GAP_PCT = 3;
export const V11_PRODUCTION_MAX_SNAPSHOT_AGE_SESSIONS = 0;
export const V11_PRODUCTION_EXIT_RANK = C1_FROZEN_OPTIONS.base.rankedExitBuffer;
export const V11_PRODUCTION_MINIMUM_HOLD_SESSIONS = 30;
export const V11_PRODUCTION_CATASTROPHIC_STOP_PCT = 14;
export const C1_EARLY_MOMENTUM_EXIT = Object.freeze({
  startSession: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.earlyRankExitStartSessions,
  endSession: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.earlyRankExitEndSessions,
  rankThreshold: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.earlyRankExitThreshold,
  confirmations: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.earlyRankExitConsecutiveSessions,
});
export const C1_PROFIT_MOMENTUM_EXIT = Object.freeze({
  activationGainPct: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.profitRankExitActivationPct,
  rankThreshold: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.profitRankExitThreshold,
  confirmations: C1_ACCOUNT_MOMENTUM_EXIT_POLICY.profitRankExitConsecutiveSessions,
});
...
export const C1_PRODUCTION_ACTIVATION_DAY = "2026-09-04";
...
export function buildV11ProductionSnapshot(session = {}, createdAt = new Date()) {
  ...
  return {
    ...
    earlyMomentumExit: C1_EARLY_MOMENTUM_EXIT,
    profitMomentumExit: C1_PROFIT_MOMENTUM_EXIT,
    v12HardGovernorEnabled: false,
    shortHorizonChaseGateEnabled: false,
    candidates,
  };
}
...
export function applyV11ProductionPolicy(rows = [], snapshot = {}) {
  ...
  // Until the frozen policy earns independent validation, it has no authority
  // to manufacture full-size entries from its own rank. Preserve only the
  // bounded safety pilot: at most three independently actionable names, with
  // the portfolio governor enforcing two-session confirmation and a 1% cap.
  const pilotRows =
    !policyReady &&
    snapshot?.status === "ready" &&
            ...
            const gate = operationalGate(
              row,
              {
                ...
                momentumPercentile: number(
                  row.researchFactors?.momentumPercentile,
                  0,
                ),
              },
              {},
  ...
  return rows
    .map((row) => {
      ...
      if (!policyReady) {
        if (pilotIndex.has(symbol))
          ...
          finalDecision = {
            ...
            reason:
              "The research policy has not earned full-size trading authority, and this setup is outside the three-name limited pilot.",
            nextTrigger:
              "Wait for an independently actionable limited-pilot slot or full evidence-gate promotion.",
            planText: "No fresh capital outside the bounded pilot.",
      ...
      const productionPolicy = {
        ...
        gate: gate || null,
        // Report effective authority, never the aspirational research target
        // carried by an old stored snapshot.  This prevents an unvalidated
        // snapshot from advertising a 33% live allocation even though the
        // decision layer correctly failed closed.
        targetWeightPct: policyReady
          ? V11_PRODUCTION_TARGET_WEIGHT_PCT
          : pilotIndex.has(symbol)
            ? 1
            : 0,
        pilotMaxWeightPct: 1,
        independentlyValidated: snapshot?.independentlyValidated === true,
        activationAuthorized: policyReady,
        evidenceStatus: policyReady
          ? snapshot?.evidenceStatus || "independently-validated"
          : snapshot?.independentlyValidated === true
            ? "independently-validated-not-authorized"
...
export function v11ProductionPositionLifecycle({
  ...
  const researchRank =
    Number.isFinite(parsedResearchRank) && parsedResearchRank >= 1
      ? parsedResearchRank
      : null;
  // Both the page snapshot and the holding must carry current authority.
  // A ready ranking alone is not permission to manage live capital.
  const policyReady = [policy, stock.productionPolicy].every((receipt) =>
    receipt?.id === V11_PRODUCTION_POLICY_ID &&
    receipt?.status === "ready" &&
    receipt?.independentlyValidated === true &&
  ...
  if (!policyReady)
    return {
      action: "Review",
      reason: "C1 portfolio authority is unavailable or unvalidated. No C1 buy, add, hold, or rank-based sell recommendation is established for this position. The displayed valuation is not a trading instruction.",
      source: "c1-portfolio-authority-unavailable",
      heldSessions,
      researchRank,
    };
  if (
  ...
  if (
    policyReady &&
    openedDay &&
    openedDay < C1_PRODUCTION_ACTIVATION_DAY &&
    stock.productionPolicy?.selected !== true
  )
    return {
      action: "Exit",
      reason: `${stock.symbol} was opened before C1 activation and is not in the current three-position C1 portfolio. Exit the legacy Swing rather than applying C1's 30-session protection retroactively.`,
      source: "c1-legacy-transition-exit",
      heldSessions,
      researchRank,
    };
  return null;
