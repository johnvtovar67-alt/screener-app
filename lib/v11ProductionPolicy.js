...
function operationalGate(row = {}, candidate = {}, snapshot = {}) {
  ...
  if (!checks.quoteVerified)
    ...
    reason = `The price is ${entryGapPct.toFixed(1)}% above the source-session close, beyond V11's 3% opening-gap limit.`;
  return {
    pass,
    checks,
    reason,
...
function selectedDecision(candidate = {}, gate = {}, selectedIndex = 0) {
  ...
  return {
    ...
    priority:
      selectedIndex === 0
        ? "Best Opportunity"
        : `V11 Rank #${candidate.researchRank}`,
    reason: `V11 ranks ${candidate.symbol} #${candidate.researchRank} in the current point-in-time cohort using its audited momentum-dominant blend. Current quote, fundamental, event, liquidity, short-run chase, and opening-gap checks pass.`,
    nextTrigger:
      "Enter only while the current operational checks remain clear; do not chase an opening gap above 3%.",
    planText: `Approximately equal-weight target (${V11_PRODUCTION_TARGET_WEIGHT_PCT.toFixed(2)}%); retain V11's 18% catastrophic invalidation and rank-deterioration lifecycle.`,
...
function gateWatchDecision(existing = {}, candidate = {}, gate = {}) {
  ...
  return {
    ...
    planText: "No fresh capital while a required V11 operational gate is blocked.",
    relativeCapitalScore: displayScore,
    capitalEfficiencyScore: displayScore,
    policyScore: candidate.policyScore,
...
function outsidePolicyDecision(existing = {}) {
  ...
  return {
    ...existing,
    action: "Watch",
    timing: "Wait for V11 Rank",
    size: "None",
    priority: "Outside V11 Target",
    reason:
      "The legacy absolute-score engine identifies a setup, but it is outside the current V11 momentum-dominant target portfolio. Fresh capital follows the promoted V11 rank rather than mixing policies.",
    nextTrigger: "Re-rank into the V11 target portfolio on a verified refresh.",
    planText: "No fresh capital outside the promoted V11 target portfolio.",
  ...
  return rows
    .map((row) => {
      ...
      if (!policyReady) {
        if (["Strong Buy", "Buy"].includes(existing.action))
          finalDecision = {
            ...
            size: "None",
            priority: "V11 Snapshot Paused",
            reason:
              "The promoted V11 policy snapshot is unavailable or stale, so fresh capital is paused rather than falling back to a different strategy.",
            nextTrigger: "Wait for a current verified V11 policy snapshot.",
            planText: "No fresh capital while the V11 policy snapshot is unavailable.",
      ...
      const productionPolicy = {
        ...
        status: policyReady ? "ready" : snapshot?.status || "unavailable",
...
export function v11ProductionPositionLifecycle({
  ...
  const researchRank = number(stock.productionPolicy?.researchRank, null);
  const policyReady =
  ...
  if (
    ...
    return {
      action: "Exit",
      reason: `V11 catastrophic invalidation: the Swing is down ${Math.abs(gainLossPct).toFixed(1)}%, beyond the frozen 18% loss limit. Exit to cash; do not widen the stop.`,
  ...
  if (
    ...
    return {
      action: "Exit",
      reason: `V11 time lifecycle: this Swing has reached ${heldSessions} market sessions, beyond the frozen 126-session holding limit. Exit to cash and require a new ranked entry.`,
  ...
  if (
    ...
    return {
      ...
      reason: Number.isFinite(researchRank)
        ? `V11 weekly rank lifecycle: after ${heldSessions} market sessions, ${stock.symbol} ranks #${researchRank}, outside the frozen top-${V11_PRODUCTION_EXIT_RANK} retention buffer. Exit to cash or a separately qualified V11 target.`
        : `V11 weekly rank lifecycle: after ${heldSessions} market sessions, ${stock.symbol} is outside the frozen top-${V11_PRODUCTION_EXIT_RANK} retention buffer. Exit to cash or a separately qualified V11 target.`,
