...
export async function collectC1HeldRankReviews({baseline,sourceHash,symbols,now=new Date(),fetcher=fetch,apiKey=process.env.FMP_API_KEY||process.env.FMP_KEY,report=details=>console.warn('C1_HELD_RANK_VERIFICATION',JSON.stringify(details))}={}){
 ...
 for(const symbol of symbols){
  ...
  try{
   ...
   const review=compileC1HeldRankReview({baseline,sourceHash,symbol,history,observedAt:now.toISOString()});
   // historyHash identifies raw normalized provider input; the private receipt
   // records the sole canonicalization used for compilation.
   rows.push({...review,historyHash:createHash('sha256').update(JSON.stringify(history.slice(-253))).digest('hex'),provider:'FMP',endpoint:'historical-price-eod/dividend-adjusted'});
   try{report({code:review.volumeReconciliation?'VERIFIED_ACCEPTED_VOLUME':'VERIFIED',stage:'compile',providerStatus,receivedRows,normalizedRows});}catch{}
  }catch(error){
   ...
   const known={
    ...
    'Accepted holding volume unavailable':'ACCEPTED_VOLUME_UNAVAILABLE',
   ...
   const mismatchFields=code==='ACCEPTED_PRICE_MISMATCH'?['open','high','low','close'].filter(k=>Array.isArray(error?.mismatchFields)&&error.mismatchFields.includes(k)):[];
   try{report({code,stage,providerStatus,receivedRows,normalizedRows,...(mismatchFields.length?{mismatchFields}:{})});}catch{}
