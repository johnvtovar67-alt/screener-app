Warning: truncated output (original token count: 166234)
Total output lines: 17894
  ...
  for (const [date, rows] of [...byDate].sort(([left], [right]) =>
    ...
    for (const row of rows) {
      const addedSymbol = unchangedIssuers.has(row.addedSymbol)
        ? ""
        : row.addedSymbol;
      const removedSymbol = unchangedIssuers.has(row.removedSy…46234 tokens truncated…onst priorBenchmark = benchmarkByDate.get(curve[index - 1]?.date);
