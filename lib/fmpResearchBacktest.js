...
const COMPILED_CHECKPOINT_SCHEMA = 2;
const COMPILE_SESSIONS_PER_RUN = 100;
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const compiledSignature = JSON.stringify({
      ...
      compilerContract: "historical-signal-evaluator-v7-research-v2-batched",
      statementSignature,
    ...
    const savedCompiledMatches = Boolean(
      savedCompiled?.schema === COMPILED_CHECKPOINT_SCHEMA &&
        ...
        Array.isArray(savedCompiled?.dataset?.sessions),
    );
    const compiledCacheHit = Boolean(
      savedCompiledMatches &&
        savedCompiled?.complete === true &&
        savedCompiled.dataset.sessions.length === rawDataset.sessions.length,
    );
    let compiled = compiledCacheHit ? savedCompiled.dataset : null;
    if (!compiledCacheHit) {
      const compilation = compilePointInTimeSignals(rawDataset, {
        liquidity: { maxCandidates: 500 },
        maxSessions: COMPILE_SESSIONS_PER_RUN,
        resume:
          savedCompiledMatches && savedCompiled?.complete !== true
            ? {
                sessions: savedCompiled.dataset.sessions,
                completedSessions:
