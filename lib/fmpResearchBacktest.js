...
const REPLAY_CHECKPOINT_SCHEMA = 1;
const REPLAY_CANDIDATES_PER_RUN = 1;
...
async function runProvisionalWindows(
  dataset,
  {
    initial = null,
    onCheckpoint = null,
    maxCandidates = REPLAY_CANDIDATES_PER_RUN,
  } = {},
) {
  ...
  for (const candidate of asArray(initial?.candidateRuns)) {
    ...
    const validFolds =
      foldRuns.length === folds.length &&
      foldRuns.every(
        (foldRun, foldIndex) =>
          foldRun?.fold === folds[foldIndex].fold &&
  ...
  const boundedCandidates = boundedInteger(maxCandidates, 1, 0, grid.length);
  let computedCandidates = 0;
  for (let index = 0; index < grid.length; index++) {
    if (candidateRuns.has(index)) continue;
    if (computedCandidates >= boundedCandidates) break;
    ...
    candidateRuns.set(index, { index, parameters, foldRuns });
    computedCandidates++;
    if (onCheckpoint)
      await onCheckpoint({
        schema: REPLAY_CHECKPOINT_SCHEMA,
        completedCandidates: candidateRuns.size,
  ...
  // End an invocation after completing its bounded work, even when it finished
  // the last candidate. The next invocation performs selection and the full
  // diagnostic from a completely durable checkpoint rather than racing the
  // function timeout after a long candidate simulation.
  if (candidateRuns.size < grid.length || computedCandidates > 0)
    return {
      status: "collecting",
      progress: {
        completedCandidates: candidateRuns.size,
        totalCandidates: grid.length,
        remainingCandidates: grid.length - candidateRuns.size,
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const replaySignature = JSON.stringify({
      ...
      thesisContract: "v7-five-thesis-walk-forward-v1",
      statementSignature,
