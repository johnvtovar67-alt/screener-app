...
const POINT_IN_TIME_NASDAQ_ALPHA_R11_EXECUTION_CONTRACT =
  "nasdaq-r11-simulator-gates-statistics-v2";
...
const POINT_IN_TIME_NASDAQ_COMPILER_CONTRACT =
  "historical-signal-evaluator-v11-nasdaq-membership-removal-v1";
...
export async function getPointInTimeNasdaqUniverseStatus() {
  try {
    ...
    return report ? pointInTimeNasdaqUniverseSummary(report) : null;
  } catch (error) {
    return { status: "unavailable", error: sanitizedError(error) };
  }
}
...
export async function getPointInTimeNasdaqDatasetStatus() {
  try {
    return await readExactPrivateJson(POINT_IN_TIME_NASDAQ_DATASET_STATUS_STORE);
  } catch (error) {
    return { status: "unavailable", error: sanitizedError(error) };
  }
}
  ...
  const sessions = spy.map((benchmarkBar) => {
      ...
      if (change.removedSymbol) {
        ...
        if (wasMember && emitUniverseRemovalActions)
          removalsEffectiveToday.push({
            ...
            treatment: "next-observable-session-open",
          });
      }
    ...
    availableMembershipObservations += universeSymbols.filter(
      (symbol) =>
        ...
        indexedHistories.get(symbol)?.has(date),
    ).length;
    universeRemovalActions += removalsEffectiveToday.length;
    universeRemovalOpenPrices += removalsEffectiveToday.filter(
      (action) => number(indexedHistories.get(action.symbol)?.get(date)?.open) > 0,
    ).length;
    const prices = [];
    for (const [symbol, bars] of indexedHistories) {
      const bar = bars.get(date);
...
export async function runPointInTimeNasdaqUniverse({ force = false } = {}) {
  ...
  if (!force && existing?.status === "complete" && existing?.version === 1)
...
export async function runPointInTimeNasdaqDatasetAcquisition({
  ...
  const limitations = [
    ...
    "Every active member-date and every universe-removal open must have a usable adjusted price before any audit result can count.",
  ];
  const apiKey = process.env.FMP_API_KEY || process.env.FMP_KEY;
  if (!apiKey)
    throw new Error("The configured research provider key is unavailable");
...
async function compilePointInTimeNasdaqDataset({
  ...
  const compilationLimitations = [
    ...limitations,
    "A membership removal is realized at the first later observable session open with the same 12 bps execution slippage as every other sale.",
...
export async function runPointInTimeNasdaqPriceIntegrity({
  ...
  if (!manifest?.complete || !asArray(manifest?.chunks).length)
    throw new Error("The completed Nasdaq checkpoint is unavailable");
  const evidenceDatasetFingerprint =
  ...
  const report = {
    ...
    assessment: {
      ...
      activeMemberDateCoverageComplete:
        membershipObservationCoveragePct === 100,
      universeRemovalOpenCoverageComplete:
        universeRemovalOpenCoveragePct === 100,
...
async function pointInTimeNasdaqR11Context() {
  ...
  if (!manifest?.complete || !asArray(manifest?.chunks).length)
    throw new Error("The completed Nasdaq dataset is unavailable");
  const evidenceDatasetFingerprint =
    manifest.evidenceDatasetFingerprint;
  if (
    !validSha256(evidenceDatasetFingerprint) ||
...
export async function finalizePointInTimeNasdaqR11() {
  ...
  const report = {
      ...
      data: {
        ...
        universeRemovalOpenCoverageComplete:
          context.integrity.assessment
            .universeRemovalOpenCoverageComplete === true,
        membershipCardinalityPlausible:
    ...
    methodology: {
      ...
      universeRemovalExecution: "first-later-session-open",
  ...
  for (const dataset of asArray(datasets)) {
    ...
    for (const [localIndex, session] of sessions.entries()) {
      ...
      for (const action of asArray(session?.corporateActions)) {
        ...
        if (actionType === "universe-removal") {
          const actionSymbol = symbolOf(action);
          if (actionSymbol) activeSymbols.add(actionSymbol);
        }
      }
