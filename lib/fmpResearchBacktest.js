...
export async function getPointInTimeSp500AlphaCreator() {
  try {
    return await readPrivateJson(POINT_IN_TIME_SP500_ALPHA_CREATOR_STORE);
  } catch (error) {
    return { status: "unavailable", error: sanitizedError(error) };
  }
}
...
export async function runPointInTimeSp500DatasetAcquisition({
  ...
  return compilePointInTimeSp500Dataset({
    blueprint,
    histories,
    statementHistory,
    symbols,
...
async function compilePointInTimeSp500Dataset({
  ...
  const compiledSignature = JSON.stringify({
    schema: 1,
    compilerContract:
      "historical-signal-evaluator-v9-explicit-session-membership",
...
function pointInTimeAlphaPhaseSummary(runs = [], windows = []) {
  const aggregate = boundedReviewExperimentSummary(
    aggregateResearchRuns(runs),
  );
...
export async function runPointInTimeSp500AlphaCreator({
  ...
  const existing = await getPointInTimeSp500AlphaCreator();
  if (
    !force &&
  ...
  try {
    ...
    const datasetStatus = await getPointInTimeSp500DatasetStatus();
    ...
    const report = {
      ...
      dataQuality: {
        pointInTimeMembershipConstructed: true,
        membershipObservationCoveragePct: number(
          manifest.datasetMetadata?.membershipObservationCoveragePct,
        ),
    ...
    await persistPrivateJson(POINT_IN_TIME_SP500_ALPHA_CREATOR_STORE, report);
    return report;
  } catch (error) {
    const failed = {
      ...running,
