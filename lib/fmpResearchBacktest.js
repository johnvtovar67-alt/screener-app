...
async function readReport() {
  const { blobs } = await list({ prefix: FMP_RESEARCH_REPORT_STORE, limit: 1 });
  const blob =
    blobs.find((item) => item.pathname === FMP_RESEARCH_REPORT_STORE) ||
    blobs[0];
  if (!blob) return null;
...
async function persistReport(report) {
  await put(FMP_RESEARCH_REPORT_STORE, JSON.stringify(report), {
    access: "private",
    allowOverwrite: true,
      ...
      try {
        ...
        if (response.status === 429 && attempt === 0) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, Math.min(5_000, Math.max(1_000, retryAfter * 1_000))),
          );
          lastError = error;
          continue;
        }
...
function parameterScore(run = {}) {
  ...
  const tradePenalty = number(metrics.closedTrades, 0) < 8 ? 2 : 0;
  return (
    number(metrics.sharpe, -10) -
    Math.abs(Math.min(0, number(metrics.maxDrawdownPct, 0))) / 20 -
    tradePenalty
  );
}
...
function runProvisionalWindows(dataset) {
  ...
  const grid = [
    { buyTargetPct: 0.04, strongBuyTargetPct: 0.07, maxPositions: 12 },
    { buyTargetPct: 0.06, strongBuyTargetPct: 0.09, maxPositions: 10 },
    { buyTargetPct: 0.07, strongBuyTargetPct: 0.1, maxPositions: 8 },
  ];
  ...
  return {
    ...
    untouchedTest: metricsSummary(test),
    fullPeriodDiagnostic: metricsSummary(full),
  };
}

async function fetchStatementHistory(client, symbols, startYear, endYear) {
  const target = new Set(symbols);
  const output = { incomeRows: [], balanceRows: [], cashFlowRows: [] };
  const endpoints = [
    ["income-statement-bulk", "incomeRows"],
    ["balance-sheet-statement-bulk", "balanceRows"],
    ["cash-flow-statement-bulk", "cashFlowRows"],
  ];
  for (let year = startYear; year <= endYear; year++) {
    for (const period of ["Q1", "Q2", "Q3", "Q4"]) {
      for (const [path, key] of endpoints) {
        ...
        output[key].push(...rows.filter((row) => target.has(symbolOf(row))));
      }
    }
  }
  return output;
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const { histories, failures: priceFailures } = await fetchPriceHistory(
      client,
      priceSymbols,
      fromDate,
      endDate,
    ...
    const report = {
      ...
      methodology: {
        ...
        untouchedChronologicalTestWindow: true,
