...
import { pointInTimeNasdaqDistinctAlphaControls, pointInTimeNasdaqDistinctAlphaDefinitions } from "./nasdaqDistinctAlphaResearch";
import { pointInTimeNasdaqEventAlphaControls, pointInTimeNasdaqEventAlphaDefinitions } from "./nasdaqEventAlphaResearch";
...
const POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_PREFIX =
  "research/pit-nasdaq-adaptive-replacement-r40-r44-placebo";
const POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SEEDS = 1_000;
const POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SHARDS = 20;
const POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_CONTRACT =
  "r41-1000-seed-five-family-maximum-statistic-v1";
const POINT_IN_TIME_NASDAQ_R45_STORE = "research/pit-nasdaq-distinct-alpha-r45-r54.json";
const POINT_IN_TIME_NASDAQ_R45_PREFIX = "research/pit-nasdaq-distinct-alpha-r45-r54";
const POINT_IN_TIME_NASDAQ_R45_CONTRACT = "parallel-distinct-nasdaq-alpha-next-open-v1";
const POINT_IN_TIME_NASDAQ_R55_STORE = "research/pit-nasdaq-event-alpha-r55-r64.json";
const POINT_IN_TIME_NASDAQ_R55_PREFIX = "research/pit-nasdaq-event-alpha-r55-r64";
const POINT_IN_TIME_NASDAQ_R55_CONTRACT = "parallel-event-nasdaq-alpha-next-open-v1";
...
export async function getPointInTimeNasdaqDistinctAlphaR45() {
  try {
    return (await readExactPrivateJson(POINT_IN_TIME_NASDAQ_R45_STORE)) || { version: 45, researchGeneration: "R45-R54", status: "pending-development", productionChanged: false, eligibleForAlphaClaim: false, eligibleForLiveCapital: false };
  } catch (error) {
    return { version: 45, researchGeneration: "R45-R54", status: "unavailable", error: sanitizedError(error), productionChanged: false, eligibleForAlphaClaim: false, eligibleForLiveCapital: false };
  }
}

export async function getPointInTimeNasdaqEventAlphaR55() {
  try {
    return (await readExactPrivateJson(POINT_IN_TIME_NASDAQ_R55_STORE)) || { version: 55, researchGeneration: "R55-R64", status: "pending-development", productionChanged: false, eligibleForAlphaClaim: false, eligibleForLiveCapital: false };
  } catch (error) {
    return { version: 55, researchGeneration: "R55-R64", status: "unavailable", error: sanitizedError(error), productionChanged: false, eligibleForAlphaClaim: false, eligibleForLiveCapital: false };
  }
}
...
export async function getPointInTimeSp500AlphaProgram() {
  const eventAlpha = await getPointInTimeNasdaqEventAlphaR55();
  if (eventAlpha?.status && eventAlpha.status !== "unavailable") return eventAlpha;
  const distinctAlpha = await getPointInTimeNasdaqDistinctAlphaR45();
  if (distinctAlpha?.status && distinctAlpha.status !== "unavailable") return distinctAlpha;
...
export function canonicalIndexMembershipChanges(changes = []) {
  const normalized = asArray(changes)
    .map((row) => ({
      ...row,
      date: String(row.date || "").slice(0, 10),
      addedSymbol: pointInTimeSecuritySymbol(row.addedSymbol),
  ...
  const result = [];
  for (const [date, rows] of [...byDate].sort(([left], [right]) =>
    left.localeCompare(right),
  )) {
    const additions = new Set(rows.map((row) => row.addedSymbol).filter(Boolean));
    const removals = new Set(
      rows.map((row) => row.removedSymbol).filter(Boolean),
    );
    const unchangedIssuers = new Set(
      [...additions].filter((symbol) => removals.has(symbol)),
    );
    for (const row of rows) {
      const addedSymbol = unchangedIssuers.has(row.addedSymbol)
        ? ""
        : row.addedSymbol;
      const removedSymbol = unchangedIssuers.has(row.removedSymbol)
        ? ""
        : row.removedSymbol;
      if (addedSymbol || removedSymbol)
        result.push({ ...row, date, addedSymbol, removedSymbol });
    }
  }
  return result.sort(
    (left, right) =>
...
export function pointInTimeIndexRawDatasetFromHistory({
  blueprint,
  profiles,
  histories,
  fundamentals,
  source = "FMP historical index-membership research dataset",
  emitUniverseRemovalActions = false,
  membershipChangesEffectiveInclusive = false,
}) {
  const privateBlueprint = blueprint.privateBlueprint || {};
  const changes = canonicalIndexMembershipChanges(privateBlueprint.changes);
  ...
  const missingMembershipObservationsBySymbol = new Map();
  let universeRemovalActions = 0;
  let universeRemovalOpenPrices = 0;
  let universeRemovalZeroRecoveryActions = 0;
  let universeRemovalResolvedOutcomes = 0;
  let minimumMembershipCount = Infinity;
  let maximumMembershipCount = 0;
  const sessions = spy.map((benchmarkBar) => {
    const date = benchmarkBar.date;
    // Nasdaq supplies dateAdded/effectiveDate and opts into same-session
    // inclusion. Older generic callers retain their prior next-session policy.
    const removalsEffectiveToday = [];
    while (
      changeIndex < changes.length &&
      (membershipChangesEffectiveInclusive
        ? changes[changeIndex].date <= date
        : changes[changeIndex].date < date)
    ) {
      const change = changes[changeIndex++];
      if (change.removedSymbol) {
        const wasMember = members.delete(change.removedSymbol);
        if (wasMember && emitUniverseRemovalActions) {
          const row = indexedHistories.get(change.removedSymbol)?.get(date);
          const exactAdjustedOpen = usableAdjustedOhlcBar(row);
          universeRemovalActions++;
          universeRemovalResolvedOutcomes++;
          if (exactAdjustedOpen) universeRemovalOpenPrices++;
          else universeRemovalZeroRecoveryActions++;
          removalsEffectiveToday.push({
            symbol: change.removedSymbol,
            type: "universe-removal",
            effectiveDate: change.date,
            treatment: exactAdjustedOpen
              ? membershipChangesEffectiveInclusive
                ? "effective-session-adjusted-open"
                : "next-session-adjusted-open"
              : "conservative-zero-recovery-missing-removal-open-v1",
            ...(exactAdjustedOpen ? {} : { valuePerShare: 0 }),
          });
        }
      }
      if (change.addedSymbol) members.add(change.addedSymbol);
    ...
    for (const symbol of universeSymbols) {
      if (usableAdjustedOhlcBar(indexedHistories.get(symbol)?.get(date))) {
        availableMembershipObservations++;
        continue;
      }
      missingMembershipObservationsBySymbol.set(
        symbol,
        number(missingMembershipObservationsBySymbol.get(symbol), 0) + 1,
      );
    }
    const prices = [];
    for (const [symbol, bars] of indexedHistories) {
    ...
    return {
      ...
      prices,
      corporateActions: [
        ...(delistingActionsByDate.get(date) || []),
        ...removalsEffectiveToday,
      ],
    };
  });
  ...
  const membershipObservationCoveragePct = requestedMembershipObservations
    ...
    : 0;
  const missingMembershipObservations = Math.max(
    0,
    requestedMembershipObservations - availableMembershipObservations,
  );
  const activeMemberDateCoverageComplete =
    requestedMembershipObservations > 0 && missingMembershipObservations === 0;
  const universeRemovalOpenCoveragePct = universeRemovalActions
    ? Math.round(
        (universeRemovalOpenPrices / universeRemovalActions) * 10_000,
      ) / 100
    : 100;
  const universeRemovalOutcomeCoveragePct = universeRemovalActions
    ? Math.round(
        (universeRemovalResolvedOutcomes / universeRemovalActions) * 10_000,
      ) / 100
    : 100;
  const universeRemovalMissingOutcomes = Math.max(
    0,
    universeRemovalActions - universeRemovalResolvedOutcomes,
  );
  const universeRemovalOutcomeCoverageComplete =
    universeRemovalMissingOutcomes === 0;
  return {
    metadata: {
      pointInTime: true,
      ...
      comparisonSymbols: ["SPY", "QQQ"],
      source,
      membershipEffectiveConvention: membershipChangesEffectiveInclusive
        ? "effective-date-inclusive"
        : "next-session-after-change-date",
      securityIdentityContract: POINT_IN_TIME_SECURITY_IDENTITY_CONTRACT,
      ...
      missingMembershipObservationsBySymbol: [
        ...
        .map(([symbol, missing]) => ({ symbol, missing }))
        .sort(
          (left, right) =>
      ...
      activeMemberDateCoverageComplete,
      universeRemovalActions,
      universeRemovalOpenCoveragePct,
      universeRemovalZeroRecoveryActions,
      universeRemovalResolvedOutcomes,
      universeRemovalOutcomeCoveragePct,
      universeRemovalMissingOutcomes,
      universeRemovalOutcomeCoverageComplete,
      universeRemovalPolicy: membershipChangesEffectiveInclusive
        ? POINT_IN_TIME_NASDAQ_REMOVAL_POLICY
...
function pointInTimeSp500RawDatasetFromHistory(args) {
  return pointInTimeIndexRawDatasetFromHistory({
    ...args,
...
function parsedIndexDate(value) {
  const text = String(value || "").trim();
  if (!text) return null;
  if (/^\d{4}-\d{2}-\d{2}/.test(text)) {
    const date = text.slice(0, 10);
    return validIsoDay(date) ? date : null;
  }
  const timestamp = new Date(`${text} 12:00:00 UTC`).getTime();
  return Number.isFinite(timestamp)
    ? new Date(timestamp).toISOString().slice(0, 10)
    : null;
}

export function reconstructIndexInitialMembers(
  currentSymbols = [],
  changes = [],
  { fromDate, throughDate } = {},
) {
  const canonicalCurrent = new Set(
    asArray(currentSymbols).map(pointInTimeSecuritySymbol).filter(Boolean),
  );
  const ordered = canonicalIndexMembershipChanges(changes);
  const throughMembers = new Set(canonicalCurrent);
  for (const change of [...ordered]
    .filter((row) => throughDate && row.date > throughDate)
    .sort((left, right) => right.date.localeCompare(left.date))) {
    if (change.addedSymbol) throughMembers.delete(change.addedSymbol);
    if (change.removedSymbol) throughMembers.add(change.removedSymbol);
  }
  const initialMembers = new Set(throughMembers);
  for (const change of [...ordered]
    .filter(
      (row) =>
  ...
  return {
    throughSymbols: [...throughMembers].sort(),
    initialSymbols: [...initialMembers].sort(),
  };
}

function pointInTimeNasdaqUniverseSummary(report = {}) {
  return {
    version: report.version,
    status: report.status,
    universeContract: report.universeContract,
    completedAt: report.completedAt,
    fromDate: report.fromDate,
    throughDate: report.throughDate,
    providerUniverseLabel: report.providerUniverseLabel,
    currentConstituents: report.currentConstituents,
    throughConstituents: report.throughConstituents,
    initialConstituents: report.initialConstituents,
    normalizedMembershipChanges: report.normalizedMembershipChanges,
    membershipSupplementContract: report.membershipSupplementContract,
    membershipSupplementFingerprint:
      report.membershipSupplementFingerprint,
    providerEventCorrectionContract:
      report.providerEventCorrectionContract,
    providerEventCorrectionFingerprint:
      report.providerEventCorrectionFingerprint,
    correctedProviderEvents: report.correctedProviderEvents,
    membershipEffectiveConvention: report.membershipEffectiveConvention,
    supplementalMembershipEvents: report.supplementalMembershipEvents,
    unionSymbols: report.unionSymbols,
    earliestMembershipEvent: report.earliestMembershipEvent,
    latestMembershipEvent: report.latestMembershipEvent,
    effectiveDateDisagreements: report.effectiveDateDisagreements,
    currentAnchorCardinalityPlausible:
      report.currentAnchorCardinalityPlausible === true,
    pointInTimeMembershipConstructed:
      report.pointInTimeMembershipConstructed === true,
    rawMembershipDigest: report.rawMembershipDigest,
    productionChanged: false,
    eligibleForAlphaClaim: false,
    nextStep: report.nextStep,
    limitations: report.limitations,
...
export async function runPointInTimeNasdaqUniverse({ force = false } = {}) {
  const existing = await getPointInTimeNasdaqUniverse().catch(() => null);
  if (!force && currentPointInTimeNasdaqUniverse(existing))
    return pointInTimeNasdaqUniverseSummary(existing);
  const apiKey = process.env.FMP_API_KEY || process.env.FMP_KEY;
  if (!apiKey)
    throw new Error("The configured research provider key is unavailable");
  const client = createFmpClient(apiKey);
  const [currentRows, changeRows] = await Promise.all([
    client.fetchStable("nasdaq-constituent", {}),
    client.fetchStable("historical-nasdaq-constituent", {}),
  ]);
  const normalize = (value) => pointInTimeSecuritySymbol(value);
  const currentSymbols = [
    ...new Set(currentRows.map((row) => normalize(row.symbol)).filter(Boolean)),
  ].sort();
  const providerChanges = changeRows
    .map((row) => {
      const dateAdded = parsedIndexDate(row.dateAdded);
      const announcementDate = parsedIndexDate(row.date);
      const effectiveDate = dateAdded || announcementDate;
      return {
        date: effectiveDate,
        effectiveDate,
        announcementDate,
        effectiveDateBasis: dateAdded ? "dateAdded" : "provider-date-fallback",
        provenance: "FMP historical-nasdaq-constituent",
        addedSymbol: normalize(
          row.addedSymbol || row.addedTicker || row.symbol,
        ),
        removedSymbol: normalize(row.removedSymbol || row.removedTicker),
        addedSecurity: String(
          row.addedSecurity || row.addedName || "",
        ).trim(),
        removedSecurity: String(
          row.removedSecurity || row.removedName || "",
        ).trim(),
  ...
  const unresolvedEffectiveDates = providerChanges.filter(
    (row) =>
      row.effectiveDateBasis !== "dateAdded" &&
      row.date >= POINT_IN_TIME_NASDAQ_RESEARCH_FROM,
  );
  if (unresolvedEffectiveDates.length)
    throw new Error(
      `Nasdaq membership has ${unresolvedEffectiveDates.length} reconstruction-relevant event dates without dateAdded; refusing to treat announcement dates as effective dates`,
    );
  const supplementalChanges = POINT_IN_TIME_NASDAQ_MEMBERSHIP_SUPPLEMENTS.map(
    (row) => ({
      ...row,
      addedSymbol: normalize(row.addedSymbol),
      removedSymbol: normalize(row.removedSymbol),
      effectiveDateBasis: "source-verified-supplement",
      sourceVerifiedSupplement: true,
    }),
  );
  const {
    providerChanges: correctedProviderChanges,
    correctionChanges,
  } = pointInTimeNasdaqApplyProviderEventCorrections(providerChanges);
  const supplementConflict = pointInTimeNasdaqMembershipSupplementConflict(
    correctedProviderChanges,
    supplementalChanges,
  );
  if (supplementConflict)
    throw new Error(
      `Nasdaq membership supplement conflicts with provider event for ${supplementConflict.supplement.addedSymbol || supplementConflict.supplement.removedSymbol}: ${supplementConflict.supplement.date}/${supplementConflict.supplement.removedSymbol || "none"} versus ${supplementConflict.provider.date}/${supplementConflict.provider.removedSymbol || "none"}`,
    );
  ...
  for (const row of [
    ...
    const key = `${row.date}|${row.addedSymbol}|${row.removedSymbol}`;
    const prior = changesByIdentity.get(key);
      ...
      prior
        ? row.effectiveDateBasis === "source-verified-supplement"
          ? {
              ...
              ...row,
              sourceVerifiedSupplement: true,
              providerProvenance: prior.provenance,
            }
          : prior
        : row,
    );
  }
  const deduplicatedChanges = [...changesByIdentity.values()].sort(
    (left, right) =>
      left.date.localeCompare(right.date) ||
      left.addedSymbol.localeCompare(right.addedSymbol) ||
      left.removedSymbol.localeCompare(right.removedSymbol),
  );
  const earliestMembershipEvent = deduplicatedChanges[0]?.date || null;
  const latestMembershipEvent = deduplicatedChanges.at(-1)?.date || null;
  const currentAnchorCardinalityPlausible =
    currentSymbols.length >= 90 && currentSymbols.length <= 125;
  if (!currentAnchorCardinalityPlausible)
    throw new Error(
      `FMP Nasdaq anchor cardinality is not index-like (${currentSymbols.length}); refusing to infer Nasdaq-100 membership`,
    );
  if (
    deduplicatedChanges.length < 40 ||
    !earliestMembershipEvent ||
    earliestMembershipEvent > POINT_IN_TIME_NASDAQ_RESEARCH_FROM
  )
    throw new Error(
      `Historical Nasdaq membership coverage is insufficient (${deduplicatedChanges.length} events from ${earliestMembershipEvent || "unknown"})`,
    );
  const reconstructed = reconstructIndexInitialMembers(
    currentSymbols,
    deduplicatedChanges,
    {
      fromDate: POINT_IN_TIME_NASDAQ_RESEARCH_FROM,
  ...
  if (
    reconstructed.throughSymbols.length < 85 ||
    reconstructed.throughSymbols.length > 125 ||
    reconstructed.initialSymbols.length < 85 ||
    reconstructed.initialSymbols.length > 125
  )
    throw new Error(
      `Reconstructed Nasdaq membership is implausible (${reconstructed.initialSymbols.length} initial, ${reconstructed.throughSymbols.length} through)`,
    );
  const relevantChanges = canonicalIndexMembershipChanges(
    deduplicatedChanges.filter(
      (row) =>
        row.date >= POINT_IN_TIME_NASDAQ_RESEARCH_FROM &&
        row.date <= POINT_IN_TIME_NASDAQ_RESEARCH_THROUGH,
    ),
  ...
  const union = new Set([
    ...reconstructed.initialSymbols,
    ...reconstructed.throughSymbols,
  ]);
  for (const change of relevantChanges) {
    if (change.addedSymbol) union.add(change.addedSymbol);
    if (change.removedSymbol) union.add(change.removedSymbol);
  ...
  const addProfile = (symbol, row = {}, name = "") => {
    const canonical = normalize(symbol);
    if (!canonical || !union.has(canonical)) return;
    profilesBySymbol[canonical] = {
      ...(profilesBySymbol[canonical] || {}),
      symbol: canonical,
      name:
        profilesBySymbol[canonical]?.name ||
        name ||
        row.name ||
        row.companyName ||
        canonical,
      companyName:
        profilesBySymbol[canonical]?.companyName ||
        name ||
        row.name ||
        row.companyName ||
        canonical,
      sector:
        profilesBySymbol[canonical]?.sector || row.sector || "Other",
      industry:
        profilesBySymbol[canonical]?.industry ||
        row.subSector ||
        row.industry ||
        "Other",
    };
  };
  for (const row of currentRows) addProfile(row.symbol, row);
  for (const row of relevantChanges) {
    addProfile(row.addedSymbol, {}, row.addedSecurity);
    addProfile(row.removedSymbol, {}, row.removedSecurity);
  }
  for (const symbol of union) addProfile(symbol);
  const rawMembershipDigest = pointInTimeNasdaqMembershipDigest({
    currentSymbols,
    throughSymbols: reconstructed.throughSymbols,
    initialSymbols: reconstructed.initialSymbols,
    changes: relevantChanges,
    membershipSupplements: supplementalChanges,
    providerEventCorrections: correctionChanges,
    unionSymbols: [...union].sort(),
    profilesBySymbol,
    delistedDates: {},
    fromDate: POINT_IN_TIME_NASDAQ_RESEARCH_FROM,
  ...
  const membershipSupplementFingerprint =
    pointInTimeNasdaqMembershipSupplementFingerprint();
  const providerEventCorrectionFingerprint =
    pointInTimeNasdaqProviderEventCorrectionFingerprint();
  const report = {
    ...
    providerEventCorrectionContract:
      POINT_IN_TIME_NASDAQ_PROVIDER_EVENT_CORRECTION_CONTRACT,
    providerEventCorrectionFingerprint,
    correctedProviderEvents: correctionChanges.length,
    membershipEffectiveConvention: "effective-date-inclusive",
    supplementalMembershipEvents: supplementalChanges.length,
    completedAt: new Date().toISOString(),
    fromDate: POINT_IN_TIME_NASDAQ_RESEARCH_FROM,
    throughDate: POINT_IN_TIME_NASDAQ_RESEARCH_THROUGH,
    providerUniverseLabel: "FMP Nasdaq index (cardinality-checked)",
    currentConstituents: currentSymbols.length,
    throughConstituents: reconstructed.throughSymbols.length,
    initialConstituents: reconstructed.initialSymbols.length,
    normalizedMembershipChanges: relevantChanges.length,
    unionSymbols: union.size,
    earliestMembershipEvent: relevantChanges[0]?.date || null,
    latestMembershipEvent: relevantChanges.at(-1)?.date || null,
    effectiveDateDisagreements: deduplicatedChanges.filter(
      (row) =>
        row.announcementDate && row.announcementDate !== row.effectiveDate,
    ).length,
    currentAnchorCardinalityPlausible,
    pointInTimeMembershipConstructed: true,
    rawMembershipDigest,
    ...
    nextStep:
      "Acquire one adjusted price history per union member and compile member-date signals with explicit effective-session removal actions.",
    limitations: [
      "FMP documents this as the Nasdaq index but does not unambiguously identify NDX; cardinality is checked and the provider label is preserved rather than overstated.",
      "dateAdded is treated as the effective membership date and is included on that market session; announcement-date fallbacks are rejected.",
      "HONA's omitted spin-off addition is supplied from the 2026 Nasdaq NDX methodology and Nasdaq Corporate Action ECA2026-399, not inferred from price availability or aliased to HON.",
      "FMP's early-dated SOLS and VSNT removals are corrected only to each security's first absent official Nasdaq NDX start-of-day weight date.",
      "Historical sector labels are unavailable, so R11 does not use sector-relative ranks or sector constraints.",
      "This is isolated research infrastructure and cannot alter V11 production recommendations.",
    ],
  };
  if (!currentPointInTimeNasdaqUniverse(report))
    throw new Error(
      `Generated Nasdaq membership failed its own structural replay and digest validation: ${JSON.stringify(pointInTimeNasdaqUniverseDiagnostics(report))}`,
    );
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_UNIVERSE_STORE, report);
  return pointInTimeNasdaqUniverseSummary(report);
}

function pointInTimeNasdaqDatasetStatusSummary(report = {}) {
  return {
    version: report.version,
    status: report.status,
    startedAt: report.startedAt,
    ...
    progress: report.progress,
    coverage: report.coverage,
    datasetContractFingerprint: report.datasetContractFingerprint,
    priceAcquisitionContract: report.priceAcquisitionContract,
    rawMembershipDigest: report.rawMembershipDigest,
    membershipSupplementFingerprint:
      report.membershipSupplementFingerprint,
    priceContract: report.priceContract,
...
function reusablePriceCheckpoint(checkpoint, { fromDate, endDate } = {}) {
  const signature = parseSignature(checkpoint?.signature);
  const contract = PRICE_HISTORY_CONTRACTS.find(
    (candidate) => candidate.id === signature?.priceContract,
  );
  const benchmarkBars = asArray(checkpoint?.histories?.SPY);
  if (
    !contract ||
    signature?.fromDate !== fromDate ||
    signature?.endDate !== endDate ||
    benchmarkBars.length < 500 ||
    !benchmarkBars.every((bar) => bar.adjusted === true)
  )
    return null;
  return { contract, benchmarkBars };
}

function subsetPriceCheckpoint(checkpoint, symbols = []) {
  if (!checkpoint) return null;
  const allowed = new Set(symbols.map(symbolOf));
  const histories = Object.fromEntries(
    Object.entries(checkpoint.histories || {}).filter(([symbol]) =>
      allowed.has(symbolOf(symbol)),
    ),
  );
  const completedSymbols = asArray(checkpoint.completedSymbols)
    .map(symbolOf)
    .filter((symbol) => allowed.has(symbol) && histories[symbol]);
  const exhaustedSymbols = asArray(checkpoint.exhaustedSymbols)
    .map(symbolOf)
  ...
  return {
    ...
    attempts: Object.fromEntries(
      Object.entries(checkpoint.attempts || {}).filter(([symbol]) =>
        allowed.has(symbolOf(symbol)),
      ),
    ),
    priceFailures: asArray(checkpoint.priceFailures).filter((failure) =>
      allowed.has(symbolOf(failure?.symbol || failure?.item)),
    ),
  };
}

function pointInTimeNasdaqPriceAcquisitionSignatureMatches(
  value,
  {
    fromDate,
    endDate,
  ...
  return Boolean(
    signature?.schema === PRICE_ACQUISITION_SCHEMA &&
      signature?.priceAcquisitionContract ===
        POINT_IN_TIME_NASDAQ_PRICE_ACQUISITION_CONTRACT &&
      signature?.rawMembershipDigest === rawMembershipDigest &&
      signature?.membershipSupplementFingerprint ===
        membershipSupplementFingerprint &&
      signature?.fromDate === fromDate &&
      signature?.endDate === endDate &&
      signature?.priceContract === priceContract &&
      sameStringArray(signature?.symbols, priceSymbols),
  );
}

export async function runPointInTimeNasdaqDatasetAcquisition({
  now = Date.now(),
} = {}) {
  let blueprint = await getPointInTimeNasdaqUniverse().catch(() => null);
  if (!currentPointInTimeNasdaqUniverse(blueprint)) {
    await runPointInTimeNasdaqUniverse();
    blueprint = await getPointInTimeNasdaqUniverse();
  }
  const unionSymbols = asArray(blueprint?.privateBlueprint?.unionSymbols)
    .map(symbolOf)
    .filter(Boolean);
  if (
    blueprint?.pointInTimeMembershipConstructed !== true ||
    unionSymbols.length < 90
  )
    throw new Error("A complete private Nasdaq membership blueprint is required");
  const previousStatus = await getPointInTimeNasdaqDatasetStatus();
  const priorClaim = new Date(previousStatus?.runClaimedAt || 0).getTime();
  if (
    previousStatus?.status === "collecting" &&
    previousStatus?.progress?.stage === "compiling" &&
    previousStatus?.runClaimedAt &&
    Number.isFinite(priorClaim) &&
    now - priorClaim < POINT_IN_TIME_SP500_COMPILATION_CLAIM_TTL_MS
  )
    return pointInTimeNasdaqDatasetStatusSummary(previousStatus);
  const startedAt =
    previousStatus?.version === 1 && previousStatus?.startedAt
      ? previousStatus.startedAt
      : new Date(now).toISOString();
  ...
  const endDate = String(blueprint.throughDate || "").slice(0, 10);
  const limitations = [
    ...asArray(blueprint.limitations),
    "R11 is price-only; unavailable revision-safe fundamentals and historical news are not filled with current values.",
    "Every active member-date must have a usable adjusted price; each removal must resolve at the fixed effective-session adjusted open or an explicitly disclosed conservative zero recovery before any audit result can count.",
  ];
  const rawMembershipDigest = blueprint.rawMembershipDigest;
  const membershipSupplementFingerprint =
    blueprint.membershipSupplementFingerprint;
  const apiKey = process.env.FMP_API_KEY || process.env.FMP_KEY;
  if (!apiKey)
    throw new Error("The configured research provider key is unavailable");
  const client = createFmpClient(apiKey);
  ...
  const [savedNasdaqPrices, savedSp500Prices] = await Promise.all([
    readExactPrivateJson(POINT_IN_TIME_NASDAQ_PRICE_CHECKPOINT_STORE).catch(
      () => null,
    ),
    readExactPrivateJson(POINT_IN_TIME_SP500_PRICE_CHECKPOINT_STORE).catch(
      () => null,
    ),
  ]);
  const reusableNasdaq = reusablePriceCheckpoint(savedNasdaqPrices, {
    fromDate,
    endDate,
  });
  const reusableSp500 = reusablePriceCheckpoint(savedSp500Prices, {
    fromDate,
    endDate,
  });
  const reusable = reusableNasdaq || reusableSp500;
  const priceContractResult = reusable
    ? {
        contract: reusable.contract,
        benchmarkBars: reusable.benchmarkBars,
        failures: [],
        checkpointReused: true,
      }
    : await resolvePriceHistoryContract(client, fromDate, endDate);
  ...
  const savedNasdaqMatches =
    pointInTimeNasdaqPriceAcquisitionSignatureMatches(
      savedNasdaqPrices?.signature,
      {
        fromDate,
        endDate,
        priceContract: priceContract.id,
  ...
  const seedSource = savedNasdaqMatches
    ? savedNasdaqPrices
    : reusableSp500?.contract?.id === priceContract.id
      ? savedSp500Prices
      : null;
  const seeded = subsetPriceCheckpoint(seedSource, priceSymbols) || {};
  const priceHistory = await fetchPriceHistory(
    client,
    priceContract,
    priceSymbols,
    ...
    endDate,
    {
      initial: {
        ...
        histories: {
          ...(seeded.histories || {}),
          SPY:
            seeded.histories?.SPY || priceContractResult.benchmarkBars,
        },
        completedSymbols: [
          ...new Set([
            "SPY",
            ...asArray(seeded.completedSymbols).map(symbolOf),
          ]),
        ],
      },
      maxSymbols: PRICE_SYMBOLS_PER_RUN,
      onCheckpoint: (checkpoint) =>
        persistPrivateJson(POINT_IN_TIME_NASDAQ_PRICE_CHECKPOINT_STORE, {
          version: PRICE_ACQUISITION_SCHEMA,
          signature: acquisitionSignature,
          updatedAt: new Date().toISOString(),
          seededFromSp500Checkpoint:
            seedSource === savedSp500Prices && Boolean(savedSp500Prices),
          ...checkpoint,
        }),
    },
  );
  if (priceHistory.remainingSymbols.length) {
    ...
    await persistPrivateJson(
      POINT_IN_TIME_NASDAQ_DATASET_STATUS_STORE,
      collecting,
    );
    return pointInTimeNasdaqDatasetStatusSummary(collecting);
  }
  const histories = new Map(Object.entries(priceHistory.histories || {}));
  if (!histories.has("SPY") || !histories.has("QQQ"))
    throw new Error("SPY and QQQ benchmark histories are required");
  const canonicalPriceHistories =
    pointInTimeCanonicalPriceHistories(histories);
  const usableSymbols = symbols.filter(
    (symbol) => (canonicalPriceHistories.get(symbol) || []).length >= 20,
  );
  if (usableSymbols.length < Math.floor(symbols.length * 0.9))
    throw new Error(
      `Nasdaq price coverage is insufficient to compile (${usableSymbols.length}/${symbols.length})`,
    );
  const runClaimedAt = new Date(now).toISOString();
  const compiling = {
    version: 1,
    ...
    coverage: {
      membershipUnionSymbols: symbols.length,
      priceUsableSymbols: usableSymbols.length,
      missingPriceSymbols: symbols.length - usableSymbols.length,
      membershipChanges: number(blueprint.normalizedMembershipChanges, 0),
    },
    pointInTimeMembershipConstructed: true,
    productionChanged: false,
    ...
    nextStep: "Compile the next isolated Nasdaq member-date signal chunk.",
    limitations,
  };
  await persistPrivateJson(
    POINT_IN_TIME_NASDAQ_DATASET_STATUS_STORE,
  ...
  try {
    return await compilePointInTimeNasdaqDataset({
      blueprint,
      histories,
      symbols,
      usableSymbols,
    ...
    await persistPrivateJson(POINT_IN_TIME_NASDAQ_DATASET_STATUS_STORE, {
      ...compiling,
      status: "ready-to-compile",
      updatedAt: new Date().toISOString(),
      runClaimedAt: null,
...
async function compilePointInTimeNasdaqDataset({
  blueprint,
  histories,
  symbols,
  usableSymbols,
  ...
  const usableSet = new Set(canonicalUsableSymbols);
  const canonicalHistories = pointInTimeCanonicalPriceHistories(histories);
  const profilesBySymbol = privateBlueprint.profilesBySymbol || {};
  const profiles = canonicalUsableSymbols.map((symbol) => {
    const source = profilesBySymbol[symbol] || {};
    return {
      ...source,
      symbol,
      name: source.name || source.companyName || symbol,
      companyName: source.companyName || source.name || symbol,
      // Current sector classifications are descriptive only. Every R11
      // definition disables sector ranks and sector concentration gates.
      sector: source.sector || "Other",
      // Nasdaq eligibility is determined by the session membership set. This
      // field is only the dataset boundary required by the shared compiler; it
      // must not pretend that a vendor history's first bar proves a listing.
      listedAt: blueprint.fromDate,
      delistedAt: null,
      isEtf: false,
      isFund: false,
    };
  ...
  const rawDataset = pointInTimeIndexRawDatasetFromHistory({
    blueprint,
    profiles,
    histories: finalHistories,
    fundamentals: [],
    source: "FMP historical Nasdaq index membership price-only dataset",
    emitUniverseRemovalActions: true,
    membershipChangesEffectiveInclusive: true,
  });
  if (rawDataset.metadata.activeMemberDateCoverageComplete !== true) {
    const largestGaps = asArray(
      rawDataset.metadata.missingMembershipObservationsBySymbol,
    )
      .slice(0, 10)
      .map((row) => `${row.symbol}:${row.missing}`)
      .join(",");
    throw new Error(
      `Nasdaq active member-date price coverage is incomplete (${rawDataset.metadata.missingMembershipObservations} missing; ${largestGaps || "unattributed"}); refusing to compile signals`,
    );
  }
  const sessionDates = rawDataset.sessions.map((session) => session.date);
  const expectedSessionDates = pointInTimeNasdaqExpectedSessionDates();
  if (
    ...
    throw new Error(
      `Nasdaq benchmark calendar is incomplete or unexpected (${sessionDates.length}/${expectedSessionDates.length} sessions)`,
    );
  const priceInputFingerprint = pointInTimePriceInputFingerprint(finalHistories);
  const compiledSignaturePayload = {
    schema: 1,
    compilerContract: POINT_IN_TIME_NASDAQ_COMPILER_CONTRACT,
    minimumSignalHistoryRows: 253,
    universeContract: POINT_IN_TIME_NASDAQ_UNIVERSE_CONTRACT,
    ...
    universeRemovalPolicy: POINT_IN_TIME_NASDAQ_REMOVAL_POLICY,
    securityIdentityContract: POINT_IN_TIME_SECURITY_IDENTITY_CONTRACT,
    rawMembershipDigest: privateBlueprint.rawMembershipDigest,
    fromDate: blueprint.fromDate,
    endDate: blueprint.throughDate,
    requestedSymbols: canonicalSymbols.slice().sort(),
    usableSymbols: canonicalUsableSymbols.slice().sort(),
    sessionDates,
    priceContract: priceContract.id,
    priceInputFingerprint,
  };
  const compiledSignature = JSON.stringify(compiledSignaturePayload);
  const datasetFingerprint = sha256Fingerprint(compiledSignature);
  const savedCompiled = await readExactPrivateJson(
    POINT_IN_TIME_NASDAQ_COMPILED_CHECKPOINT_STORE,
  ).catch(() => null);
  const savedChunks = asArray(savedCompiled?.chunks);
  let expectedChunkStart = 0;
  const chunkManifestValid = savedChunks.every((chunk) => {
    ...
    const valid =
      ...
      typeof chunk?.pathname === "string" &&
      chunk.pathname.startsWith(
        `${POINT_IN_TIME_NASDAQ_COMPILED_CHUNK_PREFIX}/${datasetFingerprint}/`,
      ) &&
      /^[a-f0-9]{64}$/.test(String(chunk?.contentSha256 || ""));
    if (valid) expectedChunkStart = end;
    return valid;
  });
  const savedCompiledMatches = Boolean(
    savedCompiled?.schema === 1 &&
      savedCompiled?.signature === compiledSignature &&
      savedCompiled?.datasetFingerprint === datasetFingerprint &&
      savedCompiled?.priceInputFingerprint === priceInputFingerprint &&
      savedCompiled?.evidenceDatasetFingerprint ===
        nasdaqEvidenceDatasetFingerprint(datasetFingerprint, savedChunks) &&
      chunkManifestValid &&
      number(savedCompiled?.completedSessions, -1) === expectedChunkStart,
  );
  const compiledCacheHit = Boolean(
  ...
  const baseCoverage = {
    ...
    priceUsableSymbols: canonicalUsableSymbols.length,
    missingPriceSymbols:
      canonicalSymbols.length - canonicalUsableSymbols.length,
    membershipChanges: number(blueprint.normalizedMembershipChanges, 0),
    membershipSupplementContract:
      POINT_IN_TIME_NASDAQ_MEMBERSHIP_SUPPLEMENT_CONTRACT,
    supplementalMembershipEvents: number(
      blueprint.supplementalMembershipEvents,
      0,
    ),
    membershipObservationCoveragePct:
      rawDataset.metadata.membershipObservationCoveragePct,
    requestedMembershipObservations:
      rawDataset.metadata.requestedMembershipObservations,
    availableMembershipObservations:
      rawDataset.metadata.availableMembershipObservations,
    missingMembershipObservations:
      rawDataset.metadata.missingMembershipObservations,
    missingMembershipObservationsBySymbol:
      rawDataset.metadata.missingMembershipObservationsBySymbol,
    activeMemberDateCoverageComplete:
      rawDataset.metadata.activeMemberDateCoverageComplete,
    universeRemovalActions: rawDataset.metadata.universeRemovalActions,
    universeRemovalOpenCoveragePct:
      rawDataset.metadata.universeRemovalOpenCoveragePct,
    universeRemovalZeroRecoveryActions:
      rawDataset.metadata.universeRemovalZeroRecoveryActions,
    universeRemovalResolvedOutcomes:
      rawDataset.metadata.universeRemovalResolvedOutcomes,
    universeRemovalOutcomeCoveragePct:
      rawDataset.metadata.universeRemovalOutcomeCoveragePct,
    universeRemovalMissingOutcomes:
      rawDataset.metadata.universeRemovalMissingOutcomes,
    universeRemovalOutcomeCoverageComplete:
      rawDataset.metadata.universeRemovalOutcomeCoverageComplete,
    universeRemovalPolicy: rawDataset.metadata.universeRemovalPolicy,
    minimumMembershipCount: rawDataset.metadata.minimumMembershipCount,
    maximumMembershipCount: rawDataset.metadata.maximumMembershipCount,
    totalSessions: sessionDates.length,
  };
  const compilationLimitations = [
    ...limitations,
    "A membership removal uses its effective-session adjusted open with 12 bps slippage; if that exact open is unavailable, the held position receives an immediate conservative zero recovery and exact-open coverage remains disclosed.",
    "A Nasdaq security emits no signal until 253 adjusted observations exist; integrity then checks that exact consumed history and fails on any internal market-session gap.",
    "R11 rankings use only adjusted prices, point-in-time membership, and benchmark histories; current profile sectors cannot affect selection.",
  ];
  if (compiledCacheHit) {
    const evidenceDatasetFingerprint =
      savedCompiled.evidenceDatasetFingerprint;
    const complete = {
      version: 1,
      status: "compiled",
      ...
      coverage: { ...baseCoverage, compiledSessions: sessionDates.length },
      datasetFingerprint: evidenceDatasetFingerprint,
      datasetInputFingerprint: datasetFingerprint,
      priceInputFingerprint,
      pointInTimeMembershipConstructed: true,
      productionChanged: false,
      eligibleForAlphaClaim: false,
      ...
      nextStep: "Run and pass the exact member-date price-integrity audit.",
      limitations: compilationLimitations,
    };
    await persistPrivateJson(
      POINT_IN_TIME_NASDAQ_DATASET_STATUS_STORE,
    ...
    return pointInTimeNasdaqDatasetStatusSummary(complete);
  }
  const completedBefore = savedCompiledMatches ? expectedChunkStart : 0;
  const compilation = compilePointInTimeSignals(rawDataset, {
    liquidity: { maxCandidates: 150 },
    minimumHistoryRows: 253,
    maxSessions: POINT_IN_TIME_NASDAQ_COMPILE_SESSIONS_PER_RUN,
    resume: savedCompiledMatches
      ? {
          sessions: [],
          completedSessions:
  ...
  compiledDataset.metadata = {
    ...
    minimumMembershipCount: rawDataset.metadata.minimumMembershipCount,
    maximumMembershipCount: rawDataset.metadata.maximumMembershipCount,
    membershipEffectiveConvention:
      rawDataset.metadata.membershipEffectiveConvention,
    compilerContract: POINT_IN_TIME_NASDAQ_COMPILER_CONTRACT,
    datasetFingerprint,
  };
  const chunkEnd = compilerProgress.completedSessions;
  if (chunkEnd <= completedBefore)
    throw new Error("Nasdaq point-in-time compilation did not advance");
  const chunkPayload = {
    metadata: compiledDataset.metadata,
    sessions: compiledDataset.sessions.map(compactReplaySession),
  };
  const contentSha256 = sha256Fingerprint(JSON.stringify(chunkPayload));
  const chunkPath = `${POINT_IN_TIME_NASDAQ_COMPILED_CHUNK_PREFIX}/${datasetFingerprint}/${String(completedBefore).padStart(4, "0")}-${String(chunkEnd).padStart(4, "0")}.json.gz`;
  const compressedBytes = await persistPrivateGzipJson(
    chunkPath,
    chunkPayload,
  );
  const chunks = [
  ...
  const evidenceDatasetFingerprint = nasdaqEvidenceDatasetFingerprint(
    datasetFingerprint,
    chunks,
  );
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_COMPILED_CHECKPOINT_STORE, {
    schema: 1,
    signature: compiledSignature,
    datasetFingerprint,
    evidenceDatasetFingerprint,
    priceInputFingerprint,
    complete: compilerProgress.complete,
    completedSessions: chunkEnd,
    completedAt: compilerProgress.complete ? new Date().toISOString() : null,
  ...
  const collecting = {
    ...
    coverage: { ...baseCoverage, compiledSessions: chunkEnd },
    datasetFingerprint: evidenceDatasetFingerprint,
    datasetInputFingerprint: datasetFingerprint,
    priceInputFingerprint,
    pointInTimeMembershipConstructed: true,
    ...
    nextStep: compilerProgress.complete
      ? "Run and pass the exact member-date price-integrity audit."
      : "Continue bounded Nasdaq member-date compilation.",
    limitations: compilationLimitations,
  };
  await persistPrivateJson(
    POINT_IN_TIME_NASDAQ_DATASET_STATUS_STORE,
...
async function verifiedNasdaqChunk(chunk) {
  const payload = await readExactPrivateGzipJson(chunk.pathname);
  if (
    asArray(payload?.sessions).length !==
      number(chunk.end) - number(chunk.start) ||
    payload?.sessions?.[0]?.date !== chunk.firstDate ||
    payload?.sessions?.at(-1)?.date !== chunk.lastDate ||
    sha256Fingerprint(JSON.stringify(payload)) !== chunk.contentSha256
  )
    throw new Error(`Nasdaq compiled chunk hash mismatch: ${chunk.pathname}`);
  return {
    ...payload,
    sessions: asArray(payload.sessions).map(compactReplaySession),
  };
}

export async function runPointInTimeNasdaqPriceIntegrity({
  force = false,
} = {}) {
  const [manifest, datasetStatus, universe] = await Promise.all([
  ...
  const earnings = await readExactPrivateJson(
    POINT_IN_TIME_NASDAQ_EARNINGS_SURPRISE_STORE,
  );
  if (
    earnings?.status !== "complete" ||
    earnings?.datasetThrough !== asArray(manifest.sessionDates).at(-1) ||
    number(earnings?.coveragePct, 0) < 80
  )
    throw new Error(
      "R13 is blocked until point-in-time Nasdaq earnings coverage is complete",
    );
  const evidenceDatasetFingerprint =
    manifest.evidenceDatasetFingerprint;
  if (
    !validSha256(evidenceDatasetFingerprint) ||
    evidenceDatasetFingerprint !==
      nasdaqEvidenceDatasetFingerprint(
        manifest.datasetFingerprint,
        manifest.chunks,
      )
  )
    throw new Error("The Nasdaq manifest content fingerprint is invalid");
  const existing = await getPointInTimeNasdaqPriceIntegrity();
  if (
    !force &&
    currentPointInTimeNasdaqIntegrity(
      existing,
  ...
  // Two streaming passes keep only one decompressed chunk resident. The first
  // builds exact active/lookback ranges; the second scans prices while carrying
  // prior closes across chunk boundaries.
  const requirements = createPointInTimePriceRequirements({
    lookbackSessions: 253,
    requireExactObservedSignalHistory: true,
  });
  let sessionIndexOffset = 0;
  for (const chunk of manifest.chunks) {
    const dataset = await verifiedNasdaqChunk(chunk);
    observePointInTimePriceRequirements(requirements, [dataset], {
      sessionIndexOffset,
    });
    sessionIndexOffset += asArray(dataset.sessions).length;
  }
  finishPointInTimePriceRequirements(requirements);
  ...
  const audit = finishPointInTimePriceIntegrityAudit(accumulator);
  ...
  const missingMembershipObservationsBySymbol = asArray(
    audit.missingMembershipObservationsBySymbol,
  );
  const activeMemberDateCoverageComplete = Boolean(
    audit.nasdaqMembershipEvidenceApplied === true &&
      audit.activeMemberDateCoverageComplete === true &&
      requestedMembershipObservations > 0 &&
      availableMembershipObservations === requestedMembershipObservations &&
      missingMembershipObservations === 0,
  );
  const universeRemovalOpenCoveragePct = number(
    audit.universeRemovalOpenCoveragePct,
    0,
  );
  const universeRemovalOpenPrices = number(
    audit.universeRemovalOpenPrices,
    0,
  );
  const universeRemovalZeroRecoveryActions = number(
    audit.universeRemovalZeroRecoveryActions,
  ...
  const universeRemovalOutcomeCoveragePct = number(
    audit.universeRemovalOutcomeCoveragePct,
    0,
  );
  const universeRemovalActions = number(
    audit.universeRemovalActions,
    0,
  );
  const universeRemovalMissingOutcomes = number(
    audit.universeRemovalMissingOutcomes,
  ...
  const universeRemovalOutcomeCoverageComplete = Boolean(
    audit.universeRemovalOutcomeCoverageComplete === true &&
      universeRemovalResolvedOutcomes === universeRemovalActions &&
      universeRemovalMissingOutcomes === 0,
  );
  const minimumMembershipCount = number(audit.minimumMembershipCount);
  const maximumMembershipCount = number(audit.maximumMembershipCount);
  const membershipCardinalityPlausible =
    minimumMembershipCount >= 85 && maximumMembershipCount <= 125;
  const expectedMembershipEvidence =
    pointInTimeNasdaqExpectedMembershipEvidence(
      universe,
      manifest.sessionDates,
  ...
  const report = {
    ...
    completedAt: new Date().toISOString(),
    datasetFingerprint: evidenceDatasetFingerprint,
    datasetInputFingerprint: manifest.datasetFingerprint,
    priceInputFingerprint: manifest.priceInputFingerprint,
    rawMembershipDigest: universe.rawMembershipDigest,
    membershipSupplementFingerprint:
      pointInTimeNasdaqMembershipSupplementFingerprint(),
    assessment: {
      adjustedPriceIntegrityPass: audit.pass === true,
      exactObservedSignalHistoryComplete:
        audit.observedSignalHistoryPass === true,
      missingPriceAttributionComplete:
        audit.missingRequiredAttributionComplete === true,
      activeMemberDateCoverageComplete,
      universeRemovalExactOpenCoverageComplete:
    ...
    membershipPathFingerprint: audit.membershipPathFingerprint,
    ...
    nextStep:
      ...
      membershipCardinalityPlausible &&
      membershipPathMatchesUniverse
        ? "Launch the three immutable R11 development-window shards concurrently."
        : "Stop: repair price or membership coverage before any R11 return is evaluated.",
  };
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_PRICE_INTEGRITY_STORE, report);
  return report;
}

function pointInTimeNasdaqAlphaR11Definitions() {
  const common = {
    ...
    buyTargetPct: 0.1633,
    strongBuyTargetPct: 0.1633,
    buyMaxPositionPct: 1 / 6,
    strongBuyMaxPositionPct: 1 / 6,
    buyMaxFactorPct: 1,
    strongBuyMaxFactorPct: 1,
    maxPositions: 6,
    minimumInitialStopPct: 22,
    maximumInitialStopPct: 22,
    // Historical sector classifications are unavailable. They cannot be used
    // as if they had been known at the decision date.
    maxSectorPositions: 6,
    maxSectorPct: 1,
    maxIssuerPositions: 1,
  ...
  const pathCandidate = ({ id, label, family, weights }) => ({
    id,
    label,
    family,
    mechanism: "dual-benchmark residual continuation with a distinct ex-ante risk/path modifier",
    weights,
    overrides: {
      ...common,
      researchRankMode: "nasdaq-residual-path-quality",
      requireNasdaqResidualPathFactors: true,
      nasdaqResidualPathWeights: weights,
    },
  });
  return [
    {
      id: "r11-r8-residual-resilience-replication",
      label:
        "R8 dual-benchmark 120-session residual rank replicated in the Nasdaq index universe",
      family: "replication",
      mechanism: "cross-universe replication of the R8 rank without sizing search",
      weights: {
        relative120: 0.85,
        relative60: 0,
    ...
    {
      id: "r11-absolute-momentum-control",
      label: "Absolute momentum matched-lifecycle control",
      family: "replication",
      mechanism: "tests whether benchmark residualization adds value",
      weights: { momentum: 1 },
      overrides: { ...common, researchRankMode: "momentum-only" },
    },
    pathCandidate({
      id: "r11-residual-continuous-information",
      label: "Residual momentum with frog-in-the-pan path continuity",
      family: "path-quality",
      weights: {
        relative120: 0.72,
        relative60: 0,
        continuity: 0.28,
        accelerationRestraint: 0,
        lotteryRestraint: 0,
        lowVolatility: 0,
    ...
    pathCandidate({
      id: "r11-residual-path-quality-composite",
      label:
        "Residual momentum with continuity, acceleration and lottery-risk restraints",
      family: "path-quality",
      weights: {
        relative120: 0.55,
        relative60: 0,
        continuity: 0.2,
        accelerationRestraint: 0.1,
        lotteryRestraint: 0.1,
        lowVolatility: 0,
    ...
    pathCandidate({
      id: "r11-residual-acceleration-restraint",
      label: "Residual momentum excluding the strongest short-run acceleration",
      family: "crash-control",
      weights: {
        relative120: 0.75,
        relative60: 0,
        continuity: 0,
        accelerationRestraint: 0.25,
        lotteryRestraint: 0,
        lowVolatility: 0,
        drawdownResilience: 0,
        anchor: 0,
    ...
    pathCandidate({
      id: "r11-residual-lottery-restraint",
      label: "Residual momentum with a maximum-daily-return lottery penalty",
      family: "crash-control",
      weights: {
        relative120: 0.75,
        relative60: 0,
    ...
    pathCandidate({
      id: "r11-residual-ivol-restraint",
      label: "Residual momentum with a soft realized-volatility penalty",
      family: "crash-control",
      weights: {
        relative120: 0.75,
        relative60: 0,
    ...
    {
      id: "r11-anchored-gradual-leadership",
      label: "52-week-high anchored gradual leadership",
      family: "anchoring",
      mechanism: "anchoring and gradual-information underreaction",
      weights: {
        anchor: 0.28,
        recency: 0.1,
        continuity: 0.22,
    ...
    {
      id: "r11-attention-shock-breakout",
      label: "High-volume near-high continuation",
      family: "attention-shock",
      mechanism: "price-and-volume attention shock with breakout follow-through",
      weights: {
        activityShock: 0.4,
    ...
    ...[
      {
        id: "r13-surprise-pure-20",
        label: "Earnings surprise with twenty-session event window",
        family: "surprise-pure",
        weights: [0.9, 0, 0, 0.1],
        eventWindow: 20,
      },
      {
        id: "r13-surprise-pure-60",
        label: "Earnings surprise with sixty-session event window",
        family: "surprise-pure",
        weights: [0.9, 0, 0, 0.1],
        eventWindow: 60,
      },
      {
        id: "r13-surprise-confirmed-20",
        label: "Earnings surprise with short residual confirmation",
        family: "surprise-confirmed",
        weights: [0.65, 0.25, 0, 0.1],
        eventWindow: 20,
      },
      {
        id: "r13-surprise-confirmed-60",
        label: "Earnings surprise with dual-horizon residual confirmation",
        family: "surprise-confirmed",
        weights: [0.55, 0.15, 0.2, 0.1],
        eventWindow: 60,
      },
      {
        id: "r13-positive-surprise-20",
        label: "Positive earnings surprise with short confirmation",
        family: "positive-surprise",
        weights: [0.65, 0.25, 0, 0.1],
        minimumSurprise: 0.25,
        eventWindow: 20,
      },
      {
        id: "r13-positive-surprise-60",
        label: "Positive earnings surprise with dual-horizon confirmation",
        family: "positive-surprise",
        weights: [0.55, 0.15, 0.2, 0.1],
        minimumSurprise: 0.25,
        eventWindow: 60,
      ...
      {
        id: "r13-large-surprise-20",
        label: "Large earnings surprise with short confirmation",
        family: "large-surprise",
        weights: [0.65, 0.25, 0, 0.1],
        minimumSurprise: 0.5,
        eventWindow: 20,
      },
      {
        id: "r13-large-surprise-60",
        label: "Large earnings surprise with dual-horizon confirmation",
        family: "large-surprise",
        weights: [0.55, 0.15, 0.2, 0.1],
        minimumSurprise: 0.5,
        eventWindow: 60,
      ...
      const [surprise, residual20, residual60, recency] = variant.weights;
      const targetCount = 8;
      return {
        id: variant.id,
        label: variant.label,
        ...
        mechanism:
          "post-earnings-announcement drift with benchmark-residual price confirmation",
        weights: { surprise, residual20, residual60, recency },
        overrides: {
          ...common,
          researchSignalSource: "earnings-event",
          researchRankMode: "post-earnings-drift",
          requireEarningsSurpriseFactors: true,
          earningsDriftWeights: { surprise, residual20, residual60, recency },
          minEarningsSurpriseScore: variant.minimumSurprise || 0,
          minSessionsSinceEarnings: 0,
          maxSessionsSinceEarnings: variant.eventWindow,
          rankedRebalanceSessions: 5,
          rankedTargetCount: targetCount,
          rankedExitBuffer: Math.ceil(targetCount * 1.67),
          rankedMinimumHoldSessions: 5,
          rankedEntryQueueCount: 12,
          buyTargetPct: 0.98 / targetCount,
          strongBuyTargetPct: 0.98 / targetCount,
          buyMaxPositionPct: 1 / targetCount,
          strongBuyMaxPositionPct: 1 / targetCount,
          maxPositions: targetCount,
          minimumAverageDollarVolume: 30_000_000,
          minimumInitialStopPct: 18,
          maximumInitialStopPct: 18,
          timeStopSessions: variant.eventWindow,
        },
      };
    }),
  ].filter((definition) => definition.id.startsWith("r13-"));
}

const POINT_IN_TIME_NASDAQ_ALPHA_R11_WINDOWS =
  POINT_IN_TIME_SP500_ALPHA_WINDOWS;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_PHASES = Object.freeze([
  "development",
  "validation",
  "historicalAudit",
  "forwardDiagnostic",
...
export function buildR11ShardPlan(phase, candidateIds = []) {
  if (!POINT_IN_TIME_NASDAQ_ALPHA_R11_PHASES.includes(phase))
    throw new Error(`Invalid R11 phase: ${phase}`);
  const windows = POINT_IN_TIME_NASDAQ_ALPHA_R11_WINDOWS[phase];
  return windows.map((window, shard) => ({
    phase,
    shard,
    shardCount: windows.length,
    window: { ...window },
...
export function r11ShardStorePath({
  datasetFingerprint,
  ...
  if (
    !validSha256(datasetFingerprint) ||
    !validSha256(candidateSetFingerprint) ||
    !validSha256(experimentFingerprint) ||
    !POINT_IN_TIME_NASDAQ_ALPHA_R11_PHASES.includes(phase) ||
    !Number.isInteger(shard) ||
    !Number.isInteger(shardCount) ||
    shard < 0 ||
    shardCount !== POINT_IN_TIME_NASDAQ_ALPHA_R11_WINDOWS[phase].length ||
    shard >= shardCount
  )
    throw new Error("Invalid R11 shard identity");
  return `${POINT_IN_TIME_NASDAQ_ALPHA_R11_PREFIX}/${datasetFingerprint}/${candidateSetFingerprint}/${experimentFingerprint}/${phase}/window-${shard}-of-${shardCount}.json`;
}

function sameR11Window(left = {}, right = {}) {
  return left?.start === right?.start && left?.end === right?.end;
}

function validR11CompactRun(run, calendar = [], window = {}) {
  const expectedDates = asArray(calendar).filter(
    (date) => date >= window.start && date <= window.end,
  );
  const curve = Array.isArray(run?.curve) ? run.curve : [];
  const trades = Array.isArray(run?.trades) ? run.trades : null;
  const skippedOrders = Array.isArray(run?.skippedOrders)
    ? run.skippedOrders
    : null;
  const dailyReturns = Array.isArray(run?.metrics?.dailyReturns)
    ? run.metrics.dailyReturns
    : [];
  const curveValues = curve.map((row) => number(row?.equity));
  if (
    expectedDates.length < 2 ||
    !sameStringArray(
      curve.map((row) => row?.date),
      expectedDates,
    ) ||
    curveValues.some((value) => !(value > 0)) ||
    run?.curveLength !== curve.length ||
    dailyReturns.length !== curve.length - 1 ||
    !trades ||
    !skippedOrders ||
    !Array.isArray(run?.unresolvedUniverseRemovals) ||
    run.unresolvedUniverseRemovals.length > 0
  )
    return false;
  const derivedDailyReturns = curveValues
    .slice(1)
    .map((value, index) => value / curveValues[index] - 1);
  if (
    derivedDailyReturns.some(
      (value, index) =>
        !Number.isFinite(number(dailyReturns[index])) ||
        Math.abs(value - number(dailyReturns[index])) > 1e-10,
    )
  )
    return false;
  const totalReturnPct =
    (curveValues.at(-1) / curveValues[0] - 1) * 100;
  let peak = curveValues[0];
  let maxDrawdown = 0;
  for (const value of curveValues) {
    peak = Math.max(peak, value);
    maxDrawdown = Math.min(maxDrawdown, value / peak - 1);
  }
  const activeExposures = curve.map((row) => number(row?.activeExposure));
  if (
  ...
  const entries = new Map(
    buyTrades.map((trade) => [trade.positionId, trade]),
  );
  const closedPositionIds = new Set(
    closedTrades.map((trade) => trade?.positionId),
  );
  const endingCash = number(run?.endingCash);
  const aggregateRoundTripPnl = closedTrades.reduce(
    (total, trade) => total + number(trade?.roundTripPnl, NaN),
    0,
  );
  if (
    entries.size !== buyTrades.length ||
    closedPositionIds.size !== closedTrades.length ||
    buyTrades.length !== closedTrades.length ||
    buyTrades.some((trade) => !closedPositionIds.has(trade?.positionId)) ||
    closedTrades.some((trade) => {
      const entry = entries.get(trade?.positionId);
      return (
        !entry ||
        !(number(entry?.shares) > 0) ||
        !(number(entry?.price) > 0) ||
        !Number.isFinite(number(trade?.roundTripPnl))
      );
    }) ||
    !Number.isFinite(endingCash) ||
    Math.abs(endingCash - curveValues.at(-1)) > 0.011 ||
    Math.abs(aggregateRoundTripPnl - (endingCash - 100_000)) > 0.011
  )
    return false;
  const closedReturns = closedTrades
    .map((trade) => {
      ...
      return entryNotional > 0
        ? (number(trade?.roundTripPnl, number(trade?.realizedPnl, 0)) /
            entryNotional) *
            100
        : null;
    })
    .filter(Number.isFinite);
  const expectedExpectancyPct = closedReturns.length
    ? roundTo(
        closedReturns.reduce((total, value) => total + value, 0) /
          closedReturns.length,
        2,
      )
    : null;
  const grossProfit = closedTrades.reduce(
    (total, trade) =>
      total +
      Math.max(0, number(trade?.roundTripPnl, number(trade?.realizedPnl, 0))),
    0,
  );
  const grossLoss = Math.abs(
    closedTrades.reduce(
      (total, trade) =>
        total +
        Math.min(0, number(trade?.roundTripPnl, number(trade?.realizedPnl, 0))),
      0,
    ),
  );
  const expectedProfitFactor = grossLoss
    ? roundTo(grossProfit / grossLoss, 3)
    : null;
  const sameNullableMetric = (actual, expected) =>
    expected === null ? actual == null : number(actual) === expected;
  if (
    Math.abs(totalReturnPct - number(run?.metrics?.totalReturnPct, Infinity)) >
      0.011 ||
    number(run?.metrics?.closedTrades, -1) !==
      closedTrades.length ||
    Math.abs(
      maxDrawdown * 100 - number(run?.metrics?.maxDrawdownPct, Infinity),
    ) > 0.011 ||
    Math.abs(
      (activeExposures.reduce((total, value) => total + value, 0) /
        activeExposures.length) *
        100 -
        number(run?.metrics?.averageActiveExposurePct, Infinity),
    ) > 0.011 ||
    !sameNullableMetric(run?.metrics?.profitFactor, expectedProfitFactor) ||
    !sameNullableMetric(
      run?.metrics?.tradeDiagnostics?.expectancyPct,
      expectedExpectancyPct,
    )
  )
    return false;
  for (const symbol of ["SPY", "QQQ"]) {
    const benchmark = run?.benchmarkCurves?.[symbol];
    if (
      ...
      benchmark.some((row) => !(number(row?.value) > 0))
    )
      return false;
    const simpleReturnPct =
      (number(benchmark.at(-1).value) / number(benchmark[0].value) - 1) * 100;
    const comparison = run?.metrics?.benchmarkComparisons?.[symbol];
    if (
      Math.abs(
        simpleReturnPct - number(comparison?.simpleReturnPct, Infinity),
      ) > 0.011 ||
      Math.abs(
        totalReturnPct -
          simpleReturnPct -
          number(comparison?.excessReturnPct, Infinity),
      ) > 0.021
    )
      return false;
  }
...
export function validateR11ShardReport(
  report,
  {
    phase,
    job,
  ...
  const expectedCandidateIds = [...candidateIds].map(String);
  const actualCandidateIds = asArray(report?.evidence?.candidateIds).map(String);
  const resultIds = asArray(report?.evidence?.results).map((result) =>
    String(result?.candidateId || ""),
  );
  const uniqueExpected = new Set(expectedCandidateIds);
  const uniqueActual = new Set(actualCandidateIds);
  ...
  const validIdentity =
    ...
    report?.status === "complete" &&
    report?.phase === phase &&
    report?.shard === job?.shard &&
    report?.shardCount === job?.shardCount &&
    report?.datasetFingerprint === datasetFingerprint &&
    report?.candidateSetFingerprint === candidateSetFingerprint &&
    report?.experimentFingerprint === experimentFingerprint &&
    report?.evidence?.phase === phase &&
    report?.evidence?.shard === job?.shard &&
    report?.evidence?.shardCount === job?.shardCount &&
    sameR11Window(report?.window, job?.window) &&
    sameR11Window(report?.evidence?.window, job?.window) &&
    uniqueExpected.size === expectedCandidateIds.length &&
    uniqueActual.size === expectedCandidateIds.length &&
    uniqueResults.size === expectedCandidateIds.length &&
    sameStringArray([...uniqueActual].sort(), [...uniqueExpected].sort()) &&
    sameStringArray([...uniqueResults].sort(), [...uniqueExpected].sort()) &&
    asArray(report?.evidence?.results).length === expectedCandidateIds.length &&
    sha256Fingerprint(JSON.stringify(report?.evidence)) ===
      report?.resultDigest &&
    report?.productionChanged === false &&
    report?.eligibleForAlphaClaim === false &&
    report?.eligibleForLiveCapital === false;
  const validCandidates = asArray(report?.evidence?.results).every((result) => {
    const candidateId = String(result?.candidateId || "");
    const expectedFingerprint =
      candidateFingerprints instanceof Map
        ? candidateFingerprints.get(candidateId)
        : candidateFingerprints?.[candidateId];
    return (
      validSha256(expectedFingerprint) &&
      result?.candidateFingerprint === expectedFingerprint &&
      validR11CompactRun(result?.run, calendar, job?.window)
    );
  });
  if (!validIdentity || !validCandidates)
    throw new Error(`R11 ${phase} shard artifact failed strict validation`);
  return true;
}

function r11SelectionStorePath(context, stage) {
  if (!["validation-selection", "audit-selection"].includes(stage))
    throw new Error("Invalid R11 selection stage");
  return `${POINT_IN_TIME_NASDAQ_ALPHA_R11_PREFIX}/${context.datasetFingerprint}/${context.candidateSetFingerprint}/${context.experimentFingerprint}/${stage}.json`;
}

function validateR11SelectionArtifact(
  selection,
  context,
  stage,
  { expectedCandidateIds = null } = {},
) {
  const candidateIds = asArray(selection?.evidence?.candidateIds).map(String);
  const uniqueCandidateIds = new Set(candidateIds);
  const knownCandidates = candidateIds.every((id) =>
    context.definitionsById.has(id),
  );
  const baseValid =
    selection?.version === 1 &&
    selection?.status === "complete" &&
    selection?.stage === stage &&
    selection?.datasetFingerprint === context.datasetFingerprint &&
    selection?.candidateSetFingerprint === context.candidateSetFingerprint &&
    selection?.experimentFingerprint === context.experimentFingerprint &&
    selection?.productionChanged === false &&
    uniqueCandidateIds.size === candidateIds.length &&
    knownCandidates &&
    sha256Fingerprint(JSON.stringify(selection?.evidence)) ===
      selection?.evidenceDigest;
  if (!baseValid)
    throw new Error(`R11 ${stage} artifact failed strict validation`);
  if (stage === "validation-selection") {
    const developmentCandidateIds = asArray(
      selection.evidence?.developmentCandidateIds,
    )
      .map(String)
      .sort();
    const expectedDevelopmentIds = context.definitions
      .map((definition) => definition.id)
      .sort();
    const development = asArray(selection.evidence?.development);
    const developmentIds = development.map((row) => String(row?.id || ""));
    const selectedRows = new Map(development.map((row) => [row.id, row]));
    const selectedFamilies = candidateIds.map(
      (id) => context.definitionsById.get(id)?.family,
    );
    if (
      !sameStringArray(developmentCandidateIds, expectedDevelopmentIds) ||
      new Set(developmentIds).size !== expectedDevelopmentIds.length ||
      !sameStringArray([...developmentIds].sort(), expectedDevelopmentIds) ||
      !development.every(
        (row) => row?.clearsPhase === r11PhaseChecksPass(row?.checks),
      ) ||
      candidateIds.length > 4 ||
      new Set(selectedFamilies).size !== selectedFamilies.length ||
      !candidateIds.every((id) => selectedRows.get(id)?.clearsPhase === true)
    )
      throw new Error("R11 validation selection coverage is invalid");
  } else {
    const validationCandidateIds = asArray(
      selection.evidence?.validationCandidateIds,
    ).map(String);
    const finalists = asArray(selection.evidence?.finalists);
    const finalistIds = finalists.map((row) => String(row?.id || ""));
    const selectedCandidateId = String(
      selection.evidence?.selectedCandidateId || "",
    );
    const selectedCandidateFingerprint = selectedCandidateId
      ? context.candidateFingerprints[selectedCandidateId]
      : null;
    if (
      (expectedCandidateIds &&
        !sameStringArray(
          [...validationCandidateIds].sort(),
          [...expectedCandidateIds].map(String).sort(),
        )) ||
      new Set(validationCandidateIds).size !== validationCandidateIds.length ||
      ...
      (candidateIds.length === 1 &&
        (candidateIds[0] !== selectedCandidateId ||
          selection.evidence?.selectedCandidateFingerprint !==
            selectedCandidateFingerprint ||
          finalists.find((row) => row.id === selectedCandidateId)
            ?.clearsPhase !== true)) ||
      (candidateIds.length === 0 &&
        (selectedCandidateId ||
          selection.evidence?.selectedCandidateFingerprint !== null))
    )
      throw new Error("R11 audit selection identity is invalid");
  }
  return selection;
}

async function pointInTimeNasdaqR11Context() {
  const [manifest, integrity, datasetStatus, universe, earnings] =
    await Promise.all([
    readExactPrivateJson(POINT_IN_TIME_NASDAQ_COMPILED_CHECKPOINT_STORE),
    getPointInTimeNasdaqPriceIntegrity(),
    getPointInTimeNasdaqDatasetStatus(),
    getPointInTimeNasdaqUniverse(),
    readExactPrivateJson(POINT_IN_TIME_NASDAQ_EARNINGS_SURPRISE_STORE),
  ]);
  assertCurrentPointInTimeNasdaqEvidence(manifest, datasetStatus, universe);
  if (
    earnings?.status !== "complete" ||
  ...
  if (
    ...
    throw new Error("The completed Nasdaq manifest fingerprint is invalid");
  if (
    !currentPointInTimeNasdaqIntegrity(
      integrity,
      manifest,
    ...
    throw new Error(
      "R11 is blocked until exact member-date price integrity passes",
    );
  const definitions = pointInTimeNasdaqAlphaR11Definitions();
  const candidateSetFingerprint = sha256Fingerprint(
    JSON.stringify(definitions),
  );
  const windows = Object.fromEntries(
    Object.entries(POINT_IN_TIME_NASDAQ_ALPHA_R11_WINDOWS).map(
      ([phase, rows]) => [phase, rows.map((window) => ({ ...window }))],
    ),
  );
  const experimentContract = {
    runner: "nasdaq-r11-window-shards-v2",
    executionContract: POINT_IN_TIME_NASDAQ_ALPHA_R11_EXECUTION_CONTRACT,
    priceIntegrityContract: POINT_IN_TIME_NASDAQ_PRICE_INTEGRITY_CONTRACT,
    executionCommit:
      process.env.VERCEL_GIT_COMMIT_SHA || "local-uncommitted",
    datasetFingerprint: evidenceDatasetFingerprint,
    datasetInputFingerprint: manifest.datasetFingerprint,
    priceInputFingerprint: manifest.priceInputFingerprint,
    rawMembershipDigest: universe.rawMembershipDigest,
    membershipSupplementFingerprint:
      pointInTimeNasdaqMembershipSupplementFingerprint(),
    candidateSetFingerprint,
    earningsFingerprint: sha256Fingerprint(JSON.stringify(earnings.rows)),
    definitions,
    windows,
    costs: { slippageBps: 12, commissionPerOrder: 0 },
    benchmarks: ["SPY", "QQQ"],
    developmentGates: POINT_IN_TIME_NASDAQ_ALPHA_R11_PHASE_CHECK_KEYS,
    auditThresholdT: 3,
    researchGenerationsCounted:
      POINT_IN_TIME_NASDAQ_ALPHA_R11_RESEARCH_GENERATIONS,
  };
  ...
  return {
    ...
    candidateFingerprints: Object.fromEntries(
      definitions.map((definition) => [
        definition.id,
        sha256Fingerprint(JSON.stringify(definition)),
      ]),
    ),
    windows,
...
async function restorePointInTimeNasdaqWindow(manifest, calendar, window) {
  const startIndex = calendar.indexOf(window.start);
  const endIndex = calendar.indexOf(window.end);
  if (startIndex < 2 || endIndex < startIndex)
    throw new Error(
      `Frozen Nasdaq window is unavailable: ${window.start} to ${window.end}`,
    );
  ...
  const restored = [];
  for (const chunk of requiredChunks) {
    const payload = await verifiedNasdaqChunk(chunk);
    restored.push(...payload.sessions);
  }
  ...
  if (!sameStringArray(sessions.map((session) => session.date), expectedDates))
    throw new Error(
      `Nasdaq restore is incomplete for ${window.start} to ${window.end}`,
    );
  return {
    metadata: {
      source: "verified Nasdaq R11 compiled chunks",
      benchmarkSymbol: "SPY",
      comparisonSymbols: ["SPY", "QQQ"],
      membershipEffectiveConvention: "effective-date-inclusive",
      universeRemovalPolicy: POINT_IN_TIME_NASDAQ_REMOVAL_POLICY,
...
function compactR11Run(run = {}) {
  return {
    metrics: run.metrics,
    trades: asArray(run.trades),
    skippedOrders: asArray(run.skippedOrders),
    curve: asArray(run.curve),
    benchmarkCurves: run.benchmarkCurves || {},
    unresolvedUniverseRemovals: asArray(run.unresolvedUniverseRemovals),
    curveLength: number(run.curveLength, asArray(run.curve).length),
    endingCash: number(run.endingCash),
  };
}

function r11WorkerSummary(report = {}) {
  return {
    version: report.version,
    researchGeneration: "R11",
    productionCandidateVersion: "V21",
    ...
    candidateCount: asArray(report.evidence?.results).length,
    datasetFingerprint: report.datasetFingerprint,
    candidateSetFingerprint: report.candidateSetFingerprint,
    experimentFingerprint: report.experimentFingerprint,
...
function r11WorkerConcurrencyProof(summaries = []) {
  const rows = asArray(summaries);
  const starts = rows
    .map((row) => new Date(row.startedAt || 0).getTime())
    .filter((value) => Number.isFinite(value) && value > 0);
  const completions = rows
    .map((row) => new Date(row.completedAt || 0).getTime())
    .filter((value) => Number.isFinite(value) && value > 0);
  return {
    workerCount: rows.length,
    workers: rows,
    allComplete:
      rows.length > 0 && rows.every((row) => row.status === "complete"),
    wallClockOverlapObserved:
      rows.length > 1 &&
      starts.length === rows.length &&
      completions.length === rows.length &&
      Math.max(...starts) < Math.min(...completions),
  };
}

async function r11CandidateIdsForPhase(context, phase) {
  if (phase === "development")
    return context.definitions.map((definition) => definition.id);
  const selectionStage =
    phase === "validation" ? "validation-selection" : "audit-selection";
  const selection = await readExactPrivateJson(
    r11SelectionStorePath(context, selectionStage),
  );
  try {
    if (selectionStage === "validation-selection")
      validateR11SelectionArtifact(selection, context, selectionStage);
    else {
      const validationSelection = await readExactPrivateJson(
        r11SelectionStorePath(context, "validation-selection"),
      );
      validateR11SelectionArtifact(
        validationSelection,
        context,
      ...
      validateR11SelectionArtifact(selection, context, selectionStage, {
        expectedCandidateIds:
          validationSelection.evidence.candidateIds,
      });
    }
  } catch {
    throw new Error(
      `R11 ${phase} is blocked until ${selectionStage} is durably frozen`,
    );
  }
  const candidateIds = asArray(selection.evidence?.candidateIds).map(String);
  if (phase !== "validation" && candidateIds.length !== 1)
    throw new Error("Exactly one frozen R11 candidate may enter audit");
  if (!candidateIds.length)
    throw new Error(`No candidate is eligible for R11 ${phase}`);
  return candidateIds;
}

export async function runPointInTimeNasdaqR11WindowShard({
  phase,
  shard,
} = {}) {
  const context = await pointInTimeNasdaqR11Context();
  const candidateIds = await r11CandidateIdsForPhase(context, phase);
  const plan = buildR11ShardPlan(phase, candidateIds);
  const shardNumber = Number(shard);
  if (!Number.isInteger(shardNumber) || !plan[shardNumber])
    throw new Error(`Invalid R11 ${phase} shard: ${shard}`);
  const job = plan[shardNumber];
  const pathname = r11ShardStorePath({
    ...context,
    phase,
    shard: shardNumber,
  ...
  if (existing) {
    if (existing.status === "complete") {
      validateR11ShardReport(existing, {
        phase,
        job,
        candidateIds,
        candidateFingerprints: context.candidateFingerprints,
      ...
      return r11WorkerSummary({ ...existing, cached: true });
    }
    const leaseUntil = new Date(existing.leaseUntil || 0).getTime();
    if (
      existing.status === "running" &&
      existing.datasetFingerprint === context.datasetFingerprint &&
      existing.candidateSetFingerprint === context.candidateSetFingerprint &&
      existing.experimentFingerprint === context.experimentFingerprint &&
      Number.isFinite(leaseUntil) &&
      leaseUntil > Date.now()
    )
      return r11WorkerSummary(existing);
  }
  ...
  await persistPrivateJson(pathname, {
    ...
    window: job.window,
    startedAt,
    leaseUntil: new Date(Date.now() + 12 * 60 * 1_000).toISOString(),
    productionChanged: false,
    eligibleForAlphaClaim: false,
    eligibleForLiveCapital: false,
  });
  let dataset = await restorePointInTimeNasdaqWindow(
    context.manifest,
    context.calendar,
    job.window,
  );
  dataset = addEarningsSurprisesToDataset(
    dataset,
    context.calendar,
    context.earnings.rows,
  );
  const results = [];
  for (const candidateId of candidateIds) {
    const definition = context.definitionsById.get(candidateId);
    if (!definition)
      throw new Error(`Unknown frozen R11 candidate: ${candidateId}`);
    const run = simulatePointInTimePortfolio(
      dataset,
      simulationOptions({
        ...definition.overrides,
        thesisId: `pit-nasdaq-r11-${definition.id}`,
        thesisLabel: definition.label,
        startDate: job.window.start,
        endDate: job.window.end,
      }),
    );
    assertCompleteResearchWindow(
      run,
      context.calendar,
      job.window,
      `${definition.label} ${job.window.start}`,
    );
    results.push({
      candidateId,
      candidateFingerprint: context.candidateFingerprints[candidateId],
      run: compactR11Run(run),
    });
  }
  const evidence = {
    phase,
  ...
  return r11WorkerSummary(report);
}

export function mergeR11ShardReports(
  reports = [],
  {
    phase,
    shardCount,
  ...
  if (!POINT_IN_TIME_NASDAQ_ALPHA_R11_PHASES.includes(phase))
    throw new Error("Invalid R11 merge phase");
  if (reports.length !== shardCount)
    throw new Error(`R11 ${phase} shards are incomplete`);
  const expectedCandidateIds = [...candidateIds].sort();
  const seenShards = new Set();
  const runsById = new Map(candidateIds.map((id) => [id, []]));
  for (const report of reports) {
    ...
    const job = {
      ...
      shard,
      shardCount,
      window: windows[shard] || report?.window,
    };
    if (
      !Number.isInteger(shard) ||
      shard < 0 ||
      ...
      throw new Error(`R11 ${phase} shard set failed integrity`);
    validateR11ShardReport(report, {
      phase,
      job,
    ...
    const results = asArray(report.evidence?.results);
    for (const result of results)
      runsById.get(result.candidateId).push({
        shard: report.shard,
        window: report.evidence.window,
        run: result.run,
      });
  }
  for (const rows of runsById.values())
    rows.sort((left, right) => left.shard - right.shard);
  return runsById;
}

async function loadR11PhaseRuns(context, phase, candidateIds) {
  const plan = buildR11ShardPlan(phase, candidateIds);
  const reports = [];
  for (const job of plan) {
    ...
    const report = await readExactPrivateJson(pathname);
    if (!report)
      throw new Error(
        `R11 ${phase} window ${job.shard + 1}/${job.shardCount} is missing`,
      );
    reports.push(report);
  }
  const runsById = mergeR11ShardReports(reports, {
    phase,
    shardCount: plan.length,
    candidateIds,
    windows: plan.map((job) => job.window),
    candidateFingerprints: context.candidateFingerprints,
    datasetFingerprint: context.datasetFingerprint,
    candidateSetFingerprint: context.candidateSetFingerprint,
    experimentFingerprint: context.experimentFingerprint,
  ...
  Object.defineProperty(runsById, "workerConcurrency", {
    value: r11WorkerConcurrencyProof(reports.map(r11WorkerSummary)),
    enumerable: false,
  });
  return runsById;
}

function r11SelectionComparator(left, right) {
  const leftWorst = number(left.summary?.worstMinimumBenchmarkAlphaPct, -Infinity);
  const rightWorst = number(
    right.summary?.worstMinimumBenchmarkAlphaPct,
    -Infinity,
  );
  const leftAggregate = minimumSimpleBenchmarkAlpha(left.summary?.aggregate);
  const rightAggregate = minimumSimpleBenchmarkAlpha(right.summary?.aggregate);
  const leftDrawdown = number(left.summary?.aggregate?.maxDrawdownPct, -Infinity);
  const rightDrawdown = number(
    right.summary?.aggregate?.maxDrawdownPct,
    -Infinity,
  );
  return (
    rightWorst - leftWorst ||
    rightAggregate - leftAggregate ||
    rightDrawdown - leftDrawdown ||
    left.definition.id.localeCompare(right.definition.id)
  );
}

function r11CompactCandidateRow(row = {}) {
  return {
    id: row.definition.id,
    label: row.definition.label,
    family: row.definition.family,
    mechanism: row.definition.mechanism,
    weights: row.definition.weights,
    configuration: row.definition.overrides,
    summary: compactPointInTimeAlphaPhase(row.summary),
    checks: row.checks,
    clearsPhase: row.clearsPhase,
  };
}

function r11FinalReportWithDigest(report) {
  const value = {
    ...report,
    reportContract: POINT_IN_TIME_NASDAQ_ALPHA_R11_FINAL_REPORT_CONTRACT,
  };
  return {
    ...value,
    reportDigest: sha256Fingerprint(JSON.stringify(value)),
  };
}

function validR11FinalReportForContext(report, context) {
  const reportDigest = report?.reportDigest;
  const digestPayload = Object.fromEntries(
    Object.entries(report || {}).filter(
      ([key]) => !["reportDigest", "cached"].includes(key),
    ),
  );
  const baseValid = Boolean(
    report?.version === POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION &&
      ...
      report?.reportContract ===
        POINT_IN_TIME_NASDAQ_ALPHA_R11_FINAL_REPORT_CONTRACT &&
      report?.datasetFingerprint === context?.datasetFingerprint &&
      report?.candidateSetFingerprint === context?.candidateSetFingerprint &&
      report?.experimentFingerprint === context?.experimentFingerprint &&
      report?.candidateSetFrozenBeforeExecution === true &&
      report?.candidateCount === context?.definitions?.length &&
      report?.productionChanged === false &&
      report?.eligibleForAlphaClaim === false &&
      report?.eligibleForLiveCapital === false &&
      report?.allEvidenceGatesPassed === false &&
      validSha256(reportDigest) &&
      sha256Fingerprint(JSON.stringify(digestPayload)) === reportDigest,
  );
  if (!baseValid) return false;
  if (report.auditCandidates === 0)
    return Boolean(
      report.selectedCandidateId == null &&
        report.selected == null &&
        report.allHistoricalScreenGatesPassed === false &&
        String(report.candidateDisposition || "").startsWith("rejected-"),
    );
  const selectedCandidateId = String(report.selectedCandidateId || "");
  const definition = context.definitionsById?.get(selectedCandidateId);
  if (
    report.auditCandidates !== 1 ||
    !definition ||
    report.selected?.id !== selectedCandidateId ||
    sha256Fingerprint(
      JSON.stringify({
        id: report.selected.id,
        label: report.selected.label,
        family: report.selected.family,
        mechanism: report.selected.mechanism,
        weights: report.selected.weights,
        overrides: report.selected.configuration,
      }),
    ) !== context.candidateFingerprints?.[selectedCandidateId]
  )
    return false;
  const statistics = report.statisticalEvidence?.excessReturnStatistics;
  const deterministicPromotionScreenPassed = r11DeterministicAuditGate({
    auditChecks: report.checks?.historicalAudit,
    spyStatistic: statistics?.SPY,
    qqqStatistic: statistics?.QQQ,
    dataIntegrityPassed:
      context.integrity?.assessment?.allDataGatesPassed === true,
    tradeConcentration: report.tradeConcentration,
  });
  return Boolean(
    report.statisticalEvidence?.significanceThresholdT === 3 &&
      report.statisticalEvidence?.strictMatchedPlacebosRun === 0 &&
      report.statisticalEvidence?.strictMatchedPlacebosRequired ===
        POINT_IN_TIME_NASDAQ_ALPHA_R11_STRICT_PLACEBO_SEEDS &&
      report.statisticalEvidence?.deterministicPromotionScreenPassed ===
        deterministicPromotionScreenPassed &&
      report.allHistoricalScreenGatesPassed ===
        deterministicPromotionScreenPassed &&
      report.checks?.statisticalSignificance
        ?.auditNeweyWestTAboveThreeVsSpy ===
        (number(statistics?.SPY?.tStatistic, -Infinity) > 3) &&
      report.checks?.statisticalSignificance
        ?.auditNeweyWestTAboveThreeVsQqq ===
        (number(statistics?.QQQ?.tStatistic, -Infinity) > 3),
  );
}

async function completedR11ReportForContext(context) {
  const report = await readExactPrivateJson(
    POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE,
  ).catch(() => null);
  return validR11FinalReportForContext(report, context) ? report : null;
}

async function persistR11TerminalRejection(context, partial) {
  const report = r11FinalReportWithDigest({
    version: POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION,
    researchGeneration: "R11",
    productionCandidateVersion: "V21",
    status: "complete",
    experiment: "R11 parallel historical Nasdaq index alpha search",
    frozenDate: POINT_IN_TIME_NASDAQ_ALPHA_R11_FROZEN_DATE,
    earliestProspectiveSession:
      POINT_IN_TIME_NASDAQ_ALPHA_R11_PROSPECTIVE_START,
    completedAt: new Date().toISOString(),
    datasetFingerprint: context.datasetFingerprint,
    candidateSetFingerprint: context.candidateSetFingerprint,
    experimentFingerprint: context.experimentFingerprint,
    ...
    eligibleForLiveCapital: false,
    productionChanged: false,
    fullPlaceboRunsAvoided: true,
    ...partial,
  });
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE, report);
  return report;
}

export async function freezePointInTimeNasdaqR11Validation() {
  const context = await pointInTimeNasdaqR11Context();
  const completed = await completedR11ReportForContext(context);
  if (completed) return completed;
  const candidateIds = context.definitions.map((definition) => definition.id);
  const runsById = await loadR11PhaseRuns(
    context,
    "development",
    candidateIds,
  );
  const rows = context.definitions
    .map((definition) => {
      const runs = runsById.get(definition.id).map((row) => row.run);
      const summary = pointInTimeAlphaPhaseSummary(
        runs,
        context.windows.development,
      );
      const checks = pointInTimeAlphaPhaseChecks(summary, {
      ...
      return {
        ...
        summary,
        checks,
        clearsPhase: r11PhaseChecksPass(checks),
      };
    })
    .sort(r11SelectionComparator);
  const championByFamily = new Map();
  for (const row of rows.filter((candidate) => candidate.clearsPhase))
    if (!championByFamily.has(row.definition.family))
      championByFamily.set(row.definition.family, row);
  const finalists = [...championByFamily.values()]
    .sort(r11SelectionComparator)
    .slice(0, 4);
  const evidence = {
    candidateIds: finalists.map((row) => row.definition.id),
    developmentCandidateIds: candidateIds,
    development: rows.map(r11CompactCandidateRow),
    developmentWorkerConcurrency: runsById.workerConcurrency,
    selectionPolicy:
      "One gate-passing development champion per economic family, then at most four by worst-window alpha, aggregate minimum benchmark alpha, and drawdown; validation and audit are unread at selection.",
  };
  const selection = {
    version: 1,
    status: "complete",
    stage: "validation-selection",
    frozenAt: new Date().toISOString(),
    datasetFingerprint: context.datasetFingerprint,
    candidateSetFingerprint: context.candidateSetFingerprint,
    experimentFingerprint: context.experimentFingerprint,
    evidence,
    evidenceDigest: sha256Fingerprint(JSON.stringify(evidence)),
    productionChanged: false,
  };
  await persistPrivateJson(
    r11SelectionStorePath(context, "validation-selection"),
  ...
  if (!finalists.length)
    return persistR11TerminalRejection(context, {
      candidateDisposition:
        "rejected-before-validation-by-development-family-funnel",
      developmentCandidates: rows.map(r11CompactCandidateRow),
      workerExecution: { development: runsById.workerConcurrency },
      validationFinalists: 0,
      selectedCandidateId: null,
      selected: null,
      nextStep:
        "No R11 family champion cleared every development gate; do not expose validation or audit as a tuning surface.",
    });
  const interim = {
    version: POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION,
    researchGeneration: "R11",
    ...
    candidateCount: context.definitions.length,
    developmentCandidates: rows.map(r11CompactCandidateRow),
    validationFinalists: finalists.map((row) => row.definition.id),
    workerExecution: { development: runsById.workerConcurrency },
    productionChanged: false,
    eligibleForAlphaClaim: false,
    eligibleForLiveCapital: false,
    nextStep: `Launch ${context.windows.validation.length} validation-window shards concurrently.`,
  };
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE, interim);
  return interim;
}

export async function freezePointInTimeNasdaqR11Audit() {
  const context = await pointInTimeNasdaqR11Context();
  const completed = await completedR11ReportForContext(context);
  if (completed) return completed;
  const validationSelection = await readExactPrivateJson(
  ...
  if (!candidateIds.length)
    throw new Error("No R11 candidate was eligible for validation");
  const developmentById = new Map(
    asArray(validationSelection.evidence?.development).map((row) => [
      row.id,
      row,
    ]),
  );
  ...
  const rows = candidateIds
    .map((candidateId) => {
      const definition = context.definitionsById.get(candidateId);
      const runs = runsById.get(candidateId).map((row) => row.run);
      const summary = pointInTimeAlphaPhaseSummary(
        runs,
      ...
      const development = developmentById.get(candidateId);
      return {
        definition,
        summary,
        checks,
        development,
        clearsPhase:
          development?.clearsPhase === true &&
          r11PhaseChecksPass(checks),
      };
    })
    .sort(r11SelectionComparator);
  const selectedRow = rows.find((row) => row.clearsPhase) || null;
  const evidence = {
    candidateIds: selectedRow ? [selectedRow.definition.id] : [],
    validationCandidateIds: candidateIds,
    selectedCandidateId: selectedRow?.definition.id || null,
    selectedCandidateFingerprint: selectedRow
      ? sha256Fingerprint(JSON.stringify(selectedRow.definition))
      : null,
    finalists: rows.map((row) => ({
      ...r11CompactCandidateRow(row),
      development: row.development,
    })),
    validationWorkerConcurrency: runsById.workerConcurrency,
    selectionPolicy:
      "Exactly one candidate that cleared both frozen development and validation gates is selected by the preregistered worst-window alpha, aggregate alpha, and drawdown order. Audit data are unread at selection.",
  };
  const selection = {
    version: 1,
    status: "complete",
  ...
  await persistPrivateJson(
    r11SelectionStorePath(context, "audit-selection"),
    selection,
  );
  if (!selectedRow)
    return persistR11TerminalRejection(context, {
      ...
      developmentCandidates: asArray(
        validationSelection.evidence?.development,
      ),
      validationFinalists: rows.map((row) => ({
        ...r11CompactCandidateRow(row),
        development: row.development,
      })),
      workerExecution: {
        development:
          validationSelection.evidence?.developmentWorkerConcurrency || null,
        validation: runsById.workerConcurrency,
      },
      auditCandidates: 0,
      selectedCandidateId: null,
      selected: null,
      nextStep:
        "No frozen R11 finalist cleared every validation gate; the audit remains sealed.",
    });
  const interim = {
    version: POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION,
    researchGeneration: "R11",
    ...
    validationFinalists: evidence.finalists,
    workerExecution: {
      development:
        validationSelection.evidence?.developmentWorkerConcurrency || null,
      validation: runsById.workerConcurrency,
    ...
    auditCandidates: 1,
    selectedCandidateId: selectedRow.definition.id,
    productionChanged: false,
    eligibleForAlphaClaim: false,
    eligibleForLiveCapital: false,
    nextStep: `Launch ${context.windows.historicalAudit.length} audit-window shards for only the frozen winner, plus the non-selecting forward diagnostic.`,
  };
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE, interim);
  return interim;
}

export function r11PhaseChecksPass(checks) {
  return Boolean(
    checks &&
      POINT_IN_TIME_NASDAQ_ALPHA_R11_PHASE_CHECK_KEYS.every(
        (key) => checks[key] === true,
      ),
  );
}

export function r11DeterministicAuditGate({
  auditChecks,
  spyStatistic,
  qqqStatistic,
  dataIntegrityPassed,
  ...
  return Boolean(
    dataIntegrityPassed === true &&
      r11PhaseChecksPass(auditChecks) &&
      number(spyStatistic?.tStatistic, -Infinity) > 3 &&
      number(qqqStatistic?.tStatistic, -Infinity) > 3 &&
      tradeConcentration?.concentrationWarning === false
  );
}

export async function finalizePointInTimeNasdaqR11() {
  const context = await pointInTimeNasdaqR11Context();
  const completed = await completedR11ReportForContext(context);
  if (completed) return completed;
  const [validationSelection, auditSelection] = await Promise.all([
    readExactPrivateJson(r11SelectionStorePath(context, "validation-selection")),
    readExactPrivateJson(r11SelectionStorePath(context, "audit-selection")),
  ]);
  validateR11SelectionArtifact(
    validationSelection,
    context,
  ...
  validateR11SelectionArtifact(auditSelection, context, "audit-selection", {
    expectedCandidateIds: validationSelection.evidence.candidateIds,
  });
  const selectedCandidateId = String(
    auditSelection.evidence?.selectedCandidateId || "",
  );
  const definition = context.definitionsById.get(selectedCandidateId);
  if (
    !definition ||
    asArray(auditSelection.evidence?.candidateIds).length !== 1 ||
    auditSelection.evidence.candidateIds[0] !== selectedCandidateId ||
    auditSelection.evidence.selectedCandidateFingerprint !==
      context.candidateFingerprints[selectedCandidateId]
  )
    throw new Error("Exactly one valid R11 audit candidate is required");
  const [auditById, forwardById] = await Promise.all([
    loadR11PhaseRuns(
      context,
      "historicalAudit",
      [selectedCandidateId],
  ...
  const auditRuns = auditById.get(selectedCandidateId).map((row) => row.run);
  const forwardRuns = forwardById
    .get(selectedCandidateId)
    .map((row) => row.run);
  const historicalAudit = pointInTimeAlphaPhaseSummary(
    auditRuns,
    context.windows.historicalAudit,
  );
  const forwardDiagnostic = pointInTimeAlphaPhaseSummary(
    forwardRuns,
    context.windows.forwardDiagnostic,
  );
  const auditChecks = pointInTimeAlphaPhaseChecks(historicalAudit, {
    minimumClosedTrades: 15,
  });
  const excessReturnStatistics = {
    SPY: benchmarkExcessReturnStatistic(auditRuns, "SPY"),
    QQQ: benchmarkExcessReturnStatistic(auditRuns, "QQQ"),
  };
  const tradeConcentration = summarizePointInTimeTradeConcentration(auditRuns);
  const deterministicPromotionScreenPassed = r11DeterministicAuditGate({
    auditChecks,
    spyStatistic: excessReturnStatistics.SPY,
    qqqStatistic: excessReturnStatistics.QQQ,
    dataIntegrityPassed:
      context.integrity.assessment.allDataGatesPassed === true,
    tradeConcentration,
  });
  const selectedValidation = asArray(
    auditSelection.evidence?.finalists,
  ).find((row) => row.id === selectedCandidateId);
  const selectedDevelopment = asArray(
    validationSelection?.evidence?.development,
  ).find((row) => row.id === selectedCandidateId);
  const manifestSignature = parseSignature(context.manifest.signature);
  const report = r11FinalReportWithDigest({
    version: POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION,
    researchGeneration: "R11",
    productionCandidateVersion: "V21",
    ...
    candidateSetFingerprint: context.candidateSetFingerprint,
    experimentFingerprint: context.experimentFingerprint,
    datasetThrough: context.calendar.at(-1) || null,
    candidateSetFrozenBeforeExecution: true,
    candidateCount: context.definitions.length,
    developmentCandidates: asArray(
      validationSelection?.evidence?.development,
    ),
    validationFinalists: asArray(auditSelection.evidence?.finalists),
    auditCandidates: 1,
    selectedCandidateId,
    selected: {
      id: definition.id,
      ...
      development: selectedDevelopment?.summary || null,
      validation: selectedValidation?.summary || null,
      historicalAudit: compactPointInTimeAlphaPhase(historicalAudit),
      forwardDiagnostic: compactPointInTimeAlphaPhase(forwardDiagnostic),
    },
    selectionPolicy:
      "Ten frozen mechanisms were evaluated across three development windows in separate concurrent workers; one champion per economic family could advance, at most four finalists were evaluated across two validation workers, and exactly one frozen candidate was evaluated across two audit workers. Audit never selected or retuned the winner.",
    workerExecution: {
      development:
        validationSelection.evidence?.developmentWorkerConcurrency || null,
      validation:
        auditSelection.evidence?.validationWorkerConcurrency || null,
      historicalAudit: auditById.workerConcurrency,
      forwardDiagnostic: forwardById.workerConcurrency,
    },
    statisticalEvidence: {
      method:
        "Newey-West HAC t-statistics with five lags on audit-only daily strategy-minus-benchmark returns; the t>3 threshold is applied independently against SPY and QQQ after the nested family funnel.",
    ...
    checks: {
      data: {
        pointInTimeMemberDateCoverageComplete:
          context.integrity.assessment.activeMemberDateCoverageComplete === true,
        universeRemovalOutcomeCoverageComplete:
          context.integrity.assessment
            .universeRemovalOutcomeCoverageComplete === true,
        membershipCardinalityPlausible:
          context.integrity.assessment.membershipCardinalityPlausible === true,
        membershipPathMatchesUniverse:
          context.integrity.assessment.membershipPathMatchesUniverse === true,
        adjustedPriceIntegrityPassed:
          context.integrity.assessment.adjustedPriceIntegrityPass === true,
        datasetAndExperimentSha256Bound: true,
        noBenchmarkCompletionSleeve: true,
      },
      selection: {
        candidateSetFrozenBeforeExecution: true,
        validationUsedOnlyDevelopmentSelections: true,
    ...
    methodology: {
      ...
      membershipEffectiveConvention: "effective-date-inclusive",
      universeRemovalExecution:
        "effective-session-adjusted-open-else-conservative-zero",
      ...
      benchmarkCompletionSleeveUsed: false,
      developmentWindowWorkers: context.windows.development.length,
      validationWindowWorkers: context.windows.validation.length,
      auditWindowWorkers: context.windows.historicalAudit.length,
    ...
    dataQuality: {
      pointInTimeMembershipConstructed: true,
      membershipObservationCoveragePct:
        context.integrity.membershipObservationCoveragePct,
      requestedMembershipObservations:
        context.integrity.requestedMembershipObservations,
      availableMembershipObservations:
        context.integrity.availableMembershipObservations,
      missingMembershipObservations:
        context.integrity.missingMembershipObservations,
      universeRemovalOpenCoveragePct:
        context.integrity.universeRemovalOpenCoveragePct,
      universeRemovalZeroRecoveryActions:
        context.integrity.universeRemovalZeroRecoveryActions,
      universeRemovalOutcomeCoveragePct:
        context.integrity.universeRemovalOutcomeCoveragePct,
      universeRemovalMissingOutcomes:
        context.integrity.universeRemovalMissingOutcomes,
      universeRemovalPolicy: context.integrity.universeRemovalPolicy,
      compiledSessions: context.calendar.length,
      compiledChunks: asArray(context.manifest.chunks).length,
      membershipUnionSymbols: asArray(
        manifestSignature?.requestedSymbols,
      ).length,
      priceUsableSymbols: asArray(manifestSignature?.usableSymbols).length,
    ...
    researchReferences: [
      ...
      },
      {
        ...
        url: "https://joim.com/article/momentum-acceleration-and-reversal/",
      },
      {
    ...
    limitations: [
      "The Nasdaq endpoint is cardinality-checked but FMP does not unambiguously identify it as NDX in the endpoint documentation.",
      "The audit was sealed within R11, but its calendar dates were visible during earlier S&P research and are not genuinely new future observations.",
      "The dual-benchmark residual terms are transparent excess-return proxies, not a full Fama-French residual regression.",
      "Ten correlated candidates and eleven research generations make the separate 1,000-seed maximum-statistic placebo stage mandatory after any deterministic pass.",
      "At least 60 genuinely new sessions and independent review remain mandatory before an alpha or live-capital claim.",
    ],
    nextStep: deterministicPromotionScreenPassed
      ? "Run 1,000 maximum-statistic matched placebos without changing the selected candidate, then begin the genuinely prospective 60-session ledger."
      : "Preserve the rejection. Do not promote V21 or change V11 production.",
  });
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE, report);
  return report;
}
...
function pointInTimeSp500AlphaR10EarningsDriftDefinitions() {
  const weightProfiles = [
    {
      id: "surprise",
      label: "earnings surprise",
      weights: { surprise: 0.9, residual20: 0, residual60: 0, recency: 0.1 },
    },
    {
      id: "surprise-short-residual",
      label: "earnings surprise plus short residual confirmation",
      weights: {
        surprise: 0.65,
        residual20: 0.25,
        residual60: 0,
    ...
    {
      id: "surprise-dual-residual",
      label: "earnings surprise plus dual-horizon residual confirmation",
      weights: {
        surprise: 0.55,
        residual20: 0.15,
        residual60: 0.2,
  ...
  const minimumSurprises = [0, 0.25, 0.5];
  const eventWindows = [20, 60];
  const definitions = [];
  for (const profile of weightProfiles) {
    for (const minimumSurprise of minimumSurprises) {
      for (const eventWindow of eventWindows) {
        definitions.push({
          id: `r10-${profile.id}-min-${String(minimumSurprise).replace(".", "p")}-age-${eventWindow}`,
          label: `${profile.label}, surprise >= ${minimumSurprise}, ${eventWindow}-session event window`,
          family: "post-earnings-announcement-drift",
          ...
          overrides: {
            ...
            requireRelativeStrength: false,
            requireEarningsSurpriseFactors: true,
            minEarningsSurpriseScore: minimumSurprise,
            minSessionsSinceEarnings: 0,
            maxSessionsSinceEarnings: eventWindow,
            minimumResearchFactorCoverage: 0,
            blockChaseEntries: false,
            maxEntryGapPct: 3,
            selectionMode: "ranked",
            researchRankMode: "post-earnings-drift",
            earningsDriftWeights: profile.weights,
            rankedRebalanceSessions: 5,
            rankedTargetCount: 8,
            rankedExitBuffer: 12,
            rankedMinimumHoldSessions: 5,
...
function normalizedEarningsSurpriseRows(rows = [], datasetThrough = null) {
  return asArray(rows)
    .map((row) => {
      const symbol = pointInTimeSecuritySymbol(row);
      const date = String(
        row.date || row.earningsDate || row.fiscalDateEnding || "",
      ).slice(0, 10);
      const actual = number(
        row.actualEarningResult ?? row.actualEPS ?? row.epsActual,
        null,
      );
      const estimate = number(
        row.estimatedEarning ?? row.estimatedEPS ?? row.epsEstimated,
        null,
      );
      if (
        !symbol ||
        ...
        actual === null ||
        estimate === null ||
        (datasetThrough && date > datasetThrough)
      )
        return null;
      return {
        symbol,
        ...
        actual,
        estimate,
        surpriseScore: roundMetric(
          Math.max(-2, Math.min(2, (actual - estimate) / Math.max(0.25, Math.abs(estimate)))),
          6,
        ),
      };
    })
...
async function getOrFetchPointInTimeEarningsSurprises(
  datasetThrough,
  requestedSymbols,
  storePath = POINT_IN_TIME_SP500_EARNINGS_SURPRISE_STORE,
  minimumSymbols = 500,
) {
  ...
  if (
    existing?.status === "complete" &&
    existing?.source === "company-earnings" &&
    existing?.datasetThrough === datasetThrough &&
    number(existing?.coveragePct, 0) >= 80
  )
    return existing;
  const apiKey = process.env.FMP_API_KEY || process.env.FMP_KEY;
  if (!apiKey) throw new Error("The configured research provider key is unavailable");
  const client = createFmpClient(apiKey);
  const symbols = [
    ...new Set(asArray(requestedSymbols).map(pointInTimeSecuritySymbol)),
  ].filter(Boolean);
  if (symbols.length < minimumSymbols)
    throw new Error(
      `The point-in-time earnings universe is incomplete (${symbols.length} symbols)`,
    );
  const canResume =
    existing?.source === "company-earnings" &&
    existing?.datasetThrough === datasetThrough;
  const completed = new Set(
    canResume ? asArray(existing?.completedSymbols).map(symbolOf) : [],
  );
  const rawRows = canResume ? [...asArray(existing?.rows)] : [];
  const failures = canResume ? [...asArray(existing?.failures)] : [];
  ...
  for (let offset = 0; offset < pending.length; offset += 12) {
    ...
    for (let index = 0; index < results.length; index++) {
      ...
      const result = results[index];
      if (result?.error) {
        failures.push({ symbol, error: result.error });
        continue;
      }
      rawRows.push(
        ...asArray(result.rows).map((row) => ({ ...row, symbol })),
      );
      completed.add(symbol);
    }
    await persistPrivateJson(storePath, {
      version: 2,
  ...
  const rows = normalizedEarningsSurpriseRows(
    rawRows,
    datasetThrough,
  );
  const usableBySymbol = new Map();
  for (const row of rows)
    usableBySymbol.set(
      row.symbol,
      number(usableBySymbol.get(row.symbol), 0) + 1,
    );
  const coveredSymbols = symbols.filter(
    (symbol) => number(usableBySymbol.get(symbol), 0) >= 4,
  ).length;
  const coveragePct = roundMetric((coveredSymbols / symbols.length) * 100, 2);
  if (coveragePct < 80)
    throw new Error(
      `Per-company historical earnings coverage is insufficient (${coveredSymbols}/${symbols.length}, ${coveragePct}%)`,
    );
  const report = {
    version: 2,
    source: "company-earnings",
    ...
    availabilityConvention: "first-market-session-strictly-after-reported-date",
    rows,
  };
  await persistPrivateJson(storePath, report);
  return report;
}

export async function preparePointInTimeNasdaqR13Earnings() {
  ...
  const datasetThrough = calendar.at(-1) || null;
  if (!manifest?.complete || !datasetThrough)
    throw new Error("The completed Nasdaq checkpoint is unavailable for R13");
  return getOrFetchPointInTimeEarningsSurprises(
    datasetThrough,
    universe?.privateBlueprint?.unionSymbols,
    POINT_IN_TIME_NASDAQ_EARNINGS_SURPRISE_STORE,
    90,
...
function secTickerSymbol(value) {
  return String(value || "")
    .replace("-", ".")
    .toUpperCase()
    .trim();
...
async function fetchSecJson(url) {
  const response = await fetch(url, {
    headers: {
      ...
      // The SEC requires automated clients to identify both the application
      // and a monitored contact address. Anonymous/generic agents are denied.
      "User-Agent":
  ...
  if (!response.ok) {
    const error = new Error(`SEC request failed (${response.status})`);
    error.status = response.status;
    throw error;
  }
...
async function secTickerCikMap() {
  const payload = await fetchSecJson(
    "https://www.sec.gov/files/company_tickers.json",
  );
  const map = new Map();
  for (const row of Object.values(payload || {})) {
    const symbol = secTickerSymbol(row?.ticker);
    const cik = String(row?.cik_str || "").replace(/\D/g, "");
    if (symbol && cik) map.set(symbol, cik.padStart(10, "0"));
  }
  if (map.size < 5_000)
    throw new Error(`SEC ticker map is implausibly small (${map.size})`);
  return map;
}

export async function getPointInTimeSp500SecFilingR14Status() {
  try {
    const report = await readExactPrivateJson(
      POINT_IN_TIME_SP500_SEC_R14_FACTS_STORE,
    );
    return compactR14FactsStatus(
      report || {
        version: 1,
        researchGeneration: "R14",
        status: "pending",
...
export async function preparePointInTimeSp500SecFilingR14({
  batchSize = 30,
} = {}) {
  const [manifest, universe, prior] = await Promise.all([
    readExactPrivateJson(POINT_IN_TIME_SP500_COMPILED_CHECKPOINT_STORE),
    getPointInTimeSp500Universe(),
    readExactPrivateJson(POINT_IN_TIME_SP500_SEC_R14_FACTS_STORE).catch(
      () => null,
    ),
  ]);
  const calendar = asArray(manifest?.sessionDates).map(String);
  const datasetThrough = calendar.at(-1) || null;
  const symbols = asArray(universe?.privateBlueprint?.unionSymbols)
    .map(pointInTimeSecuritySymbol)
    .filter(Boolean)
    .sort();
  if (!manifest?.complete || !datasetThrough || symbols.length < 450)
    throw new Error(
      "R14 requires the complete point-in-time S&P dataset and membership union",
    );
  if (
    prior?.status === "complete" &&
    prior?.datasetThrough === datasetThrough &&
    sameStringArray(prior?.requestedSymbols, symbols)
  )
    return { ...prior, cached: true };
  const resumable =
    prior?.datasetThrough === datasetThrough &&
    sameStringArray(prior?.requestedSymbols, symbols);
  const eventsBySymbol = resumable ? { ...(prior.eventsBySymbol || {}) } : {};
  const cikBySymbol = resumable ? { ...(prior.cikBySymbol || {}) } : {};
  const completed = new Set(
    resumable ? asArray(prior.completedSymbols).map(String) : [],
  );
  const exhausted = new Set(
    resumable ? asArray(prior.exhaustedSymbols).map(String) : [],
  );
  const failures = resumable ? [...asArray(prior.failures)] : [];
  const tickerMap = await secTickerCikMap();
  for (const symbol of symbols) {
    const cik = tickerMap.get(secTickerSymbol(symbol));
    if (cik) cikBySymbol[symbol] = cik;
  }
  const pending = symbols.filter(
    (symbol) => !completed.has(symbol) && !exhausted.has(symbol),
  );
  const batch = pending.slice(0, boundedInteger(batchSize, 30, 1, 40));
  const results = await mapLimited(batch, 3, async (symbol) => {
    const cik = cikBySymbol[symbol];
    if (!cik) return { symbol, exhausted: true, error: "SEC CIK unavailable" };
    try {
      const raw = await fetchSecJson(
        `https://data.sec.gov/api/xbrl/companyfacts/CIK${cik}.json`,
      );
      const compact = compactSecCompanyFacts(raw, {
        earliestFiled: "2020-01-01",
        latestFiled: datasetThrough,
      });
      return {
        symbol,
        cik,
        events: buildPointInTimeFilingEvents(compact, {
          symbol,
          cik,
          datasetThrough,
        }),
  ...
  for (const result of results) {
    if (result.events) {
      eventsBySymbol[result.symbol] = result.events;
      completed.add(result.symbol);
    } else if (result.exhausted) exhausted.add(result.symbol);
    if (result.error)
      failures.push({
  ...
  const coveredSymbols = symbols.filter(
    (symbol) => asArray(eventsBySymbol[symbol]).length >= 4,
  );
  const rows = Object.values(eventsBySymbol)
    .flatMap(asArray)
    .filter((row) => row.filedDate >= "2022-01-01")
    .sort(
      (left, right) =>
        left.filedDate.localeCompare(right.filedDate) ||
        left.symbol.localeCompare(right.symbol),
    );
  const coveragePct = roundMetric(
    (coveredSymbols.length / symbols.length) * 100,
    2,
  );
  const status = remainingSymbols.length ? "acquiring" : "complete";
  if (status === "complete" && coveragePct < 80)
    throw new Error(
      `R14 SEC filing coverage is insufficient (${coveredSymbols.length}/${symbols.length}, ${coveragePct}%)`,
    );
  const report = {
    version: 1,
    researchGeneration: "R14",
    ...
    availabilityConvention:
      "first-market-session-strictly-after-sec-filed-date",
    datasetThrough,
    updatedAt: new Date().toISOString(),
    completedAt: status === "complete" ? new Date().toISOString() : null,
    requestedSymbols: symbols,
    cikBySymbol,
    completedSymbols: [...completed].sort(),
    exhaustedSymbols: [...exhausted].sort(),
    remainingSymbols: remainingSymbols.length,
    coveredSymbols: coveredSymbols.length,
    coveragePct,
    eventRows: rows.length,
    failures: failures.slice(-150),
    eventsBySymbol,
    rows,
    dataFingerprint: sha256Fingerprint(
      JSON.stringify({ datasetThrough, symbols, cikBySymbol, rows }),
    ),
    productionChanged: false,
    eligibleForAlphaClaim: false,
    eligibleForLiveCapital: false,
    nextStep:
      status === "complete"
        ? "Run only the frozen R14 development family; validation remains sealed until a candidate clears every development gate."
        : `Acquire ${remainingSymbols.length} remaining SEC issuer histories in bounded batches.`,
    limitations: [
      "SEC Company Facts standard-taxonomy coverage varies by issuer and foreign filer; missing fields fail closed.",
      "Historical index tickers absent from the SEC current ticker map remain excluded rather than guessed or aliased.",
      "Every session through 2026-09-01 is contaminated development evidence and cannot establish prospective alpha.",
    ],
  };
  await persistPrivateJson(POINT_IN_TIME_SP500_SEC_R14_FACTS_STORE, report);
  return report;
}
...
function addEarningsSurprisesToDataset(dataset, calendar, earningsRows) {
  const observationsBySymbol = new Map();
  for (const row of earningsRows) {
    const availableIndex = firstCalendarIndexAfter(calendar, row.date);
    if (availableIndex >= calendar.length) continue;
    if (!observationsBySymbol.has(row.symbol))
      observationsBySymbol.set(row.symbol, []);
    observationsBySymbol.get(row.symbol).push({ ...row, availableIndex });
  }
  const activeBySymbol = new Map();
  const cursors = new Map();
  const sessions = asArray(dataset?.sessions).map((session) => {
    const sessionIndex = calendar.indexOf(session.date);
    const annotate = (signal) => {
      const symbol = pointInTimeSecuritySymbol(signal);
      const observations = observationsBySymbol.get(symbol) || [];
      let cursor = number(cursors.get(symbol), 0);
      while (
        cursor < observations.length &&
        observations[cursor].availableIndex <= sessionIndex
      ) {
        activeBySymbol.set(symbol, observations[cursor]);
        cursor++;
      }
      ...
      return {
        ...
        researchFactors: {
          ...
          earningsSurpriseScore: event.surpriseScore,
          sessionsSinceEarnings: sessionIndex - event.availableIndex,
          earningsReportedDate: event.date,
        },
      };
    };
    return {
      ...session,
      signals: asArray(session.signals).map(annotate),
      positionSignals: asArray(session.positionSignals).map(annotate),
    };
  });
...
function normalizePointInTimeAlphaReport(report, datasetStatus = null) {
  ...
  return {
    ...
    candidateDisposition:
      ["R8", "R9", "R10", "R14"].includes(report.researchGeneration)
...
async function evaluatePointInTimeAlphaDefinitions(
  ...
  definitions,
  windows,
  { datasetTransform = null } = {},
) {
  const runsById = new Map(definitions.map((definition) => [definition.id, []]));
  for (const window of windows) {
    let dataset = await restorePointInTimeSp500Window(
      manifest,
      calendar,
      window,
    );
    if (typeof datasetTransform === "function")
      dataset = datasetTransform(dataset, window);
...
function createPointInTimePriceIntegrityAccumulator() {
  return {
    ...
    observedRequiredPriceRows: 0,
    missingRequiredPriceRows: 0,
    missingRequiredPriceSamples: [],
    missingRequiredPriceRowsBySymbol: new Map(),
    nasdaqMembershipEvidenceApplied: false,
    requestedMembershipObservations: 0,
    availableMembershipObservations: 0,
    missingMembershipObservationsBySymbol: new Map(),
    membershipPathHash: createHash("sha256"),
    minimumMembershipCount: Infinity,
    maximumMembershipCount: 0,
    universeRemovalActions: 0,
    universeRemovalOpenPrices: 0,
    universeRemovalZeroRecoveryActions: 0,
    universeRemovalResolvedOutcomes: 0,
    universeRemovalMissingOutcomes: 0,
    requirementMetadataApplied: false,
    observedSignalHistoryRequired: false,
    shortSignalHistoryRows: 0,
    shortSignalHistorySamples: [],
    adjustedRows: 0,
    invalidOhlcRows: 0,
    nonPositiveRows: 0,
...
function addPointInTimePriceRequirement(
  ...
  if (prior && startIndex <= prior.end + 1) {
    prior.start = Math.min(prior.start, startIndex);
...
function createPointInTimePriceRequirements(
  {
    lookbackSessions = 253,
    comparisonSymbols = ["SPY", "QQQ"],
    requireExactObservedSignalHistory = false,
  } = {},
) {
  const normalizedLookbackSessions = Math.max(
    1,
    Math.floor(number(lookbackSessions, 253)),
  );
  return {
    rangesBySymbol: new Map(),
    activeRangesBySymbol: new Map(),
    signalLookbackRangesBySymbol: new Map(),
    benchmarkRangesBySymbol: new Map(),
    sessionDates: [],
    observedPriceSessionIndexesBySymbol: new Map(),
    totalSessions: 0,
    lookbackSessions: normalizedLookbackSessions,
    comparisonSymbols: comparisonSymbols.map(symbolOf).filter(Boolean),
    requireExactObservedSignalHistory:
      requireExactObservedSignalHistory === true,
    shortSignalHistoryRows: 0,
    shortSignalHistorySamples: [],
  };
...
function observePointInTimePriceRequirements(
  ...
  for (const dataset of asArray(datasets)) {
    ...
    for (const [localIndex, session] of sessions.entries()) {
      ...
      const priorSessionDate = requirements.sessionDates[sessionIndex];
      const precedingSessionDate = requirements.sessionDates[sessionIndex - 1];
      if (
        requirements.requireExactObservedSignalHistory &&
        (!validIsoDay(sessionDate) ||
          (priorSessionDate && priorSessionDate !== sessionDate) ||
          (precedingSessionDate && precedingSessionDate >= sessionDate))
      )
        throw new Error(
          `Point-in-time price requirement calendar is invalid at ${sessionIndex}`,
        );
      requirements.sessionDates[sessionIndex] = sessionDate;
      if (requirements.requireExactObservedSignalHistory) {
        for (const row of asArray(session?.prices)) {
          const symbol = symbolOf(row);
          if (!symbol) continue;
          const indexes =
            requirements.observedPriceSessionIndexesBySymbol.get(symbol) || [];
          if (indexes.at(-1) !== sessionIndex) indexes.push(sessionIndex);
          if (indexes.length > requirements.lookbackSessions) indexes.shift();
          requirements.observedPriceSessionIndexesBySymbol.set(symbol, indexes);
        }
      }
      const activeUniverseSymbols = new Set(
        asArray(session?.universeSymbols)
          .map(symbolOf)
          .filter(Boolean),
      );
      ...
      // Corporate-action execution prices are decision-relevant even when the
      // security disappears from that session's signal universe. In particular,
      // a universe-removal open must receive the same adjusted/OHLC checks as an
      // entry or ordinary exit.
      for (const action of asArray(session?.corporateActions)) {
        const actionType = String(action?.type || "").toLowerCase();
        if (
          actionType === "universe-removal" &&
          String(action?.treatment || "") !==
            "conservative-zero-recovery-missing-removal-open-v1"
        ) {
          const actionSymbol = symbolOf(action);
          if (actionSymbol) activeUniverseSymbols.add(actionSymbol);
        }
      }
      for (const symbol of signalSymbols) {
        let lookbackStart = Math.max(
          0,
          sessionIndex - requirements.lookbackSessions,
        );
        if (requirements.requireExactObservedSignalHistory) {
          const observedIndexes =
            requirements.observedPriceSessionIndexesBySymbol.get(symbol) || [];
          const exactHistoryAvailable =
            observedIndexes.length === requirements.lookbackSessions &&
            observedIndexes.at(-1) === sessionIndex;
          if (!exactHistoryAvailable) {
            requirements.shortSignalHistoryRows++;
            if (requirements.shortSignalHistorySamples.length < 20)
              requirements.shortSignalHistorySamples.push({
                date: sessionDate,
                symbol,
                observedRows: observedIndexes.length,
                requiredRows: requirements.lookbackSessions,
                currentSessionPriceObserved:
                  observedIndexes.at(-1) === sessionIndex,
              });
          }
          // The compiler consumes its last N observed rows. Requiring every
          // calendar row from the oldest consumed observation through today
          // catches internal provider gaps while making no listing-date claim.
          lookbackStart = observedIndexes[0] ?? sessionIndex;
        }
        ...
        addPointInTimePriceRequirement(
          requirements.signalLookbackRangesBySymbol,
          symbol,
          lookbackStart,
          sessionIndex,
        );
...
function finishPointInTimePriceRequirements(requirements) {
  for (const symbol of requirements.comparisonSymbols) {
    ...
    addPointInTimePriceRequirement(
      requirements.benchmarkRangesBySymbol,
      symbol,
      0,
      Math.max(0, requirements.totalSessions - 1),
    );
...
function scanPointInTimePriceDataset(
  ...
  if (requirements && !accumulator.requirementMetadataApplied) {
    accumulator.requirementMetadataApplied = true;
    accumulator.observedSignalHistoryRequired =
      requirements.requireExactObservedSignalHistory === true;
    accumulator.shortSignalHistoryRows = number(
      requirements.shortSignalHistoryRows,
      0,
    );
    accumulator.shortSignalHistorySamples = asArray(
      requirements.shortSignalHistorySamples,
    );
  }
  for (const [localIndex, session] of sessions.entries()) {
    ...
    const observedRequiredSymbols = new Set();
    const sessionPricesBySymbol = new Map();
    for (const row of asArray(session.prices)) {
      const symbol = symbolOf(row);
      if (
        symbol &&
        (!sessionPricesBySymbol.has(symbol) || usableAdjustedOhlcBar(row))
      )
        sessionPricesBySymbol.set(symbol, row);
    }
    if (requirements) {
      accumulator.requirementsApplied = true;
      for (const symbol of requirements.rangesBySymbol.keys())
        if (
      ...
      accumulator.requiredPriceRows += requiredSymbols.size;
    }
    ...
    if (requirements?.requireExactObservedSignalHistory) {
      accumulator.nasdaqMembershipEvidenceApplied = true;
      const membershipSymbols = [
        ...new Set(
          asArray(session.universeSymbols).map(symbolOf).filter(Boolean),
        ),
      ].sort();
      const membershipRemovalIdentities = asArray(session.corporateActions)
        .filter(
          (action) =>
            String(action?.type || "").toLowerCase() === "universe-removal",
        )
        .map((action) => ({
          symbol: symbolOf(action),
          effectiveDate: String(action?.effectiveDate || "").slice(0, 10),
        }))
        .sort(
          (left, right) =>
            left.symbol.localeCompare(right.symbol) ||
      ...
      accumulator.membershipPathHash.update(
        `${JSON.stringify({
          date,
          universeSymbols: membershipSymbols,
          removals: membershipRemovalIdentities,
        })}\n`,
      );
      accumulator.minimumMembershipCount = Math.min(
        accumulator.minimumMembershipCount,
        membershipSymbols.length,
      );
      accumulator.maximumMembershipCount = Math.max(
        accumulator.maximumMembershipCount,
        membershipSymbols.length,
      );
      accumulator.requestedMembershipObservations += membershipSymbols.length;
      for (const symbol of membershipSymbols) {
        if (usableAdjustedOhlcBar(sessionPricesBySymbol.get(symbol))) {
          accumulator.availableMembershipObservations++;
          continue;
        }
        accumulator.missingMembershipObservationsBySymbol.set(
          symbol,
          number(
            accumulator.missingMembershipObservationsBySymbol.get(symbol),
            0,
          ) + 1,
        );
      }
      for (const action of asArray(session.corporateActions)) {
        if (String(action?.type || "").toLowerCase() !== "universe-removal")
          continue;
        accumulator.universeRemovalActions++;
        const actionSymbol = symbolOf(action);
        const exactPriceAvailable = usableAdjustedOhlcBar(
          sessionPricesBySymbol.get(actionSymbol),
        );
        const exactResolved =
          action?.treatment === "effective-session-adjusted-open" &&
          exactPriceAvailable;
        const zeroResolved =
          action?.treatment ===
            "conservative-zero-recovery-missing-removal-open-v1" &&
          number(action?.valuePerShare) === 0 &&
          !exactPriceAvailable;
        if (exactResolved) accumulator.universeRemovalOpenPrices++;
        if (zeroResolved) accumulator.universeRemovalZeroRecoveryActions++;
        if (exactResolved || zeroResolved)
          accumulator.universeRemovalResolvedOutcomes++;
        else accumulator.universeRemovalMissingOutcomes++;
      }
    }
    for (const row of asArray(session.prices)) {
      const symbol = symbolOf(row);
      ...
      observedRequiredSymbols.add(symbol);
    ...
    if (requirements) {
      accumulator.observedRequiredPriceRows += observedRequiredSymbols.size;
      for (const symbol of requiredSymbols) {
        if (observedRequiredSymbols.has(symbol)) continue;
        accumulator.missingRequiredPriceRows++;
        const benchmark = pointInTimePriceRequirementContains(
          requirements.benchmarkRangesBySymbol,
          symbol,
          sessionIndex,
        );
        const activeOrExecution = pointInTimePriceRequirementContains(
          requirements.activeRangesBySymbol,
          symbol,
          sessionIndex,
        );
        const reason = benchmark
          ...
          : activeOrExecution
            ? "active-or-execution"
            : "signal-lookback";
        const bySymbol = accumulator.missingRequiredPriceRowsBySymbol.get(
          symbol,
        ) || {
          symbol,
          total: 0,
        ...
        bySymbol.total++;
        if (reason === "benchmark") bySymbol.benchmark++;
        else if (reason === "active-or-execution")
          bySymbol.activeOrExecution++;
        else bySymbol.signalLookback++;
        accumulator.missingRequiredPriceRowsBySymbol.set(symbol, bySymbol);
        if (accumulator.missingRequiredPriceSamples.length < 20)
          accumulator.missingRequiredPriceSamples.push({
            date,
            symbol,
            active: activeOrExecution,
            reason,
          });
      }
    }
...
function finishPointInTimePriceIntegrityAudit(accumulator) {
  const adjustedCoveragePct = accumulator.priceRows
    ...
    : 0;
  const missingRequiredPriceRowsBySymbol = [
    ...accumulator.missingRequiredPriceRowsBySymbol.values(),
  ].sort(
    (left, right) =>
      right.total - left.total || left.symbol.localeCompare(right.symbol),
  );
  const missingRequiredAttributionComplete =
    !accumulator.requirementsApplied ||
    (missingRequiredPriceRowsBySymbol.reduce(
      (total, row) => total + number(row.total, 0),
      0,
    ) === accumulator.missingRequiredPriceRows &&
      missingRequiredPriceRowsBySymbol.every(
        (row) =>
          number(row.activeOrExecution, 0) +
            number(row.signalLookback, 0) +
            number(row.benchmark, 0) ===
  ...
  const missingMembershipObservationsBySymbol = [
    ...accumulator.missingMembershipObservationsBySymbol,
  ]
    .map(([symbol, missing]) => ({ symbol, missing }))
    .sort(
      (left, right) =>
        right.missing - left.missing || left.symbol.localeCompare(right.symbol),
    );
  const missingMembershipObservations = Math.max(
    0,
    accumulator.requestedMembershipObservations -
      accumulator.availableMembershipObservations,
  );
  const membershipObservationCoveragePct =
    accumulator.requestedMembershipObservations > 0
      ? roundMetric(
          (accumulator.availableMembershipObservations /
            accumulator.requestedMembershipObservations) *
            100,
      ...
      : 0;
  const universeRemovalOpenCoveragePct = accumulator.universeRemovalActions
    ? roundMetric(
        (accumulator.universeRemovalOpenPrices /
          accumulator.universeRemovalActions) *
          100,
        4,
      )
    : 100;
  const universeRemovalOutcomeCoveragePct = accumulator.universeRemovalActions
    ? roundMetric(
        (accumulator.universeRemovalResolvedOutcomes /
          accumulator.universeRemovalActions) *
          100,
    ...
    : 100;
  const membershipPathFingerprint = accumulator.nasdaqMembershipEvidenceApplied
    ? accumulator.membershipPathHash.digest("hex")
    : null;
  const pass =
    accumulator.priceRows > 0 &&
    (!accumulator.requirementsApplied ||
      accumulator.missingRequiredPriceRows === 0) &&
    missingRequiredAttributionComplete &&
    (!accumulator.observedSignalHistoryRequired ||
      accumulator.shortSignalHistoryRows === 0) &&
  ...
  return {
    ...
    requiredPriceRows: accumulator.requirementsApplied
      ? accumulator.requiredPriceRows
      : null,
    observedRequiredPriceRows: accumulator.requirementsApplied
      ? accumulator.observedRequiredPriceRows
      : null,
    missingRequiredPriceRows: accumulator.requirementsApplied
      ? accumulator.missingRequiredPriceRows
      : null,
    missingRequiredPriceSamples: accumulator.requirementsApplied
      ? accumulator.missingRequiredPriceSamples
      : [],
    missingRequiredPriceRowsBySymbol: accumulator.requirementsApplied
      ? missingRequiredPriceRowsBySymbol
      : [],
    missingRequiredAttributionComplete,
    nasdaqMembershipEvidenceApplied:
      accumulator.nasdaqMembershipEvidenceApplied,
    ...
    activeMemberDateCoverageComplete:
      accumulator.requestedMembershipObservations > 0 &&
      missingMembershipObservations === 0,
    universeRemovalActions: accumulator.universeRemovalActions,
    universeRemovalOpenPrices: accumulator.universeRemovalOpenPrices,
    universeRemovalOpenCoveragePct,
    universeRemovalZeroRecoveryActions:
      accumulator.universeRemovalZeroRecoveryActions,
    universeRemovalResolvedOutcomes:
      accumulator.universeRemovalResolvedOutcomes,
    universeRemovalMissingOutcomes:
      accumulator.universeRemovalMissingOutcomes,
    universeRemovalOutcomeCoveragePct,
    universeRemovalOutcomeCoverageComplete:
      accumulator.universeRemovalResolvedOutcomes ===
        accumulator.universeRemovalActions &&
      accumulator.universeRemovalMissingOutcomes === 0,
    minimumMembershipCount: Number.isFinite(
      accumulator.minimumMembershipCount,
    )
      ? accumulator.minimumMembershipCount
      : 0,
    maximumMembershipCount: accumulator.maximumMembershipCount,
    observedSignalHistoryRequired:
      accumulator.observedSignalHistoryRequired,
    observedSignalHistoryPass:
      !accumulator.observedSignalHistoryRequired ||
      accumulator.shortSignalHistoryRows === 0,
    shortSignalHistoryRows: accumulator.shortSignalHistoryRows,
    shortSignalHistorySamples: accumulator.shortSignalHistorySamples,
    ...
    interpretation: accumulator.observedSignalHistoryRequired
      ? "Only active-member prices, each signal's exact 253 observed input rows plus any internal calendar gaps, and full benchmark histories determine integrity. Nasdaq signals are suppressed until all 253 input rows exist; no first-price date is treated as proof of listing. Archival post-removal rows are excluded. Extreme moves are disclosed; they fail only when both the open and close preserve a common split ratio with ordinary intraday movement, when active symbols share an identical full OHLCV row, or when adjusted/OHLC invariants fail."
      : "Only active-member prices, each signal's current row plus its 253-session lookback, and benchmark histories determine integrity. Archival post-removal rows are excluded. Extreme moves are disclosed; they fail only when both the open and close preserve a common split ratio with ordinary intraday movement, when active symbols share an identical full OHLCV row, or when adjusted/OHLC invariants fail.",
  };
}
...
export function pointInTimePriceIntegrityAudit(
  ...
  const requirements = decisionRelevantOnly
    ? pointInTimePriceRequirements(normalizedDatasets, {
        lookbackSessions,
        requireExactObservedSignalHistory,
      })
    : null;
...
function r40PlaceboStatistic(row) {
  return Math.min(
    number(row?.summary?.aggregate?.simpleDifferenceVsSpyPct, -Infinity),
    number(row?.summary?.aggregate?.simpleDifferenceVsQqqPct, -Infinity),
  );
}

async function pointInTimeNasdaqR40PlaceboContext() {
  const [context, report] = await Promise.all([
    pointInTimeNasdaqR40Context(),
    getPointInTimeNasdaqAdaptiveReplacementR40(),
  ]);
  if (report?.status !== "complete" || report?.selectedCandidateId !== "r41-two-leader-buffer-three" || report?.historicalDevelopmentGatesPassed !== true)
    throw new Error("The frozen R41 development survivor is required for strict placebo testing");
  const selected = asArray(report.developmentCandidates).find((row) => row.id === report.selectedCandidateId);
  if (!selected || selected.definitionFingerprint !== report.selectedDefinitionFingerprint)
    throw new Error("The frozen R41 definition fingerprint is inconsistent");
  const fingerprint = sha256Fingerprint(JSON.stringify({
    datasetFingerprint: report.datasetFingerprint,
    selectedDefinitionFingerprint: report.selectedDefinitionFingerprint,
    candidateSetFingerprint: report.candidateSetFingerprint,
    contract: POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_CONTRACT,
    seeds: POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SEEDS,
    shards: POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SHARDS,
  }));
  return { ...context, report, selected, placeboFingerprint: fingerprint };
}

function r40PlaceboShardPath(context, shard) {
  return `${POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_PREFIX}/${context.placeboFingerprint}/shard-${String(shard).padStart(2, "0")}.json`;
}

export async function runPointInTimeNasdaqAdaptiveReplacementPlaceboWorker({ shard } = {}) {
  const context = await pointInTimeNasdaqR40PlaceboContext();
  const shardIndex = Math.floor(number(shard, -1));
  if (shardIndex < 0 || shardIndex >= POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SHARDS) throw new Error("Unknown R40 strict-placebo shard");
  const pathname = r40PlaceboShardPath(context, shardIndex);
  const existing = await readExactPrivateJson(pathname).catch(() => null);
  if (existing?.status === "complete" && existing?.placeboFingerprint === context.placeboFingerprint) return { ...existing, cached: true };
  const seedsPerShard = POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SEEDS / POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SHARDS;
  const seedStart = shardIndex * seedsPerShard + 1;
  const seedEnd = seedStart + seedsPerShard - 1;
  const startedAt = new Date().toISOString();
  await persistPrivateJson(pathname, { status: "running", shard: shardIndex, seedStart, seedEnd, placeboFingerprint: context.placeboFingerprint, startedAt });
  try {
    const maxima = [];
    for (let seed = seedStart; seed <= seedEnd; seed += 1) {
      const placeboDefinitions = context.definitions.map((definition) => ({
        ...definition,
        id: `placebo-${seed}-${definition.id}`,
        researchGeneration: "strict-placebo",
        label: `Seed ${seed} matched ${definition.id}`,
        control: true,
        overrides: { ...definition.overrides, researchRankMode: "random-placebo", researchRandomSeed: seed },
      }));
      const evaluations = await evaluatePointInTimeNasdaqContinuousRunnerDefinitions(context, placeboDefinitions, context.developmentWindows);
      const rows = evaluations.map((evaluation) => r15CandidateRow(evaluation, 15));
      const strongest = [...rows].sort((left, right) => r40PlaceboStatistic(right) - r40PlaceboStatistic(left))[0];
      maxima.push({ seed, statisticPct: roundMetric(r40PlaceboStatistic(strongest), 3), totalReturnPct: strongest.summary.aggregate.totalReturnPct, candidateId: strongest.id, closedTrades: strongest.summary.aggregate.closedTrades });
    }
    const result = { status: "complete", shard: shardIndex, seedStart, seedEnd, seedCount: maxima.length, placeboFingerprint: context.placeboFingerprint, startedAt, completedAt: new Date().toISOString(), maxima };
    await persistPrivateJson(pathname, result);
    return result;
  } catch (error) {
    await persistPrivateJson(pathname, { status: "failed", shard: shardIndex, seedStart, seedEnd, placeboFingerprint: context.placeboFingerprint, startedAt, failedAt: new Date().toISOString(), error: sanitizedError(error) }).catch(() => {});
    throw error;
  }
}

export async function finalizePointInTimeNasdaqAdaptiveReplacementPlacebo() {
  const context = await pointInTimeNasdaqR40PlaceboContext();
  const shards = [];
  for (let shard = 0; shard < POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SHARDS; shard += 1) {
    const result = await readExactPrivateJson(r40PlaceboShardPath(context, shard));
    if (result?.status !== "complete" || result?.placeboFingerprint !== context.placeboFingerprint || asArray(result.maxima).length !== 50) throw new Error(`R40 placebo shard ${shard} failed integrity`);
    shards.push(result);
  }
  const maxima = shards.flatMap((row) => row.maxima).sort((left, right) => left.seed - right.seed);
  if (maxima.length !== POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SEEDS || new Set(maxima.map((row) => row.seed)).size !== POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_SEEDS) throw new Error("R40 placebo seed coverage failed integrity");
  const values = maxima.map((row) => row.statisticPct);
  const candidateStatisticPct = roundMetric(r40PlaceboStatistic(context.selected), 3);
  const exceedances = values.filter((value) => value >= candidateStatisticPct).length;
  const familyWiseEmpiricalPValue = roundMetric((exceedances + 1) / (values.length + 1), 6);
  const percentile95StatisticPct = roundMetric(percentileValue(values, 0.95), 3);
  const passed = candidateStatisticPct > percentile95StatisticPct && familyWiseEmpiricalPValue < 0.05;
  const intervals = shards.map((row) => ({ shard: row.shard, startedAt: row.startedAt, completedAt: row.completedAt }));
  const timestamps = intervals.flatMap((row) => [new Date(row.startedAt).getTime(), new Date(row.completedAt).getTime()]);
  const strictPlacebo = {
    status: "complete", contract: POINT_IN_TIME_NASDAQ_RUNNER_R40_PLACEBO_CONTRACT, seedCount: values.length, familyVariantCount: context.definitions.length,
    statistic: "minimum of simple return differences versus SPY and QQQ; maximum across the five predeclared lifecycle variants within each random seed",
    candidateStatisticPct, percentile95StatisticPct, medianStatisticPct: roundMetric(percentileValue(values, 0.5), 3), maximumStatisticPct: roundMetric(Math.max(...values), 3), minimumStatisticPct: roundMetric(Math.min(...values), 3),
    exceedances, familyWiseEmpiricalPValue, passed, strongestPlacebo: [...maxima].sort((left, right) => right.statisticPct - left.statisticPct)[0],
    parallelExecution: { shardCount: shards.length, seedsPerShard: 50, peakConcurrentWorkers: peakConcurrentIntervals(intervals), elapsedSeconds: roundMetric((Math.max(...timestamps) - Math.min(...timestamps)) / 1_000, 3) },
  };
  const report = {
    ...context.report,
    updatedAt: new Date().toISOString(),
    strictPlacebo,
    strictPlaceboGatePassed: passed,
    candidateDisposition: passed ? "frozen-for-prospective-tracking-only" : "rejected-by-strict-placebo",
    allHistoricalScreenGatesPassed: passed,
    allEvidenceGatesPassed: false,
    eligibleForAlphaClaim: false,
    eligibleForLiveCapital: false,
    productionChanged: false,
    nextStep: passed ? "Track R41 unchanged for at least 60 genuinely new sessions and require independent replication before any live authority." : "Preserve the strict-placebo rejection; do not promote or retune R41 on these inspected data.",
  };
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_RUNNER_R40_STORE, report);
  return report;
}

async function pointInTimeNasdaqR45Context() {
  const [context, r40] = await Promise.all([pointInTimeNasdaqR40Context(), getPointInTimeNasdaqAdaptiveReplacementR40()]);
  if (r40?.candidateDisposition !== "rejected-by-strict-placebo" || r40?.strictPlacebo?.seedCount !== 1_000) throw new Error("R45-R54 require the preserved strict R41 placebo rejection");
  const definitions = pointInTimeNasdaqDistinctAlphaDefinitions();
  const controls = pointInTimeNasdaqDistinctAlphaControls();
  const workUnits = [...definitions, ...controls];
  const candidateSetFingerprint = sha256Fingerprint(JSON.stringify(workUnits));
  const experimentFingerprint = sha256Fingerprint(JSON.stringify({ datasetFingerprint: context.datasetFingerprint, candidateSetFingerprint, contract: POINT_IN_TIME_NASDAQ_R45_CONTRACT, windows: context.developmentWindows }));
  return { ...context, definitions, controls, workUnits, definitionsById: new Map(workUnits.map((row) => [row.id, row])), candidateSetFingerprint, experimentFingerprint };
}

function r45Path(context, id) { return `${POINT_IN_TIME_NASDAQ_R45_PREFIX}/${context.experimentFingerprint}/${id}.json`; }

export async function runPointInTimeNasdaqDistinctAlphaWorker({ candidateId } = {}) {
  const context = await pointInTimeNasdaqR45Context();
  const definition = context.definitionsById.get(String(candidateId || ""));
  if (!definition) throw new Error("Unknown R45-R54 work unit");
  const path = r45Path(context, definition.id);
  const fingerprint = r15DefinitionFingerprint(definition);
  const existing = await readExactPrivateJson(path).catch(() => null);
  if (existing?.status === "complete" && existing?.definitionFingerprint === fingerprint) return { ...existing, cached: true };
  const startedAt = new Date().toISOString();
  try {
    const [evaluation] = await evaluatePointInTimeNasdaqContinuousRunnerDefinitions(context, [definition], context.developmentWindows);
    const result = r15CandidateRow(evaluation, 15);
    const report = { status: "complete", version: 45, researchGeneration: "R45-R54", candidateId: definition.id, control: definition.control === true, experimentFingerprint: context.experimentFingerprint, definitionFingerprint: fingerprint, startedAt, completedAt: new Date().toISOString(), result, productionChanged: false };
    await persistPrivateJson(path, report); return report;
  } catch (error) {
    await persistPrivateJson(path, { status: "failed", candidateId: definition.id, definitionFingerprint: fingerprint, experimentFingerprint: context.experimentFingerprint, startedAt, failedAt: new Date().toISOString(), error: sanitizedError(error) }).catch(() => {}); throw error;
  }
}

export async function finalizePointInTimeNasdaqDistinctAlphaDevelopment() {
  const context = await pointInTimeNasdaqR45Context();
  const workers = await Promise.all(context.workUnits.map((definition) => readExactPrivateJson(r45Path(context, definition.id))));
  for (const worker of workers) if (worker?.status !== "complete" || worker?.experimentFingerprint !== context.experimentFingerprint) throw new Error("R45-R54 worker integrity failed");
  const candidates = workers.filter((row) => !row.control).map((row) => row.result).sort(r14SelectionComparator);
  const controls = workers.filter((row) => row.control).map((row) => row.result).sort(r14SelectionComparator);
  const finalist = candidates.find((row) => row.clearsPhase) || null;
  const intervals = workers.map((row) => ({ candidateId: row.candidateId, startedAt: row.startedAt, completedAt: row.completedAt }));
  const timestamps = intervals.flatMap((row) => [new Date(row.startedAt).getTime(), new Date(row.completedAt).getTime()]);
  const report = { version: 45, researchGeneration: "R45-R54", status: "complete", frozenDate: "2026-09-04", datasetThrough: context.datasetThrough, datasetFingerprint: context.datasetFingerprint, candidateSetFingerprint: context.candidateSetFingerprint, experimentFingerprint: context.experimentFingerprint, executionContract: POINT_IN_TIME_NASDAQ_R45_CONTRACT, candidateCount: candidates.length, controlCount: controls.length, developmentCandidates: candidates, developmentControls: controls, selectedCandidateId: finalist?.id || null, candidateDisposition: finalist ? "frozen-for-strict-placebo-only" : "rejected-by-development-screen", historicalDevelopmentGatesPassed: Boolean(finalist), allEvidenceGatesPassed: false, eligibleForAlphaClaim: false, eligibleForLiveCapital: false, productionChanged: false, prospectiveSessions: 0, parallelExecution: { workerCount: workers.length, peakConcurrentWorkers: peakConcurrentIntervals(intervals), elapsedSeconds: roundMetric((Math.max(...timestamps) - Math.min(...timestamps)) / 1_000, 3) }, limitations: ["R45-R54 are inspected development and cannot validate alpha.", "Cash remains cash; rankings use only decision-date information.", "A survivor still requires a new strict 1,000-seed family-wise placebo and 60 genuinely new sessions."] };
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_R45_STORE, report); return report;
}

async function pointInTimeNasdaqR55Context() {
  const [context, r45] = await Promise.all([pointInTimeNasdaqR40Context(), getPointInTimeNasdaqDistinctAlphaR45()]);
  if (r45?.candidateDisposition !== "rejected-by-development-screen") throw new Error("R55-R64 require the preserved R45-R54 development rejection");
  const definitions = pointInTimeNasdaqEventAlphaDefinitions();
  const controls = pointInTimeNasdaqEventAlphaControls();
  const workUnits = [...definitions, ...controls];
  const candidateSetFingerprint = sha256Fingerprint(JSON.stringify(workUnits));
  const experimentFingerprint = sha256Fingerprint(JSON.stringify({ datasetFingerprint: context.datasetFingerprint, candidateSetFingerprint, contract: POINT_IN_TIME_NASDAQ_R55_CONTRACT, windows: context.developmentWindows }));
  return { ...context, definitions, controls, workUnits, definitionsById: new Map(workUnits.map((row) => [row.id, row])), candidateSetFingerprint, experimentFingerprint };
}

function r55Path(context, id) { return `${POINT_IN_TIME_NASDAQ_R55_PREFIX}/${context.experimentFingerprint}/${id}.json`; }

export async function runPointInTimeNasdaqEventAlphaWorker({ candidateId } = {}) {
  const context = await pointInTimeNasdaqR55Context();
  const definition = context.definitionsById.get(String(candidateId || ""));
  if (!definition) throw new Error("Unknown R55-R64 work unit");
  const path = r55Path(context, definition.id); const fingerprint = r15DefinitionFingerprint(definition);
  const existing = await readExactPrivateJson(path).catch(() => null);
  if (existing?.status === "complete" && existing?.definitionFingerprint === fingerprint) return { ...existing, cached: true };
  const startedAt = new Date().toISOString();
  try {
    const [evaluation] = await evaluatePointInTimeNasdaqContinuousRunnerDefinitions(context, [definition], context.developmentWindows);
    const result = r15CandidateRow(evaluation, 15);
    const report = { status: "complete", version: 55, researchGeneration: "R55-R64", candidateId: definition.id, control: definition.control === true, experimentFingerprint: context.experimentFingerprint, definitionFingerprint: fingerprint, startedAt, completedAt: new Date().toISOString(), result, productionChanged: false };
    await persistPrivateJson(path, report); return report;
  } catch (error) {
    await persistPrivateJson(path, { status: "failed", candidateId: definition.id, definitionFingerprint: fingerprint, experimentFingerprint: context.experimentFingerprint, startedAt, failedAt: new Date().toISOString(), error: sanitizedError(error) }).catch(() => {}); throw error;
  }
...
export async function finalizePointInTimeNasdaqEventAlphaDevelopment() {
  const context = await pointInTimeNasdaqR55Context();
  const workers = await Promise.all(context.workUnits.map((definition) => readExactPrivateJson(r55Path(context, definition.id))));
  for (const worker of workers) if (worker?.status !== "complete" || worker?.experimentFingerprint !== context.experimentFingerprint) throw new Error("R55-R64 worker integrity failed");
  const candidates = workers.filter((row) => !row.control).map((row) => row.result).sort(r14SelectionComparator);
  const controls = workers.filter((row) => row.control).map((row) => row.result).sort(r14SelectionComparator);
  const finalist = candidates.find((row) => row.clearsPhase) || null;
  const intervals = workers.map((row) => ({ candidateId: row.candidateId, startedAt: row.startedAt, completedAt: row.completedAt }));
  const timestamps = intervals.flatMap((row) => [new Date(row.startedAt).getTime(), new Date(row.completedAt).getTime()]);
  const report = { version: 55, researchGeneration: "R55-R64", status: "complete", frozenDate: "2026-09-04", datasetThrough: context.datasetThrough, datasetFingerprint: context.datasetFingerprint, candidateSetFingerprint: context.candidateSetFingerprint, experimentFingerprint: context.experimentFingerprint, executionContract: POINT_IN_TIME_NASDAQ_R55_CONTRACT, candidateCount: candidates.length, controlCount: controls.length, developmentCandidates: candidates, developmentControls: controls, selectedCandidateId: finalist?.id || null, candidateDisposition: finalist ? "frozen-for-strict-placebo-only" : "rejected-by-development-screen", historicalDevelopmentGatesPassed: Boolean(finalist), allEvidenceGatesPassed: false, eligibleForAlphaClaim: false, eligibleForLiveCapital: false, productionChanged: false, prospectiveSessions: 0, parallelExecution: { workerCount: workers.length, peakConcurrentWorkers: peakConcurrentIntervals(intervals), elapsedSeconds: roundMetric((Math.max(...timestamps) - Math.min(...timestamps)) / 1_000, 3) }, limitations: ["R55-R64 are inspected development and cannot validate alpha.", "Cash remains cash; rankings use only decision-date information.", "A survivor still requires a strict 1,000-seed family-wise placebo and 60 genuinely new sessions."] };
  await persistPrivateJson(POINT_IN_TIME_NASDAQ_R55_STORE, report); return report;
}
...
export async function runFmpResearchBacktest({
  ...
  const persistRunProgress = compileOnly
    ? async () => {}
    : persistReport;
  await persistRunProgress(running);
  const client = createFmpClient(apiKey);
  ...
  try {
    ...
    if (priceHistory.remainingSymbols.length) {
      ...
      await persistRunProgress(collecting);
      return collecting;
    }
    if (!histories.has("SPY") || !histories.has("QQQ"))
      throw new Error("SPY and QQQ benchmark histories are required");
    ...
    if (!compiledCacheHit) {
      ...
      await persistRunProgress(collecting);
      return compileOnly && compilerProgress.complete
        ? {
            status: "complete",
            mode: "compiled-dataset-refresh",
            datasetThrough: sessionDates.at(-1) || null,
            compiledSessions: compilerProgress.completedSessions,
            compiledChunks: chunks.length,
          }
        : collecting;
    ...
    await persistRunProgress(report);
    return report;
  } catch (error) {
    const failed = {
      version: REPORT_VERSION,
    ...
    await persistRunProgress(failed).catch(() => {});
    throw error;
  }
}
