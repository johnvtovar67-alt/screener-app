...
const REPLAY_CHECKPOINT_SCHEMA = 4;
...
async function runProvisionalWindows(
  ...
  const grid = [
    {
      thesisId: "v7-live-policy-control",
      thesisLabel: "V7 live policy control",
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const compiledSignature = JSON.stringify({
      ...
      fromDate,
      endDate,
      rawSessions: rawDataset.sessions.length,
      firstRawSession: rawDataset.sessions[0]?.date || null,
      lastRawSession: rawDataset.sessions.at(-1)?.date || null,
    });
    ...
    const compiledCacheHit = Boolean(
      savedCompiledMatches &&
        ...
        expectedChunkStart === rawDataset.sessions.length,
    );
    let compiled = null;
    if (!compiledCacheHit) {
      ...
      await persistPrivateJson(FMP_RESEARCH_COMPILED_CHECKPOINT_STORE, {
        ...
        sessionDates: rawDataset.sessions.map((session) => session.date),
        chunks,
      });
      const collecting = {
        version: REPORT_VERSION,
    ...
    compiled = {
      ...
      sessions: restoredChunks.flat(),
    };
    if (compiled.sessions.length !== rawDataset.sessions.length)
      throw new Error(
        `Compiled research restore is incomplete (${compiled.sessions.length}/${rawDataset.sessions.length})`,
      );
    await persistReport({
      ...running,
      updatedAt: new Date().toISOString(),
      message:
        "The point-in-time signal dataset was restored; walk-forward replay is starting.",
      progress: {
        stage: "compiled",
    ...
    const report = {
      ...
      methodology: {
        ...
        liveControlReplaysProductionPortfolioPolicy: true,
