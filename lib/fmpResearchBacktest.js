...
const REPLAY_CHECKPOINT_SCHEMA = 5;
const REPLAY_WINDOWS_PER_RUN = 1;
const REPLAY_WARMUP_SESSIONS = 2;
const V8_ACTIVE_CANDIDATE_COUNT = 3;
...
function parameterScore(run = {}) {
  ...
  const exposurePenalty=Math.max(0,55-number(metrics.averageExposurePct,0))/20;
  return number(metrics.totalReturnPct,-100)/8+
    ...
    turnoverPenalty-tradePenalty-exposurePenalty;
}
...
function buildV8ResearchFolds(sessionDates = []) {
...
function nextReplaySessionSlice(sessionDates = [], completedWindows = 0) {
  const { dates, folds } = buildV8ResearchFolds(sessionDates);
  const windowsPerFold = 3;
  const windowsPerCandidate = folds.length * windowsPerFold;
  const totalWindows = V8_ACTIVE_CANDIDATE_COUNT * windowsPerCandidate;
...
async function runProvisionalWindows(
  ...
  const calendar = Array.isArray(calendarDates) && calendarDates.length
    ? calendarDates
    : sessions;
  const { usableDates, folds } = buildV8ResearchFolds(calendar);
  ...
  const grid = [
    {
      ...
      thesisLabel: "Persistent quality leadership",
    ...
    },
    {
      ...
      thesisLabel: "Controlled acceleration leadership",
      selectionEligible: true,
    ...
    },
    {
      ...
      thesisLabel: "Quality re-acceleration, value aware",
      selectionEligible: true,
      minimumQualifiedSessions: 2,
      maxPriceVs50Pct: 10,
  ...
  if (grid.length !== V8_ACTIVE_CANDIDATE_COUNT)
    throw new Error("V8 replay candidate count is inconsistent");
  ...
  if (!selectable.length)
    throw new Error("No active V8 thesis is eligible for selection");
  ...
  const evidenceChecks = {
    ...
    positiveAlphaInMajorityOfFolds:
      alphaByFold.filter((value) => value > 0).length > folds.length / 2,
    minimumClosedRoundTrips:
      number(walkForwardMetrics.closedTrades, 0) >= 30,
    minimumAverageExposure:
      number(walkForwardMetrics.averageExposurePct, 0) >= 55,
  };
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const replaySignature = JSON.stringify({
      ...
      thesisContract: "v8-independent-factor-lifecycle-v1",
      statementSignature,
      finalSymbols: finalSymbols.slice().sort(),
      fromDate,
      endDate,
    ...
    const report = {
      ...
      limitations: [
        ...
        "The completed V7 report is retained as the external baseline rather than replayed inside V8; removing that non-selectable control does not affect V8 thesis selection.",
