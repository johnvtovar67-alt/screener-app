...
import {
  V11_DEVELOPMENT_PLACEBO_SEEDS,
  V11_EVIDENCE_REQUIREMENTS,
  V11_THESIS_ID,
  v11AuditControlDefinitions,
  v11StrategyOptions,
} from "./v11ResearchContract";
...
const REPLAY_CHECKPOINT_SCHEMA = 10;
...
const V11_ACTIVE_THESIS_COUNT = 1;
...
function buildV11ResearchFolds(sessionDates = []) {
...
function nextReplaySessionSlice(
  ...
  const { dates, folds } = buildV11ResearchFolds(sessionDates);
  const windowsPerFold = 3;
  const windowsPerCandidate = folds.length * windowsPerFold;
  const totalWindows = V11_ACTIVE_THESIS_COUNT * windowsPerCandidate;
...
async function runProvisionalWindows(
  ...
  const calendar =
    Array.isArray(calendarDates) && calendarDates.length
      ? calendarDates
      : sessions;
  const { usableDates, folds } = buildV11ResearchFolds(calendar);
  const grid = [v11StrategyOptions()];
  if (grid.length !== V11_ACTIVE_THESIS_COUNT)
    throw new Error("V11 replay thesis count is inconsistent");
  const auditControlDefinitions = v11AuditControlDefinitions();
  ...
  for (const candidate of asArray(initial?.candidateRuns)) {
    ...
    for (const foldRun of asArray(candidate?.foldRuns)) {
      ...
      if (
        ...
        foldRun?.auditControls?.priorV10Composite?.metrics &&
      ...
      for (const [key, window] of windows) {
        ...
        if (key === "rollingAudit") {
          ...
          foldRun.auditControls = {
            ...
            priorV10Composite: controlRuns[1],
            simpleQuality: controlRuns[2],
            transparentBullCyclePullback: controlRuns[3],
            randomPlacebos: Array.from(
              { length: V11_DEVELOPMENT_PLACEBO_SEEDS },
              (_, placeboIndex) => {
  ...
  if (foldRuns.length !== folds.length)
    throw new Error("The predeclared V11 thesis is missing completed folds");
  ...
  const priorV10CompositeAudit = aggregateResearchRuns(
    foldRuns.map((fold) => fold.auditControls.priorV10Composite),
  );
  ...
  const controls = {
    ...
    priorV10Composite: metricsSummary(priorV10CompositeAudit, {
      tradeLimit: 40,
      skippedLimit: 0,
    }),
    ...
    randomPlacebo: {
      seedCount: V11_DEVELOPMENT_PLACEBO_SEEDS,
      construction:
      ...
      strictPointInTimeRequirement:
        V11_EVIDENCE_REQUIREMENTS.strictPointInTimePlaceboSeeds,
    },
  };
  ...
  const evidenceChecks = {
    ...
    beatsQqqInMajorityOfFolds:
      ...
      folds.length / 2,
    beatsPriorV10CompositeControl:
      thesisTotalReturn >
      number(priorV10CompositeAudit.metrics?.totalReturnPct, Infinity),
    ...
    minimumClosedRoundTrips:
      ...
      V11_EVIDENCE_REQUIREMENTS.minimumClosedRoundTrips,
    minimumActiveStockExposure:
      number(walkForwardMetrics.averageActiveExposurePct, 0) >=
      V11_EVIDENCE_REQUIREMENTS.minimumAverageActiveStockExposurePct,
      ...
      walkForwardSelectionAudit: {
        ...
        evidenceAssessment: {
          ...
          minimumAverageActiveStockExposurePct:
            V11_EVIDENCE_REQUIREMENTS.minimumAverageActiveStockExposurePct,
          primaryAlphaMeasure: V11_EVIDENCE_REQUIREMENTS.primaryAlphaMeasure,
          selectorUsed: false,
          benchmarkCompletionSleeveUsed: false,
          developmentPlaceboSeeds: V11_DEVELOPMENT_PLACEBO_SEEDS,
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const replaySignature = JSON.stringify({
      ...
      thesisContract: v11StrategyOptions({ contractId: V11_THESIS_ID }),
      statementSignature,
      finalSymbols: finalSymbols.slice().sort(),
      fromDate,
      endDate,
    ...
    if (replayResult.status === "collecting") {
      const collecting = {
        ...
        updatedAt: new Date().toISOString(),
        message:
          "The predeclared V11 thesis is being replayed in bounded, durable batches.",
        progress: {
          stage: "replay",
    ...
    const report = {
      ...
      methodology: {
        ...
        activeThesisWasPostSelectedFromV10Control: true,
        ...
        predeclaredActiveThesisCount: V11_ACTIVE_THESIS_COUNT,
        ...
        momentumAndRelativeStrengthWeightPct: 70,
        qualityAndStabilityWeightPct: 20,
        timingAndControlledPullbackWeightPct: 10,
        ...
        developmentControls: [
          "simple momentum",
          "V10 quality-momentum blend",
          "quality",
          "transparent bull-cycle/pullback",
          "random placebo",
        ],
        developmentRandomPlaceboSeeds: V11_DEVELOPMENT_PLACEBO_SEEDS,
        ...
        sameHistoricalWindowsReusedAfterV7ThroughV10Diagnosis: true,
      },
      universe: {
      ...
      limitations: [
        ...
        "V7 through V10 are retained as external baselines and are not rerun; V9's SPY completion sleeve remains rejected.",
        ...
        "V11 uses the observed V10 momentum-control result to rebalance the blend toward momentum. Every reused historical date is therefore contaminated development data, not a new untouched test.",
      ...
      nextResearchRequirement:
        "Run the frozen V11 thesis on genuinely untouched or forward data under screener-pit-v1, including historical membership, delistings, revision-safe fundamentals, point-in-time material news and at least 1,000 matched random placebos before making an alpha claim or changing live recommendations.",
    };
