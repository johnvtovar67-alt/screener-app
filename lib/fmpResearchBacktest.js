...
const POINT_IN_TIME_SP500_ALPHA_V2_INTEGRITY_REPORT_VERSION = 2;
...
function pointInTimePriceRequirements(
  datasets = [],
  {
  ...
  const activeRangesBySymbol = new Map();
  let sessionOffset = 0;
    ...
    for (const [localIndex, session] of sessions.entries()) {
      ...
      for (const symbol of activeSymbols) {
        addPointInTimePriceRequirement(
          rangesBySymbol,
          symbol,
          Math.max(0, sessionIndex - lookbackSessions),
          sessionIndex,
        );
        addPointInTimePriceRequirement(
          activeRangesBySymbol,
          symbol,
          sessionIndex,
          sessionIndex,
        );
  ...
  for (const symbol of comparisonSymbols.map(symbolOf).filter(Boolean)) {
    addPointInTimePriceRequirement(
      rangesBySymbol,
      symbol,
      0,
      Math.max(0, sessionOffset - 1),
    );
  }
  return {
    rangesBySymbol,
    activeRangesBySymbol,
    totalSessions: sessionOffset,
    lookbackSessions,
...
export async function runPointInTimeSp500AlphaCreatorV2Integrity({
  ...
  try {
    ...
    for (const chunk of asArray(manifest.chunks)) {
      ...
      requirementDatasets.push({
        sessions: sessions.map((session) => ({
          date: session.date,
          signals: session.signals.map((signal) => ({
            symbol: symbolOf(signal),
          })),
          positionSignals: session.positionSignals.map((signal) => ({
            symbol: symbolOf(signal),
          })),
        })),
      });
    }
    const priceRequirements = pointInTimePriceRequirements(
      requirementDatasets,
      { lookbackSessions: 253 },
    );
