...
const COMPILED_CHECKPOINT_SCHEMA = 1;
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const compiledSignature = JSON.stringify({
      ...
      compilerContract: "historical-signal-evaluator-v7-research-v1",
      statementSignature,
    ...
    const compiledCacheHit = Boolean(
      savedCompiled?.schema === COMPILED_CHECKPOINT_SCHEMA &&
        ...
        savedCompiled.dataset.sessions.length,
    );
    const compiled = compiledCacheHit
      ? savedCompiled.dataset
      : compilePointInTimeSignals(rawDataset, {
          liquidity: { maxCandidates: 500 },
        });
    await persistReport({
      ...running,
      updatedAt: new Date().toISOString(),
      message: compiledCacheHit
        ? "The point-in-time signal dataset was restored; walk-forward replay is starting."
        : "The point-in-time signal dataset was compiled; its durable checkpoint is being written.",
      progress: {
        stage: "compiled",
        compiledCacheHit,
