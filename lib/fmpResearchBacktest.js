...
async function fetchStatementHistory(
  ...
  startYear,
  endYear,
  { initial = null, onYearComplete = null } = {},
) {
  ...
  const resumeYear = Math.max(
    startYear,
    number(initial?.completedThroughYear, startYear - 1) + 1,
  );
  for (let year = resumeYear; year <= endYear; year++) {
    ...
    if (onYearComplete)
      await onYearComplete({ ...output, completedThroughYear: year });
  }
  return output;
}
