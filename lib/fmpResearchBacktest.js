...
const POINT_IN_TIME_NASDAQ_UNIVERSE_CONTRACT =
  "date-added-effective-inclusive-boundary-v2";
...
function pointInTimeNasdaqManifestHasCurrentContract(manifest) {
  ...
  return Boolean(
    manifest?.complete === true &&
      asArray(manifest?.chunks).length > 0 &&
...
function assertCurrentPointInTimeNasdaqEvidence(manifest, datasetStatus) {
  if (
    !pointInTimeNasdaqManifestHasCurrentContract(manifest) ||
    datasetStatus?.status !== "compiled" ||
    datasetStatus?.datasetFingerprint !== manifest?.evidenceDatasetFingerprint
  )
    throw new Error(
...
export async function getPointInTimeNasdaqDatasetStatus() {
  try {
    ...
    if (report?.status !== "compiled") return report;
    const manifest = await readExactPrivateJson(
      POINT_IN_TIME_NASDAQ_COMPILED_CHECKPOINT_STORE,
    ).catch(() => null);
    const currentContract =
      pointInTimeNasdaqManifestHasCurrentContract(manifest);
    if (currentContract) return report;
    return {
      ...report,
      status: "stale",
      runClaimedAt: null,
...
export async function getPointInTimeNasdaqPriceIntegrity() {
  try {
    return await readExactPrivateJson(POINT_IN_TIME_NASDAQ_PRICE_INTEGRITY_STORE);
  } catch (error) {
    return { status: "unavailable", error: sanitizedError(error) };
  }
}
...
export function pointInTimeIndexRawDatasetFromHistory({
  ...
  const sessions = spy.map((benchmarkBar) => {
    ...
    availableMembershipObservations += universeSymbols.filter(
      (symbol) =>
...
export async function runPointInTimeNasdaqUniverse({ force = false } = {}) {
  ...
  const normalizedChanges = changeRows
    .map((row) => {
      const effectiveDate =
        parsedIndexDate(row.dateAdded) || parsedIndexDate(row.date);
  ...
  const deduplicatedChanges = normalizedChanges.filter(
    (row, index, rows) =>
      index === 0 ||
      `${row.date}|${row.addedSymbol}|${row.removedSymbol}` !==
        `${rows[index - 1].date}|${rows[index - 1].addedSymbol}|${rows[index - 1].removedSymbol}`,
  );
  ...
  const relevantChanges = deduplicatedChanges.filter(
    (row) =>
  ...
  const rawMembershipDigest = sha256Fingerprint(
    JSON.stringify({
      currentSymbols,
  ...
  const report = {
    ...
    nextStep:
      "Acquire one adjusted price history per union member and compile member-date signals with explicit removal-at-next-open actions.",
    limitations: [
      ...
      "dateAdded is treated as the effective membership date; the dated event is not eligible until the next observable market session.",
...
export async function runPointInTimeNasdaqDatasetAcquisition({
  ...
  const limitations = [
    ...
    "Every active member-date must have a usable adjusted price; each removal must resolve at the fixed next-session adjusted open or an explicitly disclosed conservative zero recovery before any audit result can count.",
  ];
  const apiKey = process.env.FMP_API_KEY || process.env.FMP_KEY;
  if (!apiKey)
    throw new Error("The configured research provider key is unavailable");
  ...
  const savedNasdaqSignature = parseSignature(savedNasdaqPrices?.signature);
  const savedNasdaqMatches = Boolean(
    savedNasdaqSignature?.fromDate === fromDate &&
      savedNasdaqSignature?.endDate === endDate &&
      savedNasdaqSignature?.priceContract === priceContract.id,
  );
...
async function compilePointInTimeNasdaqDataset({
  ...
  const compilationLimitations = [
    ...limitations,
    "A membership removal uses the first post-event market-session adjusted open with 12 bps slippage; if that exact open is unavailable, the held position receives an immediate conservative zero recovery and exact-open coverage remains disclosed.",
...
export async function runPointInTimeNasdaqPriceIntegrity({
  ...
  const [manifest, datasetStatus] = await Promise.all([
  ...
  if (
    !force &&
    existing?.status === "complete" &&
    existing?.datasetFingerprint === evidenceDatasetFingerprint
  )
  ...
  const requestedMembershipObservations = number(
    manifest.datasetMetadata?.requestedMembershipObservations,
    number(datasetStatus?.coverage?.requestedMembershipObservations, 0),
  );
  const availableMembershipObservations = number(
    manifest.datasetMetadata?.availableMembershipObservations,
    number(datasetStatus?.coverage?.availableMembershipObservations, 0),
  );
  const missingMembershipObservations = number(
    manifest.datasetMetadata?.missingMembershipObservations,
    number(datasetStatus?.coverage?.missingMembershipObservations, Infinity),
  );
  const activeMemberDateCoverageComplete = Boolean(
    manifest.datasetMetadata?.activeMemberDateCoverageComplete === true &&
      requestedMembershipObservations > 0 &&
      availableMembershipObservations === requestedMembershipObservations &&
      missingMembershipObservations === 0,
  );
  const universeRemovalOpenCoveragePct = number(
    manifest.datasetMetadata?.universeRemovalOpenCoveragePct,
    number(datasetStatus?.coverage?.universeRemovalOpenCoveragePct),
  );
  const universeRemovalZeroRecoveryActions = number(
    manifest.datasetMetadata?.universeRemovalZeroRecoveryActions,
    number(datasetStatus?.coverage?.universeRemovalZeroRecoveryActions, 0),
  );
  const universeRemovalResolvedOutcomes = number(
    manifest.datasetMetadata?.universeRemovalResolvedOutcomes,
    number(datasetStatus?.coverage?.universeRemovalResolvedOutcomes, 0),
  );
  const universeRemovalOutcomeCoveragePct = number(
    manifest.datasetMetadata?.universeRemovalOutcomeCoveragePct,
    number(datasetStatus?.coverage?.universeRemovalOutcomeCoveragePct),
  );
  const universeRemovalActions = number(
    manifest.datasetMetadata?.universeRemovalActions,
    number(datasetStatus?.coverage?.universeRemovalActions, 0),
  );
  const universeRemovalMissingOutcomes = number(
    manifest.datasetMetadata?.universeRemovalMissingOutcomes,
    number(datasetStatus?.coverage?.universeRemovalMissingOutcomes, Infinity),
  );
  const universeRemovalOutcomeCoverageComplete = Boolean(
    manifest.datasetMetadata?.universeRemovalOutcomeCoverageComplete === true &&
      universeRemovalResolvedOutcomes === universeRemovalActions &&
      universeRemovalMissingOutcomes === 0,
  );
  const minimumMembershipCount = number(
    manifest.datasetMetadata?.minimumMembershipCount,
  );
  const maximumMembershipCount = number(
    manifest.datasetMetadata?.maximumMembershipCount,
  );
...
async function pointInTimeNasdaqR11Context() {
  ...
  if (
    ...
    integrity?.datasetFingerprint !== evidenceDatasetFingerprint
  )
    throw new Error(
...
export async function finalizePointInTimeNasdaqR11() {
  ...
  const report = {
    ...
    methodology: {
      ...
      membershipEffectiveConvention: "next-session-after-effective-date",
      universeRemovalExecution:
        "next-session-adjusted-open-else-conservative-zero",
    ...
    dataQuality: {
      ...
      membershipUnionSymbols: number(
        context.datasetStatus?.coverage?.membershipUnionSymbols,
      ),
      priceUsableSymbols: number(
        context.datasetStatus?.coverage?.priceUsableSymbols,
      ),
      ...
      for (const symbol of requiredSymbols) {
        ...
        if (accumulator.missingRequiredPriceSamples.length < 20)
          accumulator.missingRequiredPriceSamples.push({
            date,
            symbol,
            active: pointInTimePriceRequirementContains(
              requirements.activeRangesBySymbol,
              symbol,
              sessionIndex,
            ),
