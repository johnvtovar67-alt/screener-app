...
const REPLAY_CHECKPOINT_SCHEMA = 6;
const REPLAY_WINDOWS_PER_RUN = 1;
const REPLAY_WARMUP_SESSIONS = 2;
const V9_ACTIVE_CANDIDATE_COUNT = 3;
  ...
  return Boolean(
    a &&
      ...
      latestCompletedMarketSessionDay(
        new Date(`${a.endDate}T23:59:59.000Z`),
      ) ===
        latestCompletedMarketSessionDay(
          new Date(`${b.endDate}T23:59:59.000Z`),
        ) &&
      a.schema === b.schema &&
...
function parameterScore(run = {}) {
  ...
  const profitFactor = Math.min(2.5, number(metrics.profitFactor, 0));
  const expectancy = Math.max(
    -10,
    Math.min(10, number(metrics.tradeDiagnostics?.expectancyPct, -10)),
  );
  const tradePenalty = closed < 12 ? (12 - closed) * 0.12 : 0;
  const turnoverPenalty =
    Math.max(0, number(metrics.annualizedTurnoverPct, 0) - 300) / 500;
  const exposurePenalty =
    Math.max(0, 95 - number(metrics.averageExposurePct, 0)) / 20;
  return (
    number(metrics.excessReturnPct, -100) / 4 +
    number(metrics.totalReturnPct, -100) / 20 +
    number(metrics.sharpe, -10) * 0.75 +
    profitFactor * 0.3 +
    expectancy / 10 -
    Math.abs(Math.min(0, number(metrics.maxDrawdownPct, 0))) / 12 -
    turnoverPenalty -
    tradePenalty -
    exposurePenalty
  );
...
function researchTradeDiagnostics(trades = []) {
  ...
  const completed = trades
    ...
    .map((trade) => {
      const entry = entries.get(
        trade.diagnosticPositionId ?? trade.positionId,
      );
  ...
  return {
    ...
    medianReturnPct: values.length
      ...
      : null,
    averageWinnerPct: winners.length
      ? roundMetric(average(winners))
      : null,
...
function aggregateResearchRuns(runs = []) {
  ...
  const comparisonSymbols = new Set(
    runs.flatMap((run) =>
      Object.keys(run.metrics?.benchmarkComparisons || {}),
    ),
  );
...
function buildV9ResearchFolds(sessionDates = []) {
...
function nextReplaySessionSlice(sessionDates = [], completedWindows = 0) {
  const { dates, folds } = buildV9ResearchFolds(sessionDates);
  const windowsPerFold = 3;
  const windowsPerCandidate = folds.length * windowsPerFold;
  const totalWindows = V9_ACTIVE_CANDIDATE_COUNT * windowsPerCandidate;
  const completed = boundedInteger(
    completedWindows,
    0,
    0,
    totalWindows,
  ...
  const candidateWindow = completed % windowsPerCandidate;
  ...
  const window = [fold.train, fold.validation, fold.audit][windowIndex];
  const firstIndex = dates.findIndex((date) => date >= window.start);
  let lastIndex = firstIndex;
  while (lastIndex + 1 < dates.length && dates[lastIndex + 1] <= window.end)
    lastIndex++;
  if (firstIndex < 0 || lastIndex < firstIndex)
    throw new Error(`Replay window ${window.start} to ${window.end} is unavailable`);
  return {
    complete: false,
    totalWindows,
...
async function runProvisionalWindows(
  ...
  const calendar = Array.isArray(calendarDates) && calendarDates.length
    ? calendarDates
    : sessions;
  const { usableDates, folds } = buildV9ResearchFolds(calendar);
  const independentLifecycle = {
    ...
    stopCooldownSessions: 15,
    classifyStopExits: true,
    ratchetRiskPlanStop: true,
    stopRatchetMinHoldSessions: 20,
    stopRatchetMinMfeR: 1,
    breakEvenStopMinMfeR: 1.5,
    breakEvenStopBufferPct: 0,
    relativeExitAlpha20: -8,
    relativeExitMinHoldSessions: 30,
    trendExitAlpha60: -3,
    trendExitMinHoldSessions: 30,
    timeStopSessions: 126,
    timeStopMaxReturnPct: 0,
    profitTrailActivationPct: 30,
    profitTrailDistancePct: 15,
    baseRankWeight: 0,
    shortTermAlphaRankPenalty: 0.25,
    shortTermAlphaRankPenaltyThreshold: 8,
  };
  const grid = [
    {
      ...independentLifecycle,
      thesisId: "persistent-quality-leadership",
      thesisLabel: "SPY-completed persistent quality leadership",
      ...
      minAlpha120VsSpy: 0,
      minReturn120Ex20: 0,
      relativeStrengthRankWeight: 0.3,
      researchFactorRankWeight: 0.12,
      qualityRankWeight: 0.22,
      sectorQualityRankWeight: 0.12,
      momentumRankWeight: 0.2,
      valueRankWeight: 0.03,
      stabilityRankWeight: 0.08,
      controlledPullbackRankWeight: 0.06,
    },
    {
      ...independentLifecycle,
      thesisId: "controlled-acceleration-leadership",
      thesisLabel: "SPY-completed controlled acceleration leadership",
      selectionEligible: true,
      minimumQualifiedSessions: 1,
      requireStrongEntryTiming: true,
      minShortTermTechnicalScore: 78,
      maxPriceVs50Pct: 14,
      minQualityPercentile: 45,
      minMomentumPercentile: 72,
      minCompositePercentile: 62,
      minSectorCompositePercentile: 45,
      ...
      minAlpha120VsSpy: 2,
      minReturn120Ex20: 5,
      relativeStrengthRankWeight: 0.45,
      researchFactorRankWeight: 0.1,
      qualityRankWeight: 0.12,
      sectorQualityRankWeight: 0.08,
      momentumRankWeight: 0.3,
      stabilityRankWeight: 0.04,
      controlledPullbackRankWeight: 0.06,
    },
    {
      ...independentLifecycle,
      thesisId: "quality-reacceleration-value-aware",
      thesisLabel: "SPY-completed quality re-acceleration, value aware",
      selectionEligible: true,
      minimumQualifiedSessions: 2,
      maxPriceVs50Pct: 10,
      minQualityPercentile: 72,
      minMomentumPercentile: 45,
      minCompositePercentile: 64,
      minSectorCompositePercentile: 35,
      minValuePercentile: 35,
      minStabilityPercentile: 45,
      ...
      minAlpha120VsSpy: -2,
      minReturn120Ex20: -2,
      relativeStrengthRankWeight: 0.2,
      researchFactorRankWeight: 0.12,
      qualityRankWeight: 0.3,
      sectorQualityRankWeight: 0.14,
      momentumRankWeight: 0.12,
      valueRankWeight: 0.12,
      stabilityRankWeight: 0.15,
      controlledPullbackRankWeight: 0.1,
    },
  ];
  if (grid.length !== V9_ACTIVE_CANDIDATE_COUNT)
    throw new Error("V9 replay candidate count is inconsistent");
  ...
  const selectable = evaluated.filter(
    (candidate) => candidate.parameters.selectionEligible !== false,
  );
  if (!selectable.length)
    throw new Error("No active V9 thesis is eligible for selection");
  ...
  const simpleAlphaByFold = walkForwardSelections.map((selection) =>
    number(selection.audit.metrics?.excessReturnPct),
  );
  ...
  const evidenceChecks = {
    positiveAggregateSimpleAlphaVsSpy:
      number(walkForwardMetrics.excessReturnPct, -Infinity) > 0,
    positiveAggregateExposureMatchedAlpha:
      number(walkForwardMetrics.exposureMatchedAlphaPct, -Infinity) > 0,
    positiveAggregateReturn:
      number(walkForwardMetrics.totalReturnPct, -Infinity) > 0,
    positiveExpectancy:
      number(
        walkForwardMetrics.tradeDiagnostics?.expectancyPct,
        -Infinity,
      ) > 0,
    profitFactorAboveOne:
      number(walkForwardMetrics.profitFactor, -Infinity) > 1,
    positiveAlphaInMajorityOfFolds:
      simpleAlphaByFold.filter((value) => value > 0).length > folds.length / 2,
    minimumClosedRoundTrips:
      number(walkForwardMetrics.closedTrades, 0) >= 30,
    minimumAverageExposure:
      number(walkForwardMetrics.averageExposurePct, 0) >= 95,
  };
    ...
    replay: {
      ...
      walkForwardSelectionAudit: {
        ...
        stability: {
          ...
          medianSimpleAlphaPct: [...simpleAlphaByFold].sort(
            (a, b) => a - b,
          )[Math.floor(simpleAlphaByFold.length / 2)],
        ...
        evidenceAssessment: {
          ...
          minimumAverageExposurePct: 95,
          primaryAlphaMeasure: "simple excess return versus SPY",
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const replaySignature = JSON.stringify({
      ...
      thesisContract: "v9-spy-completed-active-tilt-v1",
      statementSignature,
      finalSymbols: finalSymbols.slice().sort(),
      fromDate,
      endDate,
    ...
    const restoredChunks = await mapLimited(
      ...
      async (chunk) => {
        ...
        if (
          ...
          throw new Error(`Compiled research chunk is invalid: ${chunk.pathname}`);
    ...
    const report = {
      ...
      methodology: {
        ...
        activeThesesUseIndependentResearchLifecycle: true,
        activeThesesRequireProductionBuyLabel: false,
        ...
        residualCapitalBenchmarkCompletionSymbol: "SPY",
        ...
        volatilityScaledSizing: true,
        issuerConcentrationLimit: true,
        immutableEntryStopDiagnostics: true,
        delayedStopRatchetsUntilOneRiskUnit: true,
        breakEvenProtectionAfterOneAndHalfRiskUnits: true,
        classifiedInitialRatchetedAndTrailingStops: true,
        researchWindowsLiquidateAtFinalClose: true,
        nextOpenSizingUsesOpenMarksOnly: true,
        regimeAwareEntries: true,
        stopOutCooldown: true,
        relativeStrengthTrendAndTimeExits: true,
        ...
        sameHistoricalWindowsReusedAfterV8Diagnosis: true,
      },
      universe: {
      ...
      limitations: [
        ...
        "The completed V7 and V8 reports are retained as external baselines rather than replayed inside V9; neither prior policy can influence selection among the active V9 candidates.",
        ...
        "V9 was designed after reviewing V8 on these historical dates. Its results are a development comparison, not a new untouched test; only future or otherwise sequestered data can independently validate the redesign.",
        "The SPY completion sleeve removes cash drag but contributes only passive beta. V9 counts alpha solely as return above the corresponding SPY benchmark return.",
      ...
      nextResearchRequirement:
        "Run V9 on a genuinely untouched future or sequestered period under the strict screener-pit-v1 contract with historical membership, delistings, revision-safe fundamentals and point-in-time material news before making an alpha claim.",
    };
