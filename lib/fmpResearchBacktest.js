...
async function getOrFetchPointInTimeEarningsSurprises(datasetThrough) {
  ...
  if (
    ...
    asArray(existing?.rows).length >= 1_000
  )
    return existing;
  const apiKey = process.env.FMP_API_KEY || process.env.FMP_KEY;
  if (!apiKey) throw new Error("The configured research provider key is unavailable");
  ...
  for (const year of [2022, 2023, 2024, 2025, 2026]) {
    for (const startMonth of [0, 3, 6, 9]) {
      const from = `${year}-${String(startMonth + 1).padStart(2, "0")}-01`;
      if (from > datasetThrough) continue;
      const quarterEnd = new Date(Date.UTC(year, startMonth + 3, 0))
        .toISOString()
        .slice(0, 10);
      const to = quarterEnd < datasetThrough ? quarterEnd : datasetThrough;
      calendarRows.push(
      await client.fetchStable(
          "earnings-calendar",
          { from, to },
        { allowEmpty: true },
        ),
  ...
  if (rows.length < 1_000)
    throw new Error(
      `Historical earnings-surprise coverage is insufficient (${rows.length} usable rows)`,
    );
  const report = {
    version: 1,
    status: "complete",
...
export async function runPointInTimeSp500AlphaEarningsDriftR10({
  ...
  const [manifest, integrity, r9Report, datasetStatus] = await Promise.all([
    readPrivateJson(POINT_IN_TIME_SP500_COMPILED_CHECKPOINT_STORE),
    getPointInTimeSp500AlphaCreatorV2Integrity(),
