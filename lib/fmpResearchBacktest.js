...
const REPLAY_CHECKPOINT_SCHEMA = 8;
...
async function runProvisionalWindows(
  ...
  const transparentBullCyclePullbackAudit = aggregateResearchRuns(
    foldRuns.map(
      (fold) => fold.auditControls.transparentBullCyclePullback,
    ),
  );
  const randomPlaceboAudits = Array.from(
    ...
    (_, placeboIndex) =>
      aggregateResearchRuns(
        foldRuns.map(
          (fold) => fold.auditControls.randomPlacebos[placeboIndex],
        ),
      ),
  );
  ...
  const thesisTotalReturn = number(walkForwardMetrics.totalReturnPct, -Infinity);
    ...
    replay: {
      ...
      walkForwardSelectionAudit: {
        ...
        stability: {
          ...
          medianSimpleAlphaVsQqqPct: percentileValue(
            qqqSimpleAlphaByFold,
            0.5,
          ),
          worstExposureMatchedAlphaPct: Math.min(
