...
const REPLAY_CHECKPOINT_SCHEMA = 2;
const REPLAY_FOLDS_PER_RUN = 1;
...
async function runProvisionalWindows(
  ...
  for (const candidate of asArray(initial?.candidateRuns)) {
    ...
    const foldRuns = asArray(candidate?.foldRuns)
      .filter((foldRun) => {
        const foldIndex = folds.findIndex(
          (fold) => fold.fold === foldRun?.fold,
        );
        return Boolean(
          foldIndex >= 0 &&
            JSON.stringify(foldRun?.windows) ===
              JSON.stringify(folds[foldIndex]) &&
            foldRun?.train?.metrics &&
            foldRun?.validation?.metrics &&
            foldRun?.rollingAudit?.metrics,
        );
      })
  ...
  const boundedFolds = boundedInteger(
    maxFolds,
    1,
    0,
    grid.length * folds.length,
  );
  let computedFolds = 0;
  ...
  for (let index = 0; index < grid.length; index++) {
    ...
    for (const fold of folds) {
      if (candidate.foldRuns.some((foldRun) => foldRun.fold === fold.fold))
        continue;
      if (computedFolds >= boundedFolds) {
      ...
      if (onCheckpoint)
        await onCheckpoint({
          ...
          completedFolds: [...candidateRuns.values()].reduce(
            (total, item) => total + item.foldRuns.length,
            0,
          ),
          totalFolds: grid.length * folds.length,
          completedCandidates: [...candidateRuns.values()].filter(
            (item) => item.foldRuns.length === folds.length,
          ).length,
  ...
  const completedFolds = [...candidateRuns.values()].reduce(
    (total, candidate) => total + candidate.foldRuns.length,
    0,
  );
  const completedCandidates = [...candidateRuns.values()].filter(
    (candidate) => candidate.foldRuns.length === folds.length,
  ).length;
  ...
  // End an invocation after completing its bounded fold work, even when it
  // finished the last fold. The next invocation performs selection and the
  // full diagnostic from a completely durable checkpoint rather than racing
  // the function timeout after a long simulation.
  if (completedFolds < totalFolds || computedFolds > 0)
    return {
      status: "collecting",
      progress: {
        completedFolds,
        totalFolds,
        remainingFolds: totalFolds - completedFolds,
        completedCandidates,
        totalCandidates: grid.length,
        foldsPerCandidate: folds.length,
      },
...
export async function runFmpResearchBacktest({
  ...
  try {
    ...
    const replaySignature = JSON.stringify({
      ...
      thesisContract: "v7-five-thesis-walk-forward-v2",
      statementSignature,
