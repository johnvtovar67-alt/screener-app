...
const POINT_IN_TIME_SP500_ALPHA_V2_INTEGRITY_REPORT_VERSION = 1;
...
export async function runPointInTimeSp500DatasetAcquisition({
  ...
  if (
    ...
    now - previousCompilationClaim < RUNNING_TTL_MS
  ) {
...
function pointInTimeSp500RawDatasetFromHistory({
  ...
  const changes = asArray(privateBlueprint.changes)
    .map((row) => ({
      date: String(row.date || "").slice(0, 10),
      addedSymbol: symbolOf(row.addedSymbol),
      removedSymbol: symbolOf(row.removedSymbol),
    }))
    .filter((row) => /^\d{4}-\d{2}-\d{2}$/.test(row.date))
    .sort((left, right) => left.date.localeCompare(right.date));
  const profileSymbols = new Set(profiles.map(symbolOf));
  const delistedDates = Object.fromEntries(
    Object.entries(privateBlueprint.delistedDates || {})
      .map(([symbol, date]) => [symbolOf(symbol), String(date || "").slice(0, 10)])
      .filter(
        ([symbol, date]) =>
  ...
  const members = new Set(
    asArray(privateBlueprint.initialSymbols).map(symbolOf).filter(Boolean),
  );
  let changeIndex = 0;
...
async function compilePointInTimeSp500Dataset({
  ...
  const discoveryBySymbol = new Map(
    discoveryRows.map((row) => [symbolOf(row), row]),
  );
  const delistedDates = blueprint.privateBlueprint?.delistedDates || {};
  const usableSet = new Set(usableSymbols);
  const profiles = usableSymbols.map((symbol) => {
    const source = discoveryBySymbol.get(symbol) || {};
    const history = histories.get(symbol) || [];
    const delistedAt = String(delistedDates[symbol] || "").slice(0, 10);
    return {
      ...source,
      symbol,
  ...
  const fundamentals = buildHistoricalFundamentalRows(statementHistory).filter(
    (row) => usableSet.has(symbolOf(row)),
  );
  ...
  const compiledSignature = JSON.stringify({
    schema: 1,
    compilerContract:
      "historical-signal-evaluator-v10-explicit-membership-anchored-gradual-path-v1",
    fromDate: blueprint.fromDate,
    endDate: blueprint.throughDate,
    membershipChanges: number(blueprint.normalizedMembershipChanges, 0),
    requestedSymbols: symbols.slice().sort(),
    usableSymbols: usableSymbols.slice().sort(),
...
function scanPointInTimePriceDataset(accumulator, dataset = {}) {
  ...
  for (const session of sessions) {
    ...
    for (const row of asArray(session.prices)) {
      ...
      if (Math.abs(overnightReturnPct) >= 25 && splitRatio)
        boundedTopAbsolute(
...
function finishPointInTimePriceIntegrityAudit(accumulator) {
  ...
  return {
    ...
    interpretation:
      "Extreme moves are disclosed for review but fail integrity only when they also resemble an unadjusted common split ratio or violate adjusted/OHLC invariants.",
  };
}

export function pointInTimePriceIntegrityAudit(datasets = []) {
...
export async function runPointInTimeSp500AlphaCreatorV2({
  ...
  if (!String(manifest.signature || "").includes("anchored-gradual-path-v1"))
    throw new Error(
...
export async function runPointInTimeSp500AlphaCreatorV2Integrity({
  ...
  try {
    ...
    for (const chunk of asArray(manifest.chunks)) {
      ...
      scanPointInTimePriceDataset(priceAccumulator, { sessions });
    }
    ...
    const { primary, controls } = pointInTimeSp500AlphaV2Definitions();
    const definitions = [primary, ...controls];
    const runsByDefinition = new Map(
      ...
      for (const window of windows) {
        ...
        for (const definition of definitions) {
          const run = simulatePointInTimePortfolio(
            ...
            simulationOptions({
              ...definition.overrides,
              thesisId: `pit-sp500-alpha-integrity-${definition.id}`,
              thesisLabel: definition.label,
              startDate: window.start,
              endDate: window.end,
            }),
          ...
          assertCompleteResearchWindow(
            ...
            calendar,
            window,
            `${definition.label} integrity ${window.start}`,
          );
          runsByDefinition.get(definition.id)[phase].push(run);
        }
      }
    }
    const concentrationResults = definitions.map((definition) => ({
      id: definition.id,
      label: definition.label,
      phases: Object.fromEntries(
        Object.entries(runsByDefinition.get(definition.id)).map(
          ([phase, runs]) => [
            phase,
    ...
    const selectedConcentration = concentrationResults.find(
    ...
    const report = {
      ...
      controlTradeConcentration: concentrationResults.filter(
        (row) => row.id !== primary.id,
      ),
      assessment: {
      ...
      limitations: [
        ...
        "Large one-session moves are disclosed rather than automatically labelled errors unless they resemble a common unadjusted split ratio or violate adjusted/OHLC invariants.",
