...
export async function runResearchDataCapabilityAudit() {
  ...
  const probe = async (path, params, requiredFields) => {
    try {
      ...
      return {
        ...
        requiredFields: Object.fromEntries(
          requiredFields.map((field) => [field, sample[field] !== undefined]),
        ),
      };
    } catch (error) {
      return {
  ...
  const historicalMembership = await probe(
    ...
    {},
    ["dateAdded", "addedSymbol", "removedSymbol"],
  );
  ...
  const pointInTimeMembershipAvailable =
    ...
    ["dateAdded", "addedSymbol", "removedSymbol"].every(
      (field) => historicalMembership.requiredFields[field] === true,
    );
  const delistedUniverseAvailable =
    delistedCompanies.available === true &&
    delistedCompanies.requiredFields.symbol === true;
  const report = {
    version: 1,
    status: "complete",
    completedAt: new Date().toISOString(),
