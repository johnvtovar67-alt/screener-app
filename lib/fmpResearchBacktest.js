...
// VSNT has a historical addition but no corresponding effective removal, even
// though Nasdaq's own NDX start-of-day weights show the security disappear.
...
// before a later replacement, while FMP labels SOLS with its announcement date
// instead of Nasdaq's first start-of-day weighting date without the security.
...
export async function runPointInTimeNasdaqUniverse({ force = false } = {}) {
  ...
  const report = {
    ...
    limitations: [
      ...
      "FMP's announcement-dated SOLS removal is corrected and its omitted VSNT removal is completed only on each security's first absent official Nasdaq NDX start-of-day weight date.",
