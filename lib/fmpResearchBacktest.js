...
const POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE =
  "research/pit-nasdaq-alpha-parallel-r12.json";
const POINT_IN_TIME_NASDAQ_ALPHA_R11_PREFIX =
  "research/pit-nasdaq-alpha-parallel-r12";
const POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION = 12;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_RESEARCH_GENERATIONS = 12;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_STRICT_PLACEBO_SEEDS = 1_000;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_EXECUTION_CONTRACT =
  "nasdaq-r12-pullback-family-simulator-gates-statistics-v1";
const POINT_IN_TIME_NASDAQ_ALPHA_R11_FINAL_REPORT_CONTRACT =
  "nasdaq-r12-final-report-evidence-v1";
...
function pointInTimeNasdaqAlphaR11Definitions() {
  ...
  return [
    ...
    ...[
      {
        id: "r12-pullback-baseline",
        label: "R11 pullback leader exact replication",
        family: "pullback-baseline",
      },
      {
        id: "r12-pullback-trend-heavy",
        label: "Pullback with stronger dual-benchmark trend weight",
        family: "pullback-baseline",
        weights: [0.4, 0.45, 0.1, 0.05],
      },
      {
        id: "r12-pullback-shallow",
        label: "Shallow two-to-six percent five-session pullback",
        family: "pullback-depth",
        minReturn5Pct: -6,
        maxReturn5Pct: -2,
      },
      {
        id: "r12-pullback-deep",
        label: "Deep six-to-twelve percent five-session pullback",
        family: "pullback-depth",
        minReturn5Pct: -12,
        maxReturn5Pct: -6,
      },
      {
        id: "r12-pullback-fast",
        label: "Pullback with five-session minimum hold",
        family: "pullback-horizon",
        minimumHold: 5,
        timeStop: 20,
      },
      {
        id: "r12-pullback-patient",
        label: "Pullback with fifteen-session minimum hold",
        family: "pullback-horizon",
        minimumHold: 15,
        timeStop: 60,
      },
      {
        id: "r12-pullback-diversified",
        label: "Pullback diversified across ten equal-weight names",
        family: "pullback-portfolio",
        targetCount: 10,
      },
      {
        id: "r12-pullback-vol-managed",
        label: "Pullback with sixteen-percent volatility target",
        family: "pullback-portfolio",
        maxVolatility60Pct: 50,
      ...
      const [reversalPressure, relativeTrend, lowVolatility, liquidity] =
        variant.weights || [0.55, 0.3, 0.1, 0.05];
      const targetCount = variant.targetCount || 6;
      return {
        id: variant.id,
        label: variant.label,
        family: variant.family,
        mechanism:
          "liquidity-conditioned overreaction reversal inside an intact dual-benchmark trend",
        weights: {
          reversalPressure,
          relativeTrend,
          lowVolatility,
        ...
        overrides: {
          ...
          rankedMinimumHoldSessions: variant.minimumHold || 10,
          rankedEntryQueueCount: targetCount * 3,
          ...
          minReturn5Pct: variant.minReturn5Pct ?? -10,
          maxReturn5Pct: variant.maxReturn5Pct ?? -1,
          minAlpha120VsSpy: 0,
          minAlpha120VsQqq: 0,
          minimumAverageDollarVolume: 50_000_000,
          minimumInitialStopPct: 12,
          maximumInitialStopPct: 12,
          timeStopSessions: variant.timeStop || 40,
          maxVolatility60Pct: variant.maxVolatility60Pct ?? null,
          volatilityTargetPct: variant.volatilityTargetPct ?? null,
          riskBudgetPct: variant.riskBudgetPct ?? null,
        },
      };
    }),
  ].filter((definition) => definition.id.startsWith("r12-"));
}
...
async function getOrFetchPointInTimeEarningsSurprises(
  ...
  const existing = await readPrivateJson(
    POINT_IN_TIME_SP500_EARNINGS_SURPRISE_STORE,
  ).catch(() => null);
  if (
    existing?.status === "complete" &&
  ...
  for (let offset = 0; offset < pending.length; offset += 12) {
    ...
    await persistPrivateJson(POINT_IN_TIME_SP500_EARNINGS_SURPRISE_STORE, {
      version: 2,
  ...
  await persistPrivateJson(POINT_IN_TIME_SP500_EARNINGS_SURPRISE_STORE, report);
  return report;
}
