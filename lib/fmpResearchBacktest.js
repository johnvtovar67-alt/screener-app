...
const POINT_IN_TIME_NASDAQ_ALPHA_R11_STORE =
  "research/pit-nasdaq-alpha-parallel-r11.json";
const POINT_IN_TIME_NASDAQ_ALPHA_R11_PREFIX =
  "research/pit-nasdaq-alpha-parallel-r11";
const POINT_IN_TIME_NASDAQ_ALPHA_R11_REPORT_VERSION = 11;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_RESEARCH_GENERATIONS = 11;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_STRICT_PLACEBO_SEEDS = 1_000;
const POINT_IN_TIME_NASDAQ_ALPHA_R11_EXECUTION_CONTRACT =
  "nasdaq-r11-simulator-gates-statistics-v3";
const POINT_IN_TIME_NASDAQ_ALPHA_R11_FINAL_REPORT_CONTRACT =
  "nasdaq-r11-final-report-evidence-v2";
...
// FMP's current Nasdaq anchor includes HONA, but its historical change feed
// omits the spin-off addition. Nasdaq's NDX methodology adds spin-off
// securities, and Nasdaq Corporate Action ECA2026-399 establishes the
// 2026-06-29 ex-date and first regular-way session. Keep this as a distinct
// security from HON and make the otherwise-missing membership event explicit.
...
// FMP pairs an acquired constituent's later index replacement with its
// removal. The acquired shares can stop trading several sessions earlier.
// Split those paired rows at the source-verified corporate-action date so an
// impossible post-halt price is never required or silently forward-filled.
    ...
    const matches = adjustedProviderChanges
      ...
      .filter(
        ({ row }) =>
          ...
          pointInTimeSecuritySymbol(row.addedSymbol) === pairedAddedSymbol &&
          pointInTimeSecuritySymbol(row.removedSymbol) === removedSymbol,
      );
    if (matches.length !== 1)
      throw new Error(
        `Nasdaq provider correction ${correction?.id || removedSymbol} expected exactly one ${providerEventDate} ${pairedAddedSymbol}/${removedSymbol} event; found ${matches.length}`,
      );
...
function currentPointInTimeNasdaqUniverse(report) {
  ...
  const supplementsAppliedExactly = expectedSupplements.every((supplement) => {
    const additions = changes.filter(
      (row) => row.addedSymbol === supplement.addedSymbol,
    );
    return (
      !initialSymbols.includes(supplement.addedSymbol) &&
      additions.length === 1 &&
      additions[0].date === supplement.date &&
      additions[0].effectiveDate === supplement.effectiveDate &&
      additions[0].removedSymbol === supplement.removedSymbol &&
      additions[0].effectiveDateBasis === supplement.effectiveDateBasis &&
      additions[0].sourceVerifiedSupplement === true
    );
  });
...
export async function runPointInTimeNasdaqUniverse({ force = false } = {}) {
  ...
  if (supplementConflict)
    throw new Error(
      `Nasdaq membership supplement conflicts with provider event for ${supplementConflict.supplement.addedSymbol}: ${supplementConflict.supplement.date}/${supplementConflict.supplement.removedSymbol || "none"} versus ${supplementConflict.provider.date}/${supplementConflict.provider.removedSymbol || "none"}`,
    );
...
function pointInTimeNasdaqAlphaR11Definitions() {
  ...
  return [
    ...
    {
      id: "r11-trend-conditioned-pullback",
      label: "Short-horizon reversal inside positive dual-benchmark trend",
      family: "conditional-reversal",
      mechanism: "liquidity-conditioned overreaction reversal inside an intact trend",
      weights: {
