  ...
  for (let sessionIndex=0;sessionIndex<sessions.length;sessionIndex++) {
    ...
    if (active) {
      ...
      const candidates = (session.signals || [])
        ...
        .sort(
          (a, b) =>
            (number(b.capitalEfficiencyScore, b.score)+config.relativeStrengthRankWeight*(0.65*number(b.entryTiming?.alpha60VsSpy,0)+0.35*number(b.entryTiming?.alpha20VsSpy,0))-config.shortTermAlphaRankPenalty*Math.max(0,number(b.entryTiming?.alpha20VsSpy,0))) -
              (number(a.capitalEfficiencyScore, a.score)+config.relativeStrengthRankWeight*(0.65*number(a.entryTiming?.alpha60VsSpy,0)+0.35*number(a.entryTiming?.alpha20VsSpy,0))-config.shortTermAlphaRankPenalty*Math.max(0,number(a.entryTiming?.alpha20VsSpy,0))) ||
      ...
      for (const signal of candidates) {
        ...
        pending.push({
          ...
          symbol,
          action: signal.action,
          rank: number(signal.capitalEfficiencyScore, signal.score)+config.relativeStrengthRankWeight*(0.65*number(signal.entryTiming?.alpha60VsSpy,0)+0.35*number(signal.entryTiming?.alpha20VsSpy,0))-config.shortTermAlphaRankPenalty*Math.max(0,number(signal.entryTiming?.alpha20VsSpy,0)),
