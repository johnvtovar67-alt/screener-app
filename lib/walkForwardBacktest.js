...
function researchRank(signal = {}, config = {}) {
  ...
  const base =
    config.baseRankWeight *
    number(signal.capitalEfficiencyScore, signal.score);
  const shortTermAlpha = number(signal.entryTiming?.alpha20VsSpy, 0);
  const relative =
    ...
    config.shortTermAlphaRankPenalty *
      Math.max(
        0,
        shortTermAlpha - config.shortTermAlphaRankPenaltyThreshold,
      );
  const factor =
...
function expandedResearchSource(config = {}) {
  return config.researchSignalSource === "full-evidence";
}
...
function entrySignalQualifies(signal = {}, config = {}) {
  ...
  return Boolean(
    (expanded || ["Strong Buy", "Buy"].includes(signal.action)) &&
      ...
      (expanded
        ? timing.available === true && timing.pass === true
...
function metricsFromCurve(
  ...
  for (const [symbol, comparisonCurve] of Object.entries(benchmarkCurves || {})) {
...
export function simulatePointInTimePortfolio(dataset = {}, options = {}) {
  ...
  const benchmarkCompletionSymbol = symbolOf(config.benchmarkCompletionSymbol);
  let benchmarkCompletionReferenceClose = null;
  ...
  for (let sessionIndex=0;sessionIndex<sessions.length;sessionIndex++) {
    ...
    if (active) {
      ...
      const completionBar = benchmarkCompletionSymbol
        ? prices.get(benchmarkCompletionSymbol)
        : null;
      if (benchmarkCompletionSymbol) {
        const completionOpen = number(completionBar?.open, 0);
        const completionClose = number(completionBar?.close, 0);
        if (!(completionOpen > 0) || !(completionClose > 0))
          throw new Error(
            `Benchmark completion requires ${benchmarkCompletionSymbol} open and close prices on ${session.date}.`,
          );
        if (benchmarkCompletionReferenceClose > 0)
          cash *= completionOpen / benchmarkCompletionReferenceClose;
      }
          ...
          const sectorExposure = [...positions.values()]
            ...
            .reduce(
              (total, position) =>
                ...
                position.shares *
                  number(
                    prices.get(position.symbol)?.open,
                    position.lastPrice,
                  ),
              0,
            );
      ...
      if (benchmarkCompletionSymbol) {
        const completionBar = prices.get(benchmarkCompletionSymbol);
        const completionOpen = number(completionBar?.open, 0);
        const completionClose = number(completionBar?.close, 0);
        cash *= completionClose / completionOpen;
        benchmarkCompletionReferenceClose = completionClose;
      }
    ...
    if (active) {
      ...
      const activeExposure =
        equity > 0 ? clamp(1 - cash / equity, 0, 1) : 0;
      curve.push({
        date: session.date,
        ...
        activeExposure,
        benchmarkExposure: benchmarkCompletionSymbol ? 1 : activeExposure,
        benchmarkSleeveValue: benchmarkCompletionSymbol ? round(cash, 2) : 0,
      });
      ...
      for (const [symbol, position] of positions) {
        ...
        const trendExit=Number.isFinite(config.trendExitAlpha60)&&heldSessions>=config.trendExitMinHoldSessions&&Number.isFinite(alpha60)&&alpha60<config.trendExitAlpha60&&mark<number(signal.priceAvg50,0);
        ...
        if (relativeExit||trendExit||timeExit)
          pending.push({side:"sell",symbol,fraction:1,reason:relativeExit?"relative-strength-break":trendExit?"trend-relative-break":"time-stop"});
