    ...
    if (active) {
      ...
      const rankedPool = rankedSelection
        ? entrySignals
            ...
            .sort(
              (a, b) =>
                researchRank(b, config, {
                  date: session.date,
                  sessionIndex,
                }) -
                  researchRank(a, config, {
                    date: session.date,
                    sessionIndex,
      ...
      if (rankRebalance)
        for (const [symbol, position] of positions) {
          const heldSessions = Math.max(
            0,
            sessionIndex -
              number(position.enteredSessionIndex, sessionIndex),
          );
          if (
      ...
      for (const signal of rankedSelection && !rankRebalance ? [] : candidates) {
        const symbol = symbolOf(signal);
