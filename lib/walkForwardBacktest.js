...
function sectorOf(signal = {}) {
  return String(signal.sector || signal.primaryTheme || signal.factor || "Other");
}
...
function researchRank(signal = {}, config = {}) {
  const factors = signal.researchFactors || {};
  const base = number(signal.capitalEfficiencyScore, signal.score);
  const relative =
    config.relativeStrengthRankWeight *
      (0.65 * number(signal.entryTiming?.alpha60VsSpy, 0) +
        0.35 * number(signal.entryTiming?.alpha20VsSpy, 0)) -
    config.shortTermAlphaRankPenalty *
      Math.max(0, number(signal.entryTiming?.alpha20VsSpy, 0));
  const factor =
...
function roundTripDiagnostics(trades = []) {
  ...
  return {
    ...
    averageMfePct: completed.some((trade) => Number.isFinite(trade.mfePct))
      ? round(
          average(completed.map((trade) => trade.mfePct).filter(Number.isFinite)),
          2,
        )
      : null,
    averageMaePct: completed.some((trade) => Number.isFinite(trade.maePct))
      ? round(
          average(completed.map((trade) => trade.maePct).filter(Number.isFinite)),
          2,
        )
      : null,
    stopOutRatePct: completed.length
      ? round(
          (completed.filter((trade) => trade.reason === "invalidation-stop")
            .length /
            completed.length) *
            100,
          2,
...
function metricsFromCurve(curve = [], trades = [], benchmarkCurve = []) {
  ...
  const grossLoss = Math.abs(
    closed.reduce(
      (sum, trade) => sum + Math.min(0, tradePnl(trade)),
      0,
    ),
  );
  ...
  const exposureMatchedBenchmarkReturn = exposureMatchedBenchmark - 1;
...
export function simulatePointInTimePortfolio(dataset = {}, options = {}) {
  ...
  const executeSale = ({
    ...
    if (
      reason === "trim" &&
      typeof config.recordWinnerTrim === "function"
    )
    ...
    const trade = {
      ...
      holdingSessions: Math.max(
        0,
        currentSessionIndex - number(position.enteredSessionIndex, currentSessionIndex),
      ),
      initialStopPct:
    ...
    if (positionClosed) {
      ...
      if (reason === "invalidation-stop" && config.stopCooldownSessions > 0)
        cooldownThrough.set(symbol, number(position.lastSessionIndex, 0) + config.stopCooldownSessions);
    }
    return trade;
  };
  ...
  for (let sessionIndex=0;sessionIndex<sessions.length;sessionIndex++) {
    ...
    if (active) {
      ...
      for (const order of buyOrders.sort((a, b) => b.rank - a.rank)) {
        ...
        if (
          ...
          targetPct *= clamp(config.volatilityTargetPct / volatility60, 0.55, 1.25);
          ...
          const sectorExposure = [...positions.values()]
            ...
            .reduce(
              (total, position) =>
                ...
                position.shares *
                  number(
                    prices.get(position.symbol)?.close,
                    position.lastPrice,
                  ),
              0,
            );
        ...
        trades.push({
          ...
          signalSnapshot: {
            ...
            capitalEfficiencyScore: number(order.signal?.capitalEfficiencyScore, null),
            ...
            benchmarkRegime: order.signal?.entryTiming?.benchmarkRegime || "unknown",
    ...
    for (const [symbol, position] of positions) {
      ...
      if (row?.close > 0) {
        ...
        position.highWatermark=Math.max(number(position.highWatermark,position.entryPrice),number(row.high,row.close));
    ...
    if (active) {
      ...
      const signalMap = new Map(
        [
          ...(session.positionSignals || []),
          ...(session.signals || []),
        ].map((signal) => [symbolOf(signal), signal]),
      );
      ...
      for (const [symbol, position] of positions) {
        ...
        const heldSessions=Math.max(0,sessionIndex-number(position.enteredSessionIndex,sessionIndex));
        const pnlPct=position.entryPrice>0?(mark/position.entryPrice-1)*100:0;
        const alpha20=number(signal.entryTiming?.alpha20VsSpy,null);
        const relativeExit=Number.isFinite(config.relativeExitAlpha20)&&heldSessions>=config.relativeExitMinHoldSessions&&Number.isFinite(alpha20)&&alpha20<config.relativeExitAlpha20;
        const timeExit=Number.isFinite(config.timeStopSessions)&&heldSessions>=config.timeStopSessions&&pnlPct<=config.timeStopMaxReturnPct;
        if (relativeExit||timeExit)
          pending.push({side:"sell",symbol,fraction:1,reason:relativeExit?"relative-strength-break":"time-stop"});
          ...
          if (typeof config.winnerTrimGate === "function") {
            ...
            if (gate?.pass) {
              ...
              const rawPct =
                protection.highFroth ||
                (protection.winnerFading && pnl >= 60)
                  ? 0.45
        ...
        if (config.ratchetRiskPlanStop&&stop > 0 && stop < number(prices.get(symbol)?.close, Infinity))
          position.stopPrice = Math.max(number(position.stopPrice, 0), stop);
        if(Number.isFinite(config.profitTrailActivationPct)&&Number.isFinite(config.profitTrailDistancePct)){
          const highGain=position.entryPrice>0?(position.highWatermark/position.entryPrice-1)*100:0;
          if(highGain>=config.profitTrailActivationPct){
            const trailing=position.highWatermark*(1-config.profitTrailDistancePct/100);
            if(trailing<mark)position.stopPrice=Math.max(number(position.stopPrice,0),trailing);
          }
        }
      }
      ...
      const candidates = (session.signals || [])
        .filter(
          (signal) =>
            ...
            (!config.requireRelativeStrength||signal.entryTiming?.relativeStrengthVerified===true) &&
            (!config.requireStrongEntryTiming||signal.entryTiming?.strongPass===true) &&
            (!Number.isFinite(config.minAlpha20VsSpy)||number(signal.entryTiming?.alpha20VsSpy,-Infinity)>=config.minAlpha20VsSpy) &&
            (!Number.isFinite(config.maxAlpha20VsSpy)||number(signal.entryTiming?.alpha20VsSpy,Infinity)<=config.maxAlpha20VsSpy) &&
            (!Number.isFinite(config.minAlpha60VsSpy)||number(signal.entryTiming?.alpha60VsSpy,-Infinity)>=config.minAlpha60VsSpy) &&
            (!Number.isFinite(config.maxAlpha60VsSpy)||number(signal.entryTiming?.alpha60VsSpy,Infinity)<=config.maxAlpha60VsSpy) &&
            (!Number.isFinite(config.minAlpha60VsQqq)||number(signal.entryTiming?.alpha60VsQqq,-Infinity)>=config.minAlpha60VsQqq) &&
            (!Number.isFinite(config.minAlpha120VsSpy)||number(signal.entryTiming?.alpha120VsSpy,-Infinity)>=config.minAlpha120VsSpy) &&
            (!Number.isFinite(config.minAlpha120VsQqq)||number(signal.entryTiming?.alpha120VsQqq,-Infinity)>=config.minAlpha120VsQqq) &&
            (!Number.isFinite(config.minReturn120Ex20)||optionalNumber(signal.researchFactors?.return120Ex20,-Infinity)>=config.minReturn120Ex20) &&
            (!config.blockDefensiveEntries||signal.entryTiming?.benchmarkRegime!=="defensive") &&
            (signal.entryTiming?.benchmarkRegime!=="defensive"||!Number.isFinite(config.defensiveRegimeMinAlpha60)||number(signal.entryTiming?.alpha60VsSpy,-Infinity)>=config.defensiveRegimeMinAlpha60) &&
            optionalNumber(signal.researchFactors?.factorCoverage, 0) >= config.minimumResearchFactorCoverage &&
            (!Number.isFinite(config.minQualityPercentile) || optionalNumber(signal.researchFactors?.qualityPercentile, -Infinity) >= config.minQualityPercentile) &&
            (!Number.isFinite(config.minMomentumPercentile) || optionalNumber(signal.researchFactors?.momentumPercentile, -Infinity) >= config.minMomentumPercentile) &&
            (!Number.isFinite(config.minCompositePercentile) || optionalNumber(signal.researchFactors?.globalCompositePercentile, -Infinity) >= config.minCompositePercentile) &&
            (!Number.isFinite(config.minSectorCompositePercentile) || optionalNumber(signal.researchFactors?.sectorCompositePercentile, -Infinity) >= config.minSectorCompositePercentile) &&
            (!Number.isFinite(config.maxVolatility60Pct) || optionalNumber(signal.researchFactors?.volatility60Pct, Infinity) <= config.maxVolatility60Pct) &&
            persistenceEligible(
              signal,
              persistence,
        ...
        .sort(
          (a, b) =>
            researchRank(b, config) - researchRank(a, config) ||
      ...
      for (const signal of candidates) {
        ...
        pending.push({
          ...
          symbol,
          action: signal.action,
          rank: researchRank(signal, config),
