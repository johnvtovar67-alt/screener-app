...
function researchRank(signal = {}, config = {}, context = {}) {
  ...
  if (config.researchRankMode === "filing-inflection") {
    const weights = config.filingInflectionWeights || {};
    const weighted = (key, value) => number(weights[key], 0) * value;
    const scaled = (field, scale) => {
      const value = optionalNumber(factors[field], null);
      return value === null ? -2 : clamp(value / scale, -2, 2);
    };
    const residual20 =
      0.5 * clamp(optionalNumber(timing.alpha20VsSpy, 0) / 15, -2, 2) +
      0.5 * clamp(optionalNumber(timing.alpha20VsQqq, 0) / 15, -2, 2);
    const residual60 =
      0.5 * clamp(optionalNumber(timing.alpha60VsSpy, 0) / 25, -2, 2) +
      0.5 * clamp(optionalNumber(timing.alpha60VsQqq, 0) / 25, -2, 2);
    const age = optionalNumber(factors.sessionsSinceFiling, Infinity);
    const maxAge = Math.max(1, number(config.maxSessionsSinceFiling, 40));
    const recency = Number.isFinite(age)
      ? clamp(1 - age / maxAge, 0, 1)
      : -2;
    return (
      100 *
      (weighted("revenueGrowth", scaled("revenueGrowthPct", 30)) +
        weighted(
          "revenueAcceleration",
          scaled("revenueAccelerationPctPoints", 25),
        ) +
        weighted(
          "profitabilityGrowth",
          scaled("profitabilityGrowthPct", 50),
        ) +
        weighted("marginChange", scaled("marginChangePctPoints", 8)) +
        weighted("cashFlowGrowth", scaled("cashFlowGrowthPct", 50)) +
        weighted(
          "cashConversionChange",
          scaled("cashConversionChangePctPoints", 12),
        ) +
        weighted("residual20", residual20) +
        weighted("residual60", residual60) +
        weighted("recency", recency))
    );
  }
  if (config.researchRankMode === "post-earnings-drift") {
    const weights = config.earningsDriftWeights || {};
    const weighted = (key, value) => number(weights[key], 0) * value;
    const surprise = clamp(
      optionalNumber(factors.earningsSurpriseScore, -2),
      -2,
      2,
    );
    const residual20 =
    ...
    const residual60 =
      0.5 * clamp(optionalNumber(timing.alpha60VsSpy, 0) / 25, -2, 2) +
      0.5 * clamp(optionalNumber(timing.alpha60VsQqq, 0) / 25, -2, 2);
    const age = optionalNumber(factors.sessionsSinceEarnings, Infinity);
    const maxAge = Math.max(1, number(config.maxSessionsSinceEarnings, 60));
    const recency = Number.isFinite(age)
      ? clamp(1 - age / maxAge, 0, 1)
      : -2;
    return (
      100 *
      (weighted("surprise", surprise) +
        weighted("residual20", residual20) +
        weighted("residual60", residual60) +
        weighted("recency", recency))
    );
  ...
  if (config.researchRankMode === "nasdaq-residual-path-quality") {
    const weights = config.nasdaqResidualPathWeights || {};
    const weighted = (key, value) => number(weights[key], 0) * value;
    const relative120 =
      0.5 * clamp(optionalNumber(timing.alpha120VsSpy, 0) / 35, -2, 2) +
      0.5 * clamp(optionalNumber(timing.alpha120VsQqq, 0) / 35, -2, 2);
    ...
    return (
      ...
      (weighted("relative120", relative120) +
        ...
        weighted(
          "accelerationRestraint",
          centeredPercentile(factors.accelerationRestraintPercentile) / 50,
        ) +
        weighted(
          "lotteryRestraint",
          centeredPercentile(factors.lotteryRestraintPercentile) / 50,
        ) +
        weighted(
          "lowVolatility",
  ...
  if (config.researchRankMode === "momentum-spine") {
    const weights = config.momentumSpineWeights || {};
    const weighted = (key, value) => number(weights[key], 0) * value;
    const dualBenchmarkRelative = (spy, qqq, scale) =>
      0.5 * clamp(optionalNumber(spy, 0) / scale, -2, 2) +
      0.5 * clamp(optionalNumber(qqq, 0) / scale, -2, 2);
    const volatility20 = optionalNumber(factors.volatility20Pct, null);
    const volatility60 = optionalNumber(factors.volatility60Pct, null);
    const contraction =
      volatility20 === null || volatility60 === null || volatility60 <= 0
        ? -1
        : clamp(1 - volatility20 / volatility60, -1, 1);
    const return5 = optionalNumber(factors.return5, null);
    const return20 = optionalNumber(factors.return20, null);
    const reacceleration =
      return5 === null || return20 === null
        ? -1
        : clamp((return5 - return20 / 4) / 8, -1, 1);
    const relativeVolume = optionalNumber(factors.relativeVolume20, null);
    const volumeConfirmation =
      relativeVolume === null || relativeVolume <= 0
        ? -1
        : clamp(Math.log(relativeVolume) / Math.log(2), -1, 1);
    const shortRunRestraint =
      return5 === null
        ? -1
        : return5 >= -3 && return5 <= 5
          ? 1 - Math.abs(return5 - 1) / 8
          : clamp(1 - Math.abs(return5 - 1) / 10, -1, 1);
    return (
      100 *
      (weighted(
        "momentum",
        ...
        weighted("contraction", contraction) +
        weighted("reacceleration", reacceleration) +
        weighted("volumeConfirmation", volumeConfirmation) +
        weighted("shortRunRestraint", shortRunRestraint))
    );
  }
...
function expandedResearchSource(config = {}) {
  return ["full-evidence", "price-only", "earnings-event"].includes(
    config.researchSignalSource,
  );
}
...
function entrySignalQualifies(signal = {}, config = {}) {
  ...
  const independentlyResearchable =
    priceOnly ||
    config.researchSignalSource === "earnings-event" ||
    config.researchSignalSource === "filing-event";
  const timing = signal.entryTiming || {};
  const factors = signal.researchFactors || {};
  const price = optionalNumber(
    signal.price ?? signal.currentPrice ?? signal.lastPrice ?? signal.close,
  ...
  const earningsSurpriseFactorsComplete = [
    factors.earningsSurpriseScore,
    factors.sessionsSinceEarnings,
  ].every(Number.isFinite);
  const filingChangeFactorsComplete = [
    factors.filingCoverageFields,
    factors.filingAlignedPeriodEnds,
    factors.sessionsSinceFiling,
    factors.revenueGrowthPct,
    factors.profitabilityGrowthPct,
    factors.cashFlowGrowthPct,
    factors.marginChangePctPoints,
    factors.cashConversionChangePctPoints,
  ].every(Number.isFinite);
  const nasdaqResidualPathFactorsComplete = [
    timing.alpha60VsSpy,
    timing.alpha60VsQqq,
    timing.alpha120VsSpy,
    timing.alpha120VsQqq,
    factors.continuousInformationPercentile,
    factors.accelerationRestraintPercentile,
    factors.lotteryRestraintPercentile,
    factors.stabilityPercentile,
    factors.drawdownResiliencePercentile,
    factors.highProximityPercentile,
  ].every(Number.isFinite);
  ...
  return Boolean(
    ...
    (independentlyResearchable || signal.fundamentalDataVerified === true) &&
    (independentlyResearchable || signal.eventRiskVerified === true) &&
    (expanded
    ...
    (!config.requireEarningsSurpriseFactors ||
      earningsSurpriseFactorsComplete) &&
    (!config.requireFilingChangeFactors || filingChangeFactorsComplete) &&
    (!config.requireFilingPeriodAlignment ||
      optionalNumber(factors.filingAlignedPeriodEnds, 0) === 1) &&
    (!Number.isFinite(config.minimumFilingCoverageFields) ||
      optionalNumber(factors.filingCoverageFields, 0) >=
        config.minimumFilingCoverageFields) &&
    (!Number.isFinite(config.minSessionsSinceFiling) ||
      optionalNumber(factors.sessionsSinceFiling, -Infinity) >=
        config.minSessionsSinceFiling) &&
    (!Number.isFinite(config.maxSessionsSinceFiling) ||
      optionalNumber(factors.sessionsSinceFiling, Infinity) <=
        config.maxSessionsSinceFiling) &&
    (!Number.isFinite(config.minimumRevenueGrowthPct) ||
      optionalNumber(factors.revenueGrowthPct, -Infinity) >=
        config.minimumRevenueGrowthPct) &&
    (!Number.isFinite(config.minimumProfitabilityGrowthPct) ||
      optionalNumber(factors.profitabilityGrowthPct, -Infinity) >=
        config.minimumProfitabilityGrowthPct) &&
    (!Number.isFinite(config.minimumCashFlowGrowthPct) ||
      optionalNumber(factors.cashFlowGrowthPct, -Infinity) >=
        config.minimumCashFlowGrowthPct) &&
    (!Number.isFinite(config.minimumMarginChangePctPoints) ||
      optionalNumber(factors.marginChangePctPoints, -Infinity) >=
        config.minimumMarginChangePctPoints) &&
    (!Number.isFinite(config.minimumCashConversionChangePctPoints) ||
      optionalNumber(factors.cashConversionChangePctPoints, -Infinity) >=
        config.minimumCashConversionChangePctPoints) &&
    (!config.requireNasdaqResidualPathFactors ||
      nasdaqResidualPathFactorsComplete) &&
    (!Number.isFinite(config.minEarningsSurpriseScore) ||
      optionalNumber(factors.earningsSurpriseScore, -Infinity) >=
        config.minEarningsSurpriseScore) &&
    (!Number.isFinite(config.minSessionsSinceEarnings) ||
      optionalNumber(factors.sessionsSinceEarnings, -Infinity) >=
        config.minSessionsSinceEarnings) &&
    (!Number.isFinite(config.maxSessionsSinceEarnings) ||
      optionalNumber(factors.sessionsSinceEarnings, Infinity) <=
        config.maxSessionsSinceEarnings) &&
...
// Attribute a continuously simulated portfolio to a chronological window
// without resetting cash, positions, pending orders, or rank persistence at
// the boundary. The first curve observation is the fold base; round trips are
// attributed to the fold in which they close. This is research-only and does
// not alter the simulator or production decisions.
export function slicePointInTimePortfolioRun(run = {}, startDate, endDate) {
  const within = (date) =>
    (!startDate || date >= startDate) && (!endDate || date <= endDate);
  const curve = (run.curve || []).filter((row) => within(row.date));
  const benchmarkCurves = Object.fromEntries(
    Object.entries(run.benchmarkCurves || {}).map(([symbol, rows]) => [
      symbol,
      (rows || []).filter((row) => within(row.date)),
    ]),
  );
  const benchmarkCurve = (run.benchmarkCurve || []).filter((row) =>
    within(row.date),
  );
  const trades = (run.trades || []).filter((trade) => within(trade.date));
  ...
  const observations = (
    run.metrics?.rankRegimeDiagnostics?.observations || []
  ).filter((row) => !row.date || within(row.date));
  metrics.rankRegimeDiagnostics = {
    ...(run.metrics?.rankRegimeDiagnostics || {}),
    rebalanceObservations: observations.length,
    observations,
  };
  return {
    config: run.config,
    ...
    trades,
    openPositions: [],
    endingCash: curve.at(-1)?.cash ?? null,
    continuousLifecycleSlice: true,
...
export function simulatePointInTimePortfolio(dataset = {}, options = {}) {
  ...
  const config = {
    ...
    requireEarningsSurpriseFactors: false,
    requireFilingChangeFactors: false,
    requireFilingPeriodAlignment: false,
    requireNasdaqResidualPathFactors: false,
    minEarningsSurpriseScore: null,
    minSessionsSinceEarnings: null,
    maxSessionsSinceEarnings: null,
    minimumFilingCoverageFields: null,
    minSessionsSinceFiling: null,
    maxSessionsSinceFiling: null,
    minimumRevenueGrowthPct: null,
    minimumProfitabilityGrowthPct: null,
    minimumCashFlowGrowthPct: null,
    minimumMarginChangePctPoints: null,
    minimumCashConversionChangePctPoints: null,
    ...
    rankedAdaptiveRebalanceEnabled: false,
    ...
    rankedAdaptiveEqualWeightEnabled: false,
    rankedWeakBreadthTargetCount: null,
    rankedStrongBreadthTargetCount: null,
    rankedTargetCount: null,
    rankedExitBuffer: null,
    rankedMinimumHoldSessions: 0,
    rankedEntryQueueCount: null,
    // Research-only early momentum-loss rule. Disabled unless a candidate
    // explicitly supplies a finite rank threshold and confirmation count.
    earlyRankExitStartSessions: null,
    earlyRankExitEndSessions: null,
    earlyRankExitThreshold: null,
    earlyRankExitConsecutiveSessions: null,
    // Research-only upside momentum rule. Once a position has reached the
    // configured gain, confirmed rank deterioration can protect the winner
    // before the normal minimum-hold rank exit becomes active.
    profitRankExitStartSessions: null,
    profitRankExitEndSessions: null,
    profitRankExitActivationPct: null,
    profitRankExitThreshold: null,
    profitRankExitConsecutiveSessions: null,
  ...
  const pendingUniverseRemovalActions = new Map();
  ...
  for (let sessionIndex = 0; sessionIndex < sessions.length; sessionIndex++) {
    ...
    const universeRemovalActions = (session.corporateActions || []).filter(
      (action) =>
        String(action.type || "").toLowerCase() === "universe-removal",
    );
    const removedToday = new Set(universeRemovalActions.map(symbolOf));
    if (config.exitOnUniverseRemoval)
      for (const action of universeRemovalActions) {
        const symbol = symbolOf(action);
        if (symbol) pendingUniverseRemovalActions.set(symbol, action);
      }
    ...
    if (active) {
      ...
      for (const action of delistingActions) {
        ...
        pendingUniverseRemovalActions.delete(symbol);
      ...
      if (config.exitOnUniverseRemoval)
        for (const [symbol, action] of [
          ...pendingUniverseRemovalActions,
        ]) {
          ...
          if (!row || row.adjusted !== true || !(number(row.open) > 0)) {
            skippedOrders.push({
              date: session.date,
              symbol,
              side: "sell",
              reason: "universe-removal-open-unavailable",
            });
            continue;
          }
          executeSale({
            ...
            symbol,
            position,
            reason: "universe-removal",
            shares: position.shares,
            fill: tradePrice(row.open, "sell", config.slippageBps),
          });
          pendingUniverseRemovalActions.delete(symbol);
        }
      ...
      for (const order of buyOrders.sort((a, b) => b.rank - a.rank)) {
        ...
        let targetPct = Number.isFinite(order.targetPct)
          ? order.targetPct
    ...
    if (active) {
      ...
      const adaptiveRankContext =
        rankedSelection && config.rankedAdaptiveRebalanceEnabled === true
          ? crossSectionalRankContext(entrySignals)
          : null;
      const weakBreadth =
        adaptiveRankContext &&
        number(adaptiveRankContext.momentumBreadthPct, 0) <
          number(config.rankedWeakBreadthThresholdPct, 50);
      const effectiveRebalanceSessions = adaptiveRankContext
        ? Math.max(
            1,
            Math.floor(
              number(
                weakBreadth
                  ? config.rankedWeakBreadthRebalanceSessions
                  : config.rankedStrongBreadthRebalanceSessions,
      ...
      const rankRebalance =
        rankedSelection &&
        activeSessionNumber % effectiveRebalanceSessions === 0;
      // Ranked portfolios cannot enter or rank-exit between rebalance dates.
      // Avoid rebuilding and sorting the full cross-section on sessions where
      // the result is provably unused. This preserves decisions while cutting
      // the dominant R8-R11 CPU path by roughly the rebalance interval.
      const rawRankContext = rankRebalance
        ? adaptiveRankContext || crossSectionalRankContext(entrySignals)
        : {};
      if (
        rankRebalance &&
      ...
      const rankedPool = rankRebalance
        ? entrySignals
            .filter(
              (signal) =>
      ...
      const configuredTargetCount =
        config.rankedAdaptiveTargetEnabled === true && adaptiveRankContext
          ? weakBreadth
            ? config.rankedWeakBreadthTargetCount
            : config.rankedStrongBreadthTargetCount
          : config.rankedTargetCount;
      const rankedTargetCount = Math.max(
        1,
        Math.floor(number(configuredTargetCount, config.maxPositions)),
      );
      ...
      const earlyRankExitSymbols = new Set();
      const profitRankExitSymbols = new Set();
      if (rankRebalance)
        for (const [symbol, position] of positions) {
          ...
          const earlyStart = Number(config.earlyRankExitStartSessions);
          const earlyEnd = Number(config.earlyRankExitEndSessions);
          const earlyThreshold = Number(config.earlyRankExitThreshold);
          const earlyConfirmations = Number(
            config.earlyRankExitConsecutiveSessions,
          );
          const earlyRuleActive =
            Number.isFinite(earlyStart) &&
            Number.isFinite(earlyEnd) &&
            Number.isFinite(earlyThreshold) &&
            Number.isFinite(earlyConfirmations) &&
            earlyConfirmations > 0 &&
            heldSessions >= earlyStart &&
            heldSessions < earlyEnd;
          if (earlyRuleActive) {
            const rank =
              rankedPool.findIndex((signal) => symbolOf(signal) === symbol) + 1;
            position.earlyRankExitStreak =
              rank === 0 || rank > earlyThreshold
                ? Math.max(0, number(position.earlyRankExitStreak, 0)) + 1
                : 0;
            if (position.earlyRankExitStreak >= earlyConfirmations)
              earlyRankExitSymbols.add(symbol);
          } else {
            position.earlyRankExitStreak = 0;
          }
          const profitStart = Number(config.profitRankExitStartSessions);
          const profitEnd = Number(config.profitRankExitEndSessions);
          const profitActivation = Number(config.profitRankExitActivationPct);
          const profitThreshold = Number(config.profitRankExitThreshold);
          const profitConfirmations = Number(
            config.profitRankExitConsecutiveSessions,
          );
          ...
          const peakGainPct =
            position.entryPrice > 0
              ? (number(position.highWatermark, mark) / position.entryPrice - 1) *
                100
              : 0;
          const profitRuleActive =
            Number.isFinite(profitStart) &&
            Number.isFinite(profitEnd) &&
            Number.isFinite(profitActivation) &&
            Number.isFinite(profitThreshold) &&
            Number.isFinite(profitConfirmations) &&
            profitConfirmations > 0 &&
            heldSessions >= profitStart &&
            heldSessions < profitEnd &&
            peakGainPct >= profitActivation &&
            currentGainPct > 0;
          if (profitRuleActive) {
            const rank =
              rankedPool.findIndex((signal) => symbolOf(signal) === symbol) + 1;
            position.profitRankExitStreak =
              rank === 0 || rank > profitThreshold
                ? Math.max(0, number(position.profitRankExitStreak, 0)) + 1
                : 0;
            if (position.profitRankExitStreak >= profitConfirmations)
              profitRankExitSymbols.add(symbol);
          } else {
            position.profitRankExitStreak = 0;
          }
          if (
      ...
      for (const [symbol, position] of positions) {
        ...
        else if (profitRankExitSymbols.has(symbol))
          pending.push({
            side: "sell",
            symbol,
            fraction: 1,
            reason: "profit-rank-deterioration",
          });
        else if (earlyRankExitSymbols.has(symbol))
          pending.push({
            side: "sell",
            symbol,
            fraction: 1,
            reason: "early-rank-deterioration",
          });
      ...
      const eligibleForRankedEntry = (signal) =>
        entrySignalQualifies(signal, config) &&
        !blocked.has(symbolOf(signal)) &&
      ...
      const rankedEligibleSignals = rankedSelection
        ? rankRebalance
          ? rankedPool.filter(eligibleForRankedEntry)
          : []
        : entrySignals.filter(eligibleForRankedEntry).sort(
            (a, b) =>
              researchRank(b, config, rankContext) -
                researchRank(a, config, rankContext) ||
              symbolOf(a).localeCompare(symbolOf(b)),
      ...
      const entryCandidates =
        rankedSelection && !rankRebalance ? [] : candidates;
      for (const signal of entryCandidates) {
        const symbol = symbolOf(signal);
        ...
        pending.push({
          ...
          targetPct: Array.isArray(config.rankedTargetWeights)
            ? optionalNumber(
                config.rankedTargetWeights[
                  rankedTargetIndexBySymbol.get(symbol)
                ],
                null,
              )
            : config.rankedAdaptiveEqualWeightEnabled === true
              ? 0.99 / rankedTargetCount
            : null,
  ...
  if (config.liquidateAtEnd && lastActiveSession && positions.size) {
    for (const [symbol, position] of [...positions]) {
      if (pendingUniverseRemovalActions.has(symbol)) continue;
  ...
  const unresolvedUniverseRemovals = [...pendingUniverseRemovalActions]
    .filter(([symbol]) => positions.has(symbol))
    .map(([symbol, action]) => ({
      symbol,
