...
export const C1_HISTORICAL_PRICE_EVIDENCE_CONTRACT='c1-historical-price-evidence-v1';
...
export function validateC1AccountSeed(seed,config,sessions) {
  ...
  for(const p of seed.positions) {
    ...
    if(!baseline.universeSymbols.includes(p.symbol)&&!(p.inheritedRiskOnly===true&&baseline.legacyHoldingSignals?.some(s=>s.symbol===p.symbol&&s.sector===p.sector&&s.issuer===p.issuer)))throw new Error('An opening holding is outside the dated C1 universe: '+p.symbol);
...
export function createC1ProspectiveSeeds({portfolio,baseline,capitalRecord,adoptionConfirmed=false}={}) {
  ...
  const signals=new Map([...(baseline.legacyHoldingSignals||[]),...(baseline.positionSignals||[]),...(baseline.signals||[])].map(p=>[p.symbol,p]));
  const active=portfolio.filter(p=>p.role==='Swing'&&!cashSymbols.has(p.symbol)&&p.shares>0);
  let equity=cash;
  const outside=active.filter(p=>!baseline.universeSymbols?.includes(p.symbol)&&!baseline.legacyHoldingSignals?.some(s=>s.symbol===p.symbol&&s.inheritedRiskOnly===true)).map(p=>p.symbol);
  if(outside.length)throw new Error('Holdings outside the dated C1 universe: '+outside.join(', ')+'. Account coverage is incomplete; this is not a sell signal.');
  const missing=active.filter(p=>{const row=prices.get(p.symbol);return row?.adjusted!==true||!finite(row.close,Number.MIN_VALUE);}).map(p=>p.symbol);
  if(missing.length)throw new Error('Current adjusted account valuation required: '+missing.join(', '));
  for(const p of active)equity+=p.shares*prices.get(p.symbol).close;
  ...
  for(const p of active){
    ...
    ids.forEach((id,index)=>{if(quantities[index])allocations[id].push({symbol:p.symbol,role:'Swing',shares:quantities[index],entryPrice:p.avgCost,openedAt,
      inheritedRiskOnly:signal.inheritedRiskOnly===true,sector:signal.sector,issuer:signal.issuer||String(signal.cik||signal.cikNumber||p.symbol),stock:clone(signal)});});
  }
...
// Enrich a disposable copy of the adoption with dated, adjusted price facts.
// This never changes ownership, cash, fills, basis or the original holding date.
// Incomplete coverage is labelled rather than silently treated as complete.
export function c1HistoricalPriceEvidence(position,sessions=[],through){
  if(!position||!day(position.openedAt)||!finite(position.entryPrice,Number.MIN_VALUE)||!day(through)||!Array.isArray(sessions))throw new Error('Holding basis, dates and historical sessions required');
  const bars=[];
  for(const session of sessions){
    if(session?.date<position.openedAt||session?.date>through)continue;
    const row=session.prices?.find(p=>p.symbol===position.symbol);
    if(row?.adjusted===true&&['high','low','close'].every(k=>finite(row[k],Number.MIN_VALUE)))bars.push({date:session.date,high:row.high,low:row.low,close:row.close});
  }
  const expectedSessions=marketSessionDistance(position.openedAt,through)+1;
  const unique=[...new Map(bars.map(bar=>[bar.date,bar])).values()].sort((a,b)=>a.date.localeCompare(b.date));
  return {contract:C1_HISTORICAL_PRICE_EVIDENCE_CONTRACT,from:position.openedAt,through,sessionsObserved:unique.length,expectedSessions,complete:unique.length===expectedSessions,
    high:Math.max(position.entryPrice,...unique.map(bar=>bar.high)),low:Math.min(position.entryPrice,...unique.map(bar=>bar.low)),
    closesAboveEntry:unique.filter(bar=>bar.close>position.entryPrice).length,closesBelowEntry:unique.filter(bar=>bar.close<position.entryPrice).length,
    closesAtEntry:unique.filter(bar=>bar.close===position.entryPrice).length};
}
export function withC1HistoricalPriceEvidence(adoption,sessions=[]){
  const next=clone(adoption);
  if(next?.contract!=='c1-prospective-account-adoption-v1'||!next.seeds||!Array.isArray(sessions))throw new Error('Accepted adoption and historical sessions required');
  for(const seed of Object.values(next.seeds))for(const position of seed.positions){
    position.historicalPriceEvidence=c1HistoricalPriceEvidence(position,sessions,next.sourceSessionDate);
  }
  return next;
}
...
export function c1SeedPosition(position,sessionIndex,asOfSession,close) {
  ...
  const history=position.historicalPriceEvidence?.contract===C1_HISTORICAL_PRICE_EVIDENCE_CONTRACT&&position.historicalPriceEvidence.through===asOfSession?position.historicalPriceEvidence:null;
  const historicalHigh=finite(history?.high,Number.MIN_VALUE)?history.high:position.entryPrice,historicalLow=finite(history?.low,Number.MIN_VALUE)?history.low:position.entryPrice;
  return {...clone(position),initialShares:position.shares,entryCommission:0,realizedPnl:0,
    ...
    highWatermark:Math.max(position.entryPrice,close,historicalHigh),stopPrice:stop,initialStopPrice:stop,
    stopKind:'initial-stop',factor:position.factor||position.sector,stock:position.stock||null,
    maxPrice:Math.max(position.entryPrice,close,historicalHigh),minPrice:Math.min(position.entryPrice,close,historicalLow),
