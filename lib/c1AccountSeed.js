...
export function createC1ProspectiveSeeds({portfolio,baseline,capitalRecord,adoptionConfirmed=false}={}) {
  ...
  for(const p of portfolio){
    ...
    if(cashSymbols.has(p.symbol)){if(Number(p.avgCost||1)!==1)throw new Error('Cash balance must use dollar units');cash+=p.shares;cashDeclared=true;}
  }
  ...
  const normalized=active.map(p=>{
    ...
    return {symbol:p.symbol,role:'Swing',shares:p.shares*scale,entryPrice:p.avgCost,openedAt,
      sector:signal.sector,issuer:signal.issuer||String(signal.cik||signal.cikNumber||p.symbol),stock:clone(signal)};
  });
  const weights={base:.25,cooldown15:.5,sector40:.25},seeds={};
  for(const [sleeve,weight] of Object.entries(weights)) {
    ...
    const seed={contract:C1_ACCOUNT_SEED_CONTRACT,sleeve,asOfSession:baseline.date,prospectiveAdoptionConfirmed:true,
      cash:cash*scale,highWater:Math.max(100000,capitalRecord.highWater*scale),
      remainingCooldownSessions:remaining,resumeAtNextOpen:elapsed!=null&&elapsed>=C1_FROZEN_OPTIONS[sleeve].portfolioDrawdownCooldownSessions,positions:normalized};
    ...
    seeds[sleeve].actualDollarsPerModelDollar=weight/scale;
  }
  return {contract:'c1-prospective-account-adoption-v1',sourceSessionDate:baseline.date,
    ...
    allocationPolicy:'Prospectively attribute each current Swing position and cash 25%/50%/25%; retain original holding dates, cost basis and existing capital-loss limits.',
