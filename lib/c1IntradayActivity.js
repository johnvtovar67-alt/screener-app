...
export function recordC1IntradayActivity({account,plan,ticket,expectedRevision,now=new Date()}={}){
 ...
 if(!ticket||typeof ticket.id!=='string'||!ticket.id||ticket.id.length>100||!['buy','sell'].includes(ticket.side)||!Number.isSafeInteger(ticket.shares)||ticket.shares<=0||!positive(ticket.price)||typeof ticket.fee!=='number'||!Number.isFinite(ticket.fee)||ticket.fee<0||!Number.isFinite(time.getTime())||time>now||clock?.key!==plan?.date||clock.minutes<570||clock.minutes>=marketSessionCloseMinutes(clock.key)||easternMarketClock(now)?.key!==plan.date)throw new Error('Enter the actual regular-session trade quantity, price, fee and time');
 ...
 return {...account,revision:account.revision+1,intradayActivity:{...(prior?clone(prior):{}),date:original.date,openingObservedAt:original.observedAt,plan:clone(original),tickets:[...(prior?.tickets||[]),clone(ticket)],fills}};
}
