...
import {easternMarketClock,marketExecutionState,marketSessionDistance,marketSessionCloseMinutes} from './marketSession';
...
export const C1_OWNER_EXIT_REASON='owner-discretionary-exit';

function withOwnerExitOrders(plan,ticket){
 if(ticket?.recordingReason!==C1_OWNER_EXIT_REASON)return plan;
 if(ticket.side!=='sell')throw new Error('Only a completed sale of owned shares can be recorded as another broker trade');
 const copy=clone(plan),existing=new Set(copy.orders.filter(o=>o.reason===C1_OWNER_EXIT_REASON).map(o=>o.sleeve+':'+o.symbol));
 for(const [sleeve,book] of Object.entries(copy.openingBooks)){
  const held=book.positions[ticket.symbol]?.shares||0,key=sleeve+':'+ticket.symbol;
  if(held>0&&!existing.has(key))copy.orders.push({id:`owner-exit:${sleeve}:${copy.date}:${ticket.symbol}`,date:copy.date,sleeve,symbol:ticket.symbol,side:'sell',shares:held,reason:C1_OWNER_EXIT_REASON,recordingOnly:true,executable:false});
 }
 return copy;
}

// A broker sale is an accounting fact, not a new purchase recommendation.
// This plan may also be augmented with a confirmed owner exit below.
export function c1RecordedExitPlan({continued,now=new Date()}={}){
 ...
 return {contract:'c1-actual-account-opening-plan-v1',recordingOnly:true,providerVerified:false,executable:false,sourceSessionDate:continued.sourceSessionDate,date:day,observedAt:now.toISOString(),openingBooks:clone(continued.books),orders};
}
...
export function rebaseC1RecordedExitActivity({activity,plan,now=new Date()}={}){
 ...
 const ownerOrders=activity.plan.orders.filter(o=>o.reason===C1_OWNER_EXIT_REASON),target={...plan,orders:[...plan.orders,...ownerOrders.filter(o=>!plan.orders.some(p=>p.id===o.id))]};
 const fills=activity.fills.map(fill=>{
  const original=activity.plan.orders.find(o=>o.id===fill.orderId);
  if(original?.reason===C1_OWNER_EXIT_REASON)return fill;
 ...
 accepted(target,fills,now);
 return {...activity,recordingEvidence:activity.plan,plan:clone(target),fills};
}
...
export function recordC1IntradayActivity({account,plan,ticket,expectedRevision,now=new Date()}={}){
 ...
 if(!ticket||typeof ticket.id!=='string'||!ticket.id||ticket.id.length>100||!['buy','sell'].includes(ticket.side)||(ticket.recordingReason!=null&&ticket.recordingReason!==C1_OWNER_EXIT_REASON)||!Number.isSafeInteger(ticket.shares)||ticket.shares<=0||!positive(ticket.price)||typeof ticket.fee!=='number'||!Number.isFinite(ticket.fee)||ticket.fee<0||!Number.isFinite(time.getTime())||time>now||clock?.key!==plan?.date||clock.minutes<570||clock.minutes>=marketSessionCloseMinutes(clock.key)||easternMarketClock(now)?.key!==plan.date)throw new Error('Enter the actual regular-session trade quantity, price, fee and time');
 if(prior&&prior.date!==plan.date)throw new Error('Previous trade session has not finished reconciling');
 const original=withOwnerExitOrders(prior?.plan||plan,ticket);
 ...
 const ownerExit=ticket.recordingReason===C1_OWNER_EXIT_REASON;
 const candidates=before.outstanding.filter(o=>o.symbol===ticket.symbol&&o.side===ticket.side&&(ownerExit?o.reason===C1_OWNER_EXIT_REASON:o.reason!==C1_OWNER_EXIT_REASON)).sort((a,b)=>Number(Boolean(a.condition))-Number(Boolean(b.condition)));
 ...
 const ownerRecorded=original.orders.some(o=>o.reason===C1_OWNER_EXIT_REASON);
 return {...account,revision:account.revision+1,intradayActivity:{...(prior?clone(prior):{}),date:original.date,openingObservedAt:original.observedAt,plan:clone(original),...(ownerRecorded?{recordingEvidence:clone(original)}:{}),tickets:[...(prior?.tickets||[]),clone(ticket)],fills}};
}

// Correct broker-reported price, fee or time without changing the transaction
// identity, side, symbol, quantity or sleeve allocation.
export function correctC1IntradayTradeDetails({account,ticketId,price,fee,executedAt,expectedRevision,now=new Date()}={}){
 if(account?.revision!==expectedRevision)throw new Error('Account changed; reload before correcting the trade');
 const activity=account?.intradayActivity,ticket=activity?.tickets?.find(row=>row.id===ticketId);
 if(!ticket)throw new Error('Saved trade not found; reload before correcting it');
 const time=new Date(executedAt),clock=easternMarketClock(time);
 if(!positive(price)||typeof fee!=='number'||!Number.isFinite(fee)||fee<0)throw new Error('Enter the actual execution price and fee');
 if(!Number.isFinite(time.getTime())||time>now||clock?.key!==activity.date||clock.minutes<570||clock.minutes>=marketSessionCloseMinutes(clock.key))throw new Error('Enter the actual regular-session execution time');
 const tickets=activity.tickets.map(row=>row.id===ticketId?{...row,price,fee,executedAt:time.toISOString()}:clone(row));
 const fills=activity.fills.map(fill=>fill.id.startsWith(ticketId+':')?{...fill,price,fee:fee*fill.shares/ticket.shares,executedAt:time.toISOString()}:clone(fill)).sort((a,b)=>Date.parse(a.executedAt)-Date.parse(b.executedAt));
 accepted(activity.plan,fills,now);
 return {...account,revision:account.revision+1,intradayActivity:{...clone(activity),tickets,fills}};
}

export function correctC1IntradayTradeTime({account,ticketId,executedAt,expectedRevision,now=new Date()}={}){
 const ticket=account?.intradayActivity?.tickets?.find(row=>row.id===ticketId);
 return correctC1IntradayTradeDetails({account,ticketId,price:ticket?.price,fee:ticket?.fee,executedAt,expectedRevision,now});
}
...
export function rebaseC1UnfilledEntryPlan({activity,plan,now=new Date()}={}){
 ...
 const before=accepted(activity.plan,activity.fills,now),after=accepted(target,fills,now);
 ...
 return {...activity,entryPlanEvidence:activity.entryPlanEvidence||clone(activity.plan),plan:clone(target),fills};
}
