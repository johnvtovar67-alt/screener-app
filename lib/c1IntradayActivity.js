...
// Correct only the timestamp of an already-recorded broker fact. Economic
// fields and allocation identities remain immutable, and the entire fill set
// must still reconcile against the original saved opening plan.
export function correctC1IntradayTradeTime({account,ticketId,executedAt,expectedRevision,now=new Date()}={}){
 if(account?.revision!==expectedRevision)throw new Error('Account changed; reload before correcting the trade time');
 ...
 const tickets=activity.tickets.map(row=>row.id===ticketId?{...row,executedAt:time.toISOString()}:clone(row));
 const fills=activity.fills.map(fill=>fill.id.startsWith(ticketId+':')?{...fill,executedAt:time.toISOString()}:clone(fill)).sort((a,b)=>Date.parse(a.executedAt)-Date.parse(b.executedAt));
