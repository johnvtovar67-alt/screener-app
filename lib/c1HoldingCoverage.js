...
export async function collectC1HoldingCoverage({portfolio,baseline,now=new Date(),fetcher=fetch,
 ...
 if(!isUsMarketSessionDay(baseline?.date)||baseline.date>latestCompletedMarketSessionDay(now))throw new Error('Current completed session required for holding valuations');
 ...
 const workers=Array.from({length:Math.min(3,symbols.length)},async()=>{
  while(cursor<symbols.length){
   ...
   try{
    ...
    if(!baseline.universeSymbols?.includes(symbol)){
     const response=await fetcher('https://financialmodelingprep.com/stable/profile?'+new URLSearchParams({symbol:providerSymbol(symbol),apikey:apiKey}),
      {cache:'no-store',signal:AbortSignal.timeout(Math.max(1,Math.min(8000,deadline-Date.now())))});
     if(!response.ok)throw new Error('profile');
     const profiles=await response.json(),profile=Array.isArray(profiles)&&profiles.find(p=>[symbol,providerSymbol(symbol)].includes(p.symbol));
     if(!profile||typeof profile.sector!=='string'||!profile.sector.trim()||!profile.cik)throw new Error('classification');
     classification={symbol,sector:profile.sector,issuer:String(profile.cik),provider:'FMP',endpoint:'profile',observedAt:new Date().toISOString()};
    }
    rows.push({symbol,status:'verified',classification,date:baseline.date,price:{...bar,symbol},
