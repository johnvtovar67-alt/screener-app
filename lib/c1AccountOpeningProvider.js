...
export function validateC1OpeningObservations({baseline,symbols,membersBefore,membersAfter,observations,now=new Date()}={}){
 ...
 if(membership(membersBefore)!==[...baseline.universeSymbols].sort().join('|')||JSON.stringify(membersBefore)!==JSON.stringify(membersAfter))throw new Error('Index membership or classification changed since the account decision');
