...
export function validateC1OpeningObservations({baseline,symbols,membersBefore,membersAfter,observations,now=new Date()}={}){
 ...
 for(const symbol of symbols){
  ...
  if(anchor?.date!==baseline.date||!Number.isFinite(anchor.close)||Math.abs(anchor.close-prior.get(symbol)?.close)>1e-8*Math.max(1,anchor.close)||!prior.has(symbol))throw new Error('Adjusted account price basis changed for '+symbol+' ('+baseline.date+': saved '+String(prior.get(symbol)?.close)+', provider '+String(anchor?.close)+', provider date '+String(anchor?.date)+')');
 ...
 return {date:clock.key,universeSymbols:[...baseline.universeSymbols],corporateActions:[],prices,
  receipt:{provider:'FMP',observedAt:now.toISOString(),membershipCheckedTwice:true,previousAdjustedClosesUnchanged:true,quoteMaximumAgeSeconds:120,symbolConvention:'explicit-BF.B-BRK.B-share-class-aliases'}};
}
