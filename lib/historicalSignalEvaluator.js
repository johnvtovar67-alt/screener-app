...
function maximumDailyReturn(closes, sessions = 20) {
  if (closes.length < sessions + 1) return null;
  const window = closes.slice(-(sessions + 1));
  let maximum = -Infinity;
  for (let index = 1; index < window.length; index++) {
    const prior = number(window[index - 1]);
    const current = number(window[index]);
    if (!(prior > 0) || !(current > 0)) continue;
    maximum = Math.max(maximum, (current / prior - 1) * 100);
  }
  return Number.isFinite(maximum) ? maximum : null;
}

function pricePathDiagnostics(history = [], sessions = 120) {
  const rows = history
    .filter((row) => number(row?.close) > 0)
    .slice(-(sessions + 1));
  if (rows.length < sessions + 1)
    return {
      positiveSessionSharePct: null,
      pathEfficiencyPct: null,
      informationDiscreteness: null,
      maxDrawdownPct: null,
    };
  const logMoves = [];
  let positive = 0;
  let negative = 0;
  let peak = number(rows[0].close);
  let maxDrawdownPct = 0;
  for (let index = 1; index < rows.length; index++) {
    const prior = number(rows[index - 1].close);
    const close = number(rows[index].close);
    if (!(prior > 0) || !(close > 0)) continue;
    const move = Math.log(close / prior);
    logMoves.push(move);
    if (move > 0) positive++;
    else if (move < 0) negative++;
  ...
  const totalLogReturn = Math.log(last / first);
  const absolutePath = logMoves.reduce(
    (sum, move) => sum + Math.abs(move),
    0,
  );
  const observedDirections = positive + negative;
  const positiveShare = observedDirections ? positive / observedDirections : 0.5;
  const negativeShare = observedDirections ? negative / observedDirections : 0.5;
  // Da, Gurun and Warachka's information-discreteness construction: lower
  // values identify returns accumulated through many same-direction sessions
  // instead of one or two discontinuous jumps. It is paired with positive
  // leadership/anchor evidence so a smooth decline cannot rank as a winner.
  const informationDiscreteness =
    Math.sign(totalLogReturn) * (negativeShare - positiveShare) * 100;
  return {
    positiveSessionSharePct: positiveShare * 100,
    pathEfficiencyPct:
      absolutePath > 0 ? (Math.abs(totalLogReturn) / absolutePath) * 100 : 0,
    informationDiscreteness,
    maxDrawdownPct,
  };
}

function sessionsSinceTrailingHigh(history = [], sessions = 252) {
  const rows = history.slice(-sessions);
  if (!rows.length) return null;
  let high = -Infinity;
  let highIndex = -1;
  for (let index = 0; index < rows.length; index++) {
    const observedHigh = number(rows[index]?.high, number(rows[index]?.close));
    if (observedHigh >= high) {
      high = observedHigh;
      highIndex = index;
    }
  }
  return highIndex >= 0 ? rows.length - 1 - highIndex : null;
}
...
function momentumAccelerationDailyPct(return20, return120Ex20) {
  if (!isObservedNumber(return20) || !isObservedNumber(return120Ex20))
    return null;
  const recent = Math.log1p(Number(return20) / 100) / 20;
  const prior = Math.log1p(Number(return120Ex20) / 100) / 100;
...
export function attachCrossSectionalResearchFactors(rows = []) {
  // Negative book equity can make debt/equity negative and ROE mechanically
  // extreme. Neither value is comparable cross-sectionally in that state; do
  // not let an accounting denominator anomaly masquerade as quality.
  rows = rows.map((row) => {
    const nonPositiveBookEquity =
      isObservedNumber(row.bookValue) && Number(row.bookValue) <= 0;
    return {
      ...row,
      returnOnEquity: nonPositiveBookEquity ? null : row.returnOnEquity,
      debtToEquity:
        nonPositiveBookEquity ||
        (isObservedNumber(row.debtToEquity) && Number(row.debtToEquity) < 0)
          ? null
          : row.debtToEquity,
      accountingQualityComparable: !nonPositiveBookEquity,
    };
  });
  ...
  const ranked = rows.map((row) => {
    ...
    const highProximityPercentile = rank("distanceFromYearHighPct");
    const highRecencyPercentile = rank("yearHighRecencySessions");
    const continuousInformationPercentile = weightedScore([
      [rank("positiveSessionShare120Pct"), 0.45],
      [rank("pathEfficiency120Pct"), 0.35],
      [rank("informationDiscreteness120"), 0.2],
    ], null);
    const intermediateLeadershipPercentile = weightedScore([
      [rank("return200Ex20"), 0.55],
      [rank("return120Ex20"), 0.45],
    ], null);
    const drawdownResiliencePercentile = rank("maxDrawdown120Pct");
    const accelerationRestraintPercentile = rank(
      "momentumAccelerationDailyPct",
    );
    const lotteryRestraintPercentile = rank("maxDailyReturn20Pct");
    ...
    return {
      ...
      researchFactors: {
        ...
        return200Ex20: row.return200Ex20,
        distanceFromYearHighPct: row.distanceFromYearHighPct,
        yearHighRecencySessions: row.yearHighRecencySessions,
        positiveSessionShare120Pct: row.positiveSessionShare120Pct,
        pathEfficiency120Pct: row.pathEfficiency120Pct,
        informationDiscreteness120: row.informationDiscreteness120,
        maxDrawdown120Pct: row.maxDrawdown120Pct,
        relativeVolume20: row.relativeVolume20,
        maxDailyReturn20Pct: row.maxDailyReturn20Pct,
        momentumAccelerationDailyPct: row.momentumAccelerationDailyPct,
...
export function buildHistoricalQuote({ profile, bar, history, fundamental, decisionAt }) {
  // Every feature below is bounded to 252 sessions. Keeping only one extra
  // observation preserves the longest return calculation exactly while
  // preventing compilation cost from growing with the full history length.
  const trailingHistory = history.slice(-253);
  const closes = trailingHistory
    .map((row) => number(row.close))
    .filter((x) => x > 0);
  const prior = closes.at(-2);
  const price = closes.at(-1);
  const priorVolumes = trailingHistory
    .slice(-21, -1)
    .map((row) => number(row.volume))
    .filter((x) => x > 0);
  const last252 = trailingHistory.slice(-252);
  const yearHigh = Math.max(
    ...last252.map((row) => number(row.high, row.close)),
  );
  const path120 = pricePathDiagnostics(trailingHistory, 120);
  const averagePriorVolume20 = mean(priorVolumes);
  const shares = number(
  ...
  return {
    ...
    return200Ex20: trailingReturn(closes, 180, 20),
    distanceFromYearHighPct:
      price > 0 && yearHigh > 0 ? (price / yearHigh - 1) * 100 : null,
    yearHighRecencySessions: sessionsSinceTrailingHigh(last252, 252),
    positiveSessionShare120Pct: path120.positiveSessionSharePct,
    pathEfficiency120Pct: path120.pathEfficiencyPct,
    informationDiscreteness120: path120.informationDiscreteness,
    maxDrawdown120Pct: path120.maxDrawdownPct,
    relativeVolume20:
      number(bar.volume) > 0 && averagePriorVolume20 > 0
        ? number(bar.volume) / averagePriorVolume20
        : null,
    volatility20Pct: annualizedVolatility(closes, 20),
    volatility60Pct: annualizedVolatility(closes, 60),
    maxDailyReturn20Pct: maximumDailyReturn(closes, 20),
    momentumAccelerationDailyPct: momentumAccelerationDailyPct(
      trailingReturn(closes, 20),
      trailingReturn(closes, 100, 20),
    ),
...
export function compilePointInTimeSignals(rawDataset = {}, options = {}) {
  ...
  const requestedMinimumHistoryRows = Number(options.minimumHistoryRows);
  const minimumHistoryRowsConfigured =
    Number.isInteger(requestedMinimumHistoryRows) &&
    requestedMinimumHistoryRows >= 20;
  const minimumHistoryRows = minimumHistoryRowsConfigured
    ? requestedMinimumHistoryRows
    : 20;
  ...
  for (const session of rawSessions.slice(startIndex, endIndex)) {
    ...
    const sessionUniverse = Array.isArray(session.universeSymbols)
      ? new Set(session.universeSymbols.map(symbolOf).filter(Boolean))
      : null;
    const sourceProfiles = [...profiles.values()].filter((profile) => {
      ...
      return (
        (!sessionUniverse || sessionUniverse.has(symbol)) &&
        profile.isEtf !== true &&
        profile.isFund !== true &&
        listed &&
        listed <= session.date &&
    ...
    for (const profile of sourceProfiles) {
      ...
      if (!bar || history.length < minimumHistoryRows) continue;
    ...
    const holdingEvidenceBySymbol = new Map(
      holdingEvaluations.map((row) => [row.symbol, row]),
    );
    // The research layer must be able to evaluate an independently specified
    // factor thesis. Preserve the full-universe event and timing evidence even
    // when the production recommendation itself was only Watch/Avoid.
    const finalizedWithResearchEvidence = finalized.map((row) => {
      const evidence = holdingEvidenceBySymbol.get(row.symbol);
      if (!evidence) return row;
      const entryTiming = row.entryTiming || evidence.entryTiming;
      return {
        ...row,
        eventRisk: row.eventRisk || evidence.eventRisk,
        preTradeCheck: row.preTradeCheck || evidence.preTradeCheck,
        entryTiming,
        recommendation: {
          ...(row.recommendation || {}),
          entryTiming,
    ...
    outputSessions.push({
      date: session.date,
      decisionAt,
      universeSymbols: sessionUniverse
        ? [...sessionUniverse].sort()
        : null,
      ...
      signals: finalizedWithResearchEvidence.map((row) =>
  ...
  return {
    metadata: {
      ...
      ...(minimumHistoryRowsConfigured
        ? { minimumSignalHistoryRows: minimumHistoryRows }
        : {}),
      ...
      comparisonSymbols: Array.isArray(rawDataset.metadata?.comparisonSymbols)
        ? rawDataset.metadata.comparisonSymbols
        : [rawDataset.metadata?.benchmarkSymbol || "SPY"],
