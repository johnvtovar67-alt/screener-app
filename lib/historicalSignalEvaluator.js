...
function buildHistoricalQuote({ profile, bar, history, fundamental, decisionAt }) {
  ...
  const path120 = pricePathDiagnostics(history, 120);
  const priorVolumes20 = history
    .slice(-21, -1)
    .map((row) => number(row.volume))
    .filter((value) => value > 0);
  const averagePriorVolume20 = mean(priorVolumes20);
  const shares = number(
  ...
  return {
    ...
    yearHighRecencySessions: sessionsSinceTrailingHigh(history, 252),
