...
import { readStoredC1DatedBook } from "./c1DatedBookStore";
import { gunzipSync } from "node:zlib";
import { advanceStoredC1ForwardModel } from "./c1ForwardStore";
import { archiveC1Input } from "./c1InputArchive";
...
function assessSnapshot(snapshot, now = new Date()) {
  ...
  const model=snapshot?.forwardAccounting?.environment === (process.env.VERCEL_ENV || "local")
    ?{...snapshot.forwardAccounting,status:snapshot.forwardAccounting.sourceSessionDate===requiredSessionDate
      ?snapshot.forwardAccounting.status:'stale',executable:false,eligibleForLiveCapital:false}:null;
  return {
    ...(snapshot || {}),
    // Stored legacy metadata can never authorize the aggregate recommender.
    independentlyValidated:false,activationAuthorized:false,
    forwardAccounting:model,
    decisionSnapshot:buildC1DecisionSnapshot({model,inputArchive:model?.inputArchive,sourceSessionDate,now}),
...
export async function refreshV11ProductionSnapshot(now = new Date()) {
  // The old research cron also calls this entry point. It must read the same
  // hosted index source, never rebuild or advance the obsolete broad book.
  if (["preview", "production"].includes(process.env.VERCEL_ENV))
    return getV11ProductionSnapshot({ now, refreshIfStale: false });
  ...
  let forwardAccounting,inputArchive;
  try {
    inputArchive=await archiveC1Input(latestSession,payload.metadata,{chunkPath:latestChunk.pathname,checkpointSignature:checkpoint.signature},now);
    if(inputArchive.status==='revision-quarantined')throw new Error('Current input revision quarantined; original preserved');
    ...
    forwardAccounting = {status:"unavailable",executable:false,eligibleForLiveCapital:false,
      ...
      ...(error?.inputRevision ? {inputRevision:error.inputRevision} : {})};
  }
  const snapshot = {
    ...buildV11ProductionSnapshot(latestSession, now),
    forwardAccounting:{...forwardAccounting,...(inputArchive?{inputArchive}:{}),environment:process.env.VERCEL_ENV || "local"},
    datasetThrough,
...
export async function getV11ProductionSnapshot({
  ...
  // Both hosted environments use the declared S&P 500 source. Collection is a
  // separate operation; page reads never start hundreds of provider calls.
  // Never fall back to the broad cohort under an index label.
  if (["preview", "production"].includes(process.env.VERCEL_ENV)) {
    try {
      const { record, view, path } = await readStoredC1DatedBook("sp500", { now });
      const session = record.model.sessions.at(-1);
      return assessSnapshot({ ...buildV11ProductionSnapshot(session, now),
        forwardAccounting: { ...view.model, environment: process.env.VERCEL_ENV },
        datasetThrough: session.date, sourceChunkPath: path,
        sourceChunkEnd: record.model.sessions.length,
        snapshotId: "c1-dated-sp500:" + view.recordHash }, now);
    } catch (error) {
      return { ...assessSnapshot(null, now), status: "unavailable",
        error: String(error?.message || "Dated index model unavailable").slice(0, 240) };
    }
  }
