...
function assessSnapshot(snapshot, now = new Date()) {
  ...
  return {
    ...(snapshot || {}),
    ...(snapshot?.forwardAccounting ? {forwardAccounting:{...snapshot.forwardAccounting,
...
export async function getV11ProductionSnapshot({
  ...
  if (cached?.snapshot && Date.now() - number(cached.at, 0) < CACHE_MS) {
    ...
    if ((assessed.status === "ready" && assessed.forwardAccounting) || !refreshIfStale) return assessed;
  }
  ...
  const promise = (async () => {
    ...
    try {
      ...
      if ((assessed.status === "ready" && assessed.forwardAccounting) || !refreshIfStale) {
