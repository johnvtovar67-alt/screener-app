...
export async function getV11ProductionSnapshot({
  ...
  if (cached?.snapshot && Date.now() - number(cached.at, 0) < CACHE_MS) {
    ...
    if ((assessed.status === "ready" && assessed.forwardAccounting?.paperExecution) || !refreshIfStale) return assessed;
  }
  ...
  const promise = (async () => {
    ...
    try {
      ...
      if ((assessed.status === "ready" && assessed.forwardAccounting?.paperExecution) || !refreshIfStale) {
