...
export function reconcileBrokerageCSV(text,holdings){
 const rows=csv(text),seen=new Set(),positions=new Map(),opening=new Set(),openingPositions={},transactions=[];let cash=0,openingCash=0,lastDate='',startDate='',activity=false;
 for(let i=0;i<rows.length;i++){
  ...
  if(type.startsWith('OPEN_')){
   ...
   if(type==='OPEN_CASH'){if(i!==0||ticker||q||p||fee||amount<0)throw new Error('Invalid opening cash');cash=amount;openingCash=amount;}
   else{symbol(ticker);const shares=number(q,'opening shares');if(opening.has(ticker)||shares<=0||amount!==0||fee||p)throw new Error('Invalid opening position');opening.add(ticker);positions.set(ticker,shares);openingPositions[ticker]=shares;}
   continue;
  }
  activity=true;
  ...
  transactions.push({id,date,type,symbol:ticker||null,shares:q?number(q,'shares'):null,price:p?number(p,'price'):null,cashAmount:amount,fee});
 }
 ...
 cash,openingCash,openingPositions,transactions,reconstructedPositions:Object.fromEntries(positions),executable:false,orders:[],brokerageVerified:false,strategyValidated:false,
