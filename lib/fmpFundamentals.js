...
const BULK_RATIO_TTL_MS=24*60*60*1000;
const CACHE_KEY="__fmpFundamentalCacheV4";
const BULK_KEY="__fmpRatiosTtmBulkCacheV2";
const INFLIGHT_KEY="__fmpFundamentalInflightV1";
...
async function fetchJson(url){
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    try{
      ...
      const err=new Error(`FMP fundamentals request failed: ${r.status}${text?` - ${text.slice(0,160)}`:""}`);
      if(r.status!==429&&r.status<500)throw err;
      last=err;
      const retryAfter=Number(r.headers.get('retry-after'));
      await sleep(Number.isFinite(retryAfter)&&retryAfter>0?retryAfter*1000:400*Math.pow(2,attempt)+Math.floor(Math.random()*200));
    }catch(e){last=e;if(attempt<2)await sleep(400*Math.pow(2,attempt)+Math.floor(Math.random()*200));}
...
async function resilientOne(symbol,key,bulkRatios,store,now){
  const pending=inflight(),stale=store.get(symbol);
  if(pending.has(symbol))return pending.get(symbol);
  const p=(async()=>{try{const row=await fetchOne(symbol,key,bulkRatios);if(row.fundamentalDataStatus!=="unavailable")store.set(row.symbol,{ts:Date.now(),data:row});else if(stale&&now-stale.ts<FUNDAMENTAL_STALE_MS)return{...stale.data,fundamentalDataStaleFallback:true};return row;}catch(e){if(stale&&now-stale.ts<FUNDAMENTAL_STALE_MS)return{...stale.data,fundamentalDataStaleFallback:true};return{symbol,fundamentalDataStatus:"unavailable",fundamentalDataVerified:false,fundamentalSources:{ratiosTtm:false,incomeGrowth:false,statementFallback:false,ratioFields:0,growthFields:0}};}finally{pending.delete(symbol);}})();
  pending.set(symbol,p);return p;
}
...
export async function fetchFmpFundamentals(symbols=[]){
  ...
  if(missing.length){
    ...
    const rows=await mapLimited(missing,3,s=>resilientOne(s,key,bulkRatios,store,now));
    for(const row of rows)result.set(row.symbol,row);
  }
  return result;
}
