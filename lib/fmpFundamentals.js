...
export async function fetchFmpFundamentals(symbols=[]){
  ...
  // A broad screen used to fan out across every symbol at once. That produced roughly
  // 362 ratios calls + 362 growth calls in the user's FMP dashboard and exhausted the
  // 750 calls/min Premium limit. Bound each cold/warm pass; cached symbols are skipped,
  // so later refreshes naturally advance through the remaining universe without a storm.
  const toFetch=missing.slice(0,MAX_NEW_SYMBOLS_PER_RUN),deferred=missing.slice(MAX_NEW_SYMBOLS_PER_RUN);
  if(toFetch.length){const fetched=await mapLimited(toFetch,1,s=>resilientOne(s,key,store,now));for(const row of fetched)result.set(row.symbol,row);}
