...
const MAX_NEW_SYMBOLS_PER_RUN=12;
...
async function hydratePersistentCache(){
  ...
  globalThis[HYDRATE_KEY]=(async()=>{try{
    const{blobs}=await list({prefix:FUNDAMENTAL_STORE,limit:1}),blob=blobs.find(b=>b.pathname===FUNDAMENTAL_STORE)||blobs[0];if(!blob)return 0;
    const result=await get(blob.url,{access:'private',useCache:false});if(!result)return 0;
...
export async function fetchFmpFundamentals(symbols=[]){
  ...
  if(toFetch.length){const fetched=await mapLimited(toFetch,1,s=>resilientOne(s,key,store,now));for(const row of fetched)result.set(row.symbol,row);await persistFundamentalCache();}
