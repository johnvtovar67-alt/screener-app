...
//
// IMPORTANT: Vercel function memory is not durable. Persist verified fundamental rows
// in Blob so a cold function does not re-request the entire universe. A single cold
// invocation is also capped so it cannot consume the 750 calls/min Premium allowance.

import { put, list, get } from '@vercel/blob';

const FUNDAMENTAL_TTL_MS=24*60*60*1000;
const FUNDAMENTAL_STALE_MS=7*24*60*60*1000;
const MAX_NEW_SYMBOLS_PER_RUN=48;
const CACHE_KEY="__fmpFundamentalCacheV6";
const INFLIGHT_KEY="__fmpFundamentalInflightV3";
const COOLDOWN_KEY="__fmpFundamentalCooldownUntilV2";
const HYDRATE_KEY="__fmpFundamentalBlobHydrateV1";
const PERSIST_KEY="__fmpFundamentalBlobPersistV1";
const STORE='fmp-fundamentals-cache-v1.json';
...
async function hydratePersistentCache(){
  if(globalThis[HYDRATE_KEY])return globalThis[HYDRATE_KEY];
  globalThis[HYDRATE_KEY]=(async()=>{
    try{
      ...
      if(!result)return 0;
      const parsed=JSON.parse(await new Response(result.stream).text()),rows=Array.isArray(parsed?.rows)?parsed.rows:[],store=cache(),now=Date.now();let loaded=0;
      for(const item of rows){const symbol=normalizeSymbol(item?.symbol),ts=Number(item?.ts||0),data=item?.data;if(!symbol||!data||!ts||now-ts>=FUNDAMENTAL_STALE_MS)continue;const existing=store.get(symbol);if(!existing||Number(existing.ts||0)<ts){store.set(symbol,{ts,data});loaded++;}}
      return loaded;
    }catch(e){console.warn('fundamental cache hydrate failed:',e?.message||e);return 0;}
  })();
  return globalThis[HYDRATE_KEY];
}
async function persistFundamentalCache(){
  if(globalThis[PERSIST_KEY])return globalThis[PERSIST_KEY];
  globalThis[PERSIST_KEY]=(async()=>{
    try{
      const now=Date.now(),rows=[...cache().entries()].filter(([,x])=>x&&now-Number(x.ts||0)<FUNDAMENTAL_STALE_MS).slice(-500).map(([symbol,x])=>({symbol,ts:x.ts,data:x.data}));
      await put(STORE,JSON.stringify({version:1,updatedAt:new Date().toISOString(),rows}),{access:'private',allowOverwrite:true,addRandomSuffix:false,contentType:'application/json',cacheControlMaxAge:0});
      return true;
    }catch(e){console.warn('fundamental cache persist failed:',e?.message||e);return false;}
    finally{globalThis[PERSIST_KEY]=null;}
  })();
...
export async function fetchFmpFundamentals(symbols=[]){
  ...
  // Never let one broad screener rebuild fan out across the full universe. Cached rows
  // remain authoritative for a day; only a bounded slice of genuinely missing symbols
  // is refreshed on each invocation. Subsequent invocations naturally advance through
  // the remaining symbols because the completed slice is now cached persistently.
  const toFetch=missing.slice(0,MAX_NEW_SYMBOLS_PER_RUN),deferred=missing.slice(MAX_NEW_SYMBOLS_PER_RUN);
  if(toFetch.length){
    const fetched=await mapLimited(toFetch,1,s=>resilientOne(s,key,store,now));
    for(const row of fetched)result.set(row.symbol,row);
    void persistFundamentalCache();
  }
  for(const symbol of deferred)result.set(symbol,deferredRow(symbol));
  return result;
}
