...
export function simulatePointInTimePortfolio(dataset = {}, options = {}) {
  ...
  const config = {
    ...
    rankedRebalanceOffsetSessions: 0,
    ...
    riskBudgetPct: null,
    portfolioDrawdownStopPct: null,
    portfolioDrawdownCooldownSessions: 20,
  ...
  let portfolioHighWatermark = config.initialCapital;
  let portfolioRiskPausedThrough = -1;
  ...
  for (let sessionIndex = 0; sessionIndex < sessions.length; sessionIndex++) {
    ...
    if (active) {
      ...
      if (activeSessionNumber === portfolioRiskPausedThrough + 1)
        portfolioHighWatermark = equityValue(cash, positions, prices, "open");
      const openEquity = equityValue(cash, positions, prices, "open");
      portfolioHighWatermark = Math.max(portfolioHighWatermark, openEquity);
      const portfolioDrawdownPct =
        portfolioHighWatermark > 0
          ? ((openEquity - portfolioHighWatermark) / portfolioHighWatermark) * 100
          : 0;
      if (
        activeSessionNumber > portfolioRiskPausedThrough &&
        Number.isFinite(config.portfolioDrawdownStopPct) &&
        config.portfolioDrawdownStopPct > 0 &&
        portfolioDrawdownPct <= -config.portfolioDrawdownStopPct
      ) {
        for (const [symbol, position] of [...positions]) {
          const row = prices.get(symbol);
          ...
          executeSale({
            ...
            symbol,
            position,
            reason: "portfolio-drawdown-stop",
            shares: position.shares,
            fill: tradePrice(row.open, "sell", config.slippageBps),
          });
        }
        portfolioRiskPausedThrough =
          activeSessionNumber +
    ...
    if (active) {
      ...
      const effectiveRebalanceSessions = adaptiveRankContext
        ...
        : rankedRebalanceSessions;
      const rankedRebalanceOffsetSessions = Math.floor(
        number(config.rankedRebalanceOffsetSessions, 0),
      );
      const rankRebalance =
        rankedSelection &&
        ((activeSessionNumber - rankedRebalanceOffsetSessions) %
          effectiveRebalanceSessions +
          effectiveRebalanceSessions) %
          effectiveRebalanceSessions ===
          0;
  ...
  return {
    ...
    skippedOrders,
    pendingDecisions: config.liquidateAtEnd ? [] : JSON.parse(JSON.stringify(pending)),
