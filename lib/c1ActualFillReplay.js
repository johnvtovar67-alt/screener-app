...
export function createC1ActualFillReplay(input,seed,sessions) {
  ...
  return {
    has(date,symbol,side,reason){return date!==input.projectionDate&&Boolean(records.get(date)?.some(f=>!used.has(f.key)&&f.symbol===symbol&&f.side===side&&f.reason===reason));},
