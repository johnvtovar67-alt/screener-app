...
export async function fetchEventRiskMap(symbols=[]) {
  ...
  for(const symbol of clean){const current=map.get(symbol);if(current.blockNewCapital)continue;if(!earningsOk||!mergerOk){map.set(symbol,{...current,status:"Manual",label:"Pre-Trade: Manual Check",detail:"Automated event-risk check was incomplete. Manually verify earnings, M&A, and major news before execution.",checkComplete:false,earningsCheckComplete:earningsOk,mergerCheckComplete:mergerOk,manualCheckRequired:true});continue;}map.set(symbol,{...current,checkComplete:true,earningsCheckComplete:true,mergerCheckComplete:true});}
  return map;
}

export function applyEventRiskGate(stock={},eventRisk=null) {
  const risk=eventRisk||{status:"Manual",label:"Pre-Trade: Manual Check",detail:"Event-risk data was unavailable. Manually check earnings, M&A, and major news before execution.",blockNewCapital:false,reduceConviction:false,checkComplete:false,manualCheckRequired:true};
  ...
  if(risk.blockNewCapital&&downgraded)decisionWhy=`${risk.detail} Keep the setup on Watch until the event clears.`;
