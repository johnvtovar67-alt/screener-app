...
// Known definitive deals can carry portfolio-specific economics that a generic M&A feed does not expose.
// expectedCloseDate is an analysis estimate used only for opportunity-cost math, not a promised closing date.
const KNOWN_DEALS={
  IRDM:{
    acquirer:"Rocket Lab",acquirerSymbol:"RKLB",target:"Iridium Communications",
    announcedDate:"2026-06-28",expectedCloseDate:"2027-06-30",expectedCloseLabel:"mid-2027",
    referenceValue:54,cashPerShare:27,stockReferenceValue:27,
    collarLow:67.50,collarHigh:112.50,lowExchangeRatio:.4000,highExchangeRatio:.2400,
    structure:"$27 cash plus RKLB shares; the stock exchange ratio adjusts inside the $67.50-$112.50 RKLB collar.",
...
export function applyEventRiskGate(stock={},eventRisk=null) {
  ...
  else if(risk.blockNewCapital&&downgraded)decisionWhy=`${risk.detail} Keep the setup on Watch until the event clears or verification is completed.`;
