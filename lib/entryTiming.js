...
export function applyEntryTimingGate(stock={},timing=null){
  const supplied=timing||stock.entryTiming;if(!supplied)return stock;const requiredAsOf=latestCompletedMarketSessionDay(new Date()),staleAsOf=Boolean(supplied.available&&supplied.asOf&&requiredAsOf&&supplied.asOf<requiredAsOf),t=staleAsOf?{...supplied,available:false,pass:false,strongPass:false,status:'Historical timing verification stale',reason:`Daily timing is only verified through ${supplied.asOf}; the latest completed U.S. market session is ${requiredAsOf}. Fresh capital is paused until history catches up.`,staleVerification:true,requiredAsOf}:supplied;const rec=stock.recommendation&&typeof stock.recommendation==='object'?stock.recommendation:{},current=String(rec.displayLabel||rec.label||stock.action||'Avoid');let action=current;
  if(['Strong Buy','Buy'].includes(current)){if(!t.available||!t.pass)action='Watch';else if(current==='Strong Buy'&&!t.strongPass)action='Buy';}
  const downgraded=action!==current,blocked=!t.available||!t.pass,partialTiming=t.available&&t.pass&&!t.strongPass,suppressHigherTrigger=blocked||partialTiming,why=blocked?t.reason:partialTiming?t.reason:(rec.decisionWhy||rec.actionSummary||t.reason),e=rec.expertDecision&&typeof rec.expertDecision==='object'?rec.expertDecision:null,failures=e&&blocked?[why,...(Array.isArray(e.failures)?e.failures:[])]:e?.failures;
  const expertDecision=e&&blocked?{...e,action:'Watch',timing:'Wait for Reset',size:'None',decisionWhy:why,buyPass:false,strongBuyPass:false,partialBuyPass:false,fullBuyPass:false,failures,trendStatus:'Not Confirmed',capitalView:'Wait',historicalTimingPass:false,historicalTimingStrongPass:false}:e&&partialTiming?{...e,action:'Buy',timing:'Now',size:'Partial',decisionWhy:why,strongBuyPass:false,fullBuyPass:false,historicalTimingPass:true,historicalTimingStrongPass:false}:e?{...e,historicalTimingPass:Boolean(t.pass),historicalTimingStrongPass:Boolean(t.strongPass)}:e;
  ...
  const updatedPlan=suppressHigherTrigger&&oldPlan?{...oldPlan,addAbovePrice:null,summary:blocked?`No fresh entry. ${t.reason}`:'Partial-size entry only until short-term timing fully confirms.'}:oldPlan;
  const entryNote=blocked?`Do not initiate here. ${t.reason}`:partialTiming?'Timing supports only a partial entry. Do not add merely because price moves higher; require full short-term confirmation first.':rec.entryNote;
  const triggerNeeded=blocked?(t.chase?'Wait for a controlled pullback or consolidation, then require the timing gate to reconfirm.':'Wait for the short-term timing gate to reset and reconfirm before deploying capital.'):partialTiming?'Require full short-term timing confirmation before increasing size; a higher price alone is not a trigger.':rec.triggerNeeded;
