...
function factorWeaknessFlag(stock={}){
  const raw=[stock?.factorState,stock?.themeState,stock?.sectorState,stock?.marketCycleState,stock?.recommendation?.factorState,stock?.recommendation?.themeState].filter(Boolean).join(' ').toLowerCase();
  return stock?.factorWeakness===true||stock?.themeWeakness===true||stock?.sectorWeakness===true||/(weak|lag|deteriorat|risk-off|declin|bear)/.test(raw);
}
function exitDiagnosis({stock={},m={},held=null,opportunityCost=false}={}){
  if(opportunityCost||(held!==null&&held>=45))return 'OPPORTUNITY COST';
  const factorWeak=factorWeaknessFlag(stock),nameWeak=m.trade<58||m.technical<55||m.momentum<52||m.leadership<55||m.capital<62;
  if(factorWeak&&nameWeak)return 'BOTH — FACTOR + NAME WEAKNESS';
  if(factorWeak)return 'FACTOR WEAKNESS';
  return 'NAME / TRADE WEAKNESS';
}
...
export function reunderwriteExistingPosition({stock={},decision={},risk={},timeReview={}}={}){
  ...
  if(proofFailure){
    if(doubleDigitLoss&&!strongForward)return{override:true,action:'Exit',reason:exitReason(diagnosis(),`The trade has failed Proof — ${held} days in and down ${Math.abs(pnl).toFixed(1)}% without enough setup, momentum, leadership, and capital quality to justify more time.`),status:'Proof Failure Exit',exitDiagnosis:diagnosis(),pctSwing,crowdedFactor,crowdedPct,weightedCrowding,metrics:m,held,proofFailure:true};
  ...
  if(reunderwriteStage&&pnl<=-10&&weakForward)return{override:true,action:'Exit',reason:exitReason(diagnosis(),`${held} days in, this Swing is still down ${Math.abs(pnl).toFixed(1)}% and forward evidence is weak. The position has failed re-underwriting; recycle the capital to cash or a qualified opportunity.`),status:'Re-underwrite Exit',exitDiagnosis:diagnosis(),pctSwing,crowdedFactor,crowdedPct,weightedCrowding,metrics:m,held,opportunityCost:true};
  if(opportunityStage&&pnl<=-5&&weakForward)return{override:true,action:'Exit',reason:exitReason('OPPORTUNITY COST',`${held} days in, this Swing remains down ${Math.abs(pnl).toFixed(1)}% with weak forward evidence. Cash now has the stronger claim on this capital; no replacement hurdle is required to stop carrying dead money.`),status:'Opportunity Cost Exit',exitDiagnosis:'OPPORTUNITY COST',pctSwing,crowdedFactor,crowdedPct,weightedCrowding,metrics:m,held,opportunityCost:true};
  ...
  if(capitalReview){
    if((severeLoss||veryStale||matureReview)&&brokenForward)return{override:true,action:'Exit',reason:exitReason(diagnosis(),'The forward thesis/setup no longer earns the capital.'),status:'Exit',exitDiagnosis:diagnosis(),pctSwing,crowdedFactor,crowdedPct,weightedCrowding,metrics:m,held,opportunityCost:true};
