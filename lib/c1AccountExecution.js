...
export function continueC1ActualAccount({adoption,sessions,records,observedAt}={}) {
  ...
  for(let i=1;i<sessions.length;i++){
    ...
    const plan=planFromHistory({adoption,baseline:sessions[i-1],opening:{...session,...(record.entryRecheck?{entryRecheck:record.entryRecheck}:{})},observedAt:record.openingObservedAt,books,sessions:sessions.slice(0,i),replay,accountCompletionRules:completionRules(records.slice(0,i),adoption.sourceSessionDate)});
