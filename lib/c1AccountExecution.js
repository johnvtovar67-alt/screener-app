...
function planFromHistory({adoption,baseline,opening,observedAt,books,sessions,replay,accountCompletionRules=[]}) {
  ...
  const required=new Set(['SPY','QQQ']),runs={},orders=[],blockedOrders=[],stopPrices={},entryReviews=[];
  for(const id of ids){
  ...
  return {contract:'c1-actual-account-opening-plan-v1',status:'projection-only',sourceSessionDate:baseline.date,date:opening.date,observedAt:now.toISOString(),
    ...
    openingBooks:books,projectedBooks:projected,orders,blockedOrders,entryReviews,executable:false,providerVerified:false,
...
export function continueC1ActualAccount({adoption,sessions,records,observedAt}={}) {
  ...
  for(let i=1;i<sessions.length;i++){
    ...
    const plan=planFromHistory({adoption,baseline:sessions[i-1],opening:session,observedAt:record.openingObservedAt,books,sessions:sessions.slice(0,i),replay,accountCompletionRules:completionRules(records.slice(0,i),adoption.sourceSessionDate)});
