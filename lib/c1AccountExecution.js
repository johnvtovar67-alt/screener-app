...
export function planC1ActualAccountOpening({adoption,baseline,opening,observedAt}={}) {
  const books=accountBooks(adoption),now=new Date(observedAt),clock=easternMarketClock(now);
  if(!baseline||baseline.date!==adoption.sourceSessionDate||!opening||marketSessionDistance(baseline.date,opening.date)!==1||!clock?.isSessionDay||clock.key!==opening.date||clock.minutes<570)throw new Error('The next observed market opening is required');
  ...
  const required=new Set(['SPY','QQQ']),runs={},orders=[],blockedOrders=[];
  for(const id of ids){
    const seed=adoption.seeds[id],config={...C1_FROZEN_OPTIONS[id],startDate:baseline.date,endDate:baseline.date,liquidateAtEnd:false};
    const prior=simulatePointInTimePortfolio({sessions:[baseline]},config,seed);
    for(const p of seed.positions)required.add(p.symbol);
  ...
  for(const id of ids){
    ...
    const run=simulatePointInTimePortfolio({sessions:[baseline,session]},{...config,endDate:opening.date},seed);
    for(const [index,trade] of run.trades.entries()){
      if(trade.date!==opening.date)throw new Error('Account plan contains an unexpected execution date');
  ...
  return {contract:'c1-actual-account-opening-plan-v1',status:'projection-only',sourceSessionDate:baseline.date,date:opening.date,observedAt:now.toISOString(),
    openingBooks:books,projectedBooks:projected,orders,blockedOrders,executable:false,providerVerified:false,
    limitations:['Opening quotes and membership require server verification.','Actual fills replace estimated prices and quantities.','This first-opening adapter does not yet advance the strategy from later actual fills.']};
}
...
export function reconcileC1ActualAccountFills({plan,fills,observedAt}={}) {
  ...
  const outstanding=plan.orders.map(o=>({...o,shares:o.shares-(quantities.get(o.id)||0)})).filter(o=>o.shares>0);
