...
function withRecordingEvidence(plan,record){
  ...
  if(!evidence?.orders?.some(o=>o.reason===ownerExitReason))return plan;
  ...
  for(const order of evidence.orders.filter(o=>o.reason===ownerExitReason)){
    const held=plan.openingBooks[order.sleeve]?.positions[order.symbol]?.shares;
    if(order.side!=='sell'||order.recordingOnly!==true||!Number.isSafeInteger(order.shares)||order.shares<=0||order.shares!==held||idsSeen.has(order.id))throw new Error('Invalid recorded owner exit evidence');
...
function planFromHistory({adoption,baseline,opening,observedAt,books,sessions,replay,accountCompletionRules=[]}) {
