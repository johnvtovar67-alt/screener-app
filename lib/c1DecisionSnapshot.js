...
export function buildC1DecisionSnapshot({model,inputArchive,sourceSessionDate,now=new Date()}={}) {
  ...
  if(inputArchive?.sourcePointInTime!==true)
  ...
  return {contract:C1_DECISION_CONTRACT,
    decisionId:[C1_DECISION_CONTRACT,sourceSessionDate||'missing',inputArchive?.observedHash||'missing',model?.modelRevision??'missing'].join(':'),
    ...
    modelSource:'c1FrozenSimulator',universe:'current-production-compiler-paper-model',
