...
export async function saveC1AccountUpdate({store,path,saved,account,operation,context,book,now}={}){
 ...
 try{await store.write(path,account,saved?.etag);console.info('C1_ACCOUNT_SAVE',JSON.stringify({code:'SAVED'}));return account;}
 catch(error){
  ...
  if(operation==='refresh-analysis'){
   console.warn('C1_ACCOUNT_SAVE',JSON.stringify({code:'REFRESH_CONFLICT_RELOADED',sameVersion:latest.etag===saved.etag,weakVersion:typeof saved.etag==='string'&&saved.etag.startsWith('W/')}));
