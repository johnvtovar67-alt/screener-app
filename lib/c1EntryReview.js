...
const messages={
 'position-limit':['All three position slots are allocated to existing holdings and higher-ranked C1 buys.','A slot becomes available and this stock remains eligible.'],
...
export function c1WatchPresentation({rows=[],plan,ready=false,waitingReason}={}){
 const reviews=rows.map(row=>({row,review:row.screenReview|| (ready?c1EntryReviewText((plan?.entryReviews||[]).filter(r=>r.symbol===row.symbol)):{why:waitingReason||'Opening review unavailable.',next:'Refresh for the account opening review.'})}));
