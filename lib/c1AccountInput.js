...
// An index reconciliation proves model non-exposure, not private-account
// non-exposure. Never carry a revised basis across an actual holding or fill.
export function assertC1AccountRevisionCoverage(book, account, through) {
 ...
 for(const record of account.records)for(const fill of record.fills||[])owned.add(fill.symbol);
