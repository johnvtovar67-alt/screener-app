...
function assertReviewedAccountParity(book,account,audit){
 ...
 const state=source=>{
  const run=continueC1ActualAccount({adoption:account.adoption,sessions:source,historySessions:book.model.sessions,records,cashReconciliations:account.cashReconciliations,observedAt});
 ...
 const plan=source=>{
  const p=planC1ContinuedAccountOpening({adoption:account.adoption,sessions:source,historySessions:book.model.sessions,records,cashReconciliations:account.cashReconciliations,opening,observedAt,completionPolicy:c1CompletionPolicy(account.positionContext)});
