...
export async function connectStoredC1DatedInput(input, { holdings = [], now = new Date(), store = storage,
  path = c1DatedBookPath(input.universe), observations = [] } = {}) {
  for (let attempt = 0; attempt < 2; attempt++) {
    const saved = await store.read(path), next = acceptC1DatedInput(saved?.record || null, input, now, { observations });
    ...
    try { await store.write(path, next, saved?.etag); return view; }
      ...
      const accepted = acceptC1DatedInput(winner.record, input, now, { observations });
