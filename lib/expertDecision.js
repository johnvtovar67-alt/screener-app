...
// Exit evidence is grouped by family so correlated market/theme weakness is not double- or triple-counted.
...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  // Evidence family 1: market/technical condition. These are correlated and count only once.
  const marketTechnicalBroken=below200&&(technical<45||momentum<45||leadership<48||trade<42);
  // Evidence family 2: company/thesis deterioration.
  const thesisBroken=thesis<48;
  // Evidence family 3: abnormal risk/event deterioration.
  const riskBroken=risk>84;
  // Severe stock-specific collapse can stand on its own only at an extreme threshold.
  const severeTradeBreak=trade<30&&technical<35;
  const independentFamilyCount=[marketTechnicalBroken,thesisBroken,riskBroken].filter(Boolean).length;
  const swingExitConfirmed=severeTradeBreak||(marketTechnicalBroken&&(thesisBroken||riskBroken));
  ...
  if(role==='Core'){
    ...
    else if(below200&&thesis<38&&trade<38&&(riskBroken||technical<35)){action='Exit';reason='Core exit confirmed by a broken long-term structure plus severe thesis deterioration and an additional independent failure.';}
    ...
    else if(swingExitConfirmed){action='Exit';reason=severeTradeBreak?'Exit confirmed by an extreme stock-specific trade/technical collapse.':'Exit confirmed because market/technical weakness is accompanied by an independent thesis or abnormal-risk failure.';}
  ...
  return{action,role,capitalPriority,expertDecision:expert,reason,exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',marketTechnicalBroken,thesisBroken,riskBroken,severeTradeBreak,independentFamilyCount}};
}
