...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const technical=n(expert.metrics.technical,50),leadership=n(expert.metrics.leadership,50),momentum=n(expert.metrics.momentum,50),risk=n(expert.metrics.risk,50);
  const deployable=expert.action==='Strong Buy'||expert.action==='Buy';
  // Independent deterioration signals. A low composite score alone cannot force an Exit.
  const structureBroken=below200;
  const technicalBroken=technical<42;
  const leadershipBroken=leadership<45;
  const momentumBroken=momentum<42;
  const thesisBroken=thesis<50;
  const riskBroken=risk>82;
  const severeTradeBreak=trade<35;
  const secondaryBreaks=[technicalBroken,leadershipBroken,momentumBroken,thesisBroken,riskBroken].filter(Boolean).length;
  const swingExitConfirmed=(structureBroken&&trade<45&&secondaryBreaks>=2)||(severeTradeBreak&&secondaryBreaks>=3);
  ...
  if(role==='Core'){
    ...
    else if(below200&&thesis<40&&trade<40&&secondaryBreaks>=2){action='Exit';reason='Multiple independent failures confirm that both the long-term thesis and price structure are materially broken.';}
    ...
    else if(swingExitConfirmed){action='Exit';reason=structureBroken?'Exit confirmed by a broken long-term trend plus multiple independent deterioration signals.':'Exit confirmed by severe trade-quality deterioration plus multiple independent weakness signals.';}
    else if(below200){reason='The swing is below the long-term trend, but multiple independent failures do not yet confirm an exit.';}
    else if(severeTradeBreak){reason='Trade quality is severely weak, but the independent confirmation required for an exit is not yet present.';}
  ...
  return{action,role,capitalPriority,expertDecision:expert,reason,exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',structureBroken,severeTradeBreak,secondaryBreaks,technicalBroken,leadershipBroken,momentumBroken,thesisBroken,riskBroken}};
}
