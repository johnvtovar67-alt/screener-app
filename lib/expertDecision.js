...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const deployable=(authoritativeAction==='Strong Buy'||authoritativeAction==='Buy')&&!eventBlocksNewCapital;
  ...
  if(role==='Core'){
    if(eventBlocksNewCapital&&(authoritativeAction==='Strong Buy'||authoritativeAction==='Buy')){reason=`Hold; no new capital while event risk is unresolved. ${eventRisk.detail||'Wait for the event to clear before adding.'}`;}
