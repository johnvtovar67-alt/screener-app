...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const capitalPriority=action==='Add'?'New capital justified':action==='Exit'?'Redeploy':action==='Rotate'?`Rotation candidate vs ${rotateTarget}`:action==='Trim'?'Exceptional risk reduction':eventBlocksNewCapital?'Event hold':'No new capital';
  return{action,role,capitalPriority,expertDecision:expert,reason,eventRisk:{blocked:eventBlocksNewCapital,status:eventRisk?.status||'',label:eventRisk?.label||''},rotationReview:{candidate:rotationCandidate,opportunityGap,rotateTarget,staleSwing,fundingIndependent:true},exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',marketTechnicalBroken,thesisBroken,riskBroken,severeTradeBreak,activeDeterioration,stabilizationPending,stabilizationSignals}};
}
