...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const technical=n(expert.metrics.technical,50),leadership=n(expert.metrics.leadership,50),momentum=n(expert.metrics.momentum,50),entry=n(expert.metrics.entry,50),risk=n(expert.metrics.risk,50),day=n(expert.metrics.day,0);
  ...
  const thesisBroken=thesis<48,riskBroken=risk>84,severeTradeBreak=trade<30&&technical<35;
  ...
  const swingExitConfirmed=(severeTradeBreak||(marketTechnicalBroken&&(thesisBroken||riskBroken)))&&!stabilizationPending;
  ...
  if(role==='Core'){
    ...
    else if(below200&&thesis<38&&trade<38&&(riskBroken||technical<35)&&!stabilizationPending){action='Exit';reason='Core exit confirmed by a broken long-term structure plus severe thesis deterioration and an additional independent failure.';}
  ...
  const capitalPriority=action==='Add'?'New capital justified':action==='Exit'?'Redeploy':action==='Trim'?'Exceptional risk reduction':eventBlocksNewCapital?'Event hold':'No new capital';
  return{action,role,capitalPriority,expertDecision:expert,reason,eventRisk:{blocked:eventBlocksNewCapital,status:eventRisk?.status||'',label:eventRisk?.label||''},exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',marketTechnicalBroken,thesisBroken,riskBroken,severeTradeBreak,independentFamilyCount,stabilizationPending,stabilizationSignals}};
}
