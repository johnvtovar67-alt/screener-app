...
export function expertGates(stock={},recommendation={}){
  ...
  // Preserve the deployment logic that existed before the UI simplification.
  ...
  if(leadership<56)failures.push('Relative strength is below the minimum deployment standard.');
  if(technical<52)failures.push('Technical confirmation is below the minimum deployment standard.');

  let action='Avoid';
  if(strongBuyPass)action='Strong Buy';
  else if(buyPass)action='Buy';
  else if(raw>=52||leadership>=58||technical>=55||capitalScore>=60)action='Watch';
  ...
  return{action,timing,size,decisionWhy,buyPass,strongBuyPass,partialBuyPass,fullBuyPass,failures,
    ...
    capitalView:action==='Strong Buy'||action==='Buy'?'Deploy':action==='Watch'?'Wait':'Redeploy Elsewhere',
...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  if(roleKey==='core'){
    ...
    if(deployable&&weightPct<6){action='Add';reason='Small position has earned an upgrade; add only toward a partial position.';}
    else if(expert.action==='Avoid'&&trade<50){action='Exit';reason='Small position failed before earning a larger allocation.';}
    else if(expert.action==='Watch'&&trade<55){action='Hold';reason='Position has not confirmed; keep it small and do not add.';}
    else reason='Keep the position small until the setup earns an upgrade.';
  }else{
