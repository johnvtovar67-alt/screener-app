...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  // An Exit requires weakness that is still actively deteriorating. Low scores by themselves can describe damage that already happened.
  ...
  // Dead-money is different from invalidation. A viable but weak swing can be a rotation candidate when much stronger fresh-capital setups exist.
  ...
  const rotationCandidate=staleSwing&&opportunityGap>=20;
  ...
  if(role==='Core'){
    ...
    else if(rotationCandidate){action='Review';reason=`The swing remains viable, but its setup is weak and a materially stronger fresh-capital opportunity exists${opportunityGap?` (about a ${Math.round(opportunityGap)}-point advantage)`:''}. Review for rotation rather than treating the position as an automatic hold.`;}
  ...
  const capitalPriority=action==='Add'?'New capital justified':action==='Exit'?'Redeploy':action==='Review'?'Rotation review':action==='Trim'?'Exceptional risk reduction':eventBlocksNewCapital?'Event hold':'No new capital';
  return{action,role,capitalPriority,expertDecision:expert,reason,eventRisk:{blocked:eventBlocksNewCapital,status:eventRisk?.status||'',label:eventRisk?.label||''},rotationReview:{candidate:rotationCandidate,opportunityGap,staleSwing},exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',marketTechnicalBroken,thesisBroken,riskBroken,severeTradeBreak,activeDeterioration,stabilizationPending,stabilizationSignals}};
}
