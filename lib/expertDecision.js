...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const eventBlocksNewCapital=Boolean(eventRisk?.blockNewCapital)||String(eventRisk?.status||'').toLowerCase()==='blocked'||String(eventRisk?.status||'').toLowerCase()==='manual';
  const broadAction=String(stock?.finalDecision?.action||'');
  const broadAllowsNewCapital=!broadAction||broadAction==='Strong Buy'||broadAction==='Buy';
  const deployable=(expert.action==='Strong Buy'||expert.action==='Buy')&&!eventBlocksNewCapital&&broadAllowsNewCapital;
  ...
  // A rotation forces a sale, so demand a materially larger relative edge than an ordinary Buy.
  // This reduces whipsaw from borderline Buy candidates that can fall back to Watch on the next refresh.
  const rotationCandidate=staleSwing&&opportunityGap>=30&&Boolean(rotateTarget);
  ...
  if(role==='Core'){
    ...
    else if(!broadAllowsNewCapital&&(expert.action==='Strong Buy'||expert.action==='Buy')){reason='Standalone setup remains Buy-quality, but the current broad capital ranking does not justify adding while stronger opportunities rank ahead.';}
    ...
    else if(rotationCandidate){action='Rotate';reason=`The swing remains viable, but ${rotateTarget} has a strong enough relative edge to justify an actual rotation. The higher rotation hurdle is designed to avoid selling for borderline Buy candidates.`;}
