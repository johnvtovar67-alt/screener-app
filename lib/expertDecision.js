...
export function expertGates(stock={},recommendation={}){
  ...
  const fullBuyPass=quote.pass&&trendPass&&volumePass&&participationPass&&rrPass&&!chase&&technical>=66&&leadership>=68&&momentum>=56&&entry>=58&&risk<=75&&raw>=74&&trade>=78&&capitalScore>=72;
  const strongBuyPass=fullBuyPass&&trade>=85&&thesis>=75&&capitalScore>=80&&risk<=65;
  const partialBuyPass=quote.pass&&trendPass&&volumePass&&participationPass&&rrPass&&!chase&&raw>=70&&technical>=58&&leadership>=62&&momentum>=54&&entry>=52&&risk<=78&&trade>=74&&capitalScore>=70;
...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const deployable=(authoritativeAction==='Strong Buy'||authoritativeAction==='Buy')&&!eventBlocksNewCapital&&!quoteBlocksNewCapital;
  const deploymentReason=stock?.finalDecision?.reason||expert.decisionWhy;
  const freshCapitalTargetPct=authoritativeAction==='Strong Buy'?12:authoritativeAction==='Buy'?7.5:0;
  ...
  // A rotation forces a sale, so require both a larger capital edge and a destination with a safety margin above an ordinary Buy.
  ...
  if(role==='Core'){
    if((eventBlocksNewCapital||quoteBlocksNewCapital)&&(authoritativeAction==='Strong Buy'||authoritativeAction==='Buy')){reason=quoteBlocksNewCapital?'Hold; live quote freshness is insufficient for a new-capital decision. Refresh before adding.':`Hold; no new capital while event risk is unresolved. ${eventRisk.detail||'Wait for the event to clear before adding.'}`;}
  ...
  const capitalPriority=action==='Add'?'New capital justified':action==='Exit'?'Redeploy':action==='Rotate'?`Rotation candidate vs ${rotateTarget}`:action==='Trim'?(profitProtection?'Bank profits':'Exceptional risk reduction'):eventBlocksNewCapital?'Event hold':quoteBlocksNewCapital?'Stale quote hold':'No new capital';
  return{action,role,capitalPriority,expertDecision:expert,authoritativeAction,freshCapitalTargetPct,reason,eventRisk:{blocked:eventBlocksNewCapital,status:eventRisk?.status||'',label:eventRisk?.label||''},profitProtection:{triggered:profitProtection,pnlPct,extremeFroth,moderateFroth,winnerFading,extension,vs50,day},rotationReview:{candidate:rotationCandidate,opportunityGap,rotateTarget,staleSwing,rotationTargetEligible,fundingIndependent:true,minimumEdge:30},exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',marketTechnicalBroken,thesisBroken,riskBroken,severeTradeBreak,activeDeterioration,stabilizationPending,stabilizationSignals}};
}
