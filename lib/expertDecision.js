...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  if(role==='Core'){
    ...
    else if(rotationCandidate){action='Rotate';reason=`The swing remains viable, but ${rotateTarget} is a current Buy with about a ${Math.round(opportunityGap)}-point forward capital advantage. Rotate rather than let a weak swing become dead money.`;}
  ...
  const capitalPriority=action==='Add'?'New capital justified':action==='Exit'?'Redeploy':action==='Rotate'?`Rotate to ${rotateTarget}`:action==='Trim'?'Exceptional risk reduction':eventBlocksNewCapital?'Event hold':'No new capital';
  return{action,role,capitalPriority,expertDecision:expert,reason,eventRisk:{blocked:eventBlocksNewCapital,status:eventRisk?.status||'',label:eventRisk?.label||''},rotationReview:{candidate:rotationCandidate,opportunityGap,rotateTarget,staleSwing},exitConfirmation:{confirmed:role==='Swing'?swingExitConfirmed:action==='Exit',marketTechnicalBroken,thesisBroken,riskBroken,severeTradeBreak,activeDeterioration,stabilizationPending,stabilizationSignals}};
}
