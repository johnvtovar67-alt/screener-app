...
export function portfolioDecision({stock={},recommendation={},position={},now=new Date()}={}){
  ...
  const policyTargetPct=n(stock?.productionPolicy?.targetWeightPct,0);
  const freshCapitalTargetPct=stock?.productionPolicy?.selected&&policyTargetPct>0?policyTargetPct:authoritativeAction==='Strong Buy'?9:authoritativeAction==='Buy'?6:0;
