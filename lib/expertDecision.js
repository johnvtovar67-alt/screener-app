...
export function expertGates(stock={},recommendation={}){
  ...
  return{action,timing,size,decisionWhy,buyPass,strongBuyPass,partialBuyPass,fullBuyPass,failures,expertOverride:raw>=74&&!buyPass,expertOverrideReason:raw>=74&&!buyPass?(failures[0]||'A hard deployment gate blocks the raw Buy signal.'):'',trendStatus:trendPass?'Confirmed':'Not Confirmed',volumeStatus:participationPass?'Confirmed':'Not Confirmed',rewardRiskStatus:rrPass?'Pass':'Fail',capitalView:action==='Strong Buy'||action==='Buy'?'Deploy':action==='Watch'?'Wait':'Redeploy Elsewhere',thesisScore:thesis,tradeSetupScore:trade,capitalScore,metrics:{vs50,rawRelativeVolume:rawRv,relativeVolume:rv,marketSessionProgress:marketSessionProgress(),technical,leadership,momentum,entry,risk,extension,payoffRatio:rr,below50,below200,day,quoteFreshnessPass:quote.pass,quoteAgeMinutes:quote.ageMinutes,quoteFreshnessStatus:quote.status,fundamentalsPass,fundamentalDataStatus:fundamentalStatus}};
}
...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  const capitalPriority=action==='Add'?'New capital justified':action==='Exit'?'Redeploy':action==='Rotate'?`Rotation candidate vs ${rotateTarget}`:action==='Trim'?(profitProtection?'Bank profits':'Exceptional risk reduction'):eventBlocksNewCapital?'Event hold':fundamentalsBlockNewCapital?'Fundamental data hold':quoteBlocksNewCapital?'Stale quote hold':'No new capital';
