...
// FMP quote volume is today's accumulated volume while avgVolume is a full-day average.
// Comparing the two directly near the open makes normal early-session volume look weak.
// Convert raw relative volume into a pace-adjusted ratio during regular US market hours.
function marketSessionProgress(now=new Date()){
  try{
    ...
    if(mins<start)return null; // Premarket: do not use incomplete volume as a hard veto.
    if(mins>=end)return 1;
    return clamp((mins-start)/(end-start),0.01,1);
  }catch{return 1;}
}
function expectedVolumeFraction(progress){
  if(progress===null)return null;
  if(progress>=1)return 1;
  // Mildly front-load expected volume to reflect the normal U-shaped session profile.
  return clamp(.8*progress+.2*Math.sqrt(progress),.04,1);
}
function pacedRelativeVolume(stock={}){
  const v=volume(stock),avg=avgVolume(stock);
  if(v<=0||avg<=0)return null;
  const raw=v/avg,progress=marketSessionProgress();
  const expected=expectedVolumeFraction(progress);
...
export function expertGates(stock={},recommendation={}){
  ...
  // Strong Buy remains rare. Buy must be good enough to deploy capital today, not merely interesting.
