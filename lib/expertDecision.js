...
export function expertGates(stock={},recommendation={}){
  ...
  // Forward-entry discipline: leadership is not enough. Fresh capital should not be
  // deployed after a large move has already consumed much of the near-term asymmetry.
  // This intentionally uses multi-week trend distance (vs50/vs200) as well as today's move.
  ...
  const fullBuyPass=fundamentalsPass&&quote.pass&&trendPass&&volumePass&&participationPass&&rrPass&&forwardAsymmetryPass&&!lateTrend&&!chase&&technical>=68&&leadership>=68&&momentum>=56&&entry>=60&&risk<=72&&raw>=75&&trade>=79&&capitalScore>=74;

  // Strong Buy remains deliberately hard to earn. It now also requires a clean forward
  // entry: exceptional current quality is not enough if the move is already mature.
  ...
  const strongSafetyPass=fundamentalsPass&&quote.pass&&trendPass&&volumePass&&participationPass&&!lateTrend&&!chase&&strongForwardAsymmetryPass;
  ...
  const partialBuyPass=fundamentalsPass&&quote.pass&&trendPass&&volumePass&&participationPass&&rrPass&&!lateTrend&&!chase&&raw>=71&&technical>=60&&leadership>=62&&momentum>=54&&entry>=55&&risk<=76&&trade>=75&&capitalScore>=71;
  ...
  return{action,timing,size,decisionWhy,buyPass,strongBuyPass,partialBuyPass,fullBuyPass,failures,expertOverride:raw>=74&&!buyPass,expertOverrideReason:raw>=74&&!buyPass?(failures[0]||'A hard deployment gate blocks the raw Buy signal.'):'',trendStatus:trendPass?'Confirmed':'Not Confirmed',volumeStatus:participationPass?'Confirmed':'Not Confirmed',rewardRiskStatus:rrPass?'Pass':'Fail',capitalView:action==='Strong Buy'||action==='Buy'?'Deploy':action==='Watch'?'Wait':'Redeploy Elsewhere',thesisScore:thesis,tradeSetupScore:trade,capitalScore,strongQualityScore,metrics:{vs50,vs200,rawRelativeVolume:rawRv,relativeVolume:rv,marketSessionProgress:marketSessionProgress(),technical,leadership,momentum,entry,risk,extension,payoffRatio:rr,strongRewardRiskQuality,below50,below200,day,lateTrend,severeLateTrend,forwardAsymmetryPass,strongForwardAsymmetryPass,quoteFreshnessPass:quote.pass,quoteAgeMinutes:quote.ageMinutes,quoteFreshnessStatus:quote.status,fundamentalsPass,fundamentalDataStatus:fundamentalStatus}};
}
...
export function portfolioDecision({stock={},recommendation={},position={}}={}){
  ...
  // A trading position does not need a replacement before it can be sold. If the position
  // itself has broad forward-quality failure, cash is a valid destination. The bar is still
  // intentionally high enough that one bad day or one soft metric cannot trigger an exit.
  const forwardCapitalFailure=role==='Swing'&&!deployable&&!eventBlocksNewCapital&&!quoteBlocksNewCapital&&!fundamentalsBlockNewCapital&&
    ['Watch','Avoid'].includes(authoritativeAction)&&trade<48&&technical<48&&momentum<47&&entry<48&&leadership<52&&thesis<62&&risk>=58;
  const staleCapitalExit=forwardCapitalFailure&&!stabilizationPending&&(below50||day<0||pnlPct<=5);
  ...
  // Normal rotation should be rare: the current holding must be stale and the replacement
  // must clear a very large capital-quality gap. Otherwise hold or exit independently.
  ...
  if(merger.active){
    ...
    else if(profitProtection){action='Trim';reason=highFroth?'This has been a successful trade and the position is now materially extended. Bank part of the gain while keeping meaningful exposure.':winnerFading?'This has been a successful trade, but momentum/setup quality is fading. Bank part of the gain rather than waiting for a full breakdown.':'This winner is becoming stretched after a large gain. Take partial profits while allowing the remaining position to keep working.';}
    ...
    else if(staleCapitalExit){action='Exit';reason='This swing no longer earns trading capital on its own merits: forward setup, momentum, entry quality and relative strength are broadly weak. Exit to cash rather than waiting indefinitely for a replacement.';}
