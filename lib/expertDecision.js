...
export function expertGates(stock={},recommendation={}){
  ...
  // Capital score is the holistic fresh-money decision score. Thesis matters, but execution/setup gets more weight.
...
export function portfolioDecision({stock={},recommendation={},position={},bestOpportunity=null}={}){
  ...
  const candidateScore=n(bestOpportunity?.capitalScore??bestOpportunity?.tradeSetupScore??bestOpportunity?.score,0);
  const currentScore=expert.capitalScore;
  ...
  let action='Hold';
  if(core){if(deployable&&weightPct<35)action='Add';else if(weightPct>45)action='Trim';}
  ...
  const reason=action==='Exit'&&replacementAdvantage>=18?`A stronger available setup has about a ${Math.round(replacementAdvantage)}-point capital-allocation advantage.`:expert.failures[0]||(action==='Add'?expert.decisionWhy:core?'Core holding remains acceptable; additions still require confirmation.':'The position still earns a place, but is not a priority for fresh money.');
