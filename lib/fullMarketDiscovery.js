...
export async function refreshFullMarketDiscovery({ apiKey, now = Date.now() } = {}) {
  ...
  const [quoteRows, trailingLiquidity] = await Promise.all([
    fetchDiscoveryQuotes(coarseSymbols, key, config.quoteConcurrency),
    fetchTrailingLiquidity(coarseSymbols, key, now),
  ]);
  ...
  const snapshot = {
    ...
    provider: "FMP stable company-screener + batch-quote + EOD bulk",
    ...
    maxProviderCalls:
      3 +
      Math.ceil(config.maxQuoteUniverse / QUOTE_BATCH_SIZE) +
