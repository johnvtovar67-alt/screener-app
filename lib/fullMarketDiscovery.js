...
export const FULL_MARKET_DISCOVERY_STORE =
  "full-market-discovery-snapshot-v1.json";
export const FULL_MARKET_DISCOVERY_SCHEMA = 1;
...
const normalizeSymbol = (value) =>
  String(value || "")
    ...
    .toUpperCase()
    .trim();
const toFmpSymbol = (value) => normalizeSymbol(value).replace(".", "-");
...
const chunks = (items, size = QUOTE_BATCH_SIZE) => {
  const output = [];
...
export function fullMarketDiscoveryConfig(env = process.env) {
  return {
    ...
    quoteConcurrency: Math.floor(
      boundedNumber(env.FULL_MARKET_QUOTE_CONCURRENCY, 4, 1, 6),
    ),
  };
}
...
async function fetchDiscoveryQuotes(symbols, apiKey, concurrency) {
  const batches = chunks(symbols);
  const output = [];
  let next = 0;
  ...
  async function worker() {
    while (!terminalError) {
      ...
      const params = new URLSearchParams({
        symbols: batches[index].map(toFmpSymbol).join(","),
        apikey: apiKey,
      });
      try {
        output.push(
          ...asArray(
            await fetchJson(
              `https://financialmodelingprep.com/stable/batch-quote?${params}`,
            ),
          ),
        );
      } catch (error) {
        if ([402, 403, 404, 429].includes(error?.status)) {
          terminalError = error;
          return;
        }
        // A single transient batch failure lowers coverage; it does not fan out
        // into per-symbol retries.
        failedBatches++;
      }
    }
  }
  ...
  if (failedBatches)
    throw new Error(
      `${failedBatches} FMP discovery quote batch${failedBatches === 1 ? "" : "es"} failed; refusing to persist partial breadth`,
    );
  return output;
}
...
export async function refreshFullMarketDiscovery({ apiKey, now = Date.now() } = {}) {
  ...
  const quoteRows = await fetchDiscoveryQuotes(
    coarseRows.map((row) => row.symbol),
    key,
    config.quoteConcurrency,
  );
  ...
  const merged = coarseRows
    ...
    .map((row) => ({ ...row, ...quoteMap.get(row.symbol) }));
  ...
  const snapshot = {
    ...
    provider: "FMP stable company-screener + batch-quote",
    ...
    maxProviderCalls:
      3 + Math.ceil(config.maxQuoteUniverse / QUOTE_BATCH_SIZE),
    config,
    candidates,
  };
...
export async function getFullMarketDiscovery({
  ...
  if (snapshot && ageMs <= maxAgeMs)
    return { ...snapshot, status: "ready", stale: false, ageMs };
  if (!refreshIfStale)
    return snapshot
  ...
  if (now < Number(globalThis[COOLDOWN_KEY] || 0))
    return snapshot && ageMs <= staleAgeMs
      ? { ...snapshot, status: "stale", stale: true, ageMs }
      : { status: "unavailable", candidates: [], config, builtAt: null };
  globalThis[INFLIGHT_KEY] = (async () => {
    try {
      return await refreshFullMarketDiscovery({ now });
    } catch (error) {
      console.warn("full-market discovery refresh failed:", error?.message || error);
      ...
      return snapshot && ageMs <= staleAgeMs
        ? {
            ...
            stale: true,
            ageMs,
            refreshError: error?.message || "Discovery refresh failed",
          }
        : {
            status: "unavailable",
            candidates: [],
