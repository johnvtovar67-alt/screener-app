...
export async function refreshFullMarketDiscovery({ apiKey, now = Date.now() } = {}) {
  ...
  let quoteRows;
  try {
    // Exchange-wide quotes expose the provider's average-volume field for the
    // broad discovery floor. Exact trailing-20-session dollar liquidity is
  ...
  const liquidityCoveragePct =
    Math.round(
      (merged.filter(
        (row) =>
          number(row.avgVolume) > 0 && number(row.averageDollarVolume) > 0,
      )
        .length /
  ...
  if (liquidityCoveragePct < 95)
    throw new Error(
      `FMP average-volume coverage was ${liquidityCoveragePct}%; refusing to replace the prior complete snapshot`,
    );
  ...
  const snapshot = {
    ...
    liquiditySource: "provider_average_volume",
