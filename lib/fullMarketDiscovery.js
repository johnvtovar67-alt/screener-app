...
export function passesDiscoveryLiquidity(row = {}, config = {}) {
  ...
  const currentDollarVolume = number(
    row.liquidityProxyDollarVolume ??
      row.currentDollarVolume ??
      (price !== null && number(row.volume) !== null
        ? price * number(row.volume)
        : null),
  );
  // This function selects research candidates; it does not authorize capital.
  // When the exchange-wide payload omits avgVolume, a deliberately permissive
  // completed-session proxy prevents normally liquid stocks from disappearing
  // after one quiet day. The exact $10m trailing-20-session gate remains mandatory
  // in entryTiming before Buy/Strong Buy deployment.
  const liquidityPass =
    avgDollarVolume !== null
      ? avgDollarVolume >= config.minAvgDollarVolume
      : currentDollarVolume !== null &&
        currentDollarVolume >= config.minAvgDollarVolume * 0.25;
  return Boolean(
    price !== null &&
...
export async function refreshFullMarketDiscovery({ apiKey, now = Date.now() } = {}) {
  ...
  let quoteRows;
  try {
    // Exchange-wide quotes provide a completed-session dollar-volume proxy for
    // the non-authorizing breadth pass. Exact trailing-20-session dollar liquidity
    // is independently recomputed from symbol history before any Buy can deploy.
  ...
  const merged = coarseRows
    ...
    .map((row) => ({
      ...
      liquidityProxyDollarVolume:
        number(quoteMap.get(row.symbol)?.averageDollarVolume) ??
        number(quoteMap.get(row.symbol)?.currentDollarVolume) ??
        number(row.currentDollarVolume),
    }));
  ...
  if (liquidityCoveragePct < 95)
    throw validationError(
      `FMP completed-session dollar-volume coverage was ${liquidityCoveragePct}%; refusing to replace the prior complete snapshot`,
    );
  ...
  const snapshot = {
    ...
    liquiditySource: "latest_completed_session_proxy",
    liquidityProxyFloor: config.minAvgDollarVolume * 0.25,
