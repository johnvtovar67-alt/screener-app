...
function normalizeScreenerRow(row = {}) {
  ...
  return {
    ...
    cik: String(row.cik || row.cikNumber || "").trim(),
...
export function mergeDiscoveryRow(screenerRow = {}, quoteRow = {}) {
  // A secondary endpoint is allowed to enrich the screener row, never erase it.
  // FMP exchange quotes omit or null selected fields outside market hours; a raw
  // object spread would overwrite verified market cap/volume with null and make
  // the whole discovered universe disappear.
  const usableQuote = Object.fromEntries(
    Object.entries(quoteRow).filter(
      ([, value]) => value !== null && value !== undefined && value !== "",
    ),
  );
  const merged = { ...screenerRow, ...usableQuote };
  ...
  return {
    ...
    currentDollarVolume:
      price !== null && volume !== null
        ? price * volume
        : number(screenerRow.currentDollarVolume),
    liquidityProxyDollarVolume:
      number(usableQuote.averageDollarVolume) ??
      (price !== null && volume !== null ? price * volume : null) ??
      number(screenerRow.currentDollarVolume),
  };
}

export function passesDiscoveryResearchFloor(row = {}, config = {}) {
  const price = number(row.price);
  const marketCap = number(row.marketCap);
  // This is only a breadth/research admission floor. FMP's exchange-wide volume
  // fields can be zero after the market-day rollover, so discovery must not claim
  // that they verify liquidity. Exact trailing-20-session dollar liquidity is a
  // separate hard gate in entryTiming before any Buy/Strong Buy can deploy.
  return Boolean(
    price !== null &&
...
// Backward-compatible export for research utilities that used the older name.
// It no longer implies that breadth admission authorizes fresh capital.
export function passesDiscoveryLiquidity(row = {}, config = {}) {
  return passesDiscoveryResearchFloor(row, config);
}
...
export function fullMarketDiscoveryScore(row = {}) {
  ...
  const avgDollarVolume = number(
    row.averageDollarVolume ??
      row.liquidityProxyDollarVolume ??
      row.currentDollarVolume,
    0,
  );
...
export function selectFullMarketCandidates(rows = [], options = {}) {
  ...
  const eligible = rows
    .filter((row) => passesDiscoveryResearchFloor(row, config))
    .map((row) => ({
      ...row,
...
async function fetchJson(url, { timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      const providerText = String(await response.text().catch(() => ""))
        .replace(/apikey=[^&\s]+/gi, "apikey=[redacted]")
        .slice(0, 240);
      const error = new Error(
        `FMP discovery request failed: ${response.status}${providerText ? ` - ${providerText}` : ""}`,
      );
...
async function readDurableSnapshot() {
  ...
  const blob = blobs.find(
    (item) => item.pathname === FULL_MARKET_DISCOVERY_STORE,
  );
  if (!blob) return null;
...
export async function refreshFullMarketDiscovery({ apiKey, now = Date.now() } = {}) {
  ...
  const validationError = (message) => {
    ...
    error.discoveryStage = "coverage_validation";
  ...
  let quoteRows;
  try {
    // Exchange-wide quotes provide price, market-cap and technical fields for the
    // non-authorizing breadth pass. Exact trailing-20-session dollar liquidity is
    // independently recomputed from symbol history before any Buy can deploy.
    // Do not bootstrap with 20 date-partitioned EOD bulk calls: FMP explicitly
    // rate-limits that high-bandwidth endpoint to infrequent refreshes.
    quoteRows = await fetchDiscoveryQuotes(key);
  } catch (error) {
    if (!error.discoveryStage) error.discoveryStage = "exchange_quotes";
    error.discoveryContext = {
      sourceUniverseSize: screenerRows.length,
      sourceExchangeCounts: screener.exchangeCounts,
      sourceUniverseCapped: screener.sourceUniverseCapped,
  ...
  const merged = coarseRows
    ...
    .map((row) => mergeDiscoveryRow(row, quoteMap.get(row.symbol)));
  ...
  const liquidityCoveragePct =
    Math.round(
      (merged.filter(
        (row) =>
          number(row.liquidityProxyDollarVolume) > 0,
      )
        .length /
  ...
  const eligibleRows = merged.filter((row) =>
    passesDiscoveryResearchFloor(row, config),
  );
  const eligibleSymbols = eligibleRows.map((row) => row.symbol);
  if (!eligibleSymbols.length)
    throw validationError(
      "FMP full-market discovery produced no research-eligible common stocks; refusing to persist an empty snapshot",
    );
  ...
  const snapshot = {
    ...
    provider: "FMP stable company-screener + exchange-wide quotes",
    sourceUniverseSize: screenerRows.length,
    sourceExchangeCounts: screener.exchangeCounts,
    sourceUniverseCapped: screener.sourceUniverseCapped,
    ...
    liquidityProxyFloor: null,
    liquidityLookbackSessions: null,
    liquidityAsOf: builtAt,
    eligibleUniverseSize: eligibleSymbols.length,
    eligibleSymbols,
    researchUniverse: eligibleRows.map((row) => ({
      symbol: row.symbol,
