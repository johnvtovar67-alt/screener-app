...
export function reconcileC1VerifiedProviderRevision(prior,input,now,digest,advance,observations=[]) {
 ...
 const bySymbol=new Map(evidence.map(r=>[r.symbol,r])),proposedPrices=[],mismatches=[];
 for(const price of previous.prices){
  ...
  if(!row){proposedPrices.push({...price});continue;}
 ...
 if(!confirmations.size)reject('confirmation',mismatches.length);
 const confirmedSymbols=new Set(confirmations.keys());
 const appliedMismatches=mismatches.filter(row=>confirmedSymbols.has(row.symbol));
 const correctedPrices=previous.prices.map(price=>confirmedSymbols.has(price.symbol)?
  proposedPrices.find(row=>row.symbol===price.symbol):{...price});
 ...
 return {model:original,audit:{contract:'c1-provider-revision-reconciliation-v1',priorRecordHash:recordHash,
  ...
  mismatches:appliedMismatches,correctedPrices,deferredRevisionCount:mismatches.length-appliedMismatches.length,
  privateAccountTreatment:'preserved-recorded-basis',originalInputsPreserved:true,identicalModelEconomics:true,
