...
export function evaluateC1Account({account,book,now=new Date()}={}) {
 ...
 const continued=continueC1ActualAccount({adoption:account.adoption,sessions,records:account.records,cashReconciliations:account.cashReconciliations,observedAt:now});
...
export function pendingC1AccountSession({account,book,now=new Date()}={}) {
 ...
 const plan=planC1ContinuedAccountOpening({adoption:account.adoption,sessions:all.slice(0,index+1),records:account.records,cashReconciliations:account.cashReconciliations,opening:{...all[index+1],...(account.intradayActivity?.plan.entryRecheck?{entryRecheck:account.intradayActivity.plan.entryRecheck}:{})},observedAt:openingObservedAt,completionPolicy:c1CompletionPolicy(account.positionContext)});
...
export function recoverC1CompletedOwnerExitActivity({account,book,now=new Date()}={}) {
 ...
 const recovered=replanC1IntradayAccountOpening({adoption:account.adoption,sessions,records:account.records,cashReconciliations:account.cashReconciliations,opening,observedAt:pending.openingObservedAt,completionPolicy:c1CompletionPolicy(account.positionContext),activity,authorityPlan}).activity;
