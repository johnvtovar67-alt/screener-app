...
import { archiveC1IndexObservation } from './c1IndexObservationStore';
...
export async function refreshC1Index({ now = new Date(), prepare = prepareStoredC1DatedBook,
  collect = collectC1LiveInput, connect = connectStoredC1DatedInput, archive = archiveC1IndexObservation, onStage = () => {} } = {}) {
  onStage('prepare');
  ...
  try {
    ...
    return { status: 'advanced', ...view, observationReceipt };
  } catch (error) {
