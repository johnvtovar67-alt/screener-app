...
export function getRecommendation(row = {}) {
  ...
  if (trigger >= 72 && asymmetry >= 65 && quality >= 50) {
    return {
      label: "STRONG BUY",
      score,
      reason: "Momentum breakout with strong upside skew.",
      entryNote: "BUY NOW: starter position is actionable.",
    };
  }

  if (trigger >= 60 && asymmetry >= 55) {
    return {
      label: "BUY",
      score,
  ...
  if (trigger >= 45 || asymmetry >= 58) {
    return {
      label: "WATCH",
      score,
      reason: "Setup forming but lacks conviction.",
