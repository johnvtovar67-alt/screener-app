...
export function getArchetype(row = {}) {
  ...
  const reits = [
    "AHR", "AGNC", "O", "VICI", "PLD", "AMT", "CCI", "EQIX", "WELL", "DLR",
    "SPG", "EPR", "STAG", "ADC", "NNN", "IRM", "ARE", "BXP", "KIM", "REG",
  ];

  const crypto = [
    "MSTR", "MARA", "RIOT", "CLSK", "COIN", "HOOD", "HUT", "BTDR", "IREN",
    "WULF", "BITF", "CIFR",
  ];

  const banks = [
    "BAC", "C", "WFC", "JPM", "GS", "MS", "SCHW", "BGC", "BX", "KKR", "APO",
    "SOFI", "ALLY", "RKT", "UPST", "AFRM", "PYPL", "SQ",
  ];

  const airlines = ["AAL", "UAL", "DAL", "LUV", "JBLU", "ALK"];
  const travelCyclicals = ["RCL", "CCL", "NCLH", "ABNB", "EXPE", "BKNG"];
  ...
  const growthMomentum = [
    "PLTR", "CRWD", "NET", "DDOG", "SNOW", "SHOP", "HIMS", "CELH", "SOUN",
    "BBAI", "AAOI", "ARM", "SMCI", "MU", "ROKU", "UBER", "DKNG",
  ];

  if (reits.includes(symbol) || name.includes("REIT") || name.includes("REAL ESTATE")) {
    return "REIT / Income";
  }

  if (crypto.includes(symbol) || name.includes("BITCOIN") || name.includes("CRYPTO")) {
    return "Crypto Proxy";
  }
  ...
  if (travelCyclicals.includes(symbol)) {
    return "Travel / Cyclical";
  }

  if (banks.includes(symbol) || name.includes("BANK") || name.includes("FINANCIAL")) {
...
export function calcFundamentalScore(row = {}) {
  ...
  if (getArchetype(row) === "REIT / Income") score -= 8;
...
function buildDynamicReason(row = {}, recommendation = {}) {
  ...
  const expectationRisk = recommendation.expectationRisk ?? calcExpectationRisk(row);
  const extensionRisk = recommendation.extensionRisk ?? calcExtensionRisk(row);
  const freshBreakoutScore = recommendation.freshBreakoutScore ?? calcFreshBreakoutScore(row);
  const score = recommendation.score ?? 0;
  const maturity = recommendation.themeMaturity || getThemeMaturity(row);

  const { pctFrom50 } = trendData(row);

  if (extensionRisk >= 60) {
    return `${archetype} has strong momentum, but the move appears extended. ${maturity} profile suggests this may be a chase entry rather than an early setup.`;
  }

  if (expectationRisk >= 55) {
    return `${archetype} has an interesting story, but expectation risk is high. ${maturity} profile suggests the entry may already be crowded or extended.`;
  }

  if (recommendation.label === "BUY NOW") {
    if (freshBreakoutScore >= 75) {
      return `Actionable ${archetype} setup with strong trigger confirmation, improving relative strength, and a fresh breakout profile without excessive extension.`;
    }

    if (score < 75) {
      return `High-momentum ${archetype} setup with strong trigger confirmation and controlled expectation risk.`;
    }

    return `Actionable ${archetype} setup with green score, green trigger, strong momentum, and acceptable expectation risk.`;
  }

  if (!archetypeBuyNowAllowed(row) && trigger >= 75) {
    return `${archetype} has a strong technical trigger, but this category is capped below Buy Now for your trading style.`;
  }
  ...
  if (recommendation.label === "WATCH FOR ENTRY" && trigger >= 80 && momentum === "Building") {
    return "Strong setup with at least two green signals, but momentum is still building and not yet fully confirmed.";
  }

  if (recommendation.label === "WATCH FOR ENTRY") {
    return "Constructive setup with enough alignment to monitor closely, but not enough for Buy Now.";
  }

  if (trigger >= 80 && momentum === "Weak") {
    return "Trigger is strong, but weak momentum keeps this below Watch for Entry.";
  }

  if (score >= 60 && trigger >= 65 && momentum === "Weak") {
    return "Some setup quality is present, but weak momentum keeps the trade below entry quality.";
  }

  if (momentum === "Strong" && relative >= 65) {
    return "Leadership characteristics are improving, but the full setup is not yet aligned.";
  }
  ...
  return "Setup is not aligned enough for entry under the current trading rules.";
}
...
function buildDynamicEntry(row = {}, recommendation = {}) {
  ...
  if (extensionRisk >= 60) {
    return "Do not chase. Wait for consolidation, a pullback toward support, or a fresh reset before entry.";
  }

  if (expectationRisk >= 55) {
    return "Do not chase. Wait for consolidation, valuation reset, or a cleaner pullback setup.";
  }

  if (recommendation.label === "BUY NOW") {
    if (freshBreakoutScore >= 75) {
      return "Buy Now reflects an early breakout profile: strong trigger, strong momentum, and controlled extension risk.";
    }

    if (score < 75) {
      return "Buy Now allowed by strong trigger plus strong momentum. Use starter sizing because the overall score is not fully green.";
    }

    return "Buy Now requires green score, green trigger, strong momentum, and controlled expectation risk.";
  }

  if (!archetypeBuyNowAllowed(row) && trigger >= 75) {
    return `${archetype} is not eligible for Buy Now under the asymmetric growth/trading rules. Treat as watch only.`;
  }

  if (recommendation.label === "WATCH FOR ENTRY") {
    if (momentum === "Building") {
      return "Watch for momentum to turn Strong before this becomes a Buy Now candidate.";
    }

    return "Monitor closely, but wait for all three metrics to turn green before buying.";
  }

  if (momentum === "Weak") {
    return "Do not enter yet. Wait for momentum to improve before considering this trade.";
  }
  ...
  if (score < 75 && trigger < 80) {
    return "Needs stronger score and trigger confirmation before it becomes actionable.";
  }
...
export function getRecommendation(row = {}) {
  ...
  const triggerGreen = triggerScore >= 80;
  const momentumGreen = momentumScore >= 75;
  const momentumBuilding = momentumScore >= 55;

  const greenCount = [scoreGreen, triggerGreen, momentumGreen].filter(Boolean).length;
  ...
  const freshBreakout =
    freshBreakoutScore >= 75 &&
    extensionRisk <= 35 &&
    expectationRisk <= 38 &&
    above50 &&
    above200;

  const fullAlignmentBuyNow =
    greenCount === 3 &&
    relativeStrengthScore >= 58 &&
    cleanTrend;

  const momentumTriggerBuyNow =
    triggerScore >= 90 &&
    momentumScore >= 75 &&
    score >= 65 &&
    relativeStrengthScore >= 58 &&
  ...
  const buyNowCandidate =
    fullAlignmentBuyNow || momentumTriggerBuyNow || freshBreakoutBuyNow;

  if (buyNowCandidate && archetypeBuyNowAllowed(row)) {
    label = "BUY NOW";
  } else if (
    greenCount >= 2 &&
    momentumBuilding &&
