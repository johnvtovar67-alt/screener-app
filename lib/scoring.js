// lib/scoring.js

function num(value, fallback = null) {
  const n = Number(value);
  return Number.isFinite(n) ? n : fallback;
}

function clamp(value, min = 0, max = 100) {
  return Math.max(min, Math.min(max, value));
}

function pctDiff(a, b) {
  if (a == null || b == null || b === 0) return null;
  return ((a - b) / b) * 100;
}

function volumeRatio(row = {}) {
  const relVol = num(row.relativeVolume);
  const volume = num(row.volume);
  const avgVolume = num(row.avgVolume);

  if (relVol != null) return relVol;
  if (volume != null && avgVolume != null && avgVolume > 0) {
    return volume / avgVolume;
  }

  return null;
}

/* =========================
   BX PROXY: BUYING PRESSURE
   Measures whether buyers are in control.
========================= */

function pressureScore(row = {}) {
  let score = 0;

  const price = num(row.price);
  const day = num(row.dayChangePct);
  const ma20 = num(row.ma20);
  const ma50 = num(row.ma50 ?? row.priceAvg50);
  const ma200 = num(row.ma200 ?? row.priceAvg200);
  const vr = volumeRatio(row);

  if (price != null && ma20 != null && price > ma20) score += 1;
  if (price != null && ma50 != null && price > ma50) score += 1;
  if (price != null && ma200 != null && price > ma200) score += 1;

  if (day != null && day > 0.5) score += 1;
  if (day != null && day > 2) score += 1;

  if (vr != null && vr >= 1.05) score += 1;
  if (vr != null && vr >= 1.3) score += 1;

  if (row.breakout20) score += 1;

  return clamp(score, 0, 8);
}

function pressureState(row = {}) {
  const score = pressureScore(row);

  if (score >= 6) return "GREEN";
  if (score >= 4) return "MIXED";
  return "RED";
}

/* =========================
   33-DAY FAIR VALUE BAND PROXY
   Measures whether entry location is attractive.
========================= */

function fairValueScore(row = {}) {
  const price = num(row.price);
  const ma20 = num(row.ma20);
  const ma50 = num(row.ma50 ?? row.priceAvg50);
  const day = num(row.dayChangePct);

  if (price == null) return 50;

  let score = 50;

  const distanceFrom20 = pctDiff(price, ma20);
  const distanceFrom50 = pctDiff(price, ma50);

  // Best zone: above trend, but not too stretched.
  if (distanceFrom20 != null) {
    if (distanceFrom20 >= 0 && distanceFrom20 <= 6) score += 18;
    else if (distanceFrom20 > 6 && distanceFrom20 <= 10) score += 8;
    else if (distanceFrom20 > 12) score -= 14;
    else if (distanceFrom20 < -5) score -= 8;
  }

  if (distanceFrom50 != null) {
    if (distanceFrom50 >= 0 && distanceFrom50 <= 10) score += 18;
    else if (distanceFrom50 > 10 && distanceFrom50 <= 18) score += 6;
    else if (distanceFrom50 > 20) score -= 14;
    else if (distanceFrom50 < -3) score -= 10;
  }

  // Avoid chasing huge daily spikes.
  if (day != null) {
    if (day > 10) score -= 18;
    else if (day > 7) score -= 10;
    else if (day > 2 && day <= 6) score += 6;
    else if (day < -3) score -= 8;
  }

  if (row.pullback50) score += 10;

  return Math.round(clamp(score));
}

function fairValueState(row = {}) {
  const score = fairValueScore(row);

  if (score >= 68) return "GREEN";
  if (score >= 55) return "MIXED";
  return "RED";
}

/* =========================
   SUPPORTING SCORES
========================= */

function qualityScore(row = {}) {
  let score = 55;

  const eps = num(row.eps);
  const pe = num(row.pe);
  const marketCap = num(row.marketCap);
  const avgVolume = num(row.avgVolume);

  if (eps != null) score += eps > 0 ? 8 : -6;

  if (pe != null) {
    if (pe > 0 && pe <= 22) score += 8;
    else if (pe > 22 && pe <= 40) score += 3;
    else if (pe <= 0) score -= 6;
  }

  if (marketCap != null) {
    if (marketCap >= 300000000 && marketCap <= 10000000000) score += 8;
    else if (marketCap > 10000000000 && marketCap <= 50000000000) score += 4;
    else if (marketCap < 300000000) score -= 8;
  }

  if (avgVolume != null) {
    if (avgVolume >= 1000000) score += 7;
    else if (avgVolume >= 500000) score += 4;
    else if (avgVolume < 250000) score -= 6;
  }

  return Math.round(clamp(score));
}

function asymmetryScore(row = {}) {
  let score = 55;

  const price = num(row.price);
  const yearHigh = num(row.yearHigh);
  const yearLow = num(row.yearLow);

  if (price != null && yearHigh != null && yearHigh > price) {
    const upside = ((yearHigh - price) / price) * 100;

    if (upside >= 80) score += 18;
    else if (upside >= 50) score += 13;
    else if (upside >= 30) score += 9;
    else if (upside >= 15) score += 5;
  }

  if (price != null && yearLow != null && yearLow > 0) {
    const offLow = ((price - yearLow) / yearLow) * 100;

    if (offLow > 250) score -= 8;
    else if (offLow > 150) score -= 4;
  }

  return Math.round(clamp(score));
}

function triggerScore(row = {}) {
  let score = 50;

  const pressure = pressureScore(row);
  const fairValue = fairValueScore(row);
  const day = num(row.dayChangePct);
  const vr = volumeRatio(row);

  score += pressure * 4;

  if (fairValue >= 68) score += 10;
  else if (fairValue >= 55) score += 4;
  else score -= 8;

  if (day != null) {
    if (day > 10) score -= 8;
    else if (day > 5) score += 8;
    else if (day > 2) score += 7;
    else if (day > 0.5) score += 4;
    else if (day < -5) score -= 12;
    else if (day < -2) score -= 7;
    else if (day < 0) score -= 4;
  }

  if (vr != null) {
    if (vr >= 2) score += 8;
    else if (vr >= 1.3) score += 6;
    else if (vr >= 1.05) score += 3;
    else if (vr < 0.6) score -= 4;
  }

  return Math.round(clamp(score));
}

function compositeScore(row = {}) {
  const trigger = row.triggerScore ?? triggerScore(row);
  const asymmetry = row.asymmetryScore ?? asymmetryScore(row);
  const quality = row.qualityScore ?? qualityScore(row);
  const fairValue = fairValueScore(row);
  const pressure = pressureScore(row) * 12.5;

  return Math.round(
    clamp(
      trigger * 0.35 +
        pressure * 0.25 +
        fairValue * 0.2 +
        asymmetry * 0.12 +
        quality * 0.08
    )
  );
}

/* =========================
   LANGUAGE ENGINE
========================= */

function reasonAndEntry(row, label) {
  const pressure = pressureState(row);
  const fairValue = fairValueState(row);
  const day = num(row.dayChangePct);
  const vr = volumeRatio(row);

  if (label === "STRONG BUY") {
    return {
      reason:
        "BUY NOW: buying pressure and fair value location are both aligned.",
      entryNote:
        "BUY NOW starter. Add only if price holds strength and volume remains supportive.",
    };
  }

  if (label === "BUY") {
    if (pressure === "GREEN" && fairValue !== "GREEN") {
      return {
        reason:
          "Buying pressure is strong, but price location is not ideal yet.",
        entryNote:
          "Small starter only. Prefer pullback or consolidation before adding.",
      };
    }

    if (pressure !== "GREEN" && fairValue === "GREEN") {
      return {
        reason:
          "Entry location is attractive, but buying pressure has not fully confirmed.",
        entryNote:
          "Starter acceptable. Add only when pressure improves.",
      };
    }

    return {
      reason:
        "Setup is constructive, but one confirmation layer is still mixed.",
      entryNote:
        "Starter acceptable only if price action stays constructive.",
    };
  }

  if (label === "WATCH") {
    if (day != null && day > 7) {
      return {
        reason:
          "Buying pressure may be improving, but the move is extended.",
        entryNote:
          "Do not chase. Wait for pullback or consolidation.",
      };
    }

    if (pressure === "RED") {
      return {
        reason:
          "Fair value may be improving, but buying pressure is still weak.",
        entryNote:
          "Wait for buyers to regain control.",
      };
    }

    if (fairValue === "RED") {
      return {
        reason:
          "Buying pressure is present, but price is outside the ideal entry zone.",
        entryNote:
          "Wait for price to move back into a cleaner fair value zone.",
      };
    }

    return {
      reason:
        "Setup is forming, but the two confirmation layers are not aligned yet.",
      entryNote:
        "Wait for pressure and fair value to align.",
    };
  }

  return {
    reason:
      "Both pressure and fair value conditions are weak or unconfirmed.",
    entryNote: "Avoid for now.",
  };
}

/* =========================
   PUBLIC API
========================= */

export function calcQualityScore(row = {}) {
  return qualityScore(row);
}

export function calcAsymmetryScore(row = {}) {
  return asymmetryScore(row);
}

export function calcTriggerScore(row = {}) {
  return triggerScore(row);
}

export function getRecommendation(row = {}) {
  const trigger = row.triggerScore ?? triggerScore(row);
  const asymmetry = row.asymmetryScore ?? asymmetryScore(row);
  const quality = row.qualityScore ?? qualityScore(row);
  const score = compositeScore({
    ...row,
    triggerScore: trigger,
    asymmetryScore: asymmetry,
    qualityScore: quality,
  });

  const pressure = pressureState(row);
  const fairValue = fairValueState(row);
  const day = num(row.dayChangePct);
  const vr = volumeRatio(row);

  let label = "AVOID";

  // DiCarlo-style proxy:
  // pressure green + fair value green = strong buy
  if (
    pressure === "GREEN" &&
    fairValue === "GREEN" &&
    score >= 72 &&
    day != null &&
    day > 0.5 &&
    day < 10 &&
    (vr == null || vr >= 1.0)
  ) {
    label = "STRONG BUY";
  } else if (
    (pressure === "GREEN" && fairValue !== "RED") ||
    (fairValue === "GREEN" && pressure !== "RED")
  ) {
    label = score >= 64 ? "BUY" : "WATCH";
  } else if (score >= 58) {
    label = "WATCH";
  }

  const text = reasonAndEntry(row, label);

  return {
    label,
    score,
    reason: text.reason,
    entryNote: text.entryNote,
    pressure,
    fairValue,
  };
}

export function getStage(row = {}) {
  const rec = getRecommendation(row);
  if (rec.label === "STRONG BUY") return "Strong Buy";
  if (rec.label === "BUY") return "Buy";
  if (rec.label === "WATCH") return "Watch";
  return "Avoid";
}

export function buildTechnicalSnapshot(row = {}) {
  const price = num(row.price);
  const ma20 = num(row.ma20);
  const ma50 = num(row.ma50 ?? row.priceAvg50);
  const ma200 = num(row.ma200 ?? row.priceAvg200);
  const volume = num(row.volume);
  const avgVolume = num(row.avgVolume);
  const relVol = num(row.relativeVolume);

  return {
    oneMonthPct: row.pct20 ?? row.dayChangePct ?? null,
    threeMonthPct: row.pct60 ?? null,
    relativeVolume:
      relVol ??
      (volume != null && avgVolume != null && avgVolume > 0
        ? volume / avgVolume
        : null),
    pressure: pressureState(row),
    fairValue: fairValueState(row),
    pressureScore: pressureScore(row),
    fairValueScore: fairValueScore(row),
    above20dma: price != null && ma20 != null ? price > ma20 : null,
    above50dma: price != null && ma50 != null ? price > ma50 : null,
    above200dma: price != null && ma200 != null ? price > ma200 : null,
    pctFrom20dma: pctDiff(price, ma20),
    pctFrom50dma: pctDiff(price, ma50),
    pctFrom200dma: pctDiff(price, ma200),
    pctFrom52wHigh:
      price != null && row.yearHigh != null
        ? ((price - row.yearHigh) / row.yearHigh) * 100
        : null,
    pctFrom52wLow:
      price != null && row.yearLow != null
        ? ((price - row.yearLow) / row.yearLow) * 100
        : null,
    rsi: null,
    macd: null,
    macdSignal: null,
  };
}

export function buildFundamentalSnapshot(row = {}) {
  return {
    revenueGrowthPct: row.revenueGrowthPct ?? null,
    epsGrowthPct: row.epsGrowthPct ?? null,
    operatingMarginPct: row.operatingMarginPct ?? null,
    grossMargin: row.grossMargin ?? null,
    debtToEquity: row.debtToEquity ?? null,
    marketCap: row.marketCap ?? null,
    institutionalScore: row.institutionalScore ?? null,
    eps: row.eps ?? null,
    pe: row.pe ?? null,
  };
}
