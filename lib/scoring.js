...
function isActionableNow(row = {}) {
  ...
  const greenEnough = day != null ? day > 0.75 : false;
  const notTooExtended = day != null ? day < 10 : true;

  const volumeOk =
    volume != null && avgVolume != null && avgVolume > 0
      ? volume / avgVolume >= 1.05
      : true;

  return above50 && greenEnough && notTooExtended && volumeOk;
}
...
export function getRecommendation(row = {}) {
  ...
  const actionableNow = isActionableNow(row);

  if (score >= 77 && trigger >= 66 && asymmetry >= 62 && actionableNow) {
    return {
      label: "STRONG BUY",
      score,
      reason: "BUY NOW: momentum and upside are confirming.",
      entryNote: "BUY NOW: starter position is actionable.",
    };
  }

  if (score >= 77 && trigger >= 62 && asymmetry >= 58) {
    return {
      label: "BUY",
      score,
      reason: "High-quality setup, but not a buy-now signal yet.",
      entryNote: "WAIT: buy only on breakout, pullback, or volume confirmation.",
    };
  }

  if (score >= 70 && trigger >= 58 && asymmetry >= 55) {
    return {
      label: "WATCH",
      score,
      reason: "Setup forming, but not enough conviction yet.",
      entryNote: "Wait for stronger price and volume confirmation.",
    };
  }
