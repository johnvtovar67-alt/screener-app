...
export function getThemeMaturity(row = {}) {
  ...
  if (freshBreakoutScore >= 75 && expectationRisk <= 35 && lateChaseRisk <= 45) {
...
function getConfidenceLevel(
  ...
  if (lateChaseRisk <= 35) points += 6;

  if (lateChaseRisk >= 70) points -= 26;
  else if (lateChaseRisk >= 55) points -= 16;
  else if (lateChaseRisk >= 45) points -= 8;
  ...
  if (lateChaseRisk >= 55 && points > 68) {
    points = 68;
  }

  if (points >= 75 && cleanEntry && lateChaseRisk <= 45) {
    return {
      label: "High",
      score: points,
      tone: "green",
...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  const lateChaseRisk =
    recommendation.lateChaseRisk ?? calcLateChaseRisk(row);
  ...
  if (archetype === "Biotech / Binary Event") return "Binary event risk";

  if (lateChaseRisk >= 70) return "Late-stage chase risk";
  if (lateChaseRisk >= 55) return "Crowded momentum";

  if (historicalFadingOrBlocked(row)) {
    if (resistance != null && resistance > 7.5) return "Resistance overhead";
    if (relativeStrengthScore < 45) return "Relative strength deteriorating";
    return "Momentum fading";
  }

  if (dayBreakoutFailed(row)) return "Breakout failed on weak volume";

  if (label === "BUY NOW" && historicalConfirmedBreakout(row)) {
    if (volumeRatio != null && volumeRatio >= 1.05) {
      return "Confirmed breakout on volume";
    }

    return "Confirmed breakout";
  }

  if (label === "BUY NOW" && historicalCleanEntry(row)) {
    if (above50 === true && momentumScore >= 60) {
      return "Momentum improving above key average";
    }

    return "Clean actionable entry";
  }

  if (label === "WATCH") {
    if (resistance != null && resistance > 4.5) return "Resistance overhead";
    if (volumeRatio != null && volumeRatio < 1) return "Needs stronger volume";
    if (momentum5 != null && momentum5 < 0) return "Short-term momentum still negative";
    if (momentum10 != null && momentum10 < 0) return "Trend rebuilding";
    if (slope != null && slope < 0) return "Trend rebuilding";
    if (historicalImprovingSetup(row)) return "Trend rebuilding";
    if (above50 === true && pctFrom50 != null && pctFrom50 >= -1 && pctFrom50 <= 8) {
      return "Holding key support";
    }
    if (triggerScore >= 70) return "Trigger improving";
    if (momentumScore >= 55) return "Momentum improving";
  ...
  if (relativeStrengthScore < 45) return "Relative strength deteriorating";
  if (penalty >= 60) return "Setup quality below threshold";

  return "Not actionable now";
}

function dayBreakoutFailed(row = {}) {
  const breakout = row.breakoutAbove20High === true;
  const volumeRatio = historicalVolumeRatio(row);
...
function buildDynamicReason(row = {}, recommendation = {}) {
  ...
  if (action === "Buy Now") {
    return `Buy Now: ${context}. Actionable under normal sizing; avoid oversized chasing.`;
  }
...
function buildDynamicEntry(row = {}, recommendation = {}) {
  ...
  if (label === "WATCH") {
    if (context.includes("resistance")) {
      return "Watch only. Needs a clean close above resistance with stronger confirmation.";
    }

    if (context.includes("volume")) {
      return "Watch only. Needs stronger volume before it becomes actionable.";
    }

    if (context.includes("rebuilding") || context.includes("momentum")) {
      return "Watch only. Needs momentum confirmation before buying.";
    }

    if (context.includes("support")) {
      return "Watch only. It is holding support, but still needs a clean upside trigger.";
    }
...
export function getRecommendation(row = {}) {
  ...
  const macdMomentumHealthy = momentumScore >= 58 && triggerScore >= 70;
  ...
  const breakoutVelocityHealthy =
    (dayMove == null || (dayMove >= 0.2 && dayMove <= 8)) &&
    (rv == null || (rv >= 1.05 && rv <= 4.5));

  const resistanceNotExtended =
    extensionRisk <= 44 &&
    expectationRisk <= 48 &&
    lateChaseRisk <= 42;

  const trueBreakoutConfirmation =
    freshBreakoutScore >= 80 &&
  ...
  const cleanEntryConfirmation =
    ...
    penalty <= 62 &&
    above50 !== false &&
    (pctFrom50 == null || pctFrom50 <= 24);

  const selectiveActionableSetup =
  ...
  const disciplinedBuyNow =
    liquidEnough &&
    buyNowAllowed &&
    selectiveActionableSetup;

  if (cleanBuyNow || disciplinedBuyNow) {
    label = "BUY NOW";
  } else if (watchSetup) {
    label = "WATCH";
  }
...
export function getTradeReadiness(row = {}) {
  ...
  if (label === "BUY NOW") {
    return {
      label: "TRADE READY",
      tone: "green",
      reason: "Actionable Buy Now setup.",
    };
  }
  ...
  if (label === "WATCH") {
    return {
      label: "WATCH",
      tone: "yellow",
      reason: "Interesting but not actionable now.",
    };
  }
