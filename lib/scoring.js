...
function calcTriggerScore(stock = {}) {
  ...
  if (vs50 !== null) {
    if (vs50 >= 1 && vs50 <= 12) score += 20;
    else if (vs50 > 12 && vs50 <= 22) score += 9;
    else if (vs50 > 22) score -= 8;
    else if (vs50 >= -2) score += 4;
    else score -= 12;
  }
  if (vs200 !== null) score += vs200 >= 0 ? 10 : vs200 >= -5 ? -2 : -10;
  if (range !== null) score += range >= 60 && range <= 92 ? 10 : range > 92 ? 2 : range < 35 ? -8 : 0;
  if (change !== null) score += change >= 1 && change <= 7 ? 11 : change > 7 && change <= 11 ? 3 : change > 11 ? -4 : change < -4 ? -10 : 0;
  if (relVol !== null) score += relVol >= 1.5 ? 10 : relVol >= 1 ? 5 : relVol < 0.45 ? -5 : 0;
...
function compositeScore(stock = {}) {
  ...
  const score = fundamental * 0.18 + technical * 0.22 + momentum * 0.2 + relativeStrength * 0.14 + asymmetry * 0.1 + trigger * 0.16 - Math.max(0, risk - 65) * 0.24;
  return round(score);
}
...
function getTradeReadiness(stock = {}) {
  ...
  if (!institutionalPass && score < 72) return "Avoid";
  if (risk >= 86 || extension >= 90) return "Avoid";

  const buyAligned = score >= 74 && technical >= 66 && momentum >= 64 && trigger >= 66 && risk <= 72 && extension <= 78;
  if (buyAligned) return "Buy";

  const starterAligned = score >= 62 && technical >= 55 && momentum >= 54 && trigger >= 56 && risk <= 80 && extension <= 86;
  const breakoutStarter = getFreshBreakoutScore(stock) >= 68 && score >= 58 && risk <= 82;
  if (starterAligned || breakoutStarter) return "Starter";

  const watchAligned = score >= 48 || trigger >= 48 || momentum >= 50 || technical >= 50;
  if (watchAligned) return "Watch";
...
function getDominantReason(stock = {}) {
  ...
  if (label === "Buy") return "Trend, momentum, trigger, risk, and tradability are aligned.";
  if (label === "Starter") {
    if (hasBreakoutStructure(stock)) return "Breakout structure is constructive, but sizing should still start small.";
  ...
  if (trigger < 56 && score >= 48) return `Setup is watchable, but trigger confirmation is not strong enough. Watch a move through ${formatPrice(breakoutPrice)}.`;
