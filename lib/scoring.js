...
function buildInstitutionalScore(stock) {
  const priceScore = bucketScore(stock.price, 10, 5).score;
  const relativeVolumeScore = bucketScore(stock.relativeVolume, 1.0, 0.9).score;
  const volatilityScore = reverseBucketScore(stock.volatility, 4.5, 6.0).score;
  const operatingMarginScore = bucketScore(stock.operatingMarginPct, 8, 2).score;
  const debtScore = reverseBucketScore(stock.debtToEquity, 1.2, 2.0).score;
  const revenueScore = bucketScore(stock.revenueGrowthPct, 5, 0).score;
  const epsScore = bucketScore(stock.epsGrowthPct, 10, 0).score;
  const oneMonthScore = bucketScore(stock.oneMonthPct, 5, 0).score;
  const threeMonthScore = bucketScore(stock.threeMonthPct, 10, 3).score;

  return weightedAverage([
    { score: priceScore, weight: 0.08 },
    { score: relativeVolumeScore, weight: 0.12 },
    { score: volatilityScore, weight: 0.10 },
    { score: operatingMarginScore, weight: 0.16 },
    { score: debtScore, weight: 0.12 },
    { score: revenueScore, weight: 0.12 },
    { score: epsScore, weight: 0.12 },
    { score: oneMonthScore, weight: 0.08 },
    { score: threeMonthScore, weight: 0.10 },
  ]);
}
...
function buildAction(stock, scores) {
  ...
  const institutional = scores.institutionalScore ?? 0;
    ...
    return {
      ...
      reason:
        valuation >= 70
          ...
          : "Strong setup with trend + institutional support",
    };
  }
...
export function buildDrivers(stock) {
  ...
  const institutionalScore = buildInstitutionalScore(stock);

  const compositeScore = weightedAverage([
    { score: technicalScore, weight: 0.34 },
    { score: fundamentalScore, weight: 0.36 },
    { score: sentimentScore, weight: 0.10 },
    { score: institutionalScore, weight: 0.20 },
  ]);
