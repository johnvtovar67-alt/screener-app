...
function isBreakoutBuyCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (triggerScore < 65) return false;
  if (momentumScore < 62) return false;
  if (riskScore > (isDigitalAssetProxy(stock) && isConstructiveMomentum(stock) ? 72 : 68)) return false;
...
function isBuyImmediatelyCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (gateSummary.trigger?.status !== "PASS") return false;
  if (gateSummary.confirmation?.status !== "PASS") return false;
  if (gateSummary.risk?.status !== "PASS") return false;

  // This is intentionally rare: all major pillars must be strong at the same time.
  if (score < 88) return false;
  if (fundamental < 68) return false;
  if (technical < 76) return false;
  if (momentum < 72) return false;
  if (trigger < 76) return false;
  if (risk > 54) return false;

  // Excellent entry means participating in strength without chasing a vertical move.
  if (vs50 !== null && (vs50 < 0 || vs50 > 16)) return false;
  if (rangePosition !== null && (rangePosition < 55 || rangePosition > 94)) {
    return false;
  }
...
function isBuyNowCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (counts.fail > 0) return false;

  // Buy Now is deliberately easier than Buy Immediately. It is for strong, tradable setups
  // where most pillars are aligned even if one item is only a WATCH.
  if (score < 80) return false;
  if (technical < 66) return false;
  if (momentum < 62) return false;
  if (trigger < 66) return false;
  if (risk > (isDigitalAssetProxy(stock) && isConstructiveMomentum(stock) ? 70 : 66)) return false;

  // Do not let Buy Now become a parabolic chase bucket.
  if (vs50 !== null && vs50 > 24 && !hasBreakoutStructure(stock)) return false;
  if (rangePosition !== null && rangePosition > 98 && !hasBreakoutStructure(stock)) {
    return false;
  }
...
function getTradeReadiness(stock = {}) {
  ...
  if (isBuyImmediatelyCandidate(stock, gateSummary)) {
    return "Buy Immediately";
  }

  if (isBuyNowCandidate(stock, gateSummary)) {
    return "Buy Now";
  }
...
function buildActionSummary(label, dominantReason, score, stock = {}) {
  if (label === "Buy Immediately") {
    return "Highest-conviction setup. Fundamentals, technicals, momentum, risk, extension, and entry quality are all aligned.";
  }
...
function buildReason(label, dominantReason, stock = {}) {
  ...
  if (label === "Buy Immediately") {
    return `Buy Immediately because all six key requirements are aligned: fundamentals, technicals, momentum, risk, extension, and entry quality.`;
  }
...
function buildEntryNote(label, dominantReason, stock = {}) {
  ...
  if (label === "Buy Immediately") {
    return `Highest-conviction entry is acceptable now near ${formatPrice(
      price
    )}. Use normal sizing only with a defined stop near ${formatPrice(
      supportPrice
    )}.`;
  }
