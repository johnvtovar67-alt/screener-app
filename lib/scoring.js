...
function trendData(row = {}) {
  ...
  return {
    ...
    above200: price != null && ma200 != null ? price > ma200 : null,
...
function trueTradabilityFailure(row = {}) {
  ...
  if (price == null || price < 5) return true;

  // Important: only fail market cap or volume when FMP actually provides the value.
  // Missing FMP data lowers confidence but should not falsely fail mega-cap names.
  if (marketCap != null && marketCap < 300000000) return true;
  if (avgVolume != null && avgVolume < 300000) return true;
...
function missingDataPenalty(row = {}) {
  ...
  if (isBadNumber(row.marketCap)) penalty += 4;
  if (isBadNumber(row.avgVolume)) penalty += 4;
  if (isBadNumber(row.priceAvg50)) penalty += 5;
  if (isBadNumber(row.priceAvg200)) penalty += 5;
  if (isBadNumber(row.eps)) penalty += 2;
  if (isBadNumber(row.pe)) penalty += 2;
...
function archetypeRiskPenalty(row = {}) {
  ...
  // Biotech is NOT penalized in ranking just for being biotech.
  // It is only capped below Buy Now because we do not have catalyst/FDA/news data here.
  return 0;
}
...
function marketRelativeScore(row = {}) {
  ...
  // If we do not have market-relative data, use the stock's own move.
...
export function calcFreshBreakoutScore(row = {}) {
  ...
  score -= missingDataPenalty(row) * 0.4;
...
export function calcExpectationRisk(row = {}) {
  ...
  risk += missingDataPenalty(row) * 0.35;
...
function riskPenalty(row = {}) {
  ...
  penalty += expectationRisk * 0.28;
  penalty += extensionRisk * 0.25;
  penalty += missingDataPenalty(row) * 0.5;
...
export function calcFundamentalScore(row = {}) {
  ...
  if (marketCap != null) {
    ...
    else if (marketCap < 300000000) score -= 20;
  }
  ...
  score -= missingDataPenalty(row) * 0.35;
...
export function calcTechnicalScore(row = {}) {
  ...
  score -= missingDataPenalty(row) * 0.45;
...
export function calcAsymmetryScore(row = {}) {
  ...
  score -= missingDataPenalty(row) * 0.3;
...
export function calcTriggerScore(row = {}) {
  ...
  score -= penalty * 0.12;
  score -= expectationRisk * 0.05;
  score -= extensionRisk * 0.08;
  score -= missingDataPenalty(row) * 0.3;
...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  if (label === "BUY NOW" && freshBreakoutScore >= 72) return "Fresh breakout";
  ...
  if (label === "WATCH FOR ENTRY" && triggerScore >= 74) {
...
function buildDynamicEntry(row = {}, recommendation = {}) {
  ...
  if (label === "BUY NOW") {
    return "Actionable now under the current rules. Use normal risk control.";
  }
