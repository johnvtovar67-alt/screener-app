...
function scoreNarrative(d) {
  ...
  else if (bucket === "sub25") score = 80;
  else if (bucket === "event") score = 78;
  else if (bucket === "growth") score = 65;
  else if (bucket === "quality") score = 40;
  else if (bucket === "value" || bucket === "cyclical") score = 28;
...
export function calcAsymmetryScore(d) {
  const raw =
    scoreMomentum(d) * 0.30 +
    scoreDiscovery(d) * 0.20 +
    scoreNarrative(d) * 0.25 +
...
export function calcTriggerScore(d) {
  ...
  score += Math.round(asymmetry * 0.12);
...
function actionFromScores(triggerScore, asymmetryScore, stage) {
  if (triggerScore >= 70 && asymmetryScore >= 55 && stage !== "Broken") {
    return {
      actionLabel: "Buy",
      actionColor: "green",
