...
function isBuyNow(row = {}) {
  ...
  return above50 && greenDay && notTooExtended && volumeConfirm;
}
...
export function getRecommendation(row = {}) {
  ...
  if (totalScore >= 75 && trigger >= 70 && asymmetry >= 65) {
    return {
      ...
      score: totalScore,
      reason: buyNow
        ? "BUY NOW: momentum, trend, and volume are confirming."
        : "Strong setup, but wait for volume or price confirmation.",
      entryNote: buyNow
        ? "BUY NOW: starter position is actionable."
        : "WAIT: do not buy yet. Wait for green move with stronger volume.",
    };
  }
