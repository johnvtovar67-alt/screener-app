...
function buildEntryNote(label, dominantReason, stock = {}) {
  ...
  if (dominantReason.includes("breakout")) {
    return `Wait for a clean move above ${formatPrice(
      breakoutPrice
    )} with positive price action.`;
  }
  ...
  if (dominantReason.includes("trigger confirmation")) {
    if (hasVolumeCaution) {
      return `Wait for stronger confirmation above ${formatPrice(
        breakoutPrice
      )}. Volume data is incomplete, so confirm real-time liquidity before entering.`;
    }
