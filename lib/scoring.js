...
function getConfidenceLevel(
  ...
  if (triggerScore >= 80) points += 25;
  else if (triggerScore >= 70) points += 18;
  else if (triggerScore >= 60) points += 10;
...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  const themeMaturity = recommendation.themeMaturity || getThemeMaturity(row);
  ...
  return themeMaturity || archetype;
}
...
export function getRecommendation(row = {}) {
  ...
  const buyNowCandidate =
    ...
    liquidEnough &&
    ((greenCount === 3 && relativeOkay && cleanTrend) ||
      (triggerScore >= 90 &&
        momentumScore >= 75 &&
        score >= 65 &&
        relativeOkay &&
