...
function hasBreakoutStructure(stock = {}) {
  ...
  if (changePct !== null) {
    ...
    if (changePct > 8.5) return false;
  }

  if (vs50 !== null) {
    if (vs50 < 1) return false;
    if (vs50 > 18) return false;
  }
...
function evaluateRiskGate(stock = {}) {
  ...
  if (price > 0 && fiftyDay > 0) {
    ...
    if (extended > 18 && !hasBreakoutStructure(stock)) {
      return {
        status: "WATCH",
...
function isStarterCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (stage === "Extended Uptrend") return false;
