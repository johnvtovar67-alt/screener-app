...
function buyingPressureScore(row = {}) {
  ...
  if (vr != null) {
    if (vr >= 2) score += 12;
    else if (vr >= 1.4) score += 9;
    else if (vr >= 1.1) score += 5;
...
function composite(row = {}) {
  ...
  return Math.round(clamp(pressure * 0.45 + fairValue * 0.35 + quality * 0.2));
}
...
function language(row, label, pressure, fairValue) {
  const day = num(row.dayChangePct);
  const vr = volRatio(row);
  ...
  if (label === "STRONG BUY") {
    return {
      reason: "BUY NOW: buying pressure and fair-value location are both aligned.",
      entryNote: "Buy starter now. Add only if strength and volume hold.",
    };
  }
...
export function getRecommendation(row = {}) {
  ...
  const text = language(row, label, pressure, location);
