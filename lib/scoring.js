...
function buildRiskPlan(stock = {}, label = getTradeReadiness(stock)) {
  ...
  // Exit targets are staged. The first target is a trim/review zone, not an
  ...
  if (label === "Buy") {
    summary = `Buyable now. Reassess below ${formatPrice(invalidationPrice)}; consider trimming into ${formatPrice(firstTrimPrice)}.`;
  } else if (label === "Starter") {
    summary = `Starter acceptable. Add only above ${formatPrice(addAbovePrice)}; reassess below ${formatPrice(invalidationPrice)}.`;
  } else if (label === "Watch") {
    summary = `No new entry yet. Watch for confirmation above ${formatPrice(addAbovePrice)}.`;
  } else {
    summary = `Avoid until the setup improves; no exit target is useful until the thesis repairs.`;
  }
