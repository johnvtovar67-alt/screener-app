...
function buildDynamicReason(row = {}, recommendation = {}) {
  ...
  if (recommendation.label === "BUY NOW") {
    if (score < 75) {
      return `High-momentum ${archetype} setup with very strong trigger confirmation, strong momentum, and respectable score.`;
    }

    return `Actionable ${archetype} setup with green score, green trigger, and strong momentum confirmation.`;
  }
...
function buildDynamicEntry(row = {}, recommendation = {}) {
  ...
  if (recommendation.label === "BUY NOW") {
    ...
    return "Buy Now requires green score, green trigger, and strong momentum. Starter position is reasonable if volume and trend continue confirming.";
  }
