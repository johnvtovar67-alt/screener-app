...
function buildActionSummary(label, dominantReason, score, stock = {}) {
  if (label === "Buy") return `Buy. ${dominantReason}`;
  if (label === "Starter") return `Starter. Use reduced size; add only if leadership and price confirmation hold.`;
...
function buildReason(label, dominantReason, stock = {}) {
  ...
  if (label === "Starter") return `Starter only; add after a sustained move through ${formatPrice(breakout)} or a constructive hold above support.`;
  if (label === "Watch") return `Wait for a cleaner trigger near ${formatPrice(breakout)}.`;
...
function buildEntryNote(label, dominantReason, stock = {}) {
  const support = getSupportPrice(stock);
  const breakout = getNearTermBreakoutPrice(stock);
  if (label === "Buy") return `Buyable now if position size is disciplined. Initial risk reference: ${formatPrice(support)}.`;
  if (label === "Starter") return `Starter size only. Add above ${formatPrice(breakout)} with strength.`;
  if (label === "Watch") return `Watch for a break above ${formatPrice(breakout)} or a constructive pullback to support.`;
