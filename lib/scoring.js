...
function getTradeReadiness(stock = {}) {
  ...
  // Calibrated for a rising market: true confirmed setups should not sit
  // in Starter Only forever. Buy Now is still blocked for risky archetypes,
  // but allows one WATCH gate when trend/trigger/risk are strong.
  const strongPartialPass =
    gateSummary.tradability.status !== "FAIL" &&
    gateSummary.trend.status === "PASS" &&
    gateSummary.trigger.status === "PASS" &&
    gateSummary.confirmation.status !== "FAIL" &&
    gateSummary.risk.status !== "FAIL" &&
    counts.fail === 0;

  const eliteBreakout =
    isBreakoutBuyCandidate(stock, gateSummary) &&
    score >= 84 &&
    calcTriggerScore(stock) >= 72 &&
    calcMomentumScore(stock) >= 68 &&
    risk <= 66;

  if (!blocked && score >= 84 && corePass && risk <= 66) {
    return "Buy Now";
  }

  if (!blocked && score >= 84 && strongPartialPass && risk <= 66) {
    return "Buy Now";
  }

  if (!blocked && eliteBreakout) {
    return "Buy Now";
  }
