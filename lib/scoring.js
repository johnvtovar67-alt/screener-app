...
function trendData(row = {}) {
  ...
  const pctFrom50 = pctFrom(price, ma50);
  const pctFrom200 = pctFrom(price, ma200);
...
export function calcAsymmetryScore(row = {}) {
  ...
  if (price != null && yearHigh != null && yearHigh > price) {
    const upsideToHigh = pctFrom(yearHigh, price);

    if (upsideToHigh != null) {
      if (upsideToHigh >= 50) score += 10;
      else if (upsideToHigh >= 25) score += 6;
    }
  }

  if (price != null && yearLow != null && yearLow > 0) {
    const offLow = pctFrom(price, yearLow);

    if (offLow != null) {
      if (offLow > 250) score -= 12;
      else if (offLow > 150) score -= 6;
    }
  }
...
export function getRecommendation(row = {}) {
  ...
  if (!passesInstitutionalFilter(row)) {
    return {
      ...
      riskPenalty: penalty,
      reason: "Fails minimum institutional liquidity, size, or price filters.",
  ...
  if (
    ...
    return {
      ...
      riskPenalty: penalty,
      reason:
        "Strong trend, positive market-relative strength, and actionable trigger confirmation.",
      entryNote:
        "Buy now is reasonable for a starter position. Avoid oversized entry if extended.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      riskPenalty: penalty,
      reason:
        "Constructive setup, but timing, relative strength, or risk controls are not fully aligned.",
      entryNote:
        "Watch for stronger relative strength, volume confirmation, or a cleaner pullback entry.",
    };
  }
  ...
  return {
    ...
    riskPenalty: penalty,
    reason:
      "Weak risk-adjusted setup versus stronger opportunities.",
    entryNote:
      "Avoid for now.",
  };
}
...
/* Backward-compatible exports so older API files do not break */
export const calcQualityScore = calcFundamentalScore;

export function calcHeatScore(row = {}) {
  const trigger = calcTriggerScore(row);
  if (trigger >= 85) return 5;
  if (trigger >= 75) return 4;
  if (trigger >= 65) return 3;
  if (trigger >= 55) return 2;
  if (trigger >= 45) return 1;
  return 0;
}
