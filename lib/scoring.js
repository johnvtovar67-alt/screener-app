...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  if (lateChaseRisk >= 45 && label !== "BUY NOW") return "Extended setup";
...
function buildDynamicEntry(row = {}, recommendation = {}) {
  ...
  if (label === "BUY" && lateChaseRisk >= 45) {
    return "Starter-size only. Setup is improving, but avoid chasing size after a large move.";
  }
  ...
  if (label === "WATCH FOR ENTRY" && lateChaseRisk >= 45) {
    return "Strong theme, but entry is extended. Wait for a pullback, base, or reset.";
  }
...
export function getRecommendation(row = {}) {
  ...
  const mediumOrHighConfidence =
    confidencePreview.label === "High" ||
    confidencePreview.label === "Medium";
...
export function getTradeReadiness(row = {}) {
  ...
  if (label === "BUY NOW") {
    return {
      label: "TRADE READY",
      tone: "green",
      reason: "Highest-quality actionable setup.",
    };
  }
  ...
  if (label === "BUY") {
    return {
      label: "BUY",
      tone: "green",
      reason: "Clean starter-size setup.",
    };
  }
  ...
  if (label === "WATCH FOR ENTRY") {
    return {
      label: "WATCH CLOSELY",
      tone: "yellow",
      reason: "Constructive setup, but wait for cleaner entry.",
    };
  }
