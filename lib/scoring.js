...
function getDominantReason(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (hasOnlyVolumeCaution(gateSummary)) {
    return "Setup improving; volume data incomplete.";
  }
...
function buildEntryNote(label, dominantReason, stock = {}) {
  ...
  if (dominantReason.includes("volume data incomplete")) {
    return "Volume data is incomplete, so confirm real-time liquidity before entering. Do not reject the setup only because average volume is missing.";
  }
