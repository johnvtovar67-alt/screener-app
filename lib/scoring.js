...
function getDominantReason(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (hasOnlyVolumeCaution(gateSummary)) {
    if (score >= 86) {
      return "Setup is actionable; confirm real-time liquidity.";
    }

    return "Setup improving, but edge is not strong enough for Buy Now.";
  }
...
function buildEntryNote(label, dominantReason, stock = {}) {
  ...
  if (label === "Buy Now") {
    if (hasVolumeCaution) {
      return "Entry is acceptable now, but confirm real-time liquidity because volume data is incomplete.";
    }
  ...
  if (dominantReason.includes("edge is not strong enough")) {
    if (hasVolumeCaution) {
      return "Wait for stronger confirmation. Volume data is incomplete, so confirm real-time liquidity before entering.";
    }
