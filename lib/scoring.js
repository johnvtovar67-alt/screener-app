...
function missingDataPenalty(row = {}) {
  ...
  if (num(row.priceAvg50) == null) penalty += 8;
  if (num(row.priceAvg200) == null) penalty += 8;
...
function buildTradeGates(row = {}) {
  ...
  if (trueTradabilityFailure(row)) {
    tradability = gate("fail", "Fails tradability filter: price is too low or quote is missing.");
  ...
  } else if (triggerScore >= 65 || freshBreakoutScore >= 65) {
  ...
  if (momentumScore >= 72 && relativeStrengthScore >= 60) {
    if (rv == null || rv >= 1.05 || histVolumeRatio == null || histVolumeRatio >= 0.9) {
      confirmation = gate("pass", "Momentum and relative strength confirm the move.");
    } else {
      confirmation = gate("watch", "Momentum is improving, but volume confirmation is light.");
    }
  } else if (momentumScore >= 55 || relativeStrengthScore >= 55) {
    confirmation = gate("watch", "Momentum is building, but confirmation is incomplete.");
  ...
  if (expectationRisk >= 72 || extensionRisk >= 72 || lateChaseRisk >= 72 || penalty >= 85) {
    risk = gate("fail", "Risk is too high: extended, crowded, or low-quality setup.");
  } else if (expectationRisk >= 55 || extensionRisk >= 55 || lateChaseRisk >= 55 || penalty >= 65) {
  ...
  if (!buyNowAllowed && risk.status !== "fail") {
    risk = gate("watch", `${archetype} is not allowed to be Buy Now in this model.`);
  }
...
function decideFromGates(row = {}) {
  ...
  const buyNow =
    ...
    m.triggerScore >= 70 &&
    m.momentumScore >= 55 &&
    m.freshBreakoutScore >= 65 &&
    m.expectationRisk <= 60 &&
    m.extensionRisk <= 60 &&
    m.lateChaseRisk <= 58;
  ...
  const watch =
    failCount <= 1 &&
    pass(gates.tradability.status) &&
    (gateRank(gates.trend.status) >= 1 || gateRank(gates.trigger.status) >= 1) &&
    (m.score >= 52 || m.triggerScore >= 52 || m.momentumScore >= 50 || m.freshBreakoutScore >= 55) &&
    m.expectationRisk <= 82 &&
    m.extensionRisk <= 85 &&
    m.lateChaseRisk <= 88;
...
function strongestContext(row = {}, decision = {}) {
  ...
  if (label === "BUY NOW") {
    ...
    if (m.triggerScore >= 82 && m.freshBreakoutScore >= 75) return "Fresh breakout setup";
  ...
  if (gates.confirmation?.status === "fail") return "Relative strength deteriorating";
...
function buildEntry(row = {}, recommendation = {}) {
  ...
  if (label === "WATCH") {
    if (miss?.key === "trigger") return "Watch only. Needs a cleaner trigger before buying.";
    if (miss?.key === "confirmation") return "Watch only. Needs stronger momentum, volume, or relative-strength confirmation.";
    if (miss?.key === "trend") return "Watch only. Needs trend alignment before buying.";
    if (miss?.key === "risk") return "Watch only. Risk is elevated; wait for a better entry or reset.";
    if (miss?.key === "tradability") return "Watch only. Liquidity or data quality is not clean enough.";
