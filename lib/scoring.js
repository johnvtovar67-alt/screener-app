...
function language(row, label, pressure, fairValue, tradeReadiness) {
  ...
  if (tradeReadiness?.label === "TRADE READY" && label === "STRONG BUY") {
    return {
      reason: "Trade-ready setup: pressure, value, and timing are aligned.",
  ...
  if (label === "STRONG BUY") {
    return {
      ...
      entryNote: "Buy now is reasonable for a starter position. Do not chase if it spikes further.",
    };
  }
  ...
  if (label === "BUY") {
    if (pressure === "GREEN" && fairValue !== "GREEN") {
      return {
        ...
        entryNote: "Watch for entry. Prefer pullback, consolidation, or volume confirmation.",
      };
    }
    ...
    if (fairValue === "GREEN" && pressure !== "GREEN") {
      return {
        ...
        entryNote: "Watch for entry. Add only if pressure improves.",
      };
    }
    ...
    if (above50) {
      return {
        ...
        entryNote: "Watch for entry. Best add comes on breakout or volume confirmation.",
      };
    }

    return {
      reason: "Setup is improving, but confirmation is incomplete.",
      entryNote: "Watch for entry. Keep position size small until confirmation improves.",
    };
  }
