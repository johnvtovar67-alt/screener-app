...
function getRiskLevel(expectationRisk, extensionRisk, penalty) {
  const points = expectationRisk * 0.35 + extensionRisk * 0.45 + penalty * 0.2;

  if (points >= 60) return { label: "High", score: Math.round(points), tone: "red" };
  if (points >= 35) return { label: "Medium", score: Math.round(points), tone: "yellow" };

  return { label: "Low", score: Math.round(points), tone: "green" };
}
...
function getConfidenceLevel(
  ...
  if (points >= 75) return { label: "High", score: points, tone: "green" };
  if (points >= 50) return { label: "Medium", score: points, tone: "yellow" };

  return { label: "Low", score: points, tone: "red" };
}
...
function getContextTone(label, context) {
  ...
  if (cleanContext.includes("biotech") || cleanContext.includes("binary")) return "yellow";
...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  if (!passesInstitutionalFilter(row)) return "Fails filter";
  if (archetype === "Biotech / Binary Event" && triggerScore >= 70) return "Binary risk";

  if (extensionRisk >= 60 || (pctFrom50 != null && pctFrom50 > 28)) return "Extended";
  if (expectationRisk >= 55) return "High expectations";

  if (label === "BUY NOW" && freshBreakoutScore >= 75) return "Fresh breakout";
  if (label === "BUY NOW") return "Clean setup";

  if (label === "BUY" && freshBreakoutScore >= 70) return "Early breakout";
  if (label === "BUY" && triggerScore >= 78) return "Strong trigger";
  if (label === "BUY") return "Constructive setup";

  if (label === "WATCH FOR ENTRY" && triggerScore >= 78) return "Strong trigger";
  if (label === "WATCH FOR ENTRY" && momentumScore >= 55) return "Momentum building";

  if (!above50 || !above200) return "Trend not aligned";
  if (relativeStrengthScore < 50) return "Lagging";
  if (penalty >= 45) return "Risk controls";
...
function buildDynamicReason(row = {}, recommendation = {}) {
  ...
  const context = recommendation.context || buildInstitutionalContext(row, recommendation);
...
function buildDynamicEntry(row = {}, recommendation = {}) {
  ...
  if (label === "BUY NOW") return "Actionable now under the current rules. Use normal risk control.";
  if (label === "BUY") return "Starter-size only. Add only if the setup strengthens.";
  if (label === "WATCH FOR ENTRY") return "Watch for better confirmation before buying.";
...
export function getRecommendation(row = {}) {
  ...
  const buyNowAllowed = archetypeBuyNowAllowed(row);
  ...
  return {
    ...
    context,
    contextTone,
    reason: buildDynamicReason(row, { ...recommendation, context }),
    entryNote: buildDynamicEntry(row, { ...recommendation, context }),
    scoreTone: score >= 75 ? "green" : score >= 60 ? "yellow" : "red",
    triggerTone: triggerScore >= 85 ? "green" : triggerScore >= 70 ? "yellow" : "red",
    momentumTone: getMomentumLabel(row) === "Strong" ? "green" : getMomentumLabel(row) === "Building" ? "yellow" : "red",
    expectationTone: expectationRisk <= 25 ? "green" : expectationRisk <= 45 ? "yellow" : "red",
    setupTone: setupGrade === "A" ? "green" : setupGrade === "B" || setupGrade === "B-" ? "yellow" : "red",
