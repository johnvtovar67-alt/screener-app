...
function qualityScore(row = {}) {
  ...
  if (eps != null) score += eps > 0 ? 10 : -6;

  if (pe != null) {
    if (pe > 0 && pe <= 25) score += 8;
    else if (pe > 25 && pe <= 45) score += 3;
...
function triggerScore(row = {}) {
  ...
  if (price != null && ma50 != null) score += price > ma50 ? 12 : -7;
  if (price != null && ma200 != null) score += price > ma200 ? 8 : -5;

  if (day != null) {
    if (day > 10) score += 5;
    else if (day > 5) score += 14;
    else if (day > 2) score += 11;
    else if (day > 0.5) score += 8;
  ...
  if (volumeRatio != null) {
    ...
    else if (volumeRatio < 0.6) score -= 4;
  }
...
export function getRecommendation(row = {}) {
  ...
  const asymmetry = row.asymmetryScore ?? asymmetryScore(row);
  const quality = row.qualityScore ?? qualityScore(row);
  const score = compositeScore({ ...row, triggerScore: trigger, asymmetryScore: asymmetry, qualityScore: quality });

  const day = num(row.dayChangePct);

  if (score >= 76 && trigger >= 68 && day != null && day > 2) {
    return {
      label: "STRONG BUY",
      score,
      reason: "BUY NOW: strong momentum with favorable upside.",
      entryNote: "BUY NOW starter. Add only if strength holds.",
    };
  }

  if (score >= 68 && trigger >= 60) {
    return {
      label: "BUY",
      score,
      reason:
        day != null && day < 0
          ? "Good setup, but wait for green confirmation."
          : "Actionable starter setup with upside potential.",
      entryNote:
        day != null && day < 0
          ? "WAIT: recheck when price turns green."
          : "Starter acceptable now. Add only on confirmation.",
    };
  }
  ...
  if (score >= 60) {
    return {
      label: "WATCH",
      score,
      reason: "Setup forming, but not ready yet.",
      entryNote: "Wait for stronger price action.",
    };
  }
