...
function historicalCleanEntry(row = {}) {
  ...
  return (
    score >= 74 &&
    (resistance == null || resistance <= 3.5) &&
    (momentum5 == null || momentum5 >= 0.25) &&
    (momentum10 == null || momentum10 >= -1) &&
    (slope == null || slope >= -0.25) &&
    (volumeRatio == null || volumeRatio >= 0.85)
  );
}
...
export function calcFreshBreakoutScore(row = {}) {
  ...
  if (resistance != null) {
    ...
    else if (resistance > 6) score -= 18;
  }

  if (momentum5 != null) {
    if (momentum5 > 1) score += 6;
    else if (momentum5 < -1.5) score -= 14;
  }

  if (slope != null) {
    if (slope > 0.75) score += 6;
    else if (slope < -1.25) score -= 14;
  }
...
function riskPenalty(row = {}) {
  ...
  if (resistance != null && resistance > 6) penalty += 12;
...
export function calcTechnicalScore(row = {}) {
  ...
  if (histScore != null) {
    ...
    else if (histScore >= 74) score += 8;
    else if (histScore < 58) score -= 16;
  }

  if (resistance != null && resistance > 6) score -= 12;
...
function getConfidenceLevel(
  ...
  if (histScore != null) {
    ...
    else if (histScore >= 74) points += 10;
...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  if (label === "BUY" && freshBreakoutScore >= 68) return "Clean entry";
  if (label === "BUY" && triggerScore >= 74) return "Clean entry";
