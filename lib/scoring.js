...
function calcTechnicalScore(stock = {}) {
  ...
  if (price > 0 && fiftyDay > 0) {
    ...
    else if (vs50 > 18 && vs50 <= 24) score += 1;
    else if (vs50 > 24) score -= 8;
  ...
  if (changePct !== null) {
    ...
    else if (changePct > 5 && changePct <= 10) score += 4;
    else if (changePct > 10) score -= 8;
...
function calcMomentumScore(stock = {}) {
  ...
  if (price > 0 && fiftyDay > 0) {
    const vs50 = ((price - fiftyDay) / fiftyDay) * 100;
    score += scoreRange(vs50, -8, 18) * 0.28 - 14;
  }

  if (price > 0 && twoHundredDay > 0) {
    const vs200 = ((price - twoHundredDay) / twoHundredDay) * 100;
    score += scoreRange(vs200, -15, 35) * 0.22 - 11;
  }

  if (price > 0 && yearHigh > 0 && yearLow > 0 && yearHigh > yearLow) {
    const rangePosition = ((price - yearLow) / (yearHigh - yearLow)) * 100;
    score += scoreRange(rangePosition, 25, 88) * 0.22 - 11;
  }

  if (changePct !== null) {
    if (changePct > 0 && changePct <= 6) score += 12;
    else if (changePct > 6 && changePct <= 10.5) score += 8;
    else if (changePct > 10.5) score -= 3;
...
function calcTriggerScore(stock = {}) {
  ...
  if (price > 0 && fiftyDay > 0) {
    ...
    else if (vs50 > 12 && vs50 <= 22) score += 10;
  ...
  if (changePct !== null) {
    ...
    else if (changePct > 5 && changePct <= 10.5) score += 8;
    else if (changePct > 10.5) score -= 8;
  ...
  if (volume > 0 && avgVolume > 0) {
    ...
    if (relativeVolume >= 1.1 && relativeVolume <= 3.5) score += 8;
    else if (relativeVolume >= 0.75 && relativeVolume < 1.1) score += 2;
    else if (relativeVolume > 3.5) score += 1;
...
function calcRiskScore(stock = {}) {
  ...
  if (changePct !== null) {
    if (Math.abs(changePct) >= 12) risk += 16;
    else if (Math.abs(changePct) >= 8) risk += 8;
  ...
  if (price > 0 && fiftyDay > 0) {
    ...
    if (extended > 28) risk += 18;
    else if (extended > 22) risk += 10;
    else if (extended > 15) risk += 5;
  }

  if (price > 0 && yearHigh > 0 && yearLow > 0 && yearHigh > yearLow) {
    const rangePosition = ((price - yearLow) / (yearHigh - yearLow)) * 100;

    if (rangePosition > 98) risk += 12;
    else if (rangePosition > 94) risk += 4;
...
function hasBreakoutStructure(stock = {}) {
  ...
  if (changePct !== null) {
    ...
    if (changePct > 10.75) return false;
  }

  if (vs50 !== null) {
    if (vs50 < 0) return false;
    if (vs50 > 24) return false;
  }

  if (vs200 !== null && vs200 < -2) return false;

  if (rangePosition !== null && rangePosition > 98) return false;

  if (triggerScore < 64) return false;
  if (momentumScore < 60) return false;
...
function getStage(stock = {}) {
  ...
  if (above50 && above200 && fiftyAbove200) {
    if (rangePosition !== null && rangePosition > 94) return "Extended Uptrend";
    return "Uptrend";
  }
...
function evaluateTriggerGate(stock = {}) {
  ...
  if (triggerScore >= 72 || hasBreakoutStructure(stock)) {
    return {
      status: "PASS",
...
function evaluateRiskGate(stock = {}) {
  ...
  if (price > 0 && fiftyDay > 0) {
    ...
    if (extended > 24 && !hasBreakoutStructure(stock)) {
      return {
        status: "WATCH",
...
function isStarterCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (gateSummary.confirmation?.status === "FAIL" && !breakoutStructure) {
    return false;
  }
...
function getDominantReason(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  if (hasBreakoutStructure(stock)) {
    return `Breakout structure is improving; starter entry only near ${formatPrice(
...
function getTradeReadiness(stock = {}) {
  ...
  if (!blocked && score >= 88 && corePass && risk <= 52) {
    return "Buy Now";
  }
