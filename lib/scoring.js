...
function trendData(row = {}) {
  ...
  return {
    ...
    above200: price != null && ma200 != null && ma200 > 0 ? price > ma200 : null,
...
function trueTradabilityFailure(row = {}) {
  ...
  // Do NOT fail a name because FMP is missing marketCap, avgVolume, or moving averages.
  // That was the bug causing NVDA / AMZN / GOOG / MSFT to show "Fails filter."
...
function archetypeRiskPenalty(row = {}) {
  ...
  // Biotech is capped below Buy Now, but not punished in ranking just for being biotech.
  return 0;
}
