...
function getChangePct(stock = {}) {
  ...
  const rawChange = pick(stock, ["change", "dayChange", "priceChange", "regularMarketChange"], null);
  const dollarChange = num(rawChange, null);

  // Quote providers sometimes return percentage fields in mixed units or stale
  // formats. The safest source is the dollar change versus the implied prior
  // close. This prevents normal +0.67% moves from displaying/scoring as +67%.
  if (price > 0 && dollarChange !== null) {
    const impliedPrevious = price - dollarChange;
    if (impliedPrevious > 0) {
      const derived = (dollarChange / impliedPrevious) * 100;
      if (Number.isFinite(derived) && Math.abs(derived) <= 40) return derived;
    }
  }

  if (price > 0 && prev > 0) {
    const derived = ((price - prev) / prev) * 100;
    if (Number.isFinite(derived) && Math.abs(derived) <= 40) return derived;
  }
  ...
  if (direct !== null && Math.abs(direct) <= 40) return direct;
  return null;
}
...
function calcTriggerScore(stock = {}) {
  ...
  // Trigger should be scarce. Trend earns points, but it should not create a
  // 90-100 trigger by itself. A true trigger needs a good spot, constructive
  // daily action, and preferably volume confirmation.
  if (vs50 !== null) {
    if (vs50 >= 1 && vs50 <= 8) score += 18;
    else if (vs50 > 8 && vs50 <= 15) score += 10;
    else if (vs50 > 15 && vs50 <= 24) score += 3;
    else if (vs50 > 24) score -= 12;
    else if (vs50 >= -2) score += 2;
    else score -= 14;
  }

  if (vs200 !== null) {
    if (vs200 >= 4 && vs200 <= 45) score += 8;
    else if (vs200 >= 0) score += 4;
    else if (vs200 >= -5) score -= 3;
    else score -= 12;
  }

  if (range !== null) {
    if (range >= 60 && range <= 88) score += 8;
    else if (range > 88 && range <= 94) score += 3;
    else if (range > 94) score -= 6;
    else if (range < 35) score -= 8;
  }

  if (change !== null) {
    if (change >= 0.8 && change <= 4.5) score += 8;
    else if (change > 4.5 && change <= 7) score += 2;
    else if (change > 7) score -= 7;
    else if (change < -3.5) score -= 8;
  }

  if (relVol !== null) {
    if (relVol >= 1.6) score += 10;
    else if (relVol >= 1.15) score += 6;
    else if (relVol >= 0.85) score += 2;
...
function compositeScore(stock = {}) {
  ...
  // Final model balance:
  // - Technical structure and relative strength lead the model.
  // - Momentum matters, but a single hot day should not create a Buy by itself.
  // - Fundamentals are a secondary quality check, not a hidden industry gate.
  // - Trigger is used for timing/scarcity, but does not dominate the composite.
  const score =
    technical * 0.35 +
...
function getTradeReadiness(stock = {}) {
  ...
  // Buy must be scarce. It requires true leadership plus acceptable timing.
  // This replaces subjective ticker/category caps with transparent score gates.
  if (
    score >= 86 &&
    technical >= 82 &&
    relativeStrength >= 80 &&
  ...
  // Starter is for constructive setups that are not yet full-size Buys.
  if (
    score >= 73 &&
    technical >= 66 &&
    relativeStrength >= 64 &&
  ...
  if (score >= 55 || trigger >= 52 || momentum >= 52 || technical >= 52) {
    return "Watch";
  }
...
function getDominantReason(stock = {}) {
  ...
  if (label === "Buy") {
    if (relativeStrength >= 82 && momentum >= 76) return "Leadership, trend, momentum, and trigger are aligned.";
    return "High-conviction setup: trend, momentum, relative strength, trigger, and risk are aligned.";
  }
