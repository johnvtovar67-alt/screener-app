...
function isBiotechPlatformSymbol(stock) {
  const symbol = getSymbol(stock);

  // Quote-only data often does not include industry/description, so use a
  // conservative symbol cap for volatile biotech/platform names that should not
  // graduate to a full-size Buy from price action alone. They can still qualify
  // as Starter when momentum improves.
  return [
    "MRNA",
    "NVAX",
    "CRSP",
...
function isAirlineSymbol(stock) {
  return ["AAL", "UAL", "DAL", "LUV"].includes(getSymbol(stock));
}
...
function isBinaryBiotech(stock) {
  ...
  const biotechLike =
    ...
    isBiotechPlatformSymbol(stock);

  if (!biotechLike) return false;

  // True binary biotech should be heavily constrained. Larger platform names
  // such as MRNA are not full-size Buy candidates by default, but they should
  // still be allowed to surface as Starter/Watch when the chart improves.
  return marketCap < 2000000000 || price < 10;
}

function getCategoryRiskNote(stock) {
  // Category context is informational only. It should never hide a name or
  // mechanically cap a stock that otherwise earns a signal from fundamentals,
  // technicals, momentum, liquidity, and risk.
  if (isReit(stock)) return "REIT structure; size for rate sensitivity and income-vehicle risk.";
  if (isPipelineOrIncomeName(stock)) {
    return "Income or pipeline-style structure; size for rate sensitivity and lower upside velocity.";
  }
  if (isAirline(stock) || isAirlineSymbol(stock)) {
    return "Airline cyclicality; size for macro and fuel-cost risk.";
  }
  if (isBinaryBiotech(stock)) {
    return "Binary biotech risk; confirm catalyst and trial/approval exposure before sizing.";
  }
  if (isBiotechPlatformSymbol(stock)) {
    return "Biotech/platform volatility; confirm the business thesis before sizing.";
  }

  return "";
}

function isFullBuyQualityBlocked(stock) {
  // No subjective category caps. Buy eligibility is decided by quantitative
  // scores and hard data gates, not by ticker or industry labels.
  return false;
}

function isStarterQualityBlocked(stock) {
  // No subjective category caps. A volatile name may still be Starter if the
  // setup earns it, and it may still be Avoid if the data is weak.
  return false;
}

function isBlockedBuyNowArchetype(stock) {
  // Kept for backwards compatibility with older scoring references. It no
  // longer blocks recommendations.
  return false;
}

function getBlockedReason(stock) {
  // Do not let category notes become the dominant reason or hide opportunities.
  return "";
}
...
function isBreakoutBuyCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  const fiftyDay = getFiftyDay(stock);
  const twoHundredDay = getTwoHundredDay(stock);
  const starterBlocked = isStarterQualityBlocked(stock);
  const digitalAssetTrend = isDigitalAssetProxy(stock) && isConstructiveMomentum(stock);

  if (starterBlocked) return false;
  if (price <= 0) return false;
  if (fiftyDay <= 0 || price <= fiftyDay) return false;
  if (twoHundredDay > 0 && price <= twoHundredDay) return false;
...
function isBuyImmediatelyCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  // Elite all-clear. No ticker/category caps, but it must earn the signal on
  // objective quality, trend, trigger, momentum, and risk.
  if (score < 92) return false;
  if (technical < 74) return false;
  if (quality < 56) return false;
  if (trigger < 84) return false;
  if (momentum < 78) return false;
  if (risk > 58) return false;
  if (freshBreakoutScore < 74) return false;
  if (stage === "Downtrend") return false;

  if (vs50 !== null && (vs50 < 1 || vs50 > 22)) return false;
  if (rangePosition !== null && rangePosition < 60) return false;

  if (volume > 0 && avgVolume > 0) {
    const relativeVolume = volume / avgVolume;
    if (volume < 500000) return false;
    if (relativeVolume < 0.8) return false;
  }
...
function isBuyNowCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  // Full-size Buy is stricter than Starter, but not by subjective sector caps.
  // It must be a confirmed, liquid, non-broken setup with enough objective
  // quality to justify normal sizing.
  if (gateSummary.tradability?.status === "FAIL") return false;
  if (gateSummary.trend?.status === "FAIL") return false;
  if (gateSummary.trigger?.status === "FAIL") return false;
  if (gateSummary.confirmation?.status === "FAIL") return false;
  if (gateSummary.risk?.status === "FAIL" && !digitalAssetTrend) return false;
  if (gateSummary.tradability?.status !== "PASS") return false;
  if (gateSummary.trend?.status !== "PASS") return false;

  const maxRisk = digitalAssetTrend ? 68 : 60;
  const maxVs50 = digitalAssetTrend ? 34 : 24;

  if (score < 86) return false;
  if (technical < 70) return false;
  if (quality < 54) return false;
  if (trigger < 76) return false;
  if (momentum < 70) return false;
  if (risk > maxRisk) return false;
  if (stage === "Downtrend") return false;
  if (stage === "Pullback" && !breakoutStructure) return false;

  const triggerConfirmed = gateSummary.trigger?.status === "PASS" || breakoutStructure;
  if (!triggerConfirmed) return false;

  if (gateSummary.confirmation?.status === "WATCH") {
    if (score < 88 || trigger < 78 || momentum < 72) return false;
  }

  if (changePct !== null) {
    if (changePct < 0.35) return false;
    if (changePct > 8.5 && !digitalAssetTrend) return false;
  }

  if (vs50 !== null) {
    if (vs50 < 0.5) return false;
    if (vs50 > maxVs50) return false;
  }

  if (vs200 !== null && vs200 < 0) return false;
  if (rangePosition !== null) {
    if (rangePosition < 55) return false;
    if (rangePosition > 99 && !breakoutStructure) return false;
  }
