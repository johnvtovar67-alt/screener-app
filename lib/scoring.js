...
function isBuyImmediatelyCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  // This bucket should be rare. It requires elite score, strong trigger,
  // strong momentum, low risk, and a fresh breakout profile.
  if (score < 94) return false;
  if (trigger < 88) return false;
  if (momentum < 82) return false;
  if (risk > 55) return false;
  if (freshBreakoutScore < 80) return false;

  if (vs50 !== null && (vs50 < 0 || vs50 > 18)) return false;
  if (rangePosition !== null && rangePosition < 58) return false;
...
function isBuyNowCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  // Buy Now is full-size eligible, so it is still meaningfully harder than
  // Breakout Buy. It should be uncommon, not impossible.
  if (score < 90) return false;
  if (trigger < 80) return false;
  if (momentum < 75) return false;
  if (risk > (digitalAssetTrend ? 68 : 62)) return false;

  if (vs50 !== null && vs50 > (digitalAssetTrend ? 32 : 28)) return false;
...
function buildEntryNote(label, dominantReason, stock = {}) {
  ...
  if (dominantReason.includes("old 200-day") || dominantReason.includes("Longer-term trend is broken")) {
    return `Avoid for now. Do not wait for a distant old moving average as the entry. Recheck only after it builds a base and clears near-term resistance around ${formatPrice(
      breakoutPrice
    )} with strength.`;
  }

  if (dominantReason.includes("200-day")) {
    return `Avoid for now. Let the chart rebuild first; a usable tactical trigger would be nearer-term strength around ${formatPrice(
      breakoutPrice
    )}, not simply waiting for the old 200-day.`;
  }
