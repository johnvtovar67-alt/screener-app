...
function isBinaryBiotech(stock) {
  ...
  // Small or low-priced biotech names are outright binary. Larger platform names
  // such as MRNA are still capped from full Buy by the symbol safeguard above.
  return marketCap < 2000000000 || price < 10 || isBiotechPlatformSymbol(stock);
}
...
function isFullBuyQualityBlocked(stock) {
  return (
    ...
    isAirline(stock) ||
    isAirlineSymbol(stock) ||
    isBinaryBiotech(stock)
  );
}

function isBlockedBuyNowArchetype(stock) {
  return isFullBuyQualityBlocked(stock);
}

function getBlockedReason(stock) {
  if (isReit(stock)) return "REIT structure; capped at Starter.";
  if (isPipelineOrIncomeName(stock)) {
    return "Income or pipeline-style name; capped at Starter.";
  }
  if (isAirline(stock) || isAirlineSymbol(stock)) {
    return "Airline cyclicality; capped at Starter.";
  }
  if (isBinaryBiotech(stock)) {
    return "Biotech/platform risk; capped at Starter unless fundamentals are separately reviewed.";
  }
...
function isBuyNowCandidate(stock = {}, gateSummary = buildGateSummary(stock)) {
  ...
  // Full Buy requires business/category quality, not just a green technical day.
  // Biotech/platform, airline, REIT, and income structures can still be Starter,
  // but they should not be the screener's full-size Buy signal.
  if (isBlockedBuyNowArchetype(stock)) return false;
  if (price <= 0) return false;
  if (fiftyDay <= 0 || price <= fiftyDay) return false;
  if (twoHundredDay > 0 && price <= twoHundredDay) return false;

  // Full-size Buy can never be based on broken plumbing. These hard fails keep
  // Buy selective without making it mythical.
  if (gateSummary.tradability?.status === "FAIL") return false;
  if (gateSummary.trend?.status === "FAIL") return false;
  if (gateSummary.trigger?.status === "FAIL") return false;
  if (gateSummary.confirmation?.status === "FAIL") return false;
  ...
  // Buy = normal intended position size is allowed. It should be uncommon, but
  // not impossible. Starter remains the bucket for early or less-confirmed setups.
  const maxRisk = digitalAssetTrend ? 70 : 64;
  const maxVs50 = digitalAssetTrend ? 36 : 26;

  if (score < 82) return false;
  if (trigger < 72) return false;
  if (momentum < 66) return false;
  ...
  // Confirmation may be WATCH when the raw setup is very strong, but it cannot be weak.
  if (gateSummary.confirmation?.status === "WATCH") {
    if (score < 86 || trigger < 76 || momentum < 70) return false;
  }
