...
function scoreMomentum(d) {
  ...
  if (d.price > d.sma20) score += 10;
  if (d.price > d.sma50) score += 10;
...
export function getStage(d) {
  if (d.price < d.sma50) return "Broken";

  if (d.price > d.sma20 && d.relativeVolume20d > 1.3) {
    return "Emerging";
  }

  if (d.price > d.sma20 && d.return20d > 0.15) {
    return "Extended";
  }
