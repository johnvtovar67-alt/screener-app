...
export function getRecommendation(row = {}) {
  ...
  const badFundamentals =
    hasFundamentalData &&
    ((eps != null && eps < 0 && quality < 58) ||
      (pe != null && (pe <= 0 || pe > 100) && quality < 58));

  const frothyMove = day != null && day > 15;
  ...
  if (
    ...
    return {
      ...
      score: totalScore,
      reason:
        trigger >= 80 || (relVol != null && relVol >= 1.5)
          ? "Breakout-level momentum with strong asymmetry and acceptable fundamentals."
          : "High-conviction setup with strong alignment across key factors.",
      entryNote:
        price != null && ma50 != null
          ? `Actionable now. Consider scaling in carefully; 50DMA support near $${ma50.toFixed(2)}.`
          : "Actionable now. Consider scaling in carefully.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      score: totalScore,
      reason:
        trigger >= 70 && asymmetry >= 65
          ? "Momentum and upside are aligned, but not fully confirmed."
          : asymmetry >= 65
          ? "Strong upside potential, but momentum is still developing."
          : "Technically stable, but lacks a strong catalyst or full confirmation.",
      entryNote:
        price != null && ma50 != null
          ? `Starter position only. Better entry near 50DMA around $${ma50.toFixed(2)} or breakout above $${price.toFixed(2)}.`
          : "Starter position only. Add if price, volume, or fundamentals confirm.",
    };
  }

  if (totalScore >= 58 || trigger >= 52 || asymmetry >= 58 || quality >= 58) {
    return {
      label: "WATCH",
      score: totalScore,
      reason: badFundamentals
        ...
        : frothyMove
        ? "Big move already happened; wait for a better entry or confirmation."
