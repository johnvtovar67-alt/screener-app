...
function getRiskLevel(expectationRisk, extensionRisk, penalty) {
  ...
  if (points >= 60) {
    return { label: "High", score: Math.round(points), tone: "red" };
  }

  if (points >= 35) {
    return { label: "Medium", score: Math.round(points), tone: "yellow" };
  }
...
function buildInstitutionalContext(row = {}, recommendation = {}) {
  ...
  if (label === "WATCH FOR ENTRY" && triggerScore >= 78) {
...
export function getRecommendation(row = {}) {
  ...
  return {
    ...
    triggerTone:
      triggerScore >= 85 ? "green" : triggerScore >= 70 ? "yellow" : "red",
    momentumTone:
      getMomentumLabel(row) === "Strong"
        ...
        : getMomentumLabel(row) === "Building"
          ...
          : "red",
    expectationTone:
      expectationRisk <= 25 ? "green" : expectationRisk <= 45 ? "yellow" : "red",
    setupTone:
      setupGrade === "A" || setupGrade === "B" || setupGrade === "B-"
        ? setupGrade === "A"
          ? "green"
          : "yellow"
        : "red",
