...
function calcEntryQualityScore(stock = {}) {
  ...
  let score = trigger * 0.40 + technical * 0.34 + (100 - extension) * 0.20 + (100 - risk) * 0.06;

  // Prevent the engine from over-rotating into names only after a sharp pop.
  // Big moves can confirm leadership, but they lower entry quality for fresh capital.
  if (day !== null && day > 10) score -= 18;
  else if (day !== null && day > 7) score -= 10;
  else if (day !== null && day > 5) score -= 4;

  if (vs50 !== null && vs50 > 22) score -= 12;
  else if (vs50 !== null && vs50 > 16) score -= 6;
...
function getEntryQualityLabel(stock = {}) {
  ...
  // This is the chase-control layer. A large green day or a stock stretched
  // far above short-term support can confirm leadership, but it should not
  // automatically become a full-size fresh-money Buy.
  if ((day !== null && day >= 10) || extension >= 72 || (vs50 !== null && vs50 > 22)) {
    return "Extended";
  }

  if ((day !== null && day >= 6.5) || extension >= 62 || (vs50 !== null && vs50 > 16)) {
    return "Chase Risk";
  }

  if (stage === "Pullback") return "Pullback Entry";
  if (stage === "Base / Repair" || trigger < 60 || momentum < 55) return "Early Setup";

  return "Clean Entry";
}

function getEntryQualityRank(stock = {}) {
  const label = getEntryQualityLabel(stock);
...
function getBaseRiskPlanPrices(stock = {}) {
  const price = getPrice(stock);
  ...
  if (price <= 0) {
    return {
      ...
      stopLossPct: null,
      downsideToReviewPct: null,
      upsideToFirstTrimPct: null,
      payoffRatio: null,
      tradeQuality: "Unavailable",
      tradeQualityLabel: "Unavailable",
      tradeQualityRank: 0,
    };
  }
  ...
  // Trading-account profit review should not trigger too quickly unless the
  // chart is already near major resistance. Use the realistic nearby high if it
  // is at least ~10% away; otherwise require a minimum profit-review zone.
  let firstTrimPrice = price * 1.18;
  if (yearHigh > price * 1.10 && yearHigh <= price * 1.35) {
    firstTrimPrice = yearHigh;
  }

  const stretchTargetPrice = Math.max(firstTrimPrice * 1.1, price * 1.32);
  const downsideToReviewPct = ((price - invalidationPrice) / price) * 100;
  const stopLossPct = downsideToReviewPct;
  const upsideToFirstTrimPct = ((firstTrimPrice - price) / price) * 100;
  const payoffRatio = downsideToReviewPct > 0 ? upsideToFirstTrimPct / downsideToReviewPct : null;

  let tradeQuality = "Poor";
  let tradeQualityRank = 0;
  if (payoffRatio >= 3) {
  ...
  return {
    ...
    payoffRatio,
    tradeQuality,
    tradeQualityLabel: tradeQuality,
    tradeQualityRank,
  };
}
...
function buildGateSummary(stock = {}) {
  ...
  const tradeQualityPlan = getBaseRiskPlanPrices(stock);
  const tradeQualityRank = tradeQualityPlan.tradeQualityRank;
  const entryQualityLabel = getEntryQualityLabel(stock);
  const entryQualityRank = getEntryQualityRank(stock);
  const sharpMove = getChangePct(stock);
  const isChaseRisk = entryQualityLabel === "Chase Risk";
  const isExtended = entryQualityLabel === "Extended";
  const cleanEnoughForBuy = ["Clean Entry", "Pullback Entry"].includes(entryQualityLabel) && entryQualityScore >= 62;
  const exceptionalButStretchedStarter =
    isExtended &&
    tradeQualityRank >= 3 &&
    score >= 82 &&
    leadershipScore >= 75 &&
  ...
  return {
    ...
    institutionalPass,
    tradeQualityRank,
    tradeQuality: tradeQualityPlan.tradeQuality,
    payoffRatio: tradeQualityPlan.payoffRatio,
    buyEligible:
      institutionalPass &&
      tradeQualityRank >= 2 &&
      cleanEnoughForBuy &&
    ...
    starterEligible:
      (institutionalPass &&
        ...
        riskScore <= 82) ||
      exceptionalButStretchedStarter,
  };
}
...
function buildRiskPlan(stock = {}, label = getTradeReadiness(stock)) {
  const price = getPrice(stock);
  const base = getBaseRiskPlanPrices(stock);
  const {
    invalidationPrice,
    addAbovePrice,
    firstTrimPrice,
  ...
  if (price <= 0) {
    return {
      ...base,
      summary: "No position plan available without a usable quote.",
    };
  }

  let summary = "Define risk before entry.";
  if (label === "Buy") {
    summary = `Buyable now. Review below ${formatPrice(invalidationPrice)}; review gains above ${formatPrice(firstTrimPrice)}.`;
    ...
    summary = `Avoid until the setup improves; no profit review is useful until the thesis repairs.`;
  }
...
function getDominantReason(stock = {}, gate = buildGateSummary(stock)) {
  ...
  if (gate.entryQualityLabel === "Extended") return "Leadership is present, but the entry is stretched after a sharp move; wait for a pullback or consolidation.";
  if (gate.entryQualityLabel === "Chase Risk") return "Leadership is improving, but the stock has already moved enough today that full-size fresh capital would be chasing.";
  ...
  if (gate.tradeQualityRank === 0) return "Trade quality is poor; reward does not justify fresh capital against the downside review level.";
  if (gate.tradeQualityRank === 1) return "Trade quality is thin; acceptable only as a smaller position unless conviction improves.";
...
function buildActionSummary(label, dominantReason, score, stock = {}) {
  const entry = getEntryQualityLabel(stock);
  if (label === "Buy") return "High-conviction opportunity with clean enough entry. Normal position size is appropriate.";
  if (label === "Starter") {
    if (entry === "Chase Risk" || entry === "Extended") return "Leadership is confirmed, but entry is stretched. Keep size small and wait for digestion.";
    return "Small position is acceptable. Upgrade only after confirmation.";
  }
...
function buildEntryNote(label, dominantReason, stock = {}) {
  ...
  if (label === "Starter") {
    if (entry === "Chase Risk" || entry === "Extended") {
      return `Starter size only. Leadership is confirmed, but wait for consolidation or a constructive pullback near ${formatPrice(support)} before adding.`;
    }

    return `Starter size only. Reassess after confirmation above ${formatPrice(breakout)} or a constructive hold above support.`;
  }

  if (label === "Watch") {
    if (entry === "Chase Risk" || entry === "Extended") {
      return `Monitor only. Do not chase; revisit after consolidation or a constructive pullback near ${formatPrice(support)}.`;
    }

    return `Monitor only. Revisit after a break above ${formatPrice(breakout)} or a constructive pullback to support.`;
  }
...
function buildTechnicalSnapshot(stock = {}) {
  return {
    ...
    entryQualityLabel: getEntryQualityLabel(stock),
...
function getRecommendation(stock = {}) {
  ...
  return {
    ...
    entryQualityLabel: gateSummary.entryQualityLabel,
    entryQualityRank: gateSummary.entryQualityRank,
...
function analyzeStock(stock = {}) {
  ...
  return {
    ...
    entryQualityLabel: recommendation.entryQualityLabel,
    entryQualityRank: recommendation.entryQualityRank,
