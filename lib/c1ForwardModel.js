...
export function advanceC1ForwardModel(prior, incoming, now=new Date(), initialization={}) {
  ...
  if (!prior) {
    ...
    if(!additions.length&&Array.isArray(prior.summary?.modelQueue)&&prior.summary?.combinedPortfolio?.contract==='c1-combined-holdings-v1'&&prior.paperExecutionStatus&&prior.pendingDecisionStatus&&prior.paperExecution?.priceBasis===C1_EVENT_PRICE_BASIS&&planC1PaperEvents({before:prior.ledger,after:prior.ledger,account:prior.paperExecution,eventMetadata:[]}).status==='unchanged')return prior;
