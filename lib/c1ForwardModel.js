...
export function advanceC1ForwardModel(prior, incoming, now=new Date(), initialization={}) {
  ...
  if (!prior) {
    ...
    // An explicitly new diagnostic book may prepare tomorrow's queue from an
    // already completed close. Never migrate an existing book's start window.
    if(initialization.completedCloseQueue===true && nextSession(required)===record.firstDecisionSession) {
      record.initializationContract='completed-close-queue-v1';
      record.simulationStartSession=required;
    }
  } else {
    ...
    if(prior.firstDecisionSession!==firstFutureExecutionSession(prior.sessions[0].date,prior.createdAt)) {
      const error=new Error('Forward baseline execution predates observation or identity changed; original preserved');
      error.code='C1_BASELINE_CLOCK_CONFLICT';throw error;
    }
    if(prior.simulationStartSession && (prior.initializationContract!=='completed-close-queue-v1' ||
      prior.simulationStartSession!==prior.sessions[0].date || nextSession(prior.simulationStartSession)!==prior.firstDecisionSession))
      throw new Error('Invalid diagnostic initialization window');
    ...
    if(!additions.length&&prior.summary?.modelIdentity&&Array.isArray(prior.summary?.modelQueue)&&prior.summary?.combinedPortfolio?.contract==='c1-combined-holdings-v1'&&prior.paperExecutionStatus&&prior.pendingDecisionStatus&&prior.paperExecution?.priceBasis===C1_EVENT_PRICE_BASIS&&planC1PaperEvents({before:prior.ledger,after:prior.ledger,account:prior.paperExecution,eventMetadata:[]}).status==='unchanged')return prior;
  ...
  for(const [id,weight] of Object.entries(C1_ACCOUNTING_WEIGHTS)) {
    const run=simulatePointInTimePortfolio({metadata:{},sessions},{...C1_FROZEN_OPTIONS[id],
      startDate:record.simulationStartSession||record.firstDecisionSession,endDate:through,liquidateAtEnd:false});
    if(run.trades.some(t=>t.date<record.firstDecisionSession))throw new Error('Execution before first eligible opening');
  ...
  return {...record,ledger,paperExecution,paperExecutionStatus,pendingBySleeve,pendingDecisionStatus,summary:{contract:C1_FORWARD_CONTRACT,status:'paper-only',
    sourceSessionDate:through,modelRevision:record.revision,firstDecisionSession:record.firstDecisionSession,
    modelIdentity:[record.createdAt,record.firstDecisionSession,record.simulationStartSession||'legacy-start'].join(':'),
    ...
    modelQueue:Object.entries(pendingBySleeve).flatMap(([sleeve,orders])=>orders.map((o,index)=>({sleeve,sequence:index,side:o.side,symbol:o.symbol,rank:o.rank??null,reason:o.reason??null})))}};
}
