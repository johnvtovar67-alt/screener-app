...
export function advanceC1ForwardModel(prior, incoming, now=new Date()) {
  ...
  if (!prior) {
    ...
    if(!additions.length&&prior.paperExecutionStatus)return prior;
  ...
  return {...record,ledger,paperExecution,paperExecutionStatus,summary:{contract:C1_FORWARD_CONTRACT,status:'paper-only',
    ...
    virtualShares:marked.virtualShares,sleeves,paperExecution:paperExecutionStatus}};
}
