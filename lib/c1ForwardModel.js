import {planC1PaperTransition,initializeC1PaperExecution,C1_PAPER_EXECUTION_PRICE_BASIS} from './c1ExecutionAdapter';
...
export function advanceC1ForwardModel(prior, incoming, now=new Date()) {
  ...
  if (!prior) {
    ...
    if(!additions.length&&prior.paperExecutionStatus&&prior.pendingDecisionStatus&&prior.paperExecution?.priceBasis===C1_PAPER_EXECUTION_PRICE_BASIS)return prior;
  ...
  try{
    ...
    const transition=planC1PaperTransition({before,after:ledger,account:paperExecution,prices:Object.fromEntries(sessions.at(-1).prices.map(row=>[row.symbol,row.open]))});
    paperExecutionStatus={status:transition.status,reason:transition.reason||null,
      ...
      basis:"retrospective-opening-price-paper-mapping"};
    if(transition.nextAccount)paperExecution=transition.nextAccount;
  }catch(error){paperExecutionStatus={status:"blocked",reason:error.message,proposedOrderCount:0,executable:false,basis:"retrospective-opening-price-paper-mapping"};}
