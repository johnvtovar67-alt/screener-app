...
function validateSession(session) {
  if (!session || !/^\d{4}-\d{2}-\d{2}$/.test(session.date||'') ||
      ...
      !session.prices.length || !Array.isArray(session.signals) || !Array.isArray(session.universeSymbols))
...
export function advanceC1ForwardModel(prior, incoming, now=new Date()) {
  ...
  return {...record,ledger,summary:{contract:C1_FORWARD_CONTRACT,status:'paper-only',
    ...
    virtualShares:marked.virtualShares,sleeves}};
}
