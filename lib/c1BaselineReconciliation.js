...
import { reconcileC1ReviewedSession, reconcileC1VerifiedProviderRevision } from './c1ReviewedSessionRevision';
...
export function reconcileC1UnexposedBaseline(prior, input, now, digest, advance, observations = []) {
  let providerRevisionDiagnostic;
  try {
    return reconcileC1VerifiedProviderRevision(prior,input,now,digest,advance,observations);
  } catch (error) {
    providerRevisionDiagnostic = error?.providerRevisionDiagnostic;
    // Incomplete, conflicting or economically material provider revisions keep
    // the original rejection. The historical reviewed exceptions below remain
    // available for their already-recorded immutable captures.
  }
  try {
    const reviewed = reconcileC1ReviewedSession(prior,input,now,digest,advance,observations);
  ...
  if (originalResult.paperExecutionStatus?.status === 'blocked') reject();
    ...
    if (providerRevisionDiagnostic) error.providerRevisionDiagnostic = providerRevisionDiagnostic;
    throw error;
  }
}
