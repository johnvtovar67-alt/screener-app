...
export function reconcileC1UnexposedPrices(prior, input, now, digest, advance) {
  ...
  const revised = JSON.parse(JSON.stringify(prior.model)), evidence = [];
  for (const p of changes) {
    if (exposed.has(p.symbol) || !Number.isFinite(anchors.closes[p.symbol]) || anchors.closes[p.symbol] <= 0) reject();
  ...
  return { model:originalResult, audit:{ contract:'c1-unexposed-price-reconciliation-v1',
    ...
    basis:'no-historical-or-queued-model-exposure-and-identical-economic-replay',
    comparisonHash:digest(Object.fromEntries(fields.map(k => [k,originalResult[k]]))),
