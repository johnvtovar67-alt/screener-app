...
export function reconcileC1UnexposedBaseline(prior, input, now, digest, advance) {
  if (prior.captures.length > 1) return reconcileC1UnexposedPrices(prior, input, now, digest, advance);
...
// One separately reviewed held-close correction is identified below; all other
// historical/queued exposure continues to require separate reconciliation.
export function reconcileC1UnexposedPrices(prior, input, now, digest, advance) {
  ...
  for (const p of changes) {
    ...
    if (exposed.has(p.symbol)) {
      ...
      if (recordHash !== review.priorRecordHash || p.symbol !== review.symbol ||
          ...
          bar.volume !== review.volume || ['open','high','low'].some(k => p[k] !== review[k] || bar[k] !== review[k])) reject();
      reviewedHeldCloses.push({symbol:p.symbol, observations:[...review.observations],
