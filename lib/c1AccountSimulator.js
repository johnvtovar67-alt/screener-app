// Generated by tools/generate-c1-account-simulator.cjs. Historical reference remains unchanged.
import {validateC1AccountSeed,c1SeedPosition} from './c1AccountSeed';
import {createC1ActualFillReplay} from './c1ActualFillReplay';
import {C1_ACCOUNT_MOMENTUM_EXIT_POLICY} from './c1FrozenOptions';
...
export function simulatePointInTimePortfolio(dataset = {}, options = {}, openingAccount = null, actualExecutions = null) {
  ...
  const config = {
    ...
    ...(openingAccount ? C1_ACCOUNT_MOMENTUM_EXIT_POLICY : {}),
    ...options,
  };
  ...
  const executeSale = ({
    ...
    if(actualReplay && ['portfolio-drawdown-stop','initial-stop','invalidation-stop','ratcheted-stop','profit-trailing-stop','universe-removal','delisting-outcome','universe-removal-zero-recovery'].includes(reason))pendingActualRiskExits.set(symbol,{symbol,side:'sell',reason,fraction:1});
  ...
  for (let sessionIndex = 0; sessionIndex < sessions.length; sessionIndex++) {
    ...
    if (active) {
      ...
      if(actualReplay)for(const [symbol,order] of pendingActualRiskExits){
      ...
      for (const order of buyOrders.sort((a, b) => b.rank - a.rank)) {
        ...
        if(completion){
          ...
          entryPrice: fill,
          entryCommission: executionCommission,
          realizedPnl: 0,
        ...
        for (const [symbol, position] of positions) {
          ...
          if (accountSeed) {
            ...
            const currentGainPct = position.entryPrice > 0
              ...
              : 0;
            const peakGainPct = position.entryPrice > 0
              ? (number(position.highWatermark, mark) / position.entryPrice - 1) * 100
              : 0;
            position.peakGainPct = peakGainPct;
            position.profitProtectionArmed = peakGainPct >= config.profitRankExitActivationPct;
            const earlyRuleActive =
              holdingRankVerified &&
              heldSessions >= config.earlyRankExitStartSessions &&
              heldSessions < config.earlyRankExitEndSessions;
            position.earlyRankExitStreak = earlyRuleActive && holdingRank > config.earlyRankExitThreshold
              ? number(position.earlyRankExitStreak, 0) + 1
              : 0;
            if (position.earlyRankExitStreak >= config.earlyRankExitConsecutiveSessions)
              earlyRankExitSymbols.add(symbol);
            const profitRuleActive =
              holdingRankVerified &&
              heldSessions >= config.profitRankExitStartSessions &&
              heldSessions < config.profitRankExitEndSessions &&
              peakGainPct >= config.profitRankExitActivationPct &&
              currentGainPct > 0;
            position.profitRankExitStreak = profitRuleActive && holdingRank > config.profitRankExitThreshold
              ? number(position.profitRankExitStreak, 0) + 1
              : 0;
            if (position.profitRankExitStreak >= config.profitRankExitConsecutiveSessions)
  ...
  return {
    ...
    skippedOrders,
    pendingDecisions: config.liquidateAtEnd ? [] : JSON.parse(JSON.stringify(actualReplay ? [...pending.filter(o=>o.side!=='sell'||!pendingActualRiskExits.has(o.symbol)),...pendingActualRiskExits.values()] : pending)),
