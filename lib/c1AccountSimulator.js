...
export function simulatePointInTimePortfolio(dataset = {}, options = {}, openingAccount = null, actualExecutions = null) {
  ...
  const trades = [];
  const skippedOrders = [];
  let stockRecommendations = {date:null,rows:[]};
  const curve = [];
  ...
  for (let sessionIndex = 0; sessionIndex < sessions.length; sessionIndex++) {
    ...
    stockRecommendations = {date:session.date,rows:[]};
    ...
    if (active) {
      ...
      if(accountSeed)stockRecommendations = {date:session.date,rows:rankedEligibleSignals.slice(0,rankedSelection?Math.max(rankedTargetCount,Math.floor(number(config.rankedEntryQueueCount,rankedTargetCount*3))):undefined).map((signal,index)=>({
        symbol:symbolOf(signal),priority:index,side:'buy',sector:sectorOf(signal),signal,
        action:expandedResearchSource(config)?(signal.entryTiming?.strongPass===true?'Strong Buy':'Buy'):signal.action
      }))};
  ...
  return {
    ...
    skippedOrders,
    ...(accountSeed?{stockRecommendations}:{}),
