...
export function planC1PaperTransition({before,after,account,prices,slippageBps=12,commissionPerOrder=0}){
 ...
 try{
  ...
  return {scope:'c1-paper-transition',status:'proposed',executable:false,orders:proposal.orders,projectedCash:proposal.projectedCash,
   modelSession:changes[0].date,nextAccount:{kind:'c1-paper-execution',initialCapital:next.initialCapital,cash:proposal.projectedCash,positions:aggregate(next),modelFillCount:next.fills.length,modelHistory:JSON.stringify(next.fills)},
   limitation:'Retrospective paper mapping of model fills, not a next-session signal. nextAccount assumes every proposed fill at its estimated price; partial or actual fills must be reconciled separately.'};
...
export function initializeC1PaperExecution(model){
 ...
 return {kind:'c1-paper-execution',initialCapital:state.initialCapital,cash:state.initialCapital,positions:{},modelFillCount:0,modelHistory:'[]'};
}
