...
export function c1AccountHoldingExplanation(position,sourceSessionDate){
 ...
 if(position.inheritedRiskOnly){
  parts.push('Price and stop monitoring are active. Current momentum-ranking data does not cover this holding, so no rank-based hold, add or exit is assigned.');
 }else{
 ...
 if(stops.length)parts.push(`Recorded ${stops.length===1?'stop':'stops'}: ${stops.map(p=>'$'+p.toFixed(2)).join(', ')}. Stops and portfolio loss limits apply even during the minimum holding period.`);
