...
export function c1AccountHoldingExplanation(position,sourceSessionDate,displayPrice){
 ...
 if(Number.isFinite(displayPrice)&&displayPrice>0){
  ...
  parts.push(...age,'Stops and portfolio loss limits apply during the minimum hold.');
...
export function buildC1AccountDecision({continued,sourceHash,revision,now=new Date(),positionContext=[]}={}) {
 ...
 for(const [sleeve,run] of Object.entries(continued.sleeves)){
  ...
  for(const order of run.pendingDecisions){
  ...
  const ratings=run.stockRecommendations?.date===continued.sourceSessionDate?run.stockRecommendations.rows:[];
  // Pending entries retain their signal rating; full holdings can also qualify
  // in the observational list without creating any purchase order.
  const entries=[...ratings,...run.pendingDecisions.filter(o=>o.side==='buy')];
  for(const [index,order] of entries.entries()){
   if(['MSTR','SCHW'].includes(order.symbol))continue;
   const rating=['Buy','Strong Buy'].includes(order.action)?order.action:null;
   if(!candidates.has(order.symbol))candidates.set(order.symbol,{symbol:order.symbol,priority:order.priority??index,rating,sleeves:[],entryEvidence:{momentumScore:Number.isFinite(order.signal?.researchFactors?.momentumPercentile)?order.signal.researchFactors.momentumPercentile:null,sector:order.sector||null,close:Number.isFinite(order.signal?.price)?order.signal.price:null}});
   const row=candidates.get(order.symbol);row.priority=Math.min(row.priority,order.priority??index);
   if(!row.sleeves.includes(sleeve))row.sleeves.push(sleeve);
   if(rating==='Strong Buy'||!row.rating)row.rating=rating;
  }
 }
...
export function c1AccountPositionDecision(decision,symbol,displayPrice){
 ...
 return {action:stopReview&&!row.exits.length&&row.action!=='Exit candidate'?'Stop review':row.action,reason:displayed?c1AccountHoldingExplanation(row,decision.sourceSessionDate,displayPrice):row.reason,source:'c1-actual-account',decisionId:decision.decisionId};
}
