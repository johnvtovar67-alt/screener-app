...
export function c1AccountHoldingExplanation(position,sourceSessionDate){
 ...
 if(position.inheritedRiskOnly){
  parts.push('Ranking is unavailable for this holding in the current feed. Price and stop checks remain active; this coverage status is not a sell signal.');
 }else{
...
export function buildC1AccountDecision({continued,sourceHash,revision,now=new Date()}={}) {
 ...
 const rows=[...positions.values()].map(p=>({...p,avgCost:p.totalCost/p.shares,
  action:current?(p.exits.length?'Exit candidate':p.inheritedRiskOnly?'Ranking unavailable':'Hold'):'Refresh required',
