...
// Match the existing compiler/account share-class convention without changing
// the provider's request symbols or rewriting any observed price.
export function buildC1PriorSessionPrices(histories, symbols, date) {
  const closes = {};
  for (const symbol of symbols) {
    const canonical = ({ 'BRK-B': 'BRK.B', 'BF-B': 'BF.B' })[symbol] || symbol;
    if (Object.hasOwn(closes, canonical)) throw new Error('Duplicate canonical price-anchor symbol');
    closes[canonical] = histories[symbol]?.find(bar => bar.date === date)?.close ?? null;
  }
  return { date, closes };
}

// Retain only numeric vendor price fields for the last two observed sessions.
// Field names keep their provider meaning; this does not classify a revision as
// a split or dividend, and does not expose raw response URLs or other fields.
export function c1SourcePriceEvidence(symbol, rows) {
  const fields = ['open', 'high', 'low', 'close', 'volume', 'adjOpen', 'adjHigh', 'adjLow', 'adjClose', 'adjVolume', 'unadjustedVolume'];
  return [...rows].sort((a, b) => a.date.localeCompare(b.date)).slice(-2).map(row => ({ symbol, date: row.date,
    ...Object.fromEntries(fields.filter(key => typeof row[key] === 'number' && Number.isFinite(row[key])).map(key => [key, row[key]])) }));
}
