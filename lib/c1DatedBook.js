...
import { reconcileC1UnexposedBaseline } from './c1BaselineReconciliation';
...
export function acceptC1DatedInput(prior, input, now = new Date(), { observations = [] } = {}) {
  ...
  // Only immutable, contemporaneous provider records can support a correction.
  // These options are server-owned; the input payload cannot assert its own review.
  for (const observation of observations) {
    const payload = observation?.payload;
    if (!payload || observation.contract !== 'c1-index-observation-v1' ||
        observation.acceptedModelInput !== false || observation.executable !== false ||
        createHash('sha256').update(JSON.stringify(payload)).digest('hex') !== observation.observationHash ||
        payload.contract !== C1_LIVE_INPUT_CONTRACT || payload.universe !== input.universe ||
        observation.sourceSessionDate !== required || payload.sourceSessionDate !== required ||
        payload.session?.date !== required || observation.observedAt !== payload.observedAt ||
        payload.session.decisionAt !== payload.observedAt ||
        !Number.isFinite(Date.parse(payload.observedAt)) || Date.parse(payload.observedAt) > now.getTime() ||
        latestCompletedMarketSessionDay(new Date(payload.observedAt)) !== required ||
        payload.sourceReceipt?.membershipCheckedTwice !== true)
      throw new Error('Index correction observation integrity failed');
  }
  ...
  if (prior) {
    ...
    if (mismatches.length) {
      ...
      try {
        reconciliation = reconcileC1UnexposedBaseline(prior, input, now, digest, advanceC1ForwardModel, observations);
      } catch (cause) {
        // Unreviewed revisions and failed parity retain the original rejection.
        if (cause?.providerRevisionDiagnostic) error.providerRevisionDiagnostic = cause.providerRevisionDiagnostic;
        throw error;
      }
    }
  }
  const model = reconciliation?.model || advanceC1ForwardModel(prior?.model || null, [input.session], now, { completedCloseQueue: true });
  const content = { contract: C1_DATED_BOOK_CONTRACT, universe: input.universe,
    ...((prior?.reconciliations || reconciliation) ? { reconciliations: [
      ...(prior?.reconciliations || []), ...(reconciliation ? [reconciliation.audit] : [])] } : {}),
