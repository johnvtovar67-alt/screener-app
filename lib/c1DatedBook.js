  ...
  if (prior) {
    ...
    for (const row of previous.prices) {
      ...
      if (!Number.isFinite(value) || Math.abs(value - row.close) > 1e-8 * Math.max(1, row.close))
        throw new Error('Corporate action, removal, or price revision requires reconciliation: ' + row.symbol);
    }
  }
...
export function c1DatedBookView(record, holdings = [], now = new Date()) {
  ...
  // Keep historical provenance limitations explicit; current membership is
  // correctly identified but does not retroactively certify historical sectors.
  decision.universe = record.universe;
  decision.issues = decision.issues.filter(issue => issue.code !== 'PRODUCTION_UNIVERSE');
  decision.issues.push({ code: 'HISTORICAL_PROVENANCE', message: 'This is the observed ' + record.universe + ' index. Historical sector provenance and model-selection limitations remain unresolved.' });
