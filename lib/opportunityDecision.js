...
// Visible On Deck is intentionally close to Buy quality. Internal Watch can remain broad for research.
function visibleWatchEligible(s={}){
  ...
  // A true event-related demotion can stay visible if the underlying setup is still near-Buy quality.
  if(eventStatus==='blocked'||eventStatus==='caution')return capital>=68&&trade>=72&&technical>=56&&leadership>=60;

  // Require the two core allocation scores plus at least two of three execution/quality pillars.
  const supporting=[technical>=58,leadership>=62,raw>=68].filter(Boolean).length;
  return capital>=68&&trade>=72&&supporting>=2&&risk<=80&&extension<=65;
}
