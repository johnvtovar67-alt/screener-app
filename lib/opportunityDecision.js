...
// A high-conviction setup should not jump directly from Strong Buy quality to Watch because
// one noisy intraday input crosses a binary threshold. This continuity band is intentionally
// narrower than a normal Buy: it only rescues a Watch when the aggregate setup remains strong
// and all hard deployment-safety gates are still intact. True breaks still downgrade immediately.
function strongBuyContinuityEligible(s={}){
  ...
  if(rr>0&&rr<1.5)return false;
  const vs50=m.vs50===null||m.vs50===undefined?null:num(m.vs50,0),day=num(m.day,0),extension=num(m.extension,50);
  if(extension>=65||(vs50!==null&&vs50>20)||day>8)return false;
  ...
  return thesis>=73&&trade>=80&&capital>=77&&raw>=70&&technical>=56&&leadership>=58&&momentum>=50&&entry>=48&&risk<=75;
}
function continuityReason(s={}){
  const e=expert(s),first=Array.isArray(e?.failures)?e.failures[0]:'';
  return `High-conviction setup has softened from Strong Buy thresholds but remains Buy-quality. Use partial size while full confirmation rebuilds.${first?` Softened factor: ${first}`:''}`;
}
...
// Strong Buy hysteresis: earning the label still requires every normal Strong Buy gate.
// Once earned, a transient move back to ordinary Buy does not immediately strip the label.
// Retention has materially lower floors than entry but still requires a strong, safe setup.
// It lasts only for the current trading session window and breaks immediately on hard risk/trend failures.
...
function strongBuyRetentionEligible(s={}){
  ...
  const eventStatus=String(event?.status||'').toLowerCase();
  if(event?.blockNewCapital||event?.manualCheckRequired||['blocked','manual','caution'].includes(eventStatus))return false;
  if(m.quoteFreshnessPass===false||m.fundamentalsPass===false||m.below50||m.below200)return false;
  const rr=num(m.payoffRatio,0),rv=m.relativeVolume,vs50=m.vs50===null||m.vs50===undefined?null:num(m.vs50,0),day=num(m.day,0),extension=num(m.extension,50);
  if(rv!==null&&rv!==undefined&&Number.isFinite(Number(rv))&&Number(rv)<.4)return false;
  if(rr>0&&rr<1.35)return false;
  if(extension>=68||(vs50!==null&&vs50>22)||day>9)return false;
  ...
  return strongQuality>=80&&thesis>=74&&trade>=78&&capital>=78&&raw>=74&&technical>=66&&leadership>=66&&momentum>=50&&entry>=50&&risk<=68;
}
function retainStrongBuy(s={}){
  const d=s.finalDecision||{};
  return{...s,finalDecision:{...d,action:'Strong Buy',timing:'Now',size:'Full',priority:'Top Tier',reason:'Strong Buy retained. The setup has softened modestly from the entry threshold but remains inside the high-conviction retention band; normal intraday noise does not justify a downgrade.',source:'strong-buy-hysteresis',hysteresisProtected:true}};
}
