...
export function finalizeBroadOpportunityDecisions(rows=[]){
  ...
  let finalized=base.map(s=>{const d=s.finalDecision,a=d.action,peer=relativeCapitalScore(s);
    ...
    const capital=num(rec(s)?.capitalScore??s.capitalScore??expert(s)?.capitalScore),trade=num(rec(s)?.tradeSetupScore??s.tradeSetupScore??expert(s)?.tradeSetupScore),thesis=num(rec(s)?.thesisScore??s.thesisScore??expert(s)?.thesisScore??s.fundamentalScore),nearStrong=capital>=80&&trade>=80&&thesis>=75,competitive=nearStrong||(peer>=cutoff&&capital>=70&&trade>=74);
    if(competitive)return{...s,finalDecision:{...d,priority:nearStrong?'High-Conviction Buy':d.priority,relativeCapitalScore:Math.round(peer*10)/10,relativeCapitalCutoff:Math.round(cutoff*10)/10,source:nearStrong?'near-strong-protection':d.source}};
