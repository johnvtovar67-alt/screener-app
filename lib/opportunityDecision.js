...
export function recentStrongBuySymbols(now=Date.now(),memoryOverride=null){const out=[],memory=memoryOverride instanceof Map?memoryOverride:strongBuyMemory();for(const[symbol,prior]of memory){if(prior?.action==='Strong Buy'&&!prior?.interruptedAt&&now-Number(prior.earnedAt||0)<=STRONG_BUY_MEMORY_MS)out.push(symbol);}return out;}
...
export function finalizeBroadOpportunityDecisions(rows=[],options={}){
  const requestedNow=options?.now instanceof Date?options.now.getTime():Number(options?.now),now=Number.isFinite(requestedNow)&&requestedNow>0?requestedNow:Date.now(),memory=options?.memory instanceof Map?options.memory:strongBuyMemory();
