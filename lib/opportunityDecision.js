...
export function finalizeBroadOpportunityDecisions(rows=[]){
  ...
  let finalized=base.map(s=>{const d=s.finalDecision,a=d.action,peer=relativeCapitalScore(s),prior=memory.get(symbolOf(s)),recentStrong=prior?.action==='Strong Buy'&&now-prior.earnedAt<=STRONG_BUY_MEMORY_MS;
    ...
    if(a==='Strong Buy')return{...s,finalDecision:{...d,relativeCapitalScore:Math.round(peer*10)/10,relativeCapitalCutoff:Math.round(cutoff*10)/10,capitalConfirmed:true}};
    if(recentStrong&&strongBuyRetentionEligible(s))return retainStrongBuy({...s,finalDecision:{...d,relativeCapitalScore:Math.round(peer*10)/10,relativeCapitalCutoff:Math.round(cutoff*10)/10,capitalConfirmed:true}});
    ...
    return{...s,finalDecision:{...d,action:'Watch',timing:'Wait',size:'None',priority:'Qualified Watch',reason:'Meets the standalone Buy standard and clears the capital-confirmation margin, but stronger current setups have a clear capital-allocation edge. Keep it on Watch until its relative capital advantage improves.',nextTrigger:'Re-rank when its relative capital advantage improves.',planText:'No new capital while stronger qualified setups offer a clear edge.',relativeCapitalScore:Math.round(peer*10)/10,relativeCapitalCutoff:Math.round(cutoff*10)/10,standaloneAction:a,source:'relative-demotion',capitalConfirmed:false}};
  });
