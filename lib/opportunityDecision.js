...
function hardEntryGateBlocked(s={}){
  ...
  return Boolean(event?.blockNewCapital||event?.manualCheckRequired||['blocked','manual','caution'].includes(eventStatus)||m.quoteFreshnessPass===false||m.fundamentalsPass===false||(t&&(!t.available||!t.pass)));
}
function strongBuyRetentionEligible(s={}){
  ...
  const t=s?.entryTiming||r?.entryTiming;if(t&&!t.strongPass)return false;
  const event=s?.eventRisk||s?.preTradeCheck||r?.eventRisk||r?.preTradeCheck||{},eventStatus=String(event?.status||'').toLowerCase();
...
export function finalizeBroadOpportunityDecisions(rows=[]){
  ...
  const nextMemory=new Map();for(const s of finalized){const key=symbolOf(s),a=s.finalDecision?.action,prior=memory.get(key);if(a==='Strong Buy'){const earnedAt=s.finalDecision?.hysteresisProtected&&prior?.earnedAt?prior.earnedAt:now;nextMemory.set(key,{action:'Strong Buy',earnedAt});}else if(a==='Buy'){nextMemory.set(key,{action:'Buy',earnedAt:prior?.action==='Buy'&&prior?.earnedAt?prior.earnedAt:now});}else if(['Strong Buy','Buy'].includes(s.signalChange?.from)&&prior?.earnedAt){nextMemory.set(key,{action:prior.action,earnedAt:prior.earnedAt});}else nextMemory.set(key,{action:a,earnedAt:0});}globalThis[STRONG_BUY_MEMORY_KEY]=nextMemory;
  return finalized;
}
