...
// Entry quality is separate from stock quality. A powerful same-day impulse can make
// an otherwise excellent setup a poor fresh-money entry. Cap size or wait for a reset.
...
// Continuity may smooth ordinary intraday noise, but it cannot rescue a stock that
// has failed a hard forward-entry gate. Stability is subordinate to entry quality.
...
export function finalizeStandaloneOpportunityDecision(s={}){
  const rawAction=absoluteAction(s),impulse=freshEntryImpulse(s),continuityProtected=strongBuyContinuityEligible(s);
  const impulseBlocked=['Strong Buy','Buy'].includes(rawAction)&&impulse.hard,impulseCapped=rawAction==='Strong Buy'&&impulse.strongCap&&!impulseBlocked;
  const action=impulseBlocked?'Watch':impulseCapped?'Buy':continuityProtected?'Buy':rawAction,score=relativeCapitalScore(s);
  const reason=impulseBlocked?impulseReason(impulse,true):impulseCapped?impulseReason(impulse,false):continuityProtected?continuityReason(s):baseReason(s);
  const timing=impulseBlocked?'Wait for Reset':impulseCapped?'Now':continuityProtected?'Now':baseTiming(s),size=impulseBlocked?'None':impulseCapped?'Partial':continuityProtected?'Partial':baseSize(s),priority=impulseBlocked?'Wait for Reset':impulseCapped?'Capped Entry':continuityProtected?'High-Conviction Buy':action==='Strong Buy'?'Top Tier':action==='Buy'?'Actionable':action==='Watch'?'Watch':'Avoid';
  const source=impulseBlocked?'entry-impulse-block':impulseCapped?'entry-impulse-cap':continuityProtected?'strong-buy-continuity':'standalone';
  const nextTrigger=impulseBlocked?'Wait for consolidation or a controlled pullback before reconsidering fresh capital.':action==='Watch'?priceTrigger(s):planText(s,action);
...
// A raw Buy is not enough to deploy capital. It must clear a confirmation margin
// above the ordinary Buy/Watch boundary so a single noisy snapshot cannot spend cash.
function capitalConfirmedBuy(s={}){
  const d=s.finalDecision||{},r=rec(s),e=expert(s),m=e?.metrics||{};
  if(freshEntryImpulse(s).hard)return false;
  if(d.action==='Strong Buy')return true;
...
function capitalConfirmationWatch(s={}){
  const d=s.finalDecision||{},peer=relativeCapitalScore(s);
...
export function finalizeBroadOpportunityDecisions(rows=[]){
  ...
  let finalized=base.map(s=>{const d=s.finalDecision,a=d.action,peer=relativeCapitalScore(s),eff=capitalEfficiencyScore(s),prior=memory.get(symbolOf(s)),recentStrong=prior?.action==='Strong Buy'&&now-prior.earnedAt<=STRONG_BUY_MEMORY_MS;
  ...
  const actionableFinal=finalized.filter(s=>['Strong Buy','Buy'].includes(s.finalDecision.action));
  const bestOpportunity=[...actionableFinal].sort((a,b)=>num(b.finalDecision?.capitalEfficiencyScore,capitalEfficiencyScore(b))-num(a.finalDecision?.capitalEfficiencyScore,capitalEfficiencyScore(a)))[0];
