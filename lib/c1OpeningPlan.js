...
function projectC1Session({record,date,observedAt,opens}, includeStops=false) {
 ...
 try {
  ...
  for(const [id,weight] of Object.entries(C1_ACCOUNTING_WEIGHTS)){
   const run=simulatePointInTimePortfolio({metadata:{},sessions},{...C1_FROZEN_OPTIONS[id],startDate:record.simulationStartSession||record.firstDecisionSession,endDate:date,liquidateAtEnd:false});
