...
import { compileC1LiveInput, c1LiveMembers, C1_LIVE_UNIVERSES, buildC1PriorSessionPrices, c1SourcePriceEvidence } from './c1LiveInput';
...
export async function collectC1LiveInput({ universe = 'nasdaq', now = new Date(), fetcher = fetch,
  ...
  async function request(endpoint, params = {}) {
    ...
    catch { failed = true; throw new Error('Market data provider returned invalid JSON'); }
  ...
  const histories = {}, rawHashes = {}, priceEvidence = {};
  ...
  const workers = Array.from({ length: 3 }, async () => {
    while (cursor < symbols.length && !failed) {
      ...
      try {
        ...
        priceEvidence[symbol] = c1SourcePriceEvidence(symbol, inRange);
  ...
  const previousDate = histories.SPY.at(-2)?.date;
  const priorSessionPrices = buildC1PriorSessionPrices(histories, symbols, previousDate);
  return { ...input, priorSessionPrices, priceEvidence: { contract: 'c1-provider-price-fields-v1',
    endpoint: 'historical-price-eod/dividend-adjusted', rows: symbols.flatMap(symbol => priceEvidence[symbol]) },
    sourceReceipt: { provider: 'FMP', membershipEndpoint: spec.endpoint,
