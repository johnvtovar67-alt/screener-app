...
export async function collectC1LiveInput({ universe = 'nasdaq', now = new Date(), fetcher = fetch,
  ...
  const priorSessionPrices = { date: previousDate, closes: Object.fromEntries(symbols.map(symbol =>
    [symbol, histories[symbol].find(bar => bar.date === previousDate)?.close ?? null])) };
