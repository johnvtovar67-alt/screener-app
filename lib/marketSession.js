...
function marketHolidays(year){
  ...
  for(const y of[year-1,year,year+1]){
    if(weekdayOf(dateKey(y,1,1))!==6)out.add(observedFixed(y,1,1));out.add(nthWeekday(y,1,1,3));out.add(nthWeekday(y,2,1,3));out.add(addKeyDays(easterSunday(y),-2));out.add(lastWeekday(y,5,1));
...
// NYSE equity core-session calendar: early closes are 13:00 Eastern.
// https://www.nyse.com/markets/hours-calendars
export function marketSessionCloseMinutes(key){
  const year=Number(String(key).slice(0,4));
  const early=key===addKeyDays(nthWeekday(year,11,4,4),1)||
    (key===dateKey(year,7,3)&&isUsMarketSessionDay(key))||
    (key===dateKey(year,12,24)&&isUsMarketSessionDay(key));
  return early?13*60:16*60;
}
...
export function marketExecutionState(value=new Date()){
  ...
  const start=9*60+30,end=marketSessionCloseMinutes(clock.key),isOpen=clock.isSessionDay&&clock.minutes>=start&&clock.minutes<end;
...
export function latestCompletedMarketSessionDay(value=new Date()){
  const clock=easternMarketClock(value);if(!clock)return null;
  return clock.isSessionDay&&clock.minutes>=marketSessionCloseMinutes(clock.key)?clock.key:previousMarketSessionDay(clock.key);
}
...
export function marketSessionProgress(now=new Date()){
  ...
  const start=9*60+30,end=marketSessionCloseMinutes(clock.key);
