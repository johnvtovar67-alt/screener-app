...
function previousMarketSessionDay(key){let candidate=addKeyDays(key,-1);for(let i=0;i<10;i++){if(isUsMarketSessionDay(candidate))return candidate;candidate=addKeyDays(candidate,-1);}return candidate;}
export function marketObservationSessionDay(value=new Date()){
  ...
  try{const parts=new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(now),pick=t=>parts.find(p=>p.type===t)?.value,key=dateKey(Number(pick('year')),Number(pick('month')),Number(pick('day'))),mins=Number(pick('hour'))*60+Number(pick('minute'));return isUsMarketSessionDay(key)&&mins>=9*60+30?key:previousMarketSessionDay(key);}catch{return null;}
}
