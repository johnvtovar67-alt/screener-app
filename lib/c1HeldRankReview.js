...
export function compileC1HeldRankReview({baseline,sourceHash,symbol,history,observedAt}={}){
 ...
 if(bars?.length!==253)throw new Error('Complete adjusted holding history required: row count');
 if(bars.at(-1).date!==baseline.date)throw new Error('Complete adjusted holding history required: latest session');
 if(bars.some(b=>b.adjusted!==true||!isUsMarketSessionDay(b.date)||!['open','high','low','close','volume'].every(k=>Number.isFinite(b[k])&&b[k]>0)||b.low>Math.min(b.open,b.close)||b.high<Math.max(b.open,b.close)))throw new Error('Complete adjusted holding history required: invalid bar');
 if(bars.some((b,i)=>i&&marketSessionDistance(bars[i-1].date,b.date)!==1))throw new Error('Complete adjusted holding history required: session gap');
 const mismatchFields=['open','high','low','close'].filter(k=>!Number.isFinite(accepted?.[k])||accepted[k]<=0||Math.abs(bars.at(-1)[k]-accepted[k])>1e-7);
 if(mismatchFields.length){const error=new Error('Holding history differs from the accepted account close');error.mismatchFields=mismatchFields;throw error;}
 if(!Number.isFinite(accepted.volume)||accepted.volume<=0)throw new Error('Accepted holding volume unavailable');
 // Preserve the dated account observation when the provider revises volume.
 // Price differences still reject. Copy the bar: neither source is mutated.
 const volumeReconciliation=bars.at(-1).volume!==accepted.volume?{policy:'accepted-session-volume-v1',observedVolume:bars.at(-1).volume,acceptedVolume:accepted.volume}:null;
 bars[bars.length-1]={...bars.at(-1),volume:accepted.volume};
 ...
 return {symbol,signal,ranks,close:accepted.close,sourceSessionDate:baseline.date,sourceHash,observedAt,historyFrom:bars[0].date,historyRows:bars.length,...(volumeReconciliation?{volumeReconciliation}:{})};
}
