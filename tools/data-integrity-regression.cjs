...
assert(top5.includes('stocks: rows.map(serializeStockForClient)')&&top5.includes('delete recommendation[key]')&&top5.includes('fundamentalSnapshot: _fundamentalSnapshot'),'The public broad-screen payload must remove duplicated server-only decision evidence before reaching mobile clients');
