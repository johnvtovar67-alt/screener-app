...
const completedSessions=[];for(let daysAgo=1;completedSessions.length<4&&daysAgo<15;daysAgo++){const calendarDay=new Date(Date.now()-daysAgo*86400000).toISOString().slice(0,10),sessionDay=market.marketObservationSessionDay(`${calendarDay}T20:00:00.000Z`);if(sessionDay&&!completedSessions.includes(sessionDay))completedSessions.push(sessionDay);}
const sessionState=(action,sessionDay)=>({recordType:'state',signalState:true,symbol:'OKE',action,day:sessionDay,timestamp:`${sessionDay}T20:00:00.000Z`});
let persistence=gov.signalPersistence([sessionState('Buy',completedSessions[1]),sessionState('Buy',completedSessions[0])],'OKE');assert(persistence.persistent&&persistence.actionableDays===2,'Two uninterrupted daily Buy observations must qualify');
persistence=gov.signalPersistence([sessionState('Buy',completedSessions[3]),sessionState('Buy',completedSessions[2]),sessionState('Watch',completedSessions[1]),sessionState('Buy',completedSessions[0])],'OKE');assert(!persistence.persistent&&persistence.interrupted&&persistence.actionableDays===1,'An intervening Watch must reset Buy funding confirmation');
persistence=gov.signalPersistence([sessionState('Buy',completedSessions[3]),sessionState('Buy',completedSessions[2]),sessionState('Paused',completedSessions[1]),sessionState('Buy',completedSessions[0])],'OKE');assert(persistence.persistent&&!persistence.interrupted,'A temporary data-verification pause must not masquerade as stock deterioration or erase an otherwise uninterrupted Buy streak');
...
persistence=gov.signalPersistence([sessionState('Watch','2026-08-27'),{recordType:'state',signalState:true,symbol:'OKE',action:'Buy',day:'2026-08-28',timestamp:friday},{recordType:'state',signalState:true,symbol:'OKE',action:'Buy',day:'2026-08-29',timestamp:utcSaturdayButEtFriday}],'OKE',30);assert(!persistence.persistent&&persistence.actionableDays===1,'UTC midnight must not manufacture a second Buy session');
