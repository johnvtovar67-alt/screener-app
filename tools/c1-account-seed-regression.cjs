...
const {createC1ProspectiveSeeds:create,withC1HistoricalPriceEvidence,c1SeedPosition,C1_ACCOUNT_SEED_CONTRACT:contract}=loader.load('lib/c1AccountSeed.js');
...
const historicalSessions=JSON.parse(JSON.stringify(sessions.slice(0,36))),historicalBar=historicalSessions[10].prices.find(p=>p.symbol==='T4');
historicalBar.high=close*1.2;
const enriched=withC1HistoricalPriceEvidence(adoption,historicalSessions),historicalEvidence=enriched.seeds.base.positions[0].historicalPriceEvidence;
assert.equal(historicalEvidence.complete,true);
assert.equal(historicalEvidence.sessionsObserved,36);
assert.equal(historicalEvidence.high,close*1.2);
assert.equal(historicalEvidence.closesAboveEntry+historicalEvidence.closesBelowEntry+historicalEvidence.closesAtEntry,36);
const historicalPosition=c1SeedPosition(enriched.seeds.base.positions[0],0,baseline.date,close);
...
const historicalContinuation=continueAccount({adoption,sessions:[baseline],historySessions:historicalSessions,records:[],observedAt:baseline.date+'T21:00:00Z'});
assert.equal(historicalContinuation.holdingPriceHistory.T4.complete,true);
assert.equal(historicalContinuation.sleeves.base.openPositions[0].highWatermark,close*1.2);
assert.equal(historicalContinuation.sleeves.base.openPositions[0].profitProtectionArmed,true);
...
const cashTarget=Math.round((initialView.actualCash+.52)*100)/100;
const cashCorrected=reconcileCash({account:privateAccount,brokerCash:cashTarget,expectedRevision:0,book:baselineBook,now:new Date(baseline.date+'T21:01:00Z')});
const correctedCashView=evaluateService({account:cashCorrected,book:baselineBook,now:new Date(baseline.date+'T21:01:00Z')});
assert.ok(Math.abs(correctedCashView.actualCash-cashTarget)<1e-7);
assert.deepEqual(correctedCashView.positions.map(p=>[p.symbol,p.shares,p.avgCost,p.openedAt]),initialView.positions.map(p=>[p.symbol,p.shares,p.avgCost,p.openedAt]));
assert.equal(cashCorrected.cashReconciliations[0].amount,.52);
assert.equal(cashCorrected.adoption.actualCash,privateAccount.adoption.actualCash,'Cash reconciliation must not rewrite the adopted opening balance');
assert.throws(()=>reconcileCash({account:privateAccount,brokerCash:cashTarget,expectedRevision:1,book:baselineBook}),/Account changed/);
assert.throws(()=>reconcileCash({account:privateAccount,brokerCash:Math.round((initialView.actualCash+5.01)*100)/100,expectedRevision:0,book:baselineBook}),/transaction-level evidence/);
console.log('PASS: broker cash reconciliation is append-only, exact to the cent, and preserves all recorded holdings and adoption history.');
const {c1AccountHoldingExplanation:explainHolding}=loader.load('lib/c1AccountDecision.js');
const explanationFixture={avgCost:100,exits:[],stops:[{price:86}],reviewRules:[{heldSessions:2,minimumHold:30,exitBuffer:6,close:97}]};
const youngReason=explainHolding(explanationFixture,'2026-09-11');
assert.ok(youngReason.startsWith('Down 3.00%'));
assert.ok(youngReason.includes('11.34% below the latest close'));
...
assert.equal(carriedView.current,true);
assert.equal(carriedView.actualCash,initialView.actualCash);
assert.deepEqual(carriedView.positions.map(p=>[p.symbol,p.shares,p.avgCost]),initialView.positions.map(p=>[p.symbol,p.shares,p.avgCost]));
assert.equal(JSON.stringify(privateAccount),storedBeforeCarry,'Refreshing never writes assumed no-trade confirmations');
...
const multi=carry({account:privateAccount,book:multiBook,now:multiNow});
assert.equal(multi.records.length,4);
assert.equal(evaluateService({account:multi,book:multiBook,now:multiNow}).current,true);
assert.equal(multi.records.flatMap(r=>r.fills).length,0);
assert.equal(JSON.stringify(privateAccount),storedBeforeCarry);
const priorTradeDay=carry({account:privateAccount,book:multiBook,now:multiNow,beforeDate:sessions[39].date});
assert.equal(priorTradeDay.records.length,3);
assert.equal(pendingService({account:priorTradeDay,book:multiBook,now:multiNow}).date,sessions[39].date);
const stopBook=JSON.parse(JSON.stringify(multiBook));
stopBook.model.sessions[1].prices.find(p=>p.symbol==='T4').low=close*.80;
const carriedStopped=carry({account:privateAccount,book:stopBook,now:multiNow});
const stoppedView=evaluateService({account:carriedStopped,book:stopBook,now:multiNow});
const stoppedPosition=stoppedView.positions.find(p=>p.symbol==='T4');
assert.ok(stoppedPosition.exits.some(e=>e.reason==='initial-stop'));
assert.equal(stoppedPosition.stopTriggerEvidence.date,sessions[36].date);
assert.equal(stoppedPosition.stopTriggerEvidence.low,close*.80);
assert.equal(stoppedPosition.shares,3,'A triggered stop cannot invent an executed sale');
assert.equal(stoppedView.actualCash,initialView.actualCash);
assert.ok(carriedStopped.records.every(r=>!r.fills.length));
const dated=loader.load('lib/portfolioDateDisplay.js').portfolioDateDisplay;
for(const tz of ['America/Chicago','America/Los_Angeles','UTC','Asia/Tokyo']){
 process.env.TZ=tz;
 assert.equal(dated('2026-09-08'),'Sep 8, 2026');
 assert.equal(dated('2026-09-08T00:00:00.000Z'),'Sep 8, 2026');
}
assert.equal(dated('2026-02-30'),'—');
...
(async()=>{
 ...
 const diagnostics=[];
 const inspectFailure=async(fetcher)=>providerBox.collect({baseline:rankBaseline,sourceHash,symbols:['OUT'],now:new Date(baseline.date+'T21:00:00Z'),apiKey:'synthetic-secret',report:d=>diagnostics.push(d),fetcher});
 await inspectFailure(async()=>({ok:false,status:403}));
 assert.equal(diagnostics.at(-1).code,'PROVIDER_HTTP');assert.equal(diagnostics.at(-1).providerStatus,403);
 await inspectFailure(async()=>{throw new Error('https://provider.invalid?apikey=synthetic-secret');});
