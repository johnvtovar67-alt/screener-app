...
const portfolio=[{symbol:'T4',role:'Swing',shares:3,avgCost:close,openedAt:sessions[0].date},{symbol:'MSTR',role:'Core',shares:2},{symbol:'CASH',role:'Swing',shares:12345.6,avgCost:1}];
const capitalRecord={version:2,highWater:12345.6+3*close,triggerDay:null,portfolioSignature:signature(portfolio),reconciliationRequired:false};
...
assert.ok(Math.abs(cash-12345.6)<1e-8);assert.ok(Math.abs(shares-3)<1e-8);
...
const {planC1ActualAccountOpening:planOpening,planC1ContinuedAccountOpening:planContinued,reconcileC1ActualAccountFills:reconcile,continueC1ActualAccount:continueAccount}=loader.load('lib/c1AccountExecution.js');
const opening={...sessions[36],corporateActions:[]};
const next={...sessions[37],corporateActions:[]};
...
assert.equal(plan.orders.some(o=>o.side==='buy'&&!o.condition),false,'One completed candidate session cannot fund the limited pilot');
assert.ok(plan.blockedOrders.some(o=>o.side==='buy'&&/two completed/.test(o.blockReason)));
assert.equal(JSON.stringify(plan.pilot),JSON.stringify({contract:'c1-account-limited-pilot-v1',maxNames:3,maxPositionPct:.01,confirmationSessions:2,enforced:true,openingEquity:adoption.actualSwingEquity}));
console.log('PASS: first opening withholds one-session buys; zero actual fills preserve opening balances.');
...
const firstRecord={date:opening.date,complete:true,openingObservedAt:observedAt,fills:[]};
const confirmedPlan=planContinued({adoption,sessions:[baseline,opening],records:[firstRecord],opening:{...next,corporateActions:[]},observedAt:next.date+'T14:00:00Z'});
const planFills=confirmedPlan.orders.filter(o=>!o.condition).map((o,i)=>({id:'actual-'+i,orderId:o.id,symbol:o.symbol,side:o.side,shares:o.side==='buy'?1:o.shares,price:o.estimatedPrice,fee:.01,executedAt:next.date+'T13:31:00Z'}));
assert.ok(planFills.some(f=>f.side==='buy'),'Fixture must exercise actual partial buys');
assert.ok(confirmedPlan.orders.filter(o=>o.side==='buy').every(o=>o.shares*o.estimatedPrice<=confirmedPlan.pilot.openingEquity*.01+1e-7));
const pilotPositions=new Map();for(const book of Object.values(confirmedPlan.openingBooks))for(const [symbol,position] of Object.entries(book.positions))pilotPositions.set(symbol,(pilotPositions.get(symbol)||0)+position.shares);
for(const order of confirmedPlan.orders.filter(o=>!o.condition)){const before=pilotPositions.get(order.symbol)||0,nextShares=before+(order.side==='buy'?order.shares:-order.shares);if(nextShares)pilotPositions.set(order.symbol,nextShares);else pilotPositions.delete(order.symbol);if(order.side==='buy')assert.ok(nextShares<=order.pilot.maxPositionShares);}
assert.ok(pilotPositions.size<=3,'The account-wide pilot cannot exceed three names across its internal sleeves');
const planActual=reconcile({plan:confirmedPlan,fills:planFills,observedAt:next.date+'T14:00:00Z'});
for(const id of ['base','cooldown15','sector40']){
 const aseed=adoption.seeds[id],scale=aseed.actualDollarsPerModelDollar;
 const fills=planFills.filter(f=>confirmedPlan.orders.find(o=>o.id===f.orderId).sleeve===id).map(f=>({...f,reason:confirmedPlan.orders.find(o=>o.id===f.orderId).reason,shares:f.shares/scale,fee:f.fee/scale}));
 ...
 for(const p of run.openPositions)if(p.enteredAt===next.date)assert.ok(Math.abs(p.initialStopPrice-p.entryPrice*.86)<1e-7);
}
...
const records=[firstRecord,{date:next.date,complete:true,openingObservedAt:next.date+'T14:00:00Z',fills:planFills}];
const closedSessions=[baseline,opening,next];
const continued=continueAccount({adoption,sessions:closedSessions,records,observedAt:next.date+'T21:00:00Z'});
...
const afterNext={...sessions[38],corporateActions:[]};
const laterPlan=planContinued({adoption,sessions:closedSessions,records,opening:{...afterNext,corporateActions:[]},observedAt:afterNext.date+'T14:00:00Z'});
...
assert.throws(()=>continueAccount({adoption,sessions:closedSessions,records:[{...records[0],complete:false},records[1]],observedAt:next.date+'T21:00:00Z'}),/Confirm complete/);
...
const stopOrder=confirmedPlan.orders.find(o=>o.condition==='stop-triggered'&&planFills.some(f=>f.side==='buy'&&f.symbol===o.symbol&&confirmedPlan.orders.find(x=>x.id===f.orderId).sleeve===o.sleeve));
assert.ok(stopOrder,'Opening plans include standing stops without assuming a sale');
const stoppedFill={id:'actual-stop',orderId:stopOrder.id,symbol:stopOrder.symbol,side:'sell',shares:1,price:stopOrder.estimatedPrice*.999,fee:.01,executedAt:next.date+'T15:00:00Z'};
const stopSession={...next,prices:next.prices.map(p=>p.symbol===stopOrder.symbol?{...p,low:stopOrder.estimatedPrice*.99,close:stopOrder.estimatedPrice}:p)};
const stopped=continueAccount({adoption,sessions:[baseline,opening,stopSession],records:[firstRecord,{...records[1],fills:[...planFills,stoppedFill]}],observedAt:next.date+'T21:00:00Z'});
