...
import {factorFor,signalPersistence,portfolioRiskSnapshot,swingTargetPct,capitalAllowance,capitalSignalEligible,rotationGate,swingTimeReview} from "../lib/portfolioGovernor";
...
export default function Home(){
    ...
    try{
      ...
      for(const p of portfolio){
        try{
          ...
          rows.push({...s,...calc(p,price(s)),role:role(p.symbol,p.role),openedAt:p.openedAt||null,lastTradeAt:p.lastTradeAt||p.openedAt||null});
  ...
  function pd(s){
    ...
    if(base.action==="Trim"&&!trimPlans.has(sym(s)))return{...base,action:"Hold",reason:"This winner is stretched, but a partial sale would be too small to matter or would leave an immaterial residual position."};
