...
function OpportunityCard({stock}){
  ...
  return <article className={`ideaCard ${actionClass(a)}`}>
    ...
    <div className="badgeRow"><span className="themeBadge">{theme(stock)}</span><span className="entryBadge">Entry: {entry(stock)}</span>{er&&<span className="eventBadge">{er.label}</span>}</div>
