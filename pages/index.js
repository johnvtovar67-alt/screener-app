...
export default function Home(){
  ...
  function pd(s){
    ...
    if(base.action==="Hold"){const rw=reunderwriteExistingPosition({stock:s,decision:base,risk:riskSnapshot,timeReview:time});if(rw.override)return{...base,action:rw.action,reason:rw.reason,reunderwrite:rw};if(time.review)return{...base,reason:`${base.reason} Time review: ${time.reason}`};}
    return base;
  }
