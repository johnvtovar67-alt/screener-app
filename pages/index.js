...
import {v11ProductionPositionLifecycle} from "../lib/v11ProductionPolicy";
...
export default function Home(){
  ...
  async function fetchScreen(t,verificationPass=0,{full=false,includeSymbol=""}={}){
    const requested=includeSymbol?`&symbol=${encodeURIComponent(includeSymbol)}`:"",r=await fetch(`/api/top5?theme=${encodeURIComponent(t)}&verificationPass=${verificationPass}${full?"":"&compact=1"}${requested}`,{cache:"no-store"}),d=await r.json();if(!r.ok)throw new Error(d.detail||d.error);const performance=await performanceHistory();return{...d,stocks:decorateSnapshot(d.stocks||[],performance,d.meta),performance};
  }
  ...
  async function handleReload(){if(tab==="portfolio")return analyze();if(tab==="single")return check();manualVerificationPass.current=Math.min(20,manualVerificationPass.current+1);return load(tab==="themes"?selectedTheme:"opportunities",manualVerificationPass.current);}
  ...
  return <main>
    ...
    {accountView&&["opportunities","portfolio"].includes(tab)&&<section className="card" aria-label="C1 account status"><h2>C1 account status</h2><p><b>Diagnostic only — trading release incomplete.</b> Account-specific Hold, Risk monitoring and candidate labels are model calculations, not instructions to buy, hold or sell.</p><p>Account session: {accountView.decision.sourceSessionDate}. Transaction history and trading-release evidence require separate verification.</p></section>}
    ...
    {tab==="single"&&<section className="card"><h2>🔎 Single Symbol</h2><p className="sub">Symbols in the broad universe use the same authoritative broad decision shown on Opportunities; out-of-universe symbols use the standalone expert decision.</p><form className="single" onSubmit={check}><input value={symbol} onChange={e=>setSymbol(e.target.value)} placeholder="Ticker"/><button disabled={checking}>{checking?"Analyzing...":"Analyze"}</button></form>{snap&&<div className="grid"><Card s={snap} decision={opportunityDecision(snap)}/></div>}</section>}
