...
export default function HomePage() {
  ...
  function getPortfolioAction(label, gainLossPct) {
    if (label === "STRONG BUY") return "ADD";
    if (label === "BUY") return "HOLD / ADD SMALL";
    if (label === "WATCH") return gainLossPct < -7 ? "TRIM" : "HOLD";
    if (label === "AVOID") return gainLossPct < -5 ? "EXIT / TRIM" : "DO NOT ADD";
    return "HOLD";
  }
    ...
    try {
      ...
      for (const position of portfolioPositions) {
        ...
        results.push({
          ...
          gainLoss,
          gainLossPct,
          action: getPortfolioAction(data.recommendation?.label, gainLossPct),
        });
      }
  ...
  function getEntryNote(row) {
    return row?.recommendation?.entryNote || row?.entryNote || "Use price/volume confirmation.";
  }
  ...
  return (
    <main style={mainStyle}>
      ...
      <div style={toolbarStyle}>
        ...
        <button onClick={handleLookup} disabled={isLookingUp} style={primaryButtonStyle}>
          {isLookingUp ? "Looking up..." : "Snap Quote + Score"}
        </button>

        <button onClick={loadTop5} disabled={isLoadingTop5} style={secondaryButtonStyle}>
          {isLoadingTop5 ? "Loading..." : "Reload Screener"}
        </button>
      </div>

      <div style={filterRowStyle}>
        <button onClick={() => setUnder25Only((prev) => !prev)} style={filterButtonStyle(under25Only)}>
          Under $25
        </button>

        <button onClick={() => setProfitableOnly((prev) => !prev)} style={filterButtonStyle(profitableOnly)}>
          Profitable Only
        </button>
      ...
      <div style={portfolioWrapStyle}>
        ...
        <div style={portfolioInputRowStyle}>
          ...
          <button onClick={analyzePortfolio} disabled={isAnalyzingPortfolio} style={primaryButtonStyle}>
        ...
        <div style={miniPositionStyle}>
          {portfolioPositions.map((p) => (
            <span key={p.symbol} style={positionChipStyle}>
              ...
              <button onClick={() => removePortfolioPosition(p.symbol)} style={chipButtonStyle}>
                ×
              </button>
            </span>
          ))}
                ...
                {portfolioRows.map((row) => {
                  ...
                  return (
                    <tr key={row.symbol}>
                      ...
                      >
                        ${row.gainLoss.toFixed(0)} / {row.gainLossPct.toFixed(1)}%
                      </td>
                      <td style={tdStyle}>
        ...
        <div style={top10WrapStyle}>
          ...
          <div style={top10GridStyle}>
            {top10Rows.map((row) => {
              ...
              return (
                <button key={row.symbol} onClick={() => setSelectedSymbol(row.symbol)} style={top10CardStyle}>
                  ...
                  <div style={top10ScoreStyle}>Score: {row.recommendation?.score ?? "—"}</div>
                </button>
              );
            })}
          </div>
      ...
      {selectedRow ? (
        <>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={selectedSignalStyle}>Signal: {selectedRow.recommendation?.label}</div>
            <div style={selectedReasonStyle}>{selectedRow.recommendation?.reason}</div>
          ...
          <div style={gridStyle}>
            <div style={cardStyle}>
              ...
              <div style={metricGridStyle}>
                ...
                <Metric
                  label="Pressure"
                  value={selectedRow.technicalSnapshot?.pressure || selectedRow.recommendation?.pressure || "—"}
                />
                <Metric
                  label="Fair value"
                  value={selectedRow.technicalSnapshot?.fairValue || selectedRow.recommendation?.fairValue || "—"}
                />
              </div>
            </div>
            ...
            <div style={cardStyle}>
              ...
              <div style={metricGridStyle}>
                <Metric
                  label="Market cap"
                  value={formatBillions(selectedRow.marketCap ?? selectedRow.fundamentalSnapshot?.marketCap)}
                />
                <Metric
                  label="Avg volume"
                  value={selectedRow.avgVolume != null ? Math.round(selectedRow.avgVolume).toLocaleString() : "—"}
                />
                <Metric
                  label="EPS"
...
const titleStyle = { fontSize: 36, fontWeight: 700, marginBottom: 10, display: "flex", alignItems: "center", gap: 10 };
const subtitleStyle = { color: "#6b7280", fontSize: 14, marginBottom: 20 };
const toolbarStyle = { display: "flex", gap: 12, marginBottom: 14, alignItems: "center", flexWrap: "wrap" };
const inputStyle = { flex: "1 1 280px", minWidth: 240, padding: "12px 14px", border: "1px solid #d1d5db", borderRadius: 10, fontSize: 16 };
const smallInputStyle = { width: 120, padding: "10px 12px", border: "1px solid #d1d5db", borderRadius: 10, fontSize: 14 };
const primaryButtonStyle = { padding: "12px 16px", borderRadius: 10, border: "1px solid #111827", background: "#111827", color: "#ffffff", fontSize: 14, cursor: "pointer" };
const secondaryButtonStyle = { padding: "12px 16px", borderRadius: 10, border: "1px solid #d1d5db", background: "#ffffff", color: "#111827", fontSize: 14, cursor: "pointer" };
const filterRowStyle = { display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18, alignItems: "center" };
const metaStyle = { fontSize: 13, color: "#6b7280", marginLeft: 4 };
const errorStyle = { marginBottom: 16, padding: "12px 14px", borderRadius: 10, background: "#fef2f2", color: "#b91c1c", border: "1px solid #fecaca" };

const portfolioWrapStyle = { border: "1px solid #e5e7eb", borderRadius: 14, padding: 16, background: "#ffffff", marginBottom: 20 };
const portfolioHeaderStyle = { fontSize: 18, fontWeight: 800, marginBottom: 12 };
const portfolioInputRowStyle = { display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 12 };
const miniPositionStyle = { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 };
const positionChipStyle = { display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 999, border: "1px solid #d1d5db", fontSize: 13 };
const chipButtonStyle = { border: "none", background: "transparent", cursor: "pointer", fontWeight: 800, color: "#b91c1c" };

const top10WrapStyle = { border: "1px solid #e5e7eb", borderRadius: 14, padding: 16, background: "#ffffff", marginBottom: 20 };
const top10TitleStyle = { fontSize: 18, fontWeight: 800, marginBottom: 12 };
const top10GridStyle = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(145px, 1fr))", gap: 12 };
const top10CardStyle = { textAlign: "left", border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, background: "#f9fafb", cursor: "pointer" };
const top10SymbolStyle = { fontWeight: 800, fontSize: 16, color: "#111827" };
const top10PriceStyle = { fontSize: 14, marginTop: 2, color: "#111827" };
const top10PillStyle = { display: "inline-block", minWidth: 90, textAlign: "center", padding: "4px 8px", borderRadius: 999, fontWeight: 800, fontSize: 11, letterSpacing: 0.2 };
const top10ScoreStyle = { fontSize: 12, color: "#6b7280", marginTop: 6 };

const tableWrapStyle = { border: "1px solid #e5e7eb", borderRadius: 14, overflowX: "auto", background: "#ffffff", marginBottom: 24 };
const tableStyle = { width: "100%", minWidth: 1040, borderCollapse: "collapse" };
const thStyle = { textAlign: "left", padding: "14px 16px", fontSize: 13, color: "#6b7280", borderBottom: "1px solid #e5e7eb" };
...
const tdStyleRight = { ...tdStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" };
const pillBaseStyle = { display: "inline-block", minWidth: 120, textAlign: "center", padding: "6px 10px", borderRadius: 999, fontWeight: 800, fontSize: 12, letterSpacing: 0.2 };
...
const selectedSignalStyle = { fontSize: 15, color: "#374151", marginBottom: 10, fontWeight: 700 };
...
const gridStyle = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 };
const cardStyle = { border: "1px solid #e5e7eb", borderRadius: 14, padding: 16, background: "#ffffff" };
const cardTitleStyle = { fontSize: 16, fontWeight: 700, marginBottom: 12 };
const metricGridStyle = { display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 };
const metricStyle = { border: "1px solid #e5e7eb", borderRadius: 10, padding: "10px 12px", background: "#ffffff" };
...
const engineNoteStyle = { fontSize: 13, color: "#6b7280", marginTop: 8, marginBottom: 24 };
