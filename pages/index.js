...
function rotationStrength(gap){const g=+gap||0;if(g>=50)return{label:"Very Strong Rotation Edge",tone:"veryStrong"};if(g>=40)return{label:"Strong Rotation Edge",tone:"strong"};return{label:"Below Rotation Hurdle",tone:"meaningful"};}
...
export default function Home(){
  ...
  const minResidual=Math.max(1500,portfolioValue*.05),trimMinResidual=Math.max(1000,portfolioValue*.035),minFundingAction=Math.max(750,portfolioValue*.005);
  ...
  for(const bp of buyPlans){
    ...
    if(need>minFundingAction&&rotationTargetEligible(bp.stock)){
      const candidates=rawDecisions.filter(x=>x.s.role==="Swing"&&!CASH.includes(sym(x.s))&&x.d.action!=="Exit"&&!usedRotationSymbols.has(sym(x.s))&&sym(x.s)!==bp.symbol).map(x=>{const gap=Math.max(0,bp.score-capitalScore(x.s));const decision=portfolioDecision({stock:x.s,recommendation:rec(x.s),position:{role:x.s.role,gainLossPct:x.s.gainLossPct,weightPct:x.s.weightPct,opportunityGap:gap,rotateTarget:bp.symbol,rotationTargetEligible:true}});return{...x,gap,decision};}).filter(x=>x.gap>=40&&x.decision.action==="Rotate").sort((a,b)=>b.gap-a.gap);
