...
// A Buy can receive existing cash at the normal threshold. Forcing a sale requires a stronger margin of safety.
...
export default function Home(){
  ...
  for(const bp of buyPlans){
    ...
    for(const pool of exitPools){
      if(need<=minFundingAction)break;if(pool.remaining<=0)continue;
      const exec=executableBuy(bp,Math.min(pool.remaining,need));if(exec.amount<minFundingAction)continue;
      fundingPlan.push({symbol:pool.symbol,target:bp.symbol,kind:"Exit",amount:exec.amount,targetShares:exec.shares,sourceValue:pool.sourceValue,buyRank:bp.rank});pool.remaining-=exec.amount;need-=exec.amount;
    }
    for(const pool of cashPools){
      if(need<=minFundingAction)break;if(pool.remaining<=0)continue;
      const exec=executableBuy(bp,Math.min(pool.remaining,need));if(exec.amount<minFundingAction)continue;
      fundingPlan.push({symbol:pool.symbol,target:bp.symbol,kind:"Cash",amount:exec.amount,targetShares:exec.shares,sourceValue:pool.sourceValue,buyRank:bp.rank});pool.remaining-=exec.amount;need-=exec.amount;
    }

    if(need>minFundingAction&&rotationTargetEligible(bp.stock)){
      const candidates=rawDecisions.filter(x=>x.s.role==="Swing"&&!CASH.includes(sym(x.s))&&x.d.action!=="Exit"&&!usedRotationSymbols.has(sym(x.s))&&sym(x.s)!==bp.symbol).map(x=>{
        const gap=Math.max(0,bp.score-capitalScore(x.s));
        const decision=portfolioDecision({stock:x.s,recommendation:rec(x.s),position:{role:x.s.role,gainLossPct:x.s.gainLossPct,weightPct:x.s.weightPct,opportunityGap:gap,rotateTarget:bp.symbol,rotationTargetEligible:true}});
        return{...x,gap,decision};
      }).filter(x=>x.gap>=30&&x.decision.action==="Rotate").sort((a,b)=>b.gap-a.gap);

      for(const x of candidates){
        if(need<=minFundingAction)break;
        const srcPrice=price(x.s),srcShares=Math.floor(+x.s.shares||0),v=srcShares*srcPrice;if(!(srcPrice>0&&srcShares>0&&v>0))continue;
        let targetExec=executableBuy(bp,Math.min(v,need));if(targetExec.amount<minFundingAction)continue;
        let sellShares=Math.min(srcShares,Math.ceil((targetExec.amount-1e-6)/srcPrice)),saleProceeds=sellShares*srcPrice,residualShares=srcShares-sellShares,residualValue=residualShares*srcPrice,fullRotation=sellShares>=srcShares||(residualValue>0&&residualValue<minResidual);
        if(fullRotation){sellShares=srcShares;saleProceeds=v;residualShares=0;residualValue=0;targetExec=executableBuy(bp,Math.min(saleProceeds,need));if(targetExec.amount<minFundingAction)continue;}
        const residualCash=Math.max(0,saleProceeds-targetExec.amount);
        fundingPlan.push({symbol:sym(x.s),target:bp.symbol,kind:"Rotate",amount:targetExec.amount,targetShares:targetExec.shares,sourceValue:v,sourceSellShares:sellShares,sourceSaleProceeds:saleProceeds,sourceRemainingShares:residualShares,gap:x.gap,fullRotation,residualCash,buyRank:bp.rank});
        usedRotationSymbols.add(sym(x.s));need-=targetExec.amount;
        if(residualCash>0)cashPools.push({symbol:`${sym(x.s)} proceeds`,remaining:residualCash,sourceValue:residualCash});
      }
    }
    if(need<=minFundingAction){bp.toleranceGap=Math.max(0,need);need=0;}
    bp.remainingNeed=Math.max(0,need);bp.funded=Math.max(0,bp.need-bp.remainingNeed);
  }
  ...
  const actionGroups=[];
  for(const[source,items]of fundingBySymbol.entries()){
    const exitItems=items.filter(x=>x.kind==="Exit"),exitCash=items.filter(x=>x.kind==="Exit Cash").reduce((a,x)=>a+x.amount,0),cashItems=items.filter(x=>x.kind==="Cash"),rotateItems=items.filter(x=>x.kind==="Rotate");
    if(exitItems.length||exitCash>0)actionGroups.push({source,type:"Exit",items:exitItems,cash:exitCash});
    if(cashItems.length)actionGroups.push({source,type:"Cash",items:cashItems,cash:0});
    for(const r of rotateItems)actionGroups.push({source,type:"Rotate",items:[r],cash:r.residualCash||0,gap:r.gap,fullRotation:r.fullRotation,sourceSellShares:r.sourceSellShares,sourceSaleProceeds:r.sourceSaleProceeds,sourceRemainingShares:r.sourceRemainingShares});
  }
  ...
  function pd(s){
    const base=rawPd(s),bp=buyPlans.find(x=>x.symbol===sym(s));
    if(base.action==="Add"){
      if(!bp)return{...base,action:"Hold",reason:"The authoritative broad screen does not currently include this holding as an actionable fresh-capital Buy."};
      if(bp.need<=minFundingAction)return{...base,action:"Hold",reason:"The setup remains attractive, but the existing position is already within the target-allocation tolerance, so no additional trade is needed."};
      if(fundedFor(sym(s))<minFundingAction)return{...base,action:"Hold",reason:"The setup qualifies for additional capital, but there is no meaningful executable funding action available now. Hold rather than create an unfunded recommendation."};
    }
    if(base.action==="Trim"&&!trimPlans.has(sym(s)))return{...base,action:"Hold",reason:"This winner is stretched, but a partial sale would be too small to matter or would leave an immaterial residual position. Hold rather than create a nuisance trim."};
    if(base.action==="Rotate"&&!fundedRotationSymbols.has(sym(s)))return{...base,action:"Hold",reason:"This holding can be a rotation candidate, but no sale is required in the current executable funding plan."};
    return base;
  }
  function next(s,d){
    if(CASH.includes(sym(s)))return"Available for deployment.";
    const p=plan(s),inv=+p.invalidationPrice,fs=fundingBySymbol.get(sym(s))||[];
    if(d.action==="Add"){const amt=fundedFor(sym(s));return `Add ${tradeLabel(sym(s),amt)}${Number.isFinite(inv)?`; review below ${money(inv)}`:""}.`;}
  ...
  function PortfolioRow({s,mobile=false}){
    const d=pd(s),label=`${s.role} · ${(+s.weightPct||0).toFixed(1)}% of portfolio · ${theme(s)}`;
    if(mobile)return <article className="portfolioItem"><div className="portfolioHead"><div><h3>{sym(s)}</h3><span>{label}</span></div><b className={`pill ${cls(d.action)}`}>{d.action}</b></div><div className="mobileField"><small>Why</small><p>{d.reason}</p></div><div className="mobileField"><small>Next Move</small><p>{next(s,d)}</p></div><div className="mobileNumbers"><div><small>Price</small><b>{money(price(s))}</b></div><div><small>Gain / Loss</small><b className={s.gainLoss>=0?"pos":"neg"}>{money(s.gainLoss)} / {pct(s.gainLossPct)}</b></div></div></article>;
    return <tr><td><b>{sym(s)}</b><div>{label}</div></td><td><b className={`pill ${cls(d.action)}`}>{d.action}</b></td><td>{d.reason}</td><td>{next(s,d)}</td><td>{money(price(s))}</td><td className={s.gainLoss>=0?"pos":"neg"}>{money(s.gainLoss)} / {pct(s.gainLossPct)}</td></tr>;
  }
