...
export default function Home(){
  ...
  function trimFraction(s,d){const pp=d?.profitProtection||{},pnl=Number.isFinite(+pp.pnlPct)?+pp.pnlPct:(+s.gainLossPct||0);if(pp.extremeFroth||(pp.winnerFading&&pnl>=60))return{pct:.45,label:"Extreme"};if(pp.winnerFading||pp.moderateFroth||pnl>=75)return{pct:.35,label:"Strong"};return{pct:.25,label:"Moderate"};}
  function makeTrimPlan(s,d){const v=+s.value||0,pr=price(s),shares=Math.floor(+s.shares||0);if(!(v>0&&pr>0&&shares>1))return null;const severity=trimFraction(s,d),desired=Math.max(minFundingAction,v*severity.pct),maxSell=Math.floor(Math.max(0,v-minResidual)/pr),sellShares=Math.min(shares-1,maxSell,Math.ceil(desired/pr));if(sellShares<=0)return null;const amount=sellShares*pr,residual=v-amount,avgCost=+s.avgCost||0,realizedGain=avgCost>0?sellShares*Math.max(0,pr-avgCost):null;if(amount<minFundingAction||residual<minResidual)return null;return{shares:sellShares,amount,residual,remainingShares:shares-sellShares,trimPct:severity.pct,severity:severity.label,realizedGain};}
