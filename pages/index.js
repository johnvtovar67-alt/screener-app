...
export default function Home(){
  ...
  async function handleReload(){if(tab==="account-tools")return recoverC1Account();if(tab==="portfolio")return analyze();if(tab==="single")return check();manualVerificationPass.current=Math.min(20,manualVerificationPass.current+1);return load(tab==="themes"?selectedTheme:"opportunities",manualVerificationPass.current);}
  ...
  function rawPd(s){
    ...
    if(s.role==="Swing"){const accountDecision=c1AccountPositionDecision(accountView?.decision,sym(s),price(s));if(accountDecision)return accountDecision;}
  ...
  return <main>
    ...
    <div hidden={tab!=="account-tools"} aria-label="Account settings">
      ...
      {accountView?.pendingSession&&<C1AccountActivity key={accountView.pendingSession.date} pending={accountView.pendingSession} onSave={saveC1Activity} busy={accountBusy}/>}
