...
async function fetchQuote(symbol){const quote=await fetchFmpQuote(toFmpSymbol(symbol)),normalized=normalizeQuote(quote,symbol);if(normalized.symbol&&normalized.price!==null)return normalized;throw new Error(`No usable quote data returned for ${symbol}.`);}
